...
class DrAduProjectAnalyzer:
    ...
    def analyze_file(self, file_path):
        ...
        try:
            ...
            file_info["technical_details"] = self.extract_technical_details(
    ...
    def generate_summary(self):
        ...
        self.analysis_results["content_summary"] = {
            ...
            "technical_files": len(
                [f for f in files if "technical" in f.get("key_topics", [])]
            ),
            "business_files": len(
                [f for f in files if "business" in f.get("key_topics", [])]
            ),
        }
    ...
    def generate_recommendations(self):
        ...
        has_technical_report = any(
            "technical" in f["filename"].lower()
            or "improvements" in f["filename"].lower()
            for f in files
        )
        has_client_report = any("client" in f["filename"].lower() for f in files)
        has_visual_comparison = any(
            "comparison" in f["filename"].lower() for f in files
        )
        ...
        if len(seo_files) < 3:
            recommendations.append(
                "Expand SEO documentation with more detailed reports"
            )
    ...
    def create_organization_plan(self):
        ...
        for file_info in files:
            ...
            if "invoice" in filename or "pricing" in filename:
                ...
                "technical" in filename
                or "improvements" in filename
...
"""
        ...
        for topic, count in self.analysis_results["content_summary"][
            "most_common_topics"
        ][:10]:
...
"""
    ...
    def save_analysis(self):
        ...
        print("   📊 JSON data: project_analysis.json")
        print("   📋 Report: PROJECT_ANALYSIS_REPORT.md")
...
def main():
    ...
    print("\n📈 Analysis Complete!")
    print(
        "Check the generated files for detailed insights and organization recommendations."
    )
