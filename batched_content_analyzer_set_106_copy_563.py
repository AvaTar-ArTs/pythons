...
class BatchedContentAnalyzer:
    ...
    def extract_tool_calls(self, content, max_calls=10):
        ...
        for tool_content in tool_matches[:max_calls]:
            ...
            tool_info = {
                ...
                "tool_id": tool_id,
                "size": size,
                "content_preview": tool_content[:200] + "..."
    ...
    def analyze_single_file(self, file_path):
        ...
        try:
            ...
            file_info["code_blocks"] = self.extract_code_blocks(
                content, max_blocks=3
            )  # Limit code blocks
            file_info["tool_calls"] = self.extract_tool_calls(
                content, max_calls=5
            )  # Limit tool calls
    ...
    def save_batch_results(self, batch_results):
        ...
        # Save batch summary
        summary_file = (
            batch_dir / f"batch_{batch_results['batch_number']:03d}_summary.md"
        )
        with open(summary_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- **Files Processed:** {batch_results['statistics']['total_files']}\n"
            )
            f.write(
                f"- **Total Size:** {batch_results['statistics']['total_size']:,} bytes\n"
            )
            f.write(
                f"- **Total Words:** {batch_results['statistics']['total_words']:,}\n\n"
            )
    ...
    def scan_all_files_batched(self):
        ...
        print("🔍 Performing batched content-aware analysis...")
    ...
    def generate_master_summary(self):
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- **Total Size:** {master_stats['total_size']:,} bytes ({master_stats['total_size'] / 1024 / 1024:.2f} MB)\n"
            )
            ...
            f.write(
                f"- **Unique Categories:** {len(master_stats['unique_categories'])}\n"
            )
            f.write(
                f"- **Unique Code Types:** {len(master_stats['unique_code_types'])}\n"
            )
            f.write(
                f"- **Unique Tool Types:** {len(master_stats['unique_tool_types'])}\n\n"
            )
        ...
        print(f"   ✅ Master summary saved: {master_file}")
        print(f"   ✅ Master report saved: {report_file}")
    ...
    def run_batched_analysis(self):
        ...
        print("🚀 Starting Batched Content-Aware Chat Analysis...")
        print("📦 This will process files in small batches to avoid large file sizes")
        ...
        print("📋 Check MASTER_BATCHED_REPORT.md for overview")
        print("📦 Individual batch results in: 07_Exports/batches/")
