...
class RecursiveDeepScanner:
    ...
    def scan_folder(self, folder_path, depth=0):
        ...
        try:
            ...
            folder_info['has_readme'] = (folder_path / 'README.md').exists() or (folder_path / 'README.txt').exists()
            folder_info['has_requirements'] = (folder_path / 'requirements.txt').exists()
            ...
            for item in folder_path.iterdir():
                ...
                if item.is_dir():
                    ...
                    if item.suffix == '.py':
                        ...
                        folder_info['total_functions'] += file_analysis['function_count']
    ...
    def save_folder_csv(self):
        ...
        with open(csv_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'Folder Path', 'Folder Name', 'Depth', 'Python Files (Direct)',
                'Python Files (Total)', 'Total Files', 'Subdirs Count',
                'Size (MB)', 'Total Lines', 'Total Functions', 'Total Classes',
                'Has README', 'Has Requirements', 'Has __init__',
                'APIs Found', 'Purposes', 'Subdirectories'
            ])
    ...
    def save_file_csv(self):
        ...
        print(f"💾 Saving file analysis CSV...")

        with open(csv_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'Filename', 'Relative Path', 'Parent Folder', 'Depth',
                'Size (KB)', 'Lines', 'Functions', 'Classes', 'Imports',
                'Has Main', 'Has Docstring', 'APIs Used', 'Purposes',
                'Complexity', 'Error'
            ])
    ...
    def print_tree_summary(self):
        ...
        for folder in sorted(by_depth[1], key=lambda x: x['python_files'], reverse=True):
            py_count = folder['python_files']
            size_mb = folder['size_bytes'] / (1024*1024)
            print(f"📁 {folder['folder_name']:40} {py_count:5} files, {size_mb:7.1f} MB")

            # Show its subdirectories (depth 2)
            subdirs_at_depth_2 = [f for f in by_depth[2]
                                 if f['relative_path'].startswith(folder['relative_path'])]

            for subdir in sorted(subdirs_at_depth_2, key=lambda x: x['python_files'], reverse=True)[:5]:
    ...
    def print_statistics(self):
        ...
        total_funcs = sum(f['total_functions'] for f in self.folder_data if f['depth'] == 0)
        total_classes = sum(f['total_classes'] for f in self.folder_data if f['depth'] == 0)
    ...
    def find_issues(self):
        ...
        for folder in self.folder_data:
            if folder['depth'] > 5:
                issues.append(f"🔍 DEEP NESTING (depth {folder['depth']}): {folder['relative_path']}")

        # Find folders with no Python files but has subdirs
        for folder in self.folder_data:
            if folder['direct_py_files'] == 0 and folder['total_dirs'] == 0 and folder['total_files'] > 0:
                issues.append(f"📂 NO PY FILES: {folder['relative_path']} ({folder['total_files']} other files)")

        # Find potential duplicates (similar names at same depth)
        by_depth_and_name = defaultdict(list)
        for folder in self.folder_data:
            key = (folder['depth'], folder['folder_name'].lower())
            by_depth_and_name[key].append(folder)

        for key, folders in by_depth_and_name.items():
            if len(folders) > 1:
                paths = [f['relative_path'] for f in folders]
                issues.append(f"♻️  DUPLICATE NAME: {folders[0]['folder_name']} appears in {len(folders)} locations")
