...
Features:
    - Merges: docs.py, audio.py, vids.py, img.py, other.py, cleanup_and_organize.py,
              batch-info.py, ecosystem_scan_compare.py, config.py
    - AutoTagger integration
    - SHA256 deduplication
    - CSV-driven enforcement
    - Progress bars
"""
...
def get_code_metadata(filepath: str) -> Optional[Dict]:
    """Analyze source code files for statistics."""
    try:
...
def get_creation_date(filepath: str) -> str:
    """Get file creation date in mm-dd-yy format."""
    try:
...
def print_progress(current: int, total: int, prefix: str = "Progress", width: int = 50):
    ...
    print(
        f"\r{prefix}: |{bar}| {current}/{total} ({pct*100:.1f}%)",
        end="", flush=True
    )
    if current == total:
        print()
...
def scan_directories(
    ...
    """
    Scan directories and produce a comprehensive CSV with metadata and analytics.
    """
    ...
    # --- Analyze: Build enforcement set if applicable
    enforce_uids = set()
    ...
    # --- Analyze & Format: File Scanning and Metadata Extraction ---
    rows = []
    hash_map = defaultdict(list)
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(scan_single_file, fpath, enable_metadata, enable_dedup): fpath for fpath in all_files}
        for i, future in enumerate(as_completed(futures), 1):
            fpath = futures[future]
            try:
                result = future.result()
                if result:
                    # CSV enforcement check (analysis step)
                    if enforce_uids:
                        uid = result.get("uuid", "")
    ...
    # --- Format: Filter enforced/duplicate rows before CSV writing ---
    ...
    # --- Analyze & Format: Summary Statistics ---
...
def scan_single_file(fpath: str, enable_metadata: bool, enable_dedup: bool) -> Optional[Dict]:
    """Analyze and format a single file's metadata."""
    try:
        stat = os.stat(fpath)
...
def main():
    parser = argparse.ArgumentParser(
        description="Enhanced Universal File Scanner & Organizer"
    )
    ...
    # Use minimal excludes for non-global scans if available
