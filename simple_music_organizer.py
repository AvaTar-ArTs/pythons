...
def determine_thematic_category(name):
    ...
    elif any(keyword in normalized for keyword in ["trash", "raccoon", "trashcat", "trashy"]):
...
def organize_music_content():
    ...
    # Handle loose audio files
    print("\nOrganizing loose audio files...")
    ...
    # Handle loose data files
    print("\nOrganizing loose data files...")
    data_extensions = {".csv", ".json", ".txt", ".md", ".xlsx", ".xls"}
    ...
    # Handle loose script files
    print("\nOrganizing loose script files...")
    script_extensions = {".py", ".sh", ".js", ".ts"}

    for file in all_files:
        if file.suffix.lower() in script_extensions:
    ...
    # Handle loose documentation files
    print("\nOrganizing loose documentation files...")
    doc_extensions = {".pdf", ".doc", ".docx", ".html", ".htm"}

    for file in all_files:
        if file.suffix.lower() in doc_extensions or any(
            keyword in file.name.lower() for keyword in ["readme", "doc", "note", "analysis", "report"]
        ):
            if any(keyword in file.name.lower() for keyword in ["readme", "note", "doc"]):
