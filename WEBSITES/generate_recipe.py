...
SEEDS = [
    {
        "name": "Spicy Raccoon-Free Tacos",
        "base": ["tortillas", "beans", "onion", "garlic", "tomato", "cumin", "chili"],
    },
    {
        "name": "Krampus Cocoa Bombs",
        "base": ["cocoa", "milk", "sugar", "vanilla", "cinnamon"],
    },
    {
        "name": "Ambient Focus Smoothie",
        "base": ["banana", "spinach", "almond milk", "chia", "honey"],
    },
    {
        "name": "Dark Fantasy Mushroom Pasta",
        "base": ["pasta", "mushrooms", "garlic", "butter", "parsley", "parmesan"],
    },
]
...
def main():
    ...
    name = seed["name"]
    ingredients = seed["base"] + random.sample(
        ["olive oil", "salt", "pepper", "paprika", "lemon", "basil", "oregano"], k=3
    )
    instructions = [
    ...
    content = (
        content.replace("{{PREP_TIME}}", "10m")
        .replace("{{COOK_TIME}}", "20m")
        .replace("{{SERVES}}", "2")
    )
    content = content.replace("{{INGREDIENTS_LIST}}", to_list_block(ingredients))
    content = content.replace(
        "{{INSTRUCTIONS_LIST}}",
        to_list_block([f"Step {i + 1}: {s}" for i, s in enumerate(instructions)]),
    )
    ...
    content = (
        content.replace("{{CALORIES}}", "420")
        .replace("{{PROTEIN}}", "12")
        .replace("{{FAT}}", "14")
        .replace("{{CARBS}}", "60")
    )
    content = content.replace(
        "{{AFFILIATE_LINKS}}", "; ".join(read_affiliates()) or "—"
    )
    content = content.replace(
        "{{RECIPE_SCHEMA}}", gen_schema(name, ingredients, instructions)
    )
