...
class ASTIntelligenceAnalyzer:
    ...
    def _detect_customer_service_patterns(self, text: str) -> List[Dict[str, Any]]:
        ...
        service_matches = sum(
            1 for keyword in service_keywords if keyword in text.lower()
        )
        complaint_matches = sum(
            1 for keyword in complaint_keywords if keyword in text.lower()
        )
        compliment_matches = sum(
            1 for keyword in compliment_keywords if keyword in text.lower()
        )
            ...
            patterns.append(
                {
                    "type": "compliment",
                    "confidence": min(
                        compliment_matches / len(compliment_keywords), 1.0
                    ),
    ...
    def _detect_escalation_patterns(self, text: str) -> List[Dict[str, Any]]:
        ...
        escalation_matches = sum(
            1 for keyword in escalation_keywords if keyword in text.lower()
        )
            ...
            patterns.append(
                {
                    ...
                    "confidence": min(
                        escalation_matches / len(escalation_keywords), 1.0
                    ),
    ...
    def _detect_scheduling_patterns(self, text: str) -> List[Dict[str, Any]]:
        ...
        scheduling_matches = sum(
            1 for keyword in scheduling_keywords if keyword in text.lower()
        )
            ...
            patterns.append(
                {
                    ...
                    "confidence": min(
                        scheduling_matches / len(scheduling_keywords), 1.0
                    ),
    ...
    def _classify_intent(self, conversation_history: List[Dict[str, str]]) -> str:
        ...
        recent_text = " ".join(
            [turn.get("content", "") for turn in conversation_history[-3:]]
        )
        ...
        intent_scores["service_inquiry"] = sum(
            1 for keyword in service_keywords if keyword in recent_text.lower()
        )
        ...
        intent_scores["scheduling"] = sum(
            1 for keyword in scheduling_keywords if keyword in recent_text.lower()
        )

        # Complaint scoring
        complaint_keywords = ["problem", "issue", "wrong", "complaint", "disappointed"]
        intent_scores["complaint"] = sum(
            1 for keyword in complaint_keywords if keyword in recent_text.lower()
        )

        # Sales scoring
        sales_keywords = ["price", "cost", "quote", "estimate", "interested", "sign up"]
        intent_scores["sales"] = sum(
            1 for keyword in sales_keywords if keyword in recent_text.lower()
        )

        # Information scoring
        info_keywords = ["about", "tell me", "what", "how", "information"]
        intent_scores["information"] = sum(
            1 for keyword in info_keywords if keyword in recent_text.lower()
        )
    ...
    def _calculate_confidence(:
        ...
        base_confidence = min(
            conversation_length / 10, 1.0
        )  # More turns = higher confidence

        # Confidence based on pattern detection
        patterns = self._detect_patterns(
            " ".join([turn.get("content", "") for turn in conversation_history])
        )
    ...
    def _generate_recommendations(:
        ...
        recent_text = " ".join(
            [turn.get("content", "") for turn in conversation_history[-2:]]
        )

        # Service inquiry recommendations
        if any(
            keyword in recent_text.lower()
            for keyword in ["clean", "service", "house", "office"]
        ):
            recommendations.append(
                "Provide detailed service information and pricing options"
            )

        # Scheduling recommendations
        if any(
            keyword in recent_text.lower()
            for keyword in ["schedule", "appointment", "book", "time"]
        ):
            recommendations.append(
                "Offer available time slots and confirm booking details"
            )

        # Sales recommendations
        if any(
            keyword in recent_text.lower()
            for keyword in ["price", "cost", "quote", "estimate"]
        ):
            recommendations.append(
                "Provide clear pricing information and special offers"
            )

        # Complaint recommendations
        if any(
            keyword in recent_text.lower()
            for keyword in ["problem", "issue", "wrong", "complaint"]
        ):
    ...
    def _extract_semantic_tags(self, text: str) -> List[str]:
        ...
        if any(
            keyword in text.lower() for keyword in ["commercial", "office", "business"]
        ):
        ...
        if any(
            ...
            for keyword in ["happy", "satisfied", "great", "excellent"]
        ):
            tags.append("positive_sentiment")
        if any(
            keyword in text.lower()
            for keyword in ["angry", "upset", "disappointed", "problem"]
        ):
            tags.append("negative_sentiment")
        if any(
            keyword in text.lower()
            for keyword in ["question", "what", "how", "tell me"]
        ):
...
class HeavenlyHandsCallCenterAgent:
    ...
    def create_call_session(:
        self, call_id: str, caller_id: str = "unknown"
    ) -> CallSession:
...
def handle_message():
    ...
    return {"response": response, "confidence": confidence, "call_id": call_id}
...
def process_speech():
    ...
    if speech_result:
        ...
        agent_response = call_center_agent.process_customer_message(
            call_sid, speech_result
        )
...
def main():
    ...
    if missing_twilio:
        print("ℹ️  Twilio not configured. Phone integration will be disabled.")
