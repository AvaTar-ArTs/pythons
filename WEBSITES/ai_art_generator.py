...
class AIArtGenerator:
    ...
    def __init__(self, api_key: str = None):
        self.base_path = Path(
            "/Users/steven/ai-sites/retention-products-suite/digital-products/creative-asset-packs"
        )
    ...
    def generate_art_collection(:
        self, style_name: str, collection_size: int = 20
    ) -> Dict:
        ...
        collection_path = (
            self.output_path / f"{style_name.lower().replace(' ', '_')}_collection.json"
        )
    ...
    def _generate_single_artwork(self, style: ArtStyle, index: int) -> Dict:
        ...
        prompt = style.prompt_template.format(
            subject=subject, color_palette=color, mood=style.mood
        )
        ...
        artwork = {
            ...
            "pricing": {"standard": 9.99, "premium": 19.99, "commercial": 49.99},
    ...
    def _generate_mock_image(self, style: ArtStyle, subject: str, index: int) -> Path:
        ...
        # Save image
        image_path = (
            self.output_path / f"{style.name.lower().replace(' ', '_')}_{index:03d}.png"
        )
        img.save(image_path)
    ...
    def _generate_collection_preview(self, collection: Dict):
        ...
        for i, artwork in enumerate(collection["artworks"][:16]):  # Max 16 for preview
            ...
            color = (
                artwork["color_palette"][0] if artwork["color_palette"] else "#000000"
            )
        ...
        preview_path = (
            self.output_path
            / f"{collection['style'].lower().replace(' ', '_')}_preview.png"
        )
