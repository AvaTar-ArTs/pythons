...
class CityGalleryInitTestCase(unittest.TestCase):
    ...
    def test_gallery_creation_possible(self):
        ...
        with TempDirectory() as tempdir:
            ...
            self.assertTrue(
                gallery_init.check_if_gallery_creation_possible(tempdir.path)
            )

            # Should not be possible in non-existing directory
            self.assertFalse(
                gallery_init.check_if_gallery_creation_possible("/nonexistent/path")
            )
    ...
    def test_gallery_initialization(self, input):
        ...
        with TempDirectory() as tempdir:
            ...
            self.assertTrue(
                os.path.exists(os.path.join(tempdir.path, "public", "images"))
            )
            self.assertTrue(
                os.path.exists(os.path.join(tempdir.path, "public", "images", "photos"))
            )
            self.assertTrue(
                os.path.exists(
                    os.path.join(tempdir.path, "public", "images", "thumbnails")
                )
            )
