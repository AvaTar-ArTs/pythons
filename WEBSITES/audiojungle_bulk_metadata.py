...
SUNO_DIRS = [MUSIC_DIR / "SUNO", MUSIC_DIR / "nocTurneMeLoDieS", MUSIC_DIR / "mp3-bin"]
...
def create_upload_csv(tracks, output_path=None):
    ...
    if output_path is None:
        output_path = (
            OUTPUT_DIR
            / f"audiojungle_upload_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
        )
...
def create_checklist(tracks, output_path=None):
    ...
    if output_path is None:
        output_path = (
            OUTPUT_DIR
            / f"audiojungle_checklist_{datetime.now().strftime('%Y%m%d_%H%M')}.md"
        )
    ...
    for category, items in sorted(by_category.items()):
        ...
        for track, meta in items:
            ...
            content.append(
                f"- **Format:** {track['format']} ({track['size_mb']:.1f} MB)\n"
            )
...
try:
        ...
        print(
            f"   Est. revenue potential: ${len(tracks) * 19 * 5}/month"
        )  # $19 * 5 sales avg
