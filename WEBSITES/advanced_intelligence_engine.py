    ...
    def _calculate_quality_metrics(:
        ...
        if functions:
            total_complexity = sum(
                self._count_decision_points(func) for func in functions
            )
        ...
        metrics["maintainability_index"] = max(
            0,
            100
            - metrics["function_complexity"] * 10
            - metrics["average_line_length"] / 10,
        )
...
class SemanticPatternRecognizer:
    ...
    def recognize_patterns(self, project_files: List[str]) -> Dict[str, Any]:
        ...
        pattern_results["technical_classification"] = self._classify_technical_stack(
            all_content
        )
        pattern_results["architecture_classification"] = self._classify_architecture(
            all_content
        )

        # Calculate overall confidence
        pattern_results["overall_confidence"] = self._calculate_overall_confidence(
            pattern_results
        )
        pattern_results["pattern_consistency"] = self._calculate_pattern_consistency(
            file_patterns
        )
    ...
    def _classify_technical_stack(self, content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "primary_focus": max(tech_scores.items(), key=lambda x: x[1]["score"])[0]
            if tech_scores
            else "unknown",
        }
    ...
    def _classify_architecture(self, content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "primary_pattern": max(arch_scores.items(), key=lambda x: x[1]["score"])[0]
            if arch_scores
            else "unknown",
        }
    ...
    def _calculate_overall_confidence(self, pattern_results: Dict[str, Any]) -> float:
        ...
        arch_scores = pattern_results["architecture_classification"][
            "architecture_patterns"
        ]
        if arch_scores:
    ...
    def _calculate_pattern_consistency(:
        self, file_patterns: List[Dict[str, Any]]
    ) -> float:
            ...
            if len(pattern_lists) > 1:
                ...
                for i in range(len(pattern_lists)):
                    for j in range(i + 1, len(pattern_lists)):
                        ...
                        similarity = (
                            len(set1 & set2) / len(set1 | set2) if set1 | set2 else 0
                        )
...
class ArchitecturalPatternDetector:
    ...
    def detect_patterns(self, project_structure: Dict[str, Any]) -> Dict[str, Any]:
        ...
        detection_results["architecture_quality"] = (
            self._calculate_architecture_quality(detection_results["detected_patterns"])
        )

        # Analyze design principles
        detection_results["design_principles"] = self._analyze_design_principles(
            structure_analysis
        )

        # Generate recommendations
        detection_results["recommendations"] = (
            self._generate_architectural_recommendations(detection_results)
        )
                ...
                for pattern_name, config in self.architectural_patterns.items():
                    for indicator in config["indicators"]:
                        if indicator in dir_name:
                            if (
                                pattern_name
                                not in structure_analysis["directory_patterns"]
                            ):
                                structure_analysis["directory_patterns"][
                                    pattern_name
                                ] = []
                            structure_analysis["directory_patterns"][
                                pattern_name
                ...
                for pattern_name, config in self.architectural_patterns.items():
                    for indicator in config["indicators"]:
                        if indicator in file_name:
                            ...
                            structure_analysis["file_patterns"][pattern_name].append(
                                file_path
                            )
    ...
    def _calculate_pattern_confidence(:
        self, structure_analysis: Dict[str, Any], config: Dict[str, Any]
    ) -> float:
        ...
        for pattern_name, directories in structure_analysis[
            "directory_patterns"
        ].items():
        ...
        confidence = (
            evidence_count / max_possible_evidence if max_possible_evidence > 0 else 0
        )
    ...
    def _analyze_design_principles(:
        ...
        if structure_analysis["file_patterns"]:
            avg_files_per_pattern = sum(
                len(files) for files in structure_analysis["file_patterns"].values()
            ) / len(structure_analysis["file_patterns"])
            principles["single_responsibility"] = max(
                0, 1 - (avg_files_per_pattern - 1) / 10
            )
    ...
    def _generate_architectural_recommendations(:
        ...
        if architecture_quality < 0.5:
            recommendations.append(
                "Consider implementing a clear architectural pattern (MVC, microservices, or layered architecture)"
            )

        if not detected_patterns:
            recommendations.append(
                "Add architectural documentation and clear separation of concerns"
            )

        if "mvc_pattern" in detected_patterns:
            recommendations.append(
                "Ensure proper separation between models, views, and controllers"
            )
...
class AICategorizationEngine:
    ...
    def categorize_project(self, project_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        categorization["complexity_level"] = self._determine_complexity_level(
            project_analysis
        )

        # Determine maturity level
        categorization["maturity_level"] = self._determine_maturity_level(
            project_analysis
        )
        ...
        categorization["confidence_scores"] = self._calculate_categorization_confidence(
            categorization
        )
    ...
    def _apply_categorization_rules(:
        ...
        for category, config in rules.items():
            if isinstance(config, dict) and "indicators" in config:
                ...
                matches = [
                    indicator for indicator in indicators if indicator in content
                ]
    ...
    def _determine_complexity_level(self, project_analysis: Dict[str, Any]) -> str:
        ...
        complexity_indicators = {"low": 0, "medium": 0, "high": 0}
    ...
    def _determine_maturity_level(self, project_analysis: Dict[str, Any]) -> str:
        ...
        maturity_indicators = {"early_stage": 0, "developing": 0, "mature": 0}
    ...
    def _calculate_categorization_confidence(:
        self, categorization: Dict[str, Any]
    ) -> Dict[str, float]:
        ...
        for category_type in ["project_type", "business_domain", "technology_stack"]:
            if category_type in categorization:
                ...
                if isinstance(category_data, dict) and category_data:
                    ...
                    if confidences:
                        confidence_scores[category_type] = sum(confidences) / len(
                            confidences
                        )
                    else:
...
class ContentAwarenessIntelligence:
    ...
    def analyze_content_awareness(:
        ...
        content_analysis["business_intelligence"] = self._analyze_business_intelligence(
            project_content
        )

        # Analyze technical intelligence
        content_analysis["technical_intelligence"] = (
            self._analyze_technical_intelligence(project_content)
        )

        # Analyze user experience
        content_analysis["user_experience"] = self._analyze_user_experience(
            project_content
        )

        # Analyze content quality
        content_analysis["content_quality"] = self._analyze_content_quality(
            project_analysis
        )

        # Calculate overall awareness score
        content_analysis["awareness_score"] = self._calculate_awareness_score(
            content_analysis
        )

        # Generate recommendations
        content_analysis["recommendations"] = self._generate_content_recommendations(
            content_analysis
        )
    ...
    def _analyze_business_intelligence(self, content: str) -> Dict[str, Any]:
        ...
        revenue_patterns = self.content_patterns["business_intelligence"][
            "revenue_patterns"
        ]
        revenue_matches = [
            pattern for pattern in revenue_patterns if pattern in content
        ]
        bi_analysis["revenue_focus"] = len(revenue_matches) / len(revenue_patterns)

        # Analyze customer patterns
        customer_patterns = self.content_patterns["business_intelligence"][
            "customer_patterns"
        ]
        customer_matches = [
            pattern for pattern in customer_patterns if pattern in content
        ]
        bi_analysis["customer_focus"] = len(customer_matches) / len(customer_patterns)

        # Analyze market patterns
        market_patterns = self.content_patterns["business_intelligence"][
            "market_patterns"
        ]
        ...
        bi_analysis["overall_score"] = (
            bi_analysis["revenue_focus"]
            + bi_analysis["customer_focus"]
            + bi_analysis["market_focus"]
        ) / 3
    ...
    def _analyze_technical_intelligence(self, content: str) -> Dict[str, Any]:
        ...
        perf_patterns = self.content_patterns["technical_intelligence"][
            "performance_patterns"
        ]
        ...
        sec_patterns = self.content_patterns["technical_intelligence"][
            "security_patterns"
        ]
        ...
        scale_patterns = self.content_patterns["technical_intelligence"][
        ...
        # Calculate overall score
        tech_analysis["overall_score"] = (
            tech_analysis["performance_focus"]
            + tech_analysis["security_focus"]
            + tech_analysis["scalability_focus"]
        ) / 3
    ...
    def _analyze_user_experience(self, content: str) -> Dict[str, Any]:
        ...
        usability_patterns = self.content_patterns["user_experience"][
            "usability_patterns"
        ]
        usability_matches = [
            pattern for pattern in usability_patterns if pattern in content
        ]
        ux_analysis["usability_focus"] = len(usability_matches) / len(
            usability_patterns
        )

        # Analyze accessibility patterns
        accessibility_patterns = self.content_patterns["user_experience"][
            "accessibility_patterns"
        ]
        accessibility_matches = [
            pattern for pattern in accessibility_patterns if pattern in content
        ]
        ux_analysis["accessibility_focus"] = len(accessibility_matches) / len(
            accessibility_patterns
        )
        ...
        # Calculate overall score
        ux_analysis["overall_score"] = (
            ux_analysis["usability_focus"]
            + ux_analysis["accessibility_focus"]
            + ux_analysis["mobile_focus"]
        ) / 3
    ...
    def _analyze_content_quality(:
        ...
        doc_matches = [
            indicator for indicator in doc_indicators if indicator in doc_content
        ]
        quality_analysis["documentation_quality"] = len(doc_matches) / len(
            doc_indicators
        )
        ...
        if "architectural_patterns" in project_analysis:
            ...
            if isinstance(arch_data, dict) and "architecture_quality" in arch_data:
                quality_analysis["structure_quality"] = arch_data[
                    "architecture_quality"
                ]

        # Calculate overall quality score
        quality_scores = [
            quality_analysis["documentation_quality"],
            quality_analysis["code_quality"],
            quality_analysis["structure_quality"],
        ]
    ...
    def _calculate_awareness_score(self, content_analysis: Dict[str, Any]) -> float:
        ...
        if "technical_intelligence" in content_analysis:
            tech_score = content_analysis["technical_intelligence"].get(
                "overall_score", 0
            )
        ...
        return (
            sum(awareness_components) / len(awareness_components)
    ...
    def _generate_content_recommendations(:
        ...
        if awareness_score < 0.3:
            recommendations.append(
                "Improve overall content awareness and documentation"
            )
...
class HeavenlyHandsAdvancedIntelligenceEngine:
    ...
    def __init__(self, project_path: str = "/Users/steven/ai-sites/heavenlyHands"):
        ...
        logger.info(
            f"Initialized Heavenly Hands Advanced Intelligence Engine for {project_path}"
        )
    ...
    def analyze_project(self) -> ProjectAnalysis:
        ...
        architectural_patterns = self.architectural_detector.detect_patterns(
            project_structure
        )
        ...
        content_analysis = self.content_intelligence.analyze_content_awareness(
            initial_analysis
        )
    ...
    def _perform_ast_analysis(self, project_files: List[str]) -> Dict[str, Any]:
        ...
        ast_results["average_complexity"] = (
            total_complexity / file_count if file_count > 0 else 0
        )
        ...
        for file_path in self.project_path.rglob("*"):
            if file_path.is_file():
                ...
                if ext in [".html", ".css", ".js"]:
                    ...
                    structure["documentation"].append(
                        str(file_path.relative_to(self.project_path))
                    )
    ...
    def _calculate_intelligence_metrics(:
        ...
        confidence_score = (
            ast_score + semantic_coherence + architectural_quality + content_awareness
        ) / 4
    ...
    def _calculate_confidence_scores(:
        ...
        confidence_scores = {
            ...
            "architectural_patterns": architectural_patterns.get(
