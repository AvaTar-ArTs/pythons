...
try:
    ...
    subprocess.run(
        ["bash", "-c", "source ~/.env.d/loader.sh && env"],
        capture_output=True,
        text=True,
    )
...
def run_dalle_generator(theme):
    ...
    result = subprocess.run(
        ["python3", str(script), "daily", theme], capture_output=True, text=True
    )
    return result.returncode == 0
...
def full_content_pipeline(topic, include_image=True):
    ...
    pipeline_log["success_count"] = sum(
        1 for s in pipeline_log["steps"] if s["status"] == "success"
    )
    ...
    print("\n" + "=" * 60)
    print(
        f"✅ Pipeline complete: {pipeline_log['success_count']}/{pipeline_log['total_steps']} successful"
    )
