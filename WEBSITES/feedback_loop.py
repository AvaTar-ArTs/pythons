...
def analyze_performance(days=30):
    ...
    by_type = defaultdict(
        lambda: {"views": 0, "engagement": 0, "revenue": 0, "count": 0}
    )
    ...
    for ctype, stats in by_type.items():
        if stats["count"] > 0:
            score = (
                (stats["views"] / 100) + (stats["engagement"] / 10) + stats["revenue"]
            )
            top_types.append(
                {
                    "type": ctype,
    ...
    if top_types:
        ...
        recommendations.append(
            f"✅ Focus on {best_type['type']} (score: {best_type['score']}) - it's your top performer"
        )

    if top_platforms:
        best_platform = top_platforms[0]
        recommendations.append(
            f"✅ Prioritize {best_platform['platform']} - highest engagement and revenue"
        )
    ...
    if len(top_types) > 1:
        ...
        if worst_type["score"] < best_type["score"] * 0.3:
            recommendations.append(
                f"⚠️ Consider reducing {worst_type['type']} - underperforming"
            )
    ...
    if total_revenue > 0:
        ...
        recommendations.append(
            f"💎 Best revenue source: {top_revenue_platform['platform']} (${top_revenue_platform['total_revenue']:.2f})"
        )
...
def generate_report(days=30, output_path=None):
    ...
    if output_path is None:
        output_path = (
            REPORTS_DIR / f"performance_report_{datetime.now().strftime('%Y%m%d')}.md"
        )
...
def print_analysis(days=30):
    ...
    for item in analysis["top_content_types"][:3]:
        print(
            f"  {item['type']:20s}  Score: {item['score']:6.1f}  Views: {item['avg_views']:6,}  Revenue: ${item['total_revenue']:6.2f}"
        )

    print("\n📱 Top Platforms:")
    for item in analysis["top_platforms"][:3]:
        print(
            f"  {item['platform']:20s}  Score: {item['score']:6.1f}  Views: {item['avg_views']:6,}  Revenue: ${item['total_revenue']:6.2f}"
        )
...
if __name__ == "__main__":
    if "--track" in sys.argv:
        ...
        if len(sys.argv) < 5:
            print(
                "Usage: feedback_loop.py --track <content_id> <type> <platform> <views> [likes] [shares] [revenue]"
            )
            print(
