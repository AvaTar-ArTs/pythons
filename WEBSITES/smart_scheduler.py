...
def export_to_buffer(schedule, output_path=None):
    ...
    if output_path is None:
        output_path = (
            SCHEDULER_DIR / f"buffer_schedule_{datetime.now().strftime('%Y%m%d')}.csv"
        )
...
def log_post_performance(platform, posted_at, views, engagement_rate):
    ...
    with open(csv_path, "a", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([posted_at, views, engagement_rate, datetime.now().isoformat()])
...
if __name__ == "__main__":
    if "--log" in sys.argv:
        ...
        if len(sys.argv) < 5:
            print(
                "Usage: smart_scheduler.py --log <platform> <posted_at> <views> <engagement_rate>"
            )
            sys.exit(1)
        ...
        platforms = (
            sys.argv[1:]
            if len(sys.argv) > 1
            else ["instagram", "tiktok", "youtube", "twitter"]
        )
        days = 7
