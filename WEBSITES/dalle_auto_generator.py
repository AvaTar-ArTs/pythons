...
def generate_dalle_image(prompt, size="1024x1024", quality="standard", n=1):
    ...
    try:
        ...
        return {"url": image_url, "revised_prompt": revised_prompt, "data": img_data}
...
def generate_product_mockups(design_theme, products=None):
    ...
    for product in products:
        ...
        if result:
            ...
            results.append({"product": product, "filepath": str(filepath)})
...
""")
    ...
    if command == "daily":
        ...
        manifest_file = (
            OUTPUT_DIR
            / f"series_{theme.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.json"
        )
