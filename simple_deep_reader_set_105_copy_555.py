#!/usr/bin/env python3
"""
Simple Deep Reader for Dr. Adu SEO Project
...
class SimpleDrAduReader:
    ...
    def search_for_dr_adu_content(self):
        ...
        print("🔍 Deep searching for Dr. Adu SEO content...")

        if not self.source_dir.exists():
            print(f"❌ Source directory not found: {self.source_dir}")
            print(f"   Please check the path: {self.source_dir}")
            return
        ...
        for file_path in self.source_dir.rglob("*"):
            if file_path.is_file() and file_path.suffix in [
                ...
                print(f"   📄 Scanning: {file_path.relative_to(self.source_dir)}")
                ...
                try:
                    ...
                    if relevance_score > 0:
                        ...
                        print(
                            f"      ✅ Found relevant content (score: {relevance_score})"
                        )
    ...
    def calculate_relevance_score(self, content):
        ...
        # High priority terms (exact matches)
        high_priority = [
            "dr. adu",
            "dr lawrence adu",
            "lawrence adu",
    ...
    def extract_dr_adu_content(self):
        ...
        for file_info in self.found_files:
            try:
                ...
                print(f"   ❌ Error extracting from {file_info['file_path']}: {e}")
    ...
    def organize_by_content_type(self):
        ...
        for content_info in self.dr_adu_content:
            ...
            if any(
                term in source_file for term in ["invoice", "pricing", "cost", "value"]
            ):
                content_groups["invoices"].append(content_info)
            elif any(
                term in source_file
                for term in ["technical", "implementation", "code", "html"]
            ):
                content_groups["technical"].append(content_info)
            elif any(
                term in source_file for term in ["client", "non_technical", "business"]
            ):
                content_groups["client_facing"].append(content_info)
            elif any(
                term in source_file
                for term in ["seo", "analysis", "audit", "optimization"]
            ):
                content_groups["seo_analysis"].append(content_info)
            elif any(
                term in source_file for term in ["comparison", "visual", "before_after"]
            ):
    ...
    def create_organized_files(self, content_groups):
        ...
        for group_name, content_list in content_groups.items():
            ...
            f"DR_ADU_{group_name.upper()}_COMPREHENSIVE.md"
            ...
            with open(file_path, "w", encoding="utf-8") as f:
                ...
                for i, content_info in enumerate(content_list, 1):
                    ...
                    f.write(
                        f"**Matched Terms:** {', '.join(content_info['file_metadata']['matched_terms'])}\n\n"
                    )

                    for j, section in enumerate(content_info["sections"], 1):
                        f.write(
                            f"### Section {j} (Score: {section['relevance_score']})\n"
                        )
    ...
    def copy_relevant_files(self):
        ...
        print("📋 Copying most relevant files...")

        # Copy top 10 most relevant files
        top_files = self.found_files[:10]
    ...
    def create_master_index(self):
        ...
        with open(index_path, "w", encoding="utf-8") as f:
            ...
            f.write("## Most Relevant Files (Top 15)\n")
            for i, file_info in enumerate(self.found_files[:15], 1):
                f.write(
                    f"{i}. **{file_info['relative_path']}** (Score: {file_info['relevance_score']})\n"
                )
                f.write(
                    f"   - Matched Terms: {', '.join(file_info['matched_terms'])}\n"
                )
    ...
    def generate_analysis_report(self):
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- **Relevant Files Found:** {len([f for f in self.found_files if f['relevance_score'] > 0])}\n"
            )
            f.write(f"- **Content Extracts:** {len(self.dr_adu_content)}\n\n")
    ...
    def run_analysis(self):
        ...
        print("🚀 Starting Deep Read and Organization for Dr. Adu SEO Project...")
...
def main():
    ...
    target_dir = "/Users/steven/Dr_Adu_GainesvillePFS_SEO_Project/ORGANIZED_CONTENT"

    analyzer = SimpleDrAduReader(source_dir, target_dir)
