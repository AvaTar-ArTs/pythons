...
Output JSON: [{{"keyword": "...", "growth": 80, "upvotes_trend": "rising"}}]"""
    ...
    def _combine_and_score_trends(:
        ...
        for trend in google:
            ...
            if keyword:
                ...
                keyword_data[keyword]["growth_rates"].append(trend.get("growth", 75))
                keyword_data[keyword]["competition"] = trend.get(
                    "competition", "medium"
                )
        ...
        for keyword, data in keyword_data.items():
            ...
            avg_growth = (
                sum(data["growth_rates"]) / len(data["growth_rates"])
            ...
            competition_penalty = {"low": 0, "medium": -5, "high": -15}.get(
                data["competition"], -5
            )

            # Calculate final score
            trend_score = (
                avg_growth + platform_bonus + mention_bonus + competition_penalty
            )
    ...
    def generate_trending_content(:
        self, trend: HotTrend, content_type: str = "youtube", use_assets: bool = True
...
Generate 3 title options, then pick the BEST one. Output ONLY the final title."""
    ...
    def _generate_trending_description(:
        self, trend: HotTrend, title: str, content_type: str
    ) -> str:
...
"""
    ...
    def _generate_hashtags(self, trend: HotTrend, content_type: str) -> List[str]:
        ...
        # Trending indicators
        hashtags.extend(
            ["#Trending", "#TrendingNow", "#HotTrending", "#Viral", "#GoingViral"]
        )
    ...
    def _generate_trending_tags(self, trend: HotTrend, content_type: str) -> List[str]:
        ...
        tags.extend(
            type_tags.get(content_type, ["content", "video", "trending content"])
        )
...
"""
    ...
    def _calculate_content_seo_score(:
        self, title: str, description: str, tags: List[str], trend: HotTrend
    ) -> float:
    ...
    def _calculate_aeo_score(:
        self, title: str, description: str, trend: HotTrend
    ) -> float:
        ...
        if (
            "•" in description
            or "-" in description
            or any(str(i) in desc_lower for i in range(1, 10))
        ):
            score += 10
    ...
    def _analyze_keyword_density(:
        self, description: str, trend: HotTrend
    ) -> Dict[str, float]:
    ...
    def _generate_related_keywords(:
        self, trend: HotTrend, content_type: str
    ) -> List[str]:
    ...
    def _estimate_performance(:
        self, trend: HotTrend, seo_score: float, aeo_score: float
    ) -> Dict[str, float]:
        ...
        # Competition factor
        competition_factor = {"low": 1.5, "medium": 1.0, "high": 0.7}.get(
            trend.competition, 1.0
        )
        ...
        seo_ctr_boost = (
            (seo_score - 70) / 10 if seo_score > 70 else 0
        )  # Bonus for excellent SEO
        ...
        aeo_retention_boost = (
            aeo_score / 5 if aeo_score > 70 else 0
        )  # AEO content is more engaging
        estimated_retention = (
            base_retention + trend_retention_boost + aeo_retention_boost
        )
    ...
    def save_content_package(self, package: TrendingContentPackage, output_dir: Path):
        ...
        # Sanitize filename
        safe_keyword = "".join(
            c for c in package.trend.keyword if c.isalnum() or c in (" ", "-", "_")
        ).rstrip()
...
def main():
    ...
    parser.add_argument(
        "--niche",
        default="content creation, AI, music, art, automation, technology",
        help="Content niche to analyze",
    )
    parser.add_argument(
        "--min-score", type=float, default=75.0, help="Minimum trend score (0-100)"
    )
    parser.add_argument(
        "--max-results", type=int, default=5, help="Maximum number of trends to return"
    )
    parser.add_argument(
        "--generate",
        action="store_true",
    ...
    trends = engine.discover_hot_trends(
        niche=args.niche, min_trend_score=args.min_score, max_results=args.max_results
    )
    ...
    if args.generate:
        ...
        for trend in trends[:3]:  # Generate for top 3
            ...
            print(
                f"   Combined Score: {package.estimated_performance['combined_score']:.1f}/100"
            )
            print(
                f"   Estimated Views (30d): {package.estimated_performance['estimated_views_30d']:,.0f}"
            )
            print(
                f"   Estimated CTR: {package.estimated_performance['estimated_ctr']:.1f}%"
            )
            print(
                f"   Estimated Retention: {package.estimated_performance['estimated_retention']:.1f}%"
            )
            print(
                f"   Keyword Density: {package.keyword_density.get('primary_keyword', 0):.2f}%"
            )
