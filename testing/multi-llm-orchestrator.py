...
class AIOrchestrator:
    ...
    def _initialize_models(self) -> Dict[str, AIModel]:
        ...
        if os.getenv("COHERE_API_KEY"):
            models["cohere"] = AIModel(
                ...
                strengths=[TaskType.CLASSIFICATION, TaskType.EMBEDDINGS],
    ...
    async def multi_model_consensus(:
        self, prompt: str, task_type: TaskType, num_models: int = 3
        ...
        tasks = [self.query_model(model_key, prompt) for model_key in selected_models]
        results = await asyncio.gather(*tasks)

        # Find best result (highest confidence, lowest cost)
        best_result = max(
            results,
            key=lambda r: r.confidence * 0.7
            + (1 - r.cost / max(1, max(res.cost for res in results))) * 0.3,
        )

        return best_result.response, results

    def analyze_codebase(self, directory: str, max_files: int = 100) -> Dict[str, Any]:
        """
    ...
    def generate_documentation(self, code: str, doc_type: str = "comprehensive") -> str:
        """
...
def main():
    ...
    for key, model in orchestrator.models.items():
        ...
        print(
            f"{status} {model.name:20} | Speed: {model.speed_score}/10 | Quality: {model.quality_score}/10"
        )
    ...
    best_model = orchestrator.select_best_model(
        TaskType.CODE_GENERATION, priority="quality"
    )
    ...
    best_model = orchestrator.select_best_model(
        TaskType.FAST_INFERENCE, priority="speed"
    )
    print(f"   → Selected: {orchestrator.models[best_model].name}\n")
