...
class AIReceptionist:
        ...
        try:
            ...
            """,
                (
                    ...
                    client.industry,
                    client.phone_number,
                    json.dumps(client.business_hours),
                    json.dumps(client.services),
                    json.dumps(client.pricing),
                    client.ai_personality,
                    json.dumps(client.custom_responses),
                    client.monthly_fee,
                    client.is_active,
                    client.created_at,
                    client.updated_at,
        ...
        try:
            ...
            if row:
                return BusinessClient(
                    ...
                    industry=row[2],
                    phone_number=row[3],
                    business_hours=json.loads(row[4]),
                    services=json.loads(row[5]),
                    pricing=json.loads(row[6]),
                    ai_personality=row[7],
                    custom_responses=json.loads(row[8]),
                    monthly_fee=row[9],
    ...
    def get_business_analytics(self, client_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            appointments_booked = len(
                [log for log in call_logs if log.outcome == "appointment_booked"]
            )
            leads_captured = len(
                [log for log in call_logs if log.outcome == "lead_captured"]
            )

            conversion_rate = (
                (appointments_booked / total_calls * 100) if total_calls > 0 else 0
            )
