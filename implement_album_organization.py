...
class MusicCollectionOrganizer:
    ...
    def organize_collection(self):
        """Main method to organize the music collection"""
        print("Starting music collection organization...")

        # Create target directories
        major_dir = self.target_dir / "MAJOR_COLLECTIONS"
        medium_dir = self.target_dir / "MEDIUM_COLLECTIONS"
        small_dir = self.target_dir / "SMALL_COLLECTIONS"
        single_dir = self.target_dir / "SINGLE_TRACKS"
        support_dir = self.target_dir / "SUPPORTING_CONTENT"
        ...
        file_groups = self.group_files_by_normalized_title()
        ...
        for normalized_title, files in file_groups.items():
            ...
            for file_path in files:
                standard_name = self.get_standard_filename(file_path)
                ...
                # Log the mapping
                self.mapping_log[str(file_path)] = str(target_path)
        ...
        print(f"\nNew organization created at: {self.target_dir}")

        # Save mapping log
        mapping_file = self.target_dir / "original_to_new_mapping.csv"
        with open(mapping_file, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["Original_Path", "New_Path"])
            for orig, new in self.mapping_log.items():
...
def main():
    ...
    target_directory = "/Users/steven/Music/nocTurneMeLoDieS/MUSIC_ALBUMS_NEW"

    organizer = MusicCollectionOrganizer(source_directory, target_directory)
    organizer.organize_collection()
