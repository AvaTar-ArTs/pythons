#!/usr/bin/env python3
"""
Accurate script to rename UUID-named directories and files to proper song titles
...
def parse_suno_csv_for_mappings(csv_file_path):
    ...
    # Looking for patterns like: UUID,,,,title, or UUID,title,
    for line in lines:
        if not line.strip():
            continue

        # First, try to match UUID followed by 4+ commas and then a title
        ...
        for uuid, title_candidate in matches1:
        ...
        # Second, try to match UUID followed by a title in the second position
        # Example: 01bbae67-5f8f-4285-8926-24545afe184e,stitch by stitch4,description...
        pattern2 = r'([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}),([^,\n"]+?)[,]'
        matches2 = re.findall(pattern2, line)

        for uuid, title_candidate in matches2:
            # Clean up the title candidate
            title_candidate = re.sub(r'^"|"$', "", title_candidate.strip())
            title_candidate = re.sub(r"^\'|\'$", "", title_candidate.strip())
