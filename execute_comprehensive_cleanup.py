...
class ComprehensiveCleanup:
    ...
    def __init__(self, base_dir: Path):
        ...
        self.consolidations = {
            'video-downloader': [
                'youtube-downloader', 'tiktok-video-downloader',
                'Auto-YouTube', 'youtube-python',
                'scrape-youtube-channel-videos-url'
            ],
            'video-generator': [
                'videoGenerator', 'Automatic-Video-Generator-for-youtube',
                'reddit_video_maker', 'RedditVideoMakerBot-master',
                'redditVideoGenerator', 'TikTok-Compilation-Video-Generator',
                'tiktok-generator'
            ],
            'bot-automation': [
                'instagram-follower-scraper', 'InstagramReportBot',
                'InstaReport', 'Instagram-Mass-report', 'botty',
                'FB-Script-Auto-Post-All-Group'
            ],
            'gallery-generator': [
                'simplegallery', 'simplegallery-bin',
                'netlify-gallery-deployer'
            ],
            'web-scraper': [
                'web-scraping', 'web_scraping',
                'selenium-image-scraper', 'fiverr-scraping-api'
            ],
            'transcribe-analysis': [
                'transcription_analyzer', 'audio-transcriber'
            ],
            'upscaler': [
                'upscale-python'
            ],
            'file-organizer': [
                'Python-organize', 'clean-organizer'
            ],
            'backup-tool': [
                'archive-utility'
            ],
        ...
        self.terrible_dirs = [
            'af', 'b2', 'b6', 'b8', 'ba', 'bb', 'bd', 'bf',
            'c2', 'c4', 'c7', 'c8', 'cb',
            'd0', 'd5', 'd6', 'd8', 'dd',
            'e0', 'e2', 'e6', 'e7', 'e9', 'ea',
            'f1', 'f2', 'f3', 'f6', 'f7', 'f8', 'f9', 'fb', 'fc', 'fd', 'fe'
        ]
    ...
    def consolidate_redundant_groups(self, dry_run=True):
        ...
        for target_dir, source_dirs in self.consolidations.items():
            ...
            for source_dir in existing_sources:
                ...
                if dry_run:
                    ...
                    print(f"   [DRY RUN] Would merge: {source_dir} ({file_count} items)")
                else:
        ...
        print(f"   Total: {total_consolidated} directories {'would be' if dry_run else ''} consolidated")
        print()
