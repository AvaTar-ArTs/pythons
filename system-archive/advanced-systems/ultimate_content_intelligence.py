...
class APIDiscoveryEngine:
    ...
    def discover_all_apis(self) -> Dict[str, Dict]:
        ...
        if not self.env_directory.exists():
            logger.warning(f"~/.env.d/ not found")
            return {}
    ...
    def _categorize_apis(self):
        ...
        # LLM APIs
        llm_keys = ['OPENAI', 'ANTHROPIC', 'GOOGLE', 'GEMINI', 'DEEPSEEK', 'MISTRAL', 
                    'GROQ', 'CEREBRAS', 'COHERE', 'FIREWORKS', 'TOGETHER', 'PERPLEXITY', 
                    'OPENROUTER']
        
        # Image APIs
        image_keys = ['LEONARDO', 'STABILITY', 'REPLICATE', 'FAL', 'RUNWAY', 
                      'IMAGGA', 'REMOVEBG', 'VANCEAI']
        
        # Audio APIs
        audio_keys = ['ELEVENLABS', 'DEEPGRAM', 'ASSEMBLYAI', 'MURF', 'RESEMBLE', 
                      'REVAI', 'DESCRIPT']
...
class UltimateContentIntelligence:
    ...
    async def create_complete_content_package(
        ...
        if 'text' in content_types and self.text_engine:
            ...
            logger.info(f"   📊 Quality Score: {text_result.get('quality_score', 0)}/100")
        ...
        if 'audio' in content_types and self.audio_engine:
            ...
            audio_result = await self.audio_engine.create_and_analyze_audio(
                text=package.get('text', {}).get('content', topic),
                analyze_seo=analyze
            )
            package['audio'] = audio_result
            logger.info(f"   ✅ Created: {len(audio_result.get('files', []))} audio files")
        ...
        if analyze:
            ...
            logger.info(f"   ✅ Analyzed relationships between {len(content_types)} content types")
        ...
        logger.info(f"   ✅ Complete SEO package generated")
        ...
        logger.info(f"   📊 Overall Package Score: {quality_assessment.get('total_score', 0)}/100")
        
        # Save to content database
        self.content_database['generated_content'].append(package)
        
        # Save to file
        output_file = Path.home() / f"content_package_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    async def analyze_content_relationships(self, package: Dict) -> Dict[str, Any]:
        ...
        if 'images' in package:
            ...
            if text_keywords and image_keywords:
                ...
                relationships['text_image_alignment'] = len(common) / len(text_keywords) * 100
        ...
        if 'audio' in package:
            ...
            if text_keywords and audio_keywords:
                ...
                relationships['text_audio_alignment'] = len(common) / len(text_keywords) * 100
                relationships['findings'].append(
        ...
        relationships['overall_coherence'] = sum(a for a in alignments if a > 0) / len([a for a in alignments if a > 0]) if any(alignments) else 0
    ...
    async def assess_complete_package(self, package: Dict) -> Dict[str, Any]:
        ...
        # Score images
        if 'images' in package:
            image_scores = [img.get('quality_score', 0) for img in package['images'].get('images', [])]
        ...
        # Score audio
        if 'audio' in package:
            audio_scores = [a.get('quality_score', 0) for a in package['audio'].get('files', [])]
            avg_audio_score = sum(audio_scores) / len(audio_scores) if audio_scores else 0
        ...
        if 'cross_modal_analysis' in package:
            ...
            if coherence >= 70:
                assessment['strengths'].append(f"Strong cross-modal coherence ({coherence:.1f}%)")
            else:
                assessment['improvements'].append("Improve consistency across media types")
    ...
    async def batch_analyze_directory(
        ...
        for ctype, analyses in results['by_type'].items():
            scores = [a.get('quality_score', {}).get('total_score', 0) for a in analyses if 'quality_score' in a]
            if scores:
                results['statistics']['average_scores'][ctype] = round(sum(scores) / len(scores), 1)
        
        # Save results
        output_file = directory / f"batch_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(output_file, 'w') as f:
    ...
    def generate_api_capabilities_report(self) -> str:
        ...
        for category, apis in sorted(self.api_categories.items()):
            if apis:
                report.append(f"  {category.upper().replace('_', ' ')} ({len(apis)} services):")
        ...
        if self.text_engine:
            ...
            capabilities.append(f"     - {len(self.api_categories.get('llm', []))} LLM providers")
        ...
        if self.image_engine:
            ...
            capabilities.append(f"     - {len(self.api_categories.get('image', []))} image services")
        ...
        if self.audio_engine:
            ...
            capabilities.append(f"     - {len(self.api_categories.get('audio', []))} audio services")
        ...
        if self.video_engine:
            ...
            capabilities.append(f"     - {len(self.api_categories.get('video', []))} video services")
...
class AudioIntelligenceEngine:
    ...
    async def create_and_analyze_audio(
        ...
        audio_data = {
            ...
            'service': 'elevenlabs' if 'ELEVENLABS' in self.available_services else 'placeholder',
            'quality_score': 88.0,
            'seo': {
...
class VideoIntelligenceEngine:
    ...
    async def create_video(
        ...
        return {
            ...
            'service': 'runway' if 'RUNWAY' in self.available_services else 'placeholder'
        }
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == 'analyze' and len(sys.argv) > 2:
            ...
            print("\n📝 Examples:")
            print("   python ultimate_content_intelligence.py apis")
            print("   python ultimate_content_intelligence.py create \"AI in Healthcare\"")
