...
class AdvancedContentPipeline:
    ...
    async def generate_content_score(self, content: str, content_type: str = "blog_post") -> Dict[str, Any]:
        """
    ...
    def _calculate_engagement_score(self, content: str) -> float:
        ...
        cta_keywords = ['click', 'subscribe', 'join', 'learn more', 'try', 'discover', 'explore']
        cta_count = sum(1 for keyword in cta_keywords if keyword in content.lower())
        score += min(cta_count * 2, 5)
        
        # Check for emotional words
        emotional_words = ['amazing', 'incredible', 'powerful', 'essential', 'revolutionary', 'transform']
    ...
    def _calculate_structure_score(self, content: str, content_type: str) -> float:
        ...
        if len(paragraphs) >= 3:
            ...
            avg_sentences = sum(p.count('.') + p.count('!') + p.count('?') for p in paragraphs) / len(paragraphs)
        ...
        opening = content[:150]
        if any(word in opening.lower() for word in ['imagine', 'discover', 'learn', 'what if', '?']):
            score += 5
        
        # Closing strength (has clear conclusion)
        closing = content[-200:].lower()
        if any(word in closing for word in ['conclusion', 'summary', 'remember', 'action', 'start']):
            score += 5
    ...
    async def extract_keywords(self, content: str, count: int = 10) -> List[Tuple[str, int]]:
        """
        ...
        stop_words = {
            ...
            'we', 'they', 'what', 'who', 'when', 'where', 'why', 'how', 'all',
    ...
    async def generate_ab_test_variants(self, content: str, variations: int = 2) -> List[Dict[str, Any]]:
        """
    ...
    async def generate_seo_metadata(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if "anthropic" in self.clients:
                ...
                titles = ["Title suggestion 1", "Title suggestion 2", "Title suggestion 3"]
    ...
    async def analyze_audience_fit(self, content: str, target_audience: str) -> Dict[str, Any]:
        """
        ...
        try:
            if "anthropic" in self.clients:
                ...
                return {
                    ...
                    'recommendation': 'Good fit' if avg_score >= 7 else 'Needs adjustment'
                }
        except Exception as e:
...
async def main():
    ...
    logger.info(f"   Score: {content_score.get('total_score')}/100 - {content_score.get('grade')}")
    ...
    logger.info(f"   Top keywords: {', '.join([f'{kw[0]} ({kw[1]})' for kw in keywords[:5]])}")
    ...
    if 'title_options' in seo_metadata:
        logger.info(f"   Title options:")
        for i, title in enumerate(seo_metadata['title_options'][:3], 1):
            logger.info(f"     {i}. {title}")
    logger.info(f"   Meta description: {seo_metadata.get('meta_description', 'N/A')[:80]}...")
    ...
    for platform in ['twitter', 'linkedin', 'instagram']:
        platform_opt = await pipeline.optimize_for_platform(text_content[:500], platform)
        result[f'{platform}_optimized'] = platform_opt
        logger.info(f"   {platform.title()}: {platform_opt.get('optimized_length', 0)} chars")
    
    # STEP 6: Generate A/B test variants
    logger.info("\n🔬 Generating A/B test variants...")
    ab_variants = await pipeline.generate_ab_test_variants(text_content[:300], variations=2)
    ...
    target_audiences = ['tech-savvy creators', 'marketing professionals', 'small business owners']
    audience_analyses = {}
    ...
    logger.info("\n📊 GENERATION SUMMARY:")
    logger.info(f"   Content Length: {len(text_content)} chars / {len(text_content.split())} words")
