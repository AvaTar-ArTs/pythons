...
class ImageIntelligence:
    ...
    def initialize_clients(self):
        ...
        if not self.clients:
            logger.warning("⚠️  No AI vision clients available. Image analysis will be limited.")
    ...
    async def analyze_image(self, image_path: Path) -> Dict[str, Any]:
        ...
        # SEO optimization
        result['seo'] = await self._generate_seo_metadata(image_path, result['ai_analysis'])
    ...
    async def _analyze_with_gpt4_vision(self, image_path: Path, image_data: str) -> Dict[str, Any]:
    ...
    async def _generate_seo_metadata(self, image_path: Path, ai_analysis: Dict) -> Dict[str, Any]:
        """Generate SEO-optimized metadata for image"""
        seo = {}

        # Alt text (crucial for accessibility and SEO)
        seo['alt_text'] = ai_analysis.get('alt_text') or self._generate_alt_text(image_path, ai_analysis)
    ...
    def _extract_keywords(self, analysis: Dict) -> List[str]:
        ...
        if 'objects' in analysis:
            objects = analysis['objects'].split(',') if isinstance(analysis['objects'], str) else []
    ...
    def _generate_recommendations(self, analysis: Dict) -> List[str]:
        ...
        if width < 1200:
            recommendations.append("⚠️ Resolution: Consider using higher resolution (min 1200px wide) for better quality")

        if size_mb > 1:
            recommendations.append("🗜️ Optimization: File size is large. Compress to under 1MB for faster loading")

        if technical.get('format') not in ['WEBP', 'PNG', 'JPEG']:
            recommendations.append("🔄 Format: Convert to WebP or JPEG for better web compatibility")

        # SEO recommendations
        alt_text = seo.get('alt_text', '')
        if len(alt_text) < 20:
            recommendations.append("📝 Alt Text: Add more descriptive alt text (50-125 characters recommended)")

        if len(seo.get('keywords', [])) < 5:
            recommendations.append("🏷️ Keywords: Add more relevant keywords for better SEO")
        ...
        if suggested_name and suggested_name != current_name.name:
            recommendations.append(f"📁 Filename: Rename to '{suggested_name}' for better SEO")

        # Quality-based recommendations
        if quality.get('total_score', 0) < 70:
            recommendations.append("🎯 Overall: Image needs optimization for web use. Focus on SEO metadata first.")

        if not recommendations:
            recommendations.append("✅ Image is well-optimized! No major improvements needed.")
    ...
    async def analyze_directory(
        ...
        # Save results
        output_file = directory / f"image_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        # Demo mode
        print("\n💡 Usage:")
        print("   python image_intelligence_seo.py <image_file>     # Analyze single image")
        print("   python image_intelligence_seo.py <directory>      # Analyze directory")
        print("   python image_intelligence_seo.py <directory> -r   # Recursive analysis")
        print("\n📝 Example:")
