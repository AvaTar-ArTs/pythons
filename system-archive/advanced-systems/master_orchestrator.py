...
"""
    ...
    async def execute_intelligent_task(
        ...
        if any(word in task_lower for word in ['generate', 'create', 'write', 'content']):
        ...
        elif any(word in task_lower for word in ['document', 'search', 'find', 'knowledge']):
    ...
    async def _route_to_omniscient(self, task: str, context: Dict) -> Dict:
        ...
        if self.omniscient:
            return await self.omniscient.intelligent_route(task, context.get('input_data'))
...
async def interactive_menu():
    ...
    while True:
        ...
        if choice == '1':
            ...
            result = await orchestrator.execute_intelligent_task(f"create content about {topic}")
            print(f"\n✅ Result: {result}")
        
        elif choice == '2':
            path = input("🖼️  Image directory path: ")
            result = await orchestrator.execute_intelligent_task(
                f"analyze images in directory",
            ...
            if orchestrator.document_intelligence:
                results = await orchestrator.document_intelligence.semantic_search(query)
                print(f"\n✅ Found {len(results)} relevant documents")
                for i, doc in enumerate(results[:5], 1):
                    print(f"   {i}. {doc['name']} (relevance: {doc['relevance_score']:.2f})")
