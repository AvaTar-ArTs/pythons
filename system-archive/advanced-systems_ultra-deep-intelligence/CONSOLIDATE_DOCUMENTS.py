...
class DocumentsConsolidator:
    ...
    def consolidate_folders(self):
        ...
        for target_folder, source_folders in self.consolidation_plan.items():
            ...
            existing_sources = [f for f in source_folders
                               if (self.docs_path / f).exists()]
            ...
            for source in existing_sources:
                ...
                print(f"{C.CYAN}     Files: {file_count:,}, Size: {size / (1024**2):.1f} MB{C.END}")
...
"""
    ...
    def generate_consolidation_report(self):
        ...
        report_path = Path(f"/Users/steven/advanced-systems/ultra-deep-intelligence/reports/CONSOLIDATION_REPORT_{timestamp}.md")
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"| **Space Organized** | {self.stats['space_organized'] / (1024**2):.2f} MB |\n\n")
    ...
    def run(self):
        ...
        print("╔═══════════════════════════════════════════════════════════════════════════════╗")
        print("║                                                                               ║")
        print("║           🔄 DOCUMENTS CONSOLIDATION SYSTEM 🔄                               ║")
        print("║                                                                               ║")
        print("║        Intelligent Organization Based on Deep Analysis                        ║")
        print("║                                                                               ║")
        print("╚═══════════════════════════════════════════════════════════════════════════════╝")
        print(f"{C.END}\n")
        ...
        if self.dry_run:
            ...
            print(f"{C.CYAN}💡 Use --execute flag to perform actual consolidation{C.END}\n")
        ...
        print(f"  📁 Folders to merge: {C.BOLD}{sum(len(v) for v in self.consolidation_plan.values())}{C.END}")
        print(f"  🗑️  Empty dirs to remove: {C.BOLD}4{C.END}")
        print(f"  🗂️  New structure folders: {C.BOLD}{len(self.consolidation_plan)}{C.END}")
