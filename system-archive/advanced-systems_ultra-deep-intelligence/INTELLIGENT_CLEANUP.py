...
class IntelligentCleanup:
    ...
    def __init__(self):
        ...
        self.backup_path = Path("/Users/steven/Documents/Backups/cleanup_backup_" +
    ...
    def find_duplicates(self):
        ...
        for file_path in self.docs_path.rglob('*'):
            if file_path.is_file():
                ...
                if any(skip in str(file_path) for skip in ['.git', 'node_modules', '__pycache__', '.venv', 'Library']):
                    continue
        ...
        print(f"{C.GREEN}✅ Found {len(self.cleanup_targets['duplicates'])} duplicate files{C.END}\n")
        ...
        for pattern in temp_patterns:
            for file_path in self.docs_path.rglob(pattern):
                if file_path.is_file():
                    ...
                    if 'important' in str(file_path).lower() or 'keep' in str(file_path).lower():
                        continue

                    # Add large log files (>10MB)
                    if file_path.suffix == '.log' and file_path.stat().st_size > 10_000_000:
                        self.cleanup_targets['large_logs'].append(file_path)
                    else:
                        self.cleanup_targets['temp_files'].append(file_path)

        print(f"{C.GREEN}✅ Found {len(self.cleanup_targets['temp_files'])} temp files{C.END}")
        print(f"{C.YELLOW}   Found {len(self.cleanup_targets['large_logs'])} large log files{C.END}\n")
    ...
    def find_empty_directories(self):
        ...
        for dir_path in self.docs_path.rglob('*'):
            if dir_path.is_dir():
                ...
                if any(skip in str(dir_path) for skip in ['.git', 'node_modules', '__pycache__']):
                    continue
        ...
        print(f"{C.GREEN}✅ Found {len(self.cleanup_targets['empty_dirs'])} empty directories{C.END}\n")
    ...
    def find_old_scripts(self):
        ...
        print(f"{C.GREEN}✅ Found {len(self.cleanup_targets['old_scripts'])} old scripts{C.END}\n")
    ...
    def perform_cleanup(self):
        ...
        if self.dry_run:
            print(f"{C.YELLOW}{E.WARN} DRY RUN MODE - No files will be deleted{C.END}\n")

        # Clean duplicates
        print(f"{C.CYAN}Removing {len(self.cleanup_targets['duplicates'])} duplicates...{C.END}")
        ...
        # Clean temp files
        print(f"{C.CYAN}Removing {len(self.cleanup_targets['temp_files'])} temp files...{C.END}")
        ...
        print(f"{C.CYAN}Removing {len(self.cleanup_targets['empty_dirs'])} empty directories...{C.END}")
    ...
    def generate_report(self):
        ...
        report_path = Path("/Users/steven/advanced-systems/ultra-deep-intelligence/reports/CLEANUP_REPORT_" +
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"| **Space Freed** | {self.stats['space_freed'] / (1024**2):.2f} MB |\n")
            ...
            if len(self.cleanup_targets['duplicates']) > 20:
                f.write(f"- ... and {len(self.cleanup_targets['duplicates']) - 20} more\n")
            f.write("\n")
            ...
            for ext, count in sorted(temp_by_type.items(), key=lambda x: x[1], reverse=True):
                f.write(f"- `{ext}`: {count} files\n")
            f.write("\n")

            f.write(f"### Empty Directories ({len(self.cleanup_targets['empty_dirs'])})\n")
    ...
    def run(self, dry_run: bool = True):
        ...
        print("╔═══════════════════════════════════════════════════════════════════════════════╗")
        print("║                                                                               ║")
        print("║           🧹 INTELLIGENT CLEANUP SYSTEM 🧹                                    ║")
        print("║                                                                               ║")
        print("║        Safe, Smart, and Comprehensive Cleanup                                 ║")
        print("║                                                                               ║")
        print("╚═══════════════════════════════════════════════════════════════════════════════╝")
        print(f"{C.END}\n")

        if dry_run:
            print(f"{C.YELLOW}{E.WARN} Running in DRY RUN mode - no files will be deleted{C.END}")
        ...
        if not dry_run and (self.cleanup_targets['duplicates'] or
                           self.cleanup_targets['temp_files'] or
                           self.cleanup_targets['old_scripts']):
            self.create_backup()
        ...
        print(f"  {E.FILE} Files to remove: {C.BOLD}{len(self.cleanup_targets['duplicates']) + len(self.cleanup_targets['temp_files'])}{C.END}")
        print(f"  {E.FOLDER} Directories to remove: {C.BOLD}{len(self.cleanup_targets['empty_dirs'])}{C.END}")
        print(f"  💾 Space to free: {C.BOLD}{sum(f.stat().st_size for f in self.cleanup_targets['duplicates'] + self.cleanup_targets['temp_files']) / (1024**2):.2f} MB{C.END}")
