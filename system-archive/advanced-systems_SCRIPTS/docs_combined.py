...
def compare_csvs(old_csvs: List[str], new_csv: str, output_dir: str = ".") -> None:
    ...
    logger.info(f"Rows in BOTH that have changed columns: {rows_changed_in_both.shape[0]}")
    ...
    rows_changed_in_both.to_csv(os.path.join(output_dir, "docs_modified.csv"), index=False)
