...
class ContentManagementSystem:
    ...
    def analyze_all_content(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for system_key, system_name in self.systems.items():
                ...
                analysis_results["total_content_items"] += system_results["total_content_items"]
    ...
    def _is_content_file(self, filename: str) -> bool:
        ...
        content_extensions = {'.md', '.txt', '.html', '.py', '.js', '.json', '.yml', '.yaml'}
    ...
    def _calculate_seo_score(self, content: str) -> float:
        ...
        try:
            ...
            # Check for title
            if re.search(r'<title>', content, re.IGNORECASE) or re.search(r'^#\s+', content):
                score += 20
            ...
            # Check for headings
            heading_count = len(re.findall(r'<h[1-6]>', content, re.IGNORECASE)) + len(re.findall(r'^#+\s+', content))
        ...
        try:
            ...
            for hash_val, group_items in hash_groups.items():
                if len(group_items) > 1:
                    ...
                    recommended_action = self._determine_recommended_action(content_items, similarity_score)
    ...
    def _determine_recommended_action(self, items: List[ContentItem], similarity_score: float) -> str:
...
"""
            ...
            for system_name, system_data in analysis.get("system_breakdown", {}).items():
                report += f"""
### {system_name}
...
"""
            ...
            for i, group in enumerate(analysis.get("duplicate_groups", [])[:10]):  # Show first 10
                report += f"""
