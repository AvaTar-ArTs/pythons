...
class IntelligentEnvironmentAnalyzer:
    ...
    def _analyze_env_structure(self):
        ...
        if len(env_files) > 10:
            env_analysis['optimization_opportunities'].append("Consider consolidating environment files")
    ...
    def _analyze_python_projects(self):
        ...
        for py_file in python_path.rglob("*.py"):
            if py_file.is_file():
                ...
                revenue_keywords = ['revenue', 'income', 'profit', 'monetize', 'subscription', 'pricing']
                if any(keyword in content.lower() for keyword in revenue_keywords):
                    python_analysis['revenue_potential'].append({
                        'file': str(py_file),
                        'keywords_found': [kw for kw in revenue_keywords if kw in content.lower()]
                    })
        
        # Optimization opportunities
        if python_analysis['total_projects'] > 50:
            python_analysis['optimization_opportunities'].append("Consider consolidating similar projects")
        
        if len(python_analysis['ai_tools']) > 10:
            python_analysis['optimization_opportunities'].append("Create AI tools marketplace")
    ...
    def _analyze_html_projects(self):
        ...
        for html_file in html_path.rglob("*.html"):
            if html_file.is_file():
                ...
                try:
                    ...
                    revenue_indicators = ['buy', 'purchase', 'price', 'subscription', 'contact']
                    if any(indicator in content.lower() for indicator in revenue_indicators):
                        html_analysis['revenue_potential'].append({
                            'file': str(html_file),
                            'indicators': [ind for ind in revenue_indicators if ind in content.lower()]
                        })
        ...
        if html_analysis['total_projects'] > 20:
            html_analysis['optimization_opportunities'].append("Create template system for common projects")
        
        if len(html_analysis['seo_opportunities']) > 0:
            html_analysis['optimization_opportunities'].append("Implement SEO optimization across all HTML projects")
    ...
    def _analyze_pdf_documents(self):
        ...
        if pdf_analysis['document_categories']['AI/Technology'] > 5:
            pdf_analysis['content_opportunities'].append("Create AI knowledge base from PDFs")
        
        if pdf_analysis['document_categories']['Design/Creative'] > 3:
            pdf_analysis['content_opportunities'].append("Develop creative asset library")
    ...
    def _analyze_claude_projects(self):
        ...
        if claude_analysis['project_types']['Skills'] > 0:
            claude_analysis['optimization_opportunities'].append("Package skills for marketplace")
        
        if claude_analysis['project_types']['Courses'] > 0:
            claude_analysis['optimization_opportunities'].append("Create educational content platform")
    ...
    def _analyze_code_directory(self):
        ...
        if code_analysis['file_types']['.html'] > 10:
            code_analysis['development_opportunities'].append("Create web development portfolio")
        
        if code_analysis['file_types']['.py'] > 20:
            code_analysis['development_opportunities'].append("Package Python tools for distribution")
    ...
    def _analyze_other_directories(self):
        ...
        other_dirs = ['Archives', 'CsV', 'markD', 'json', 'cursor-agent', 'Docs', 'openai-cookbook', 'Notion']
        ...
        for dir_name in other_dirs:
            ...
            if dir_path.exists():
                ...
                if file_count > 100:
                    other_analysis['content_opportunities'].append(f"Large content repository in {dir_name}")
                
                if dir_name == 'json' and file_count > 50:
                    other_analysis['optimization_opportunities'].append("Create JSON data processing tools")
                
                if dir_name == 'Notion' and file_count > 20:
                    other_analysis['optimization_opportunities'].append("Integrate Notion with AI content generation")
    ...
    def _generate_optimization_recommendations(self):
        ...
        if self.analysis_results['environment_analysis'].get('env_d', {}).get('missing_apis'):
        ...
        python_analysis = self.analysis_results['content_analysis'].get('python_projects', {})
        ...
        html_analysis = self.analysis_results['content_analysis'].get('html_projects', {})
...
def main():
    ...
    with open("/Users/steven/ai-sites/automation/reports/environment_analysis.json", "w") as f:
        json.dump(results, f, indent=2)
    
    with open("/Users/steven/ai-sites/automation/reports/environment_analysis_report.md", "w") as f:
        f.write(report)
    
    print("✅ Analysis complete!")
    print(f"📊 Results saved to: /Users/steven/ai-sites/automation/reports/")
    print(f"📋 Report: environment_analysis_report.md")
    print(f"📈 Data: environment_analysis.json")
    
    # Print summary
    print("\n🎯 Key Findings:")
    print(f"  - Python Projects: {results['content_analysis'].get('python_projects', {}).get('total_projects', 0)}")
    print(f"  - HTML Projects: {results['content_analysis'].get('html_projects', {}).get('total_projects', 0)}")
