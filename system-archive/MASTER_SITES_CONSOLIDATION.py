...
class SitesConsolidator:
    ...
    def scan_ecosystem_docs(self):
        ...
        for md_file in self.ecosystem_dir.rglob('*.md'):
            category = md_file.parent.name if md_file.parent != self.ecosystem_dir else 'root'
    ...
    def scan_python_docs(self):
        ...
        planning_docs = []
        for pattern in ['_*.md', '*PLAN*.md', '*STRATEGY*.md', '*EXECUTION*.md', '*SEO*.md']:
    ...
    def check_site_documentation(self):
        ...
        if (self.ecosystem_dir / 'docs' / 'PROPERTIES').exists():
            ecosystem_properties = {
                f.stem for f in (self.ecosystem_dir / 'docs' / 'PROPERTIES').glob('*.md')
            }
    ...
    def generate_markdown_report(self, index):
        ...
        with open(report_file, 'w') as f:
            ...
            # Sort by size
            sorted_sites = sorted(self.sites.items(), key=lambda x: x[1]['size'], reverse=True)
            ...
            for name, info in sorted_sites[:15]:
                ...
                f.write(f"| {name} | {info['size']} | {info['file_count']} | {status} |\n")
            
            # Documentation section
            f.write("\n## 📚 DOCUMENTATION INVENTORY\n\n")
            
            ecosystem_docs = index.get('ecosystem_docs', {})
            f.write(f"**ECOSYSTEM Docs:** {sum(len(docs) for docs in ecosystem_docs.values())} files\n\n")
