    ...
    async def _analyze_technical_ultra(self, image_path: Path) -> Dict[str, Any]:
        ...
        if PIL_AVAILABLE:
            with Image.open(image_path) as img:
                ...
                technical['compression_ratio'] = round(size_bytes / (img.width * img.height), 4)
                ...
                if img.mode == 'RGB':
                    ...
                    if colors:
                        ...
                        technical['dominant_colors'] = [
                            {'rgb': color, 'count': count, 'hex': f'#{color[0]:02x}{color[1]:02x}{color[2]:02x}'}
...
Format as JSON."""
    ...
    async def _generate_perfect_seo(self, image_path: Path, analysis: Dict) -> Dict[str, Any]:
    ...
    def _generate_ultra_recommendations(self, analysis: Dict) -> List[str]:
        ...
        if not technical.get('is_high_res'):
            recs.append("⚡ CRITICAL: Increase resolution to minimum 1920px for professional use")
