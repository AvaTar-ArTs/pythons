...
class DocumentsOrganizer:
    ...
    def organize_files(self):
        ...
        for item in self.docs_dir.iterdir():
            ...
            elif 'report' in name_lower or 'summary' in name_lower or 'analysis' in name_lower:
                dest = org_structure['reports']
            elif 'strategy' in name_lower or 'plan' in name_lower or 'roadmap' in name_lower:
                dest = org_structure['strategies']
            elif 'guide' in name_lower or 'readme' in name_lower or 'setup' in name_lower:
                dest = org_structure['guides']
            elif 'template' in name_lower or 'invoice' in name_lower or 'contract' in name_lower:
                dest = org_structure['templates']
            elif name_lower.startswith('cat') or 'pasted-content' in name_lower or item.suffix == '.txttxt':
                dest = org_structure['temp-files']
            elif item.suffix in ['.json', '.csv', '.txt'] and item.stat().st_size < 10000:
