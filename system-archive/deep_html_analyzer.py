...
class HTMLAnalyzer:
    ...
    def categorize_files(self):
        ...
        for file_info in self.files:
            ...
            elif any(x in name for x in ['landing', 'service', 'portfolio', 'about', 'index']):
    ...
    def generate_report(self):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"**Total Size:** {sum(fi['size_mb'] for fi in self.files):.1f} MB\n\n")
            f.write("---\n\n")
            ...
            if self.duplicates:
                ...
                for i, (hash_val, files) in enumerate(list(self.duplicates.items())[:10], 1):
                    f.write(f"\n### Duplicate Set {i}\n\n")
            ...
            ai_conv_size = sum(f['size_mb'] for f in self.categories.get('ai_conversations', []))
            if ai_conv_size > 500:
                f.write(f"1. **AI Conversations:** {ai_conv_size:.0f} MB - Consider deleting old exports\n")
            
            if self.duplicates:
                dup_size = sum(sum(f['size_mb'] for f in files[1:]) for files in self.duplicates.values())
                f.write(f"2. **Duplicates:** {len(self.duplicates)} sets - {dup_size:.0f} MB can be freed\n")
