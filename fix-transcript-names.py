    ...
    def save_report(self):
        ...
        with open(report_file, 'w', newline='', encoding='utf-8') as f:
            ...
            for rename in self.rename_suggestions:
                # Per-row status set during execute_renames(); rows never
                # processed (e.g. beyond the display/execution cap) stay
                # 'SUGGESTED' so the revert script never treats them as renamed.
                status = rename.get('status', 'SUGGESTED')
