...
class RichDocGenerator:
    ...
    def add_rename(self, current_name: str, description: str, new_name: str, 
                   action: str = "RENAME", category: str = "", 
                   why_rename: str = "", impact: str = "", apis_used: list = None):
    ...
    def generate_rich_markdown(self):
        ...
        with open(output_path, 'w') as f:
            ...
            f.write(f"This document details the renaming of **{total_files} Python scripts** ")
            f.write("to create a more intuitive, organized, and maintainable codebase. ")
            f.write("Each rename reflects what the script **actually does** rather than vague or technical naming.\n\n")
            
            f.write("### Quick Stats\n\n")
            f.write(f"- 🏷️ **{by_action.get('RENAME', 0)} files** being renamed for clarity\n")
            f.write(f"- 🗑️ **{by_action.get('DELETE', 0)} files** marked for deletion (duplicates, library code)\n")
            f.write(f"- ✅ **{by_action.get('KEEP', 0)} files** already have perfect names\n")
            f.write(f"- 📦 **{by_action.get('MOVE', 0)} files** being moved to appropriate directories\n")
            f.write(f"- 📂 **{len(by_category)} categories** identified for better organization\n\n")
            ...
            f.write("The goal is to create **self-documenting filenames** that immediately ")
            f.write("communicate purpose and functionality. Instead of:\n\n")
            f.write("- ❌ `analyze-metadata.py` (vague - analyze what metadata?)\n")
            f.write("- ✅ `transcript-to-image-prompts.py` (clear - converts transcripts to image prompts)\n\n")
            f.write("### Naming Principles Applied\n\n")
            f.write("1. **Action-First Naming** - Use verbs that describe the core action\n")
            f.write("2. **Input-Output Clarity** - Show what goes in and what comes out\n")
            f.write("3. **Technology Transparency** - Indicate key technologies used (GPT, Claude, etc.)\n")
            f.write("4. **Purpose Over Implementation** - Focus on WHAT it does, not HOW\n\n")
            f.write("---\n\n")
            ...
            for category in sorted(by_category.keys()):
                ...
                for i, item in enumerate(sorted(items, key=lambda x: x['current_name']), 1):
                    action = item['action']
                    
                    if action == 'DELETE':
                        f.write(f"#### {i}. ~~`{item['current_name']}`~~ → 🗑️ DELETE\n\n")
                        f.write(f"**Why Delete:** {item['description']}\n\n")
                    elif action == 'KEEP':
                        f.write(f"#### {i}. `{item['current_name']}` → ✅ KEEP\n\n")
                        f.write(f"**Why Keep:** {item['description']}\n\n")
                    else:
                        f.write(f"#### {i}. `{item['current_name']}` → `{item['new_name']}`\n\n")
            ...
            for item in sorted(self.renames, key=lambda x: x['current_name']):
                ...
                new = item['new_name'] if item['action'] == 'RENAME' else f"**{item['action']}**"
                ...
                f.write(f"| `{current}` | `{new}` | {cat} | {action_emoji} {action} |\n")
            ...
            for item in self.renames:
                if item['action'] == 'RENAME':
                    ...
                    f.write(f"rm {item['current_name']}  # {item['description'][:50]}\n")
            
            f.write("```\n\n")
            
            f.write("### Step 3: Update Documentation\n\n")
            f.write("Update any README files or documentation that reference the old filenames.\n\n")
            ...
            f.write("- This reorganization makes your toolkit more professional and maintainable\n")
            ...
            f.write("---\n\n")
            f.write(f"*Documentation generated by Rich Doc Generator on {datetime.now().strftime('%B %d, %Y')}*\n")
