#!/usr/bin/env python3
"""
Comprehensive Chat Analysis Organizer
Similar to ChatGPT Exporter - organizes and sorts all chat analysis content
"""
...
class ComprehensiveChatOrganizer:
    ...
    def scan_all_files(self):
        ...
        print("🔍 Scanning all chat analysis files...")
        ...
        for file_path in self.source_dir.rglob("*"):
            if file_path.is_file():
                print(f"   📄 Found: {file_path.relative_to(self.source_dir)}")
    ...
    def organize_content(self):
        ...
        for file_info in self.all_files:
            for project in file_info["projects"]:
                if project not in self.organized_content["projects"]:
                    self.organized_content["projects"][project] = []
                self.organized_content["projects"][project].append(file_info)
        ...
        # Create timeline
        self.organized_content["timeline"] = sorted(
            self.all_files, key=lambda x: x["modified"], reverse=True
        )
    ...
    def create_organized_structure(self):
        ...
        for project in self.organized_content["projects"].keys():
            project_dir = (
                self.target_dir / "01_Projects" / self.sanitize_filename(project)
            )
    ...
    def copy_file_to_directory(self, file_info, target_dir):
        ...
        organized_name = f"{timestamp}_{file_info['name']}"
        target_path = target_dir / organized_name

        try:
            shutil.copy2(source_path, target_path)
            print(f"   ✅ Copied: {file_info['name']} -> {target_dir.name}")
        except Exception as e:
    ...
    def generate_master_index(self):
        ...
        with open(index_path, "w", encoding="utf-8") as f:
            ...
            f.write("## Overview\\n")
            f.write(
                f"- **Total Files:** {self.organized_content['statistics']['total_files']}\\n"
            )
            f.write(
                f"- **Total Size:** {self.organized_content['statistics']['total_size']:,} bytes\\n"
            )
            f.write(
                f"- **Total Words:** {self.organized_content['statistics']['total_words']:,}\\n"
            )
            f.write(
                f"- **Projects:** {self.organized_content['statistics']['projects_count']}\\n"
            )
            f.write(
                f"- **Topics:** {self.organized_content['statistics']['topics_count']}\\n\\n"
            )
            ...
            f.write("\\n## File Types\\n")
            for file_type, stats in self.organized_content["statistics"][
                "file_types"
            ].items():
                f.write(
                    f"- **{file_type}:** {stats['count']} files ({stats['total_size']:,} bytes)\\n"
                )
    ...
    def generate_project_reports(self):
        ...
        for project, files in self.organized_content["projects"].items():
            report_path = (
                self.target_dir
                / "01_Projects"
                / self.sanitize_filename(project)
                / f"{self.sanitize_filename(project)}_REPORT.md"
            )
    ...
    def generate_file_type_reports(self):
        ...
        for file_type, files in self.organized_content["file_types"].items():
            report_path = (
                self.target_dir / "02_File_Types" / file_type / f"{file_type}_REPORT.md"
            )
    ...
    def generate_timeline_report(self):
        ...
        with open(timeline_path, "w", encoding="utf-8") as f:
            ...
            for file_info in self.organized_content["timeline"]:
                f.write(
                    f"### {file_info['modified'].strftime('%Y-%m-%d %H:%M:%S')} - {file_info['name']}\\n"
                )
                f.write(f"- **Projects:** {', '.join(file_info['projects'])}\\n")
                f.write(f"- **Topics:** {', '.join(file_info['topics'])}\\n")
                f.write(f"- **Type:** {file_info['file_type']}\\n")
    ...
    def generate_statistics_report(self):
        ...
        with open(stats_path, "w", encoding="utf-8") as f:
            ...
            for file_type, type_stats in stats["file_types"].items():
                ...
                f.write(
                    f"- **Average Size:** {type_stats['total_size'] // type_stats['count']:,} bytes\\n\\n"
                )
    ...
    def run_organization(self):
        ...
        print("🚀 Starting Comprehensive Chat Analysis Organization...")
        ...
        print(f"📁 Organized content saved to: {self.target_dir}")
        print("📋 Check MASTER_INDEX.md for navigation")


def main():
    """Main execution function"""
    source_dir = "/Users/steven/Documents/cursor-agent/chat_analysis "
    target_dir = "/Users/steven/COMPREHENSIVE_CHAT_ORGANIZATION"

    organizer = ComprehensiveChatOrganizer(source_dir, target_dir)
