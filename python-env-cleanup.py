    ...
    for pattern in cache_patterns:
        if '*' in pattern:
            ...
            try:
                ...
                cache_files.extend(result.stdout.strip().split('\n') if result.stdout.strip() else [])
            except:
                pass
        else:
            # Directory pattern
            try:
                ...
                cache_dirs.extend(result.stdout.strip().split('\n') if result.stdout.strip() else [])
            except:
                pass
    ...
    for findings in all_findings:
        ...
        if 'cache_files' in findings:
            if findings['cache_files'] > 0 or findings['cache_dirs'] > 0:
                print(f"   ⚠️  Cache: {findings['cache_files']} files, {findings['cache_dirs']} directories")
        ...
        if 'subdirs' in findings and findings['subdirs']:
            ...
            for subdir, size in sorted(findings['subdirs'].items(), key=lambda x: x[1], reverse=True)[:5]:
...
def cleanup_cache(dry_run=True):
    ...
    for name, path in DIRS.items():
        ...
        if cache_files or cache_dirs:
            print(f"📁 {name}:")
            print(f"   Found {len(cache_files)} cache files, {len(cache_dirs)} cache directories")
            ...
            if not dry_run:
                ...
                print(f"   Would remove {len(cache_dirs)} directories and {len(cache_files)} files")
            print()
