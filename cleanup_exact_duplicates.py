...
def cleanup_exact_duplicates(root_path, dry_run=True):
    ...
    for location, files in sorted(by_location.items(), key=lambda x: len(x[1]), reverse=True):
        size = sum(f['size'] for f in files)
        print(f"     {location:20s} {len(files):4d} files ({size/1024/1024/1024:.2f} GB)")
        ...
        for file_info in files_to_remove:
            try:
                ...
                if removed_count % 100 == 0:
                    print(f"     Removed {removed_count}/{len(files_to_remove)} files...", end='\r')
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        print("  python cleanup_exact_duplicates.py /path/to/dir --execute # Actually remove")
        sys.exit(1)
