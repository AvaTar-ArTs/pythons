...
def analyze_custom_code():
    ...
    if not json_files:
        print("❌ No analysis found. Please run compare_content_and_functions.py first.")
        return
    ...
    print(f"   Duplicate functions: {len([f for f in all_functions if f.get('hash') in duplicate_hashes])}")
    ...
    with open(report_path, 'w') as f:
        ...
        f.write(f"Duplicate functions: {len([f for f in all_functions if f.get('hash') in duplicate_hashes])}\n")
        ...
        sorted_files = sorted(functions_by_file.items(), key=lambda x: len(x[1]), reverse=True)
    ...
    json_report = {
        ...
        'summary': {
            ...
            'duplicate_functions': len([f for f in all_functions if f.get('hash') in duplicate_hashes]),
