...
get_ipython().system("pip install -q bitsandbytes>=0.39.0")
get_ipython().system("pip install -q git+https://github.com/huggingface/accelerate.git")
get_ipython().system(
...
axs[0].hist(
    weights, bins=150, alpha=0.5, label="Original weights", color="blue", range=(-2, 2)
)
axs[0].hist(
    weights_abs, bins=150, alpha=0.5, label="Absmax weights", color="red", range=(-2, 2)
)
...
for ax in axs:
    ...
    ax.yaxis.set_major_formatter(
        ticker.EngFormatter()
    )  # Make y-ticks more human readable
