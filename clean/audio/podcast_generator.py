        ...
        try:
            ...
            for file_path in files:
                try:
                    ...
                    temp_root = os.path.realpath(temp.gettempdir())
                    if (
                        os.path.commonpath([candidate, temp_root]) == temp_root
                        and os.path.basename(candidate).startswith(TEMP_PREFIX)
                        and os.path.isfile(candidate)
                    ):
                        os.remove(candidate)
                        logger.debug(f"Cleaned up temporary file: {file_path}")
                    else:
                        logger.warning("Refusing to remove non-owned temporary path: %s", file_path)
                except OSError as e:
    ...
    async def _generate_speech_file(
        ...
        try:
            ...
            temp_file = temp.NamedTemporaryFile(
                prefix=TEMP_PREFIX, suffix=".mp3", delete=False, delete_on_close=False
            )
