...
def create_albums_from_folders(images_data: Dict[str, Any], images_path: str) -> Dict[str, List[str]]:
    """
...
def paginate_images(images_data_list: List[Dict], images_per_page: int = 100) -> List[List[Dict]]:
    """
...
def build_large_gallery_html(
    ...
    try:
        ...
        if not os.path.exists(images_data_file):
            raise spg_common.SPGException(f"Images data file not found: {images_data_file}")
        ...
        if enable_albums:
            ...
            albums = create_albums_from_folders(images_data, gallery_config["images_path"])
        ...
        if not os.path.exists(templates_path):
            raise spg_common.SPGException(f"Templates path does not exist: {templates_path}")
...
def main():
    ...
    if ConfigValidator:
        gallery_config = ConfigValidator.migrate_config(gallery_config, from_version="2.0")
