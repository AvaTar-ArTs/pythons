...
def build_html(gallery_config):
    ...
    # Use first image filename (without extension) as title if title matches folder name
    if images_data_list and gallery_config.get("title") == parent_folder_name:
        first_image_name = images_data_list[0]["name"]
        # Remove file extension for cleaner title
        first_image_title = os.path.splitext(first_image_name)[0]
        gallery_config["title"] = first_image_title
