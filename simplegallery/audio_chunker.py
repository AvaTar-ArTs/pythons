...
class AudioChunker:
    ...
    def calculate_chunks(self, total_duration: float) -> List[Tuple[float, float]]:
        ...
        while current_start < total_duration:
            ...
            if current_end - current_start < self.min_chunk_duration and len(chunks) > 0:
        ...
        try:
            ...
            with AudioFileClip(audio_path) as audio:
                for i, (start_time, end_time) in enumerate(chunks, 1):
                    ...
                    logger.info(f"Creating chunk {i}/{len(chunks)}: {start_time:.1f}s - {end_time:.1f}s")
    ...
    def merge_transcripts(self, chunk_transcripts: List[dict], chunk_files: List[str]) -> dict:
        ...
        try:
            ...
            for i, (chunk_transcript, chunk_file) in enumerate(zip(chunk_transcripts, chunk_files)):
            ...
            merged_transcript = {
                'full_transcript': ' '.join(segment['text'].strip() for segment in all_segments),
                'segments': all_segments,
                'language': chunk_transcripts[0].get('language', 'unknown') if chunk_transcripts else 'unknown',
            ...
            for segment in all_segments:
                ...
                chunk_info = f" [Chunk {segment['chunk']}]" if len(chunk_transcripts) > 1 else ""
                text = segment['text'].strip()
                timestamped_lines.append(f"[{start_time} - {end_time}]{chunk_info} {text}")
