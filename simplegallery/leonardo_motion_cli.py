...
def poll_json(url: str, headers: dict, key_path: list[str], ok_values: set[str], timeout: int=300, interval: int=5) -> dict:
    ...
    while True:
        ...
        if status and isinstance(status, str):
            ...
            if status_lower in {"failed","error","cancelled"}:
                raise RuntimeError(f"Job failed with status={status}: {json.dumps(data)[:400]}")
        if time.time()-start > timeout:
            raise TimeoutError(f"Timeout waiting for job. Last response: {json.dumps(data)[:400]}")
        time.sleep(interval)
...
def main():
    ...
    ap.add_argument("--model-id", default="ac614f96-1082-45bf-be9d-757f2d31c174", help="Leonardo modelId")
    ...
    ap.add_argument("--api-key", default=os.getenv("LEONARDO_API_KEY"), help="Leonardo API key (or env LEONARDO_API_KEY)")
    ap.add_argument("--outdir", type=Path, default=Path("./leonardo_output"))
    args = ap.parse_args()

    if not args.api_key:
        print("❌ Missing API key. Set LEONARDO_API_KEY or pass --api-key.", file=sys.stderr)
        sys.exit(2)
    ...
    if r.status_code >= 400:
        print(f"❌ Generation request failed: {r.status_code} {r.text}", file=sys.stderr); sys.exit(1)
    gen_resp = r.json()
    ...
    if not generation_id:
        print("❌ No generationId in response.", file=sys.stderr); sys.exit(1)

    # 2) Poll generation
    data = poll_json(f"{BASE}/generations/{generation_id}", bearer(args.api_key),
                     key_path=["generations_by_pk"], ok_values={"complete","completed","succeeded"},
                     timeout=args.timeout, interval=args.poll_interval)
    ...
    if not images:
        print("❌ No generated_images!", file=sys.stderr); sys.exit(1)
    ...
    if r.status_code >= 400:
        print(f"❌ Upscale request failed: {r.status_code} {r.text}", file=sys.stderr); sys.exit(1)
    up_resp = r.json()
    (args.outdir / "03_upscale_request.json").write_text(json.dumps(up_resp, indent=2))
    up_id = up_resp.get("sdUpscaleJob", {}).get("id")
    if not up_id:
        print("❌ No upscale job id.", file=sys.stderr); sys.exit(1)

    up_data = poll_json(f"{BASE}/variations/{up_id}", bearer(args.api_key),
                        key_path=["generated_image_variation_generic"], ok_values={"complete","completed","succeeded"},
                        timeout=args.timeout, interval=args.poll_interval)
    ...
    # 4) Motion (SVD)
    r = requests.post(f"{BASE}/generations-motion-svd", headers=bearer(args.api_key),
                      json={"imageId": var_id, "motionStrength": 5, "isVariation": True}, timeout=60)
    if r.status_code >= 400:
        print(f"❌ Motion request failed: {r.status_code} {r.text}", file=sys.stderr); sys.exit(1)
    mv_req = r.json()
    ...
    if not mv_gen_id:
        print("❌ No motion generationId.", file=sys.stderr); sys.exit(1)

    mv_data = poll_json(f"{BASE}/generations/{mv_gen_id}", bearer(args.api_key),
                        key_path=["generations_by_pk"], ok_values={"complete","completed","succeeded"},
                        timeout=args.timeout, interval=args.poll_interval)
    ...
    for item in vids:
        for k in ("video", "video_url", "url", "image_path"):
            if item.get(k):
                best_url = item[k]; break
        if best_url: break
    if best_url:
        ext = ".mp4" if ".mp4" in best_url else ".webm"
        download_file(best_url, args.outdir / f"motion{ext}")
        print(f"✅ Saved: {args.outdir / f'motion{ext}'}")
    else:
        print("⚠️ Motion job completed but no downloadable URL found. See JSON files.", file=sys.stderr)
