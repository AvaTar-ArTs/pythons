...
# Import enhanced_metadata from root level
try:
    from simplegallery.enhanced_metadata import get_enhanced_metadata
except ImportError:
    # Fallback for direct import
    import enhanced_metadata
    get_enhanced_metadata = enhanced_metadata.get_enhanced_metadata
import logging
...
def build_enhanced_html(gallery_config, images_data, background_photo, remote_data):
    ...
    template_name = "enhanced_index_template.jinja" if gallery_config.get("template", "enhanced") == "enhanced" else "index_template.jinja"
    ...
    for image_name, image_data in images_data.items():
        ...
        if enhanced_features.get("enhanced", False):
            try:
                ...
                thumbnail_path = os.path.join(gallery_config["thumbnails_path"], image_name)
                public_path = gallery_config["public_path"]
                
                # Get enhanced metadata
                enhanced_metadata = get_enhanced_metadata(image_path, thumbnail_path, public_path)
                ...
                enhanced_image_data.update({
                    ...
                    'geographic_data': {},
                    'device_info': {},
                    'aesthetic_score': {'overall_aesthetic_score': 0.5, 'aesthetic_grade': 'C'}
                })
...
def generate_content_analysis_report(enhanced_data, gallery_config):
    ...
    images_with_faces = sum(1 for img in enhanced_data.values() 
                           if img.get('face_analysis', {}).get('faces_detected', False))
    images_with_text = sum(1 for img in enhanced_data.values() 
    ...
    report = {
        ...
        'face_percentage': (images_with_faces / total_images * 100) if total_images > 0 else 0,
        'images_with_text': images_with_text,
        'text_percentage': (images_with_text / total_images * 100) if total_images > 0 else 0,
        'quality_distribution': quality_grades,
        'top_colors': sorted(all_colors, key=lambda x: x.get('percentage', 0), reverse=True)[:10],
        'scene_categories': dict(sorted(scene_categories.items(), key=lambda x: x[1], reverse=True)),
        'generated_at': str(os.path.getctime(os.path.join(gallery_config["public_path"], "index.html")))
    }
    
    # Save report
    report_path = os.path.join(gallery_config["public_path"], "content_analysis_report.json")
...
def copy_enhanced_assets(gallery_config):
    ...
    # Copy enhanced CSS
    enhanced_css_src = os.path.join(os.path.dirname(__file__), "data", "public", "css", "enhanced-main.css")
    enhanced_css_dst = os.path.join(gallery_config["public_path"], "css", "enhanced-main.css")
    ...
    enhanced_js_src = os.path.join(os.path.dirname(__file__), "data", "public", "js", "enhanced-main.js")
    enhanced_js_dst = os.path.join(gallery_config["public_path"], "js", "enhanced-main.js")
    ...
    enhanced_template_src = os.path.join(os.path.dirname(__file__), "data", "templates", "enhanced_index_template.jinja")
    enhanced_template_dst = os.path.join(gallery_config["templates_path"], "enhanced_index_template.jinja")
    ...
    enhanced_macros_src = os.path.join(os.path.dirname(__file__), "data", "templates", "enhanced_gallery_macros.jinja")
    enhanced_macros_dst = os.path.join(gallery_config["templates_path"], "enhanced_gallery_macros.jinja")
...
def main():
    ...
    if enhanced_features.get("enhanced", False):
        try:
            ...
            enhanced_images_data = enhance_images_data(images_data, gallery_config, enhanced_features)
            
            # Save enhanced images data
            enhanced_images_data_file = os.path.join(gallery_root, "enhanced_images_data.json")
            with open(enhanced_images_data_file, "w", encoding="utf-8") as f:
                json.dump(enhanced_images_data, f, indent=2, ensure_ascii=False, default=str)
