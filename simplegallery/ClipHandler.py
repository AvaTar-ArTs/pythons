...
class ClipHandler:
    ...
    def __init__(self, game: str, asset_path: str, output_path: str):
        ...
        model_file_path = Path(
            os.path.join(asset_path, utils.get_valid_game_name(self.game), "game_detection.h5")
        )
        ...
        if self.model:
            ...
            for t in range(1, int(clip.duration)):
                ...
                resized_frame = tf.keras.preprocessing.image.smart_resize(
                    frame, (224, 224), interpolation="nearest"
                )
    ...
    def get_game_clips(self, headers: dict, timespan: str, language: str, filter_information: dict):
        ...
        while not self.is_required_length(filter_information):
            ...
            if resp.status_code == 200:
                ...
                self.retries -= 1
                logging.warning(
                    f"Status Code: 401 -> retrying with new token for {self.retries} more times"
                )
                if self.retries > 0:
