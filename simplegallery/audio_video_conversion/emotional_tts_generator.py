...
class EmotionalTTSGenerator:
    ...
    def generate_emotional_speech(self, text, output_path, emotion="neutral", voice="alloy", speed=1.0):
        """
    ...
    def process_text_with_emotions(self, text, output_folder, base_filename):
        ...
        for i, paragraph in enumerate(paragraphs[:10]):  # Limit to first 10 paragraphs
            ...
            emotion = "neutral"
            if any(word in paragraph.lower() for word in ["foreword", "preface", "introduction"]):
                emotion = "thoughtful"
            elif any(word in paragraph.lower() for word in ["chapter", "section"]):
                emotion = "authoritative"
            elif any(word in paragraph.lower() for word in ["conclusion", "ending", "final"]):
                emotion = "inspiring"
