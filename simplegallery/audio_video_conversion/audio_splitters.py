...
def split_file(
    ...
    if smart_split and duration_ms > chunk_ms and SILENCE_DETECTION_AVAILABLE:
        if verbose:
            print(colored("  🎯 Finding optimal split points...", "cyan"))
...
def validate_environment():
    ...
    if issues:
        print(colored("⚠️  Environment Warnings:", "yellow"))
        for issue in issues:
...
Examples:
  ...
  %(prog)s podcast.mp3 --chunk-seconds 300
  ...
  %(prog)s long_audio.mp3 --min-minutes 2 --max-minutes 20
    ...
    parser.add_argument(
        "files", nargs="+", help="Input audio files (.mp3, .wav, .m4a, etc.)",
    )
    parser.add_argument(
        "-o",
        "--out-dir",
    ...
    parser.add_argument(
        "--bitrate", type=str, default=None, help="MP3 bitrate (e.g., 192k, 256k)",
    )
    parser.add_argument(
        "--min-minutes",
        type=int,
    ...
    print(colored("\n" + "=" * 70, "blue"))
    print(colored("🎵 Audio Splitter for Transcription", "blue"))
    print(colored("=" * 70 + "\n", "blue"))
    ...
    for idx, f in enumerate(args.files, 1):
        ...
        print(colored(f"[{idx}/{total_files}] {file_path.name}", "yellow"))
    ...
    if summary:
        ...
        print(colored("\n" + "=" * 70, "blue"))
        print(
            colored(f"✅ Complete: {successful}/{total_files} files processed", "green"),
        )
        print(colored(f"📁 Output: {out_dir}", "cyan"))
        print(colored(f"📊 Summary: {summary_path.name}", "cyan"))
        print(colored("=" * 70 + "\n", "blue"))
    else:
