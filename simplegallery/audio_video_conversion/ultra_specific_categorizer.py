...
ULTRA_SPECIFIC_FUNCTIONS = {
    ...
    'upscaler': (['upscale', 'upscaler', 'enlarge', 'enhance', 'super-resolution', 'esrgan'], ['*upscale*', '*upscaler*'], 10),
    'image-resizer': (['resize', 'downsize', 'thumbnail', 'scale', 'dimension'], ['*resize*', '*downsize*', '*scale*'], 9),
    'image-converter': (['convert', 'img2img', 'format-convert', 'webp', 'png', 'jpg'], ['*img2img*', '*convert*'], 8),
    'gallery-generator': (['gallery', 'album', 'grid', 'collage', 'photo-grid'], ['*gallery*', '*grid*', '*album*'], 10),
    'pdf-converter': (['pdf', 'pdf2img', 'img2pdf', 'pdf-to-image'], ['*pdf*'], 9),
    'watermark-tool': (['watermark', 'overlay', 'logo'], ['*watermark*', '*overlay*'], 8),

    # Video-specific actions
    'transcribe-analysis': (['transcribe', 'transcription', 'whisper', 'speech-to-text', 'subtitle'], ['*transcribe*', '*whisper*', '*subtitle*'], 10),
    'video-downloader': (['download', 'yt-dlp', 'youtube-dl', 'ytdl'], ['*download*', '*ytdl*', '*yt-dlp*'], 10),
    'video-editor': (['edit', 'trim', 'cut', 'merge', 'moviepy'], ['*edit*', '*trim*', '*cut*'], 8),
    'video-converter': (['ffmpeg', 'convert', 'encode', 'transcode'], ['*ffmpeg*', '*convert*', '*encode*'], 8),
    'thumbnail-generator': (['thumbnail', 'preview', 'poster'], ['*thumbnail*', '*preview*'], 9),

    # Audio-specific actions
    'audio-transcriber': (['transcribe', 'whisper', 'speech'], ['*transcribe*', '*whisper*'], 10),
    'audio-converter': (['mp3', 'wav', 'audio-convert'], ['*audio*', '*mp3*', '*wav*'], 8),
    'text-to-speech': (['tts', 'text-to-speech', 'synthesize'], ['*tts*', '*text-to-speech*'], 9),

    # Web-specific actions
    'web-scraper': (['scrape', 'scraper', 'beautifulsoup', 'selenium'], ['*scrape*', '*scraper*'], 10),
    'api-client': (['api', 'client', 'request', 'endpoint'], ['*api*', '*client*'], 7),
    'bot-automation': (['bot', 'instagram', 'twitter', 'facebook', 'telegram'], ['*bot*', '*insta*', '*twitter*'], 9),
    'html-generator': (['html', 'generator', 'web-page'], ['*html*', '*web*'], 8),

    # Data-specific actions
    'csv-processor': (['csv', 'excel', 'xlsx', 'spreadsheet'], ['*csv*', '*excel*', '*xlsx*'], 9),
    'json-processor': (['json', 'json-parser'], ['*json*'], 7),
    'database-migrator': (['migrate', 'migration', 'sql'], ['*migrate*', '*migration*'], 9),
    'data-analyzer': (['analyze', 'analysis', 'pandas', 'stats'], ['*analyze*', '*analysis*'], 8),

    # Automation-specific actions
    'file-organizer': (['organize', 'organizer', 'sort', 'cleanup'], ['*organize*', '*organizer*', '*cleanup*'], 9),
    ...
    'ai-image-generator': (['dall-e', 'stable-diffusion', 'midjourney', 'ai-image'], ['*dall*', '*stable*', '*ai-image*'], 10),
    'ai-text-generator': (['gpt', 'llm', 'openai', 'claude'], ['*gpt*', '*llm*', '*openai*'], 9),
    'ai-chatbot': (['chatbot', 'chat', 'conversation'], ['*chatbot*', '*chat*'], 9),

    # System utilities
    'monitor-tool': (['monitor', 'watch', 'track'], ['*monitor*', '*watch*'], 7),
    'notification-sender': (['notify', 'notification', 'alert'], ['*notify*', '*alert*'], 8),
...
class UltraSpecificCategorizer:
    ...
    def analyze_file(self, file_path: Path) -> str:
        ...
        for category, (keywords, patterns, priority) in ULTRA_SPECIFIC_FUNCTIONS.items():
            score = 0
    ...
    def categorize_directory(self):
        ...
        broad_categories = [
            'api-development', 'automation', 'youtube-automation',
            'image-processing', 'video-processing', 'audio-processing',
            'social-media-automation', 'content-generation',
            'data-analysis', 'web-scraping', 'file-utilities',
            'database', 'utilities', 'general-scripts'
        ]
        ...
        for file_path in python_files:
            ...
            if category:
                ...
                self.categorized[category].append(str(file_path.relative_to(self.base_dir)))
            else:
        ...
        print(f"📄 Files Categorized: {sum(len(files) for files in self.categorized.values())}")
        print(f"❓ Uncategorized: {len(self.uncategorized)}")
        print()

        # Show categories
        for category, files in sorted(self.categorized.items(), key=lambda x: len(x[1]), reverse=True):
    ...
    def save_analysis(self):
        ...
        analysis = {
            ...
            'total_files_categorized': sum(len(files) for files in self.categorized.values()),
...
def main():
    ...
    analysis_file = categorizer.save_analysis()
