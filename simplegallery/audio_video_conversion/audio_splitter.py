...
def compute_chunk_ms_from_target_size(
    file_path: Path, audio: AudioSegment, target_size_mb: float,
) -> int:
    ...
    min_ms = 60 * 1000  # 1 minute
    max_ms = 15 * 60 * 1000  # 15 minutes
...
def find_silence_split_points(
    ...
    while target_point < duration_ms:
        ...
        search_start = max(0, target_point - 30000)
        search_end = min(duration_ms, target_point + 30000)
        ...
        try:
            ...
            print(
                colored(
                    f"  ⚠️  Silence detection failed, using target point: {e}", "yellow",
                ),
            )
            split_points.append(target_point)
...
def split_file(
    ...
    """
    print(colored(f"  📂 Loading {file_path.name}...", "cyan"))
    audio = AudioSegment.from_file(file_path)
    duration_ms = len(audio)

    print(colored(f"  ⏱️  Duration: {human_time(duration_ms)}", "cyan"))
    ...
    if smart_split and duration_ms > chunk_ms:
        print(colored("  🎯 Finding optimal split points near silence...", "cyan"))
    ...
    print(colored(f"  ✂️  Creating {len(split_points) - 1} chunks...", "yellow"))
...
def validate_dependencies():
    ...
    if not shutil.which("ffmpeg"):
        print(colored("❌ Error: FFmpeg is not installed or not in PATH", "red"))
        print(colored("   Install on macOS: brew install ffmpeg", "yellow"))
        print(colored("   Install on Linux: sudo apt-get install ffmpeg", "yellow"))
        sys.exit(1)
...
Examples:
  ...
  python audio_splitter.py podcast.mp3 --chunk-seconds 300
  ...
  python audio_splitter.py song.mp3 --no-smart-split
    ...
    parser.add_argument(
        "files", nargs="+", help="Input audio files (e.g., .mp3, .wav, .m4a)",
    )
    parser.add_argument(
        "-o",
        "--out-dir",
    ...
    print(colored("\n" + "=" * 70, "blue"))
    print(colored("🎵 Audio Splitter for Transcription", "blue", attrs=["bold"]))
    print(colored("=" * 70 + "\n", "blue"))
    ...
    for idx, f in enumerate(args.files, 1):
        ...
        try:
            ...
            if args.chunk_seconds:
                chunk_ms = int(args.chunk_seconds * 1000)
            else:
            ...
            print(
                colored(
                    f"  ✅ Complete: {len(parts)} parts, "
                    f"~{human_time(chunk_ms)} each",
                    "green",
                    attrs=["bold"],
                ),
            )
            print(
                colored(
                    f"     Source: {human_bytes(total_src)} → "
                    f"Parts: {human_bytes(total_parts)}",
                    "green",
                ),
            )
            print()
            ...
            print(colored(f"  ❌ Error processing {file_path.name}: {e}", "red"))
            continue
    ...
    if summary:
        ...
        print(
            colored(
                f"✅ All done! Processed {len(summary)}/{total_files} files",
                "green",
                attrs=["bold"],
            ),
        )
        print(colored(f"📁 Output directory: {out_dir}", "cyan"))
        print(colored(f"📊 Summary saved to: {summary_path}", "cyan"))
        print(colored("=" * 70 + "\n", "blue"))
    else:
        print(colored("❌ No files were successfully processed", "red"))
