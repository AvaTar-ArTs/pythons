...
def best_video_frame(
    video_path: Path, sample_secs=(0.5, 1.0, 1.5, 2.0),
) -> Image.Image | None:
...
def gpt_refine_and_describe(
    raw_title: str, content_hint: str, need_typography: bool,
) -> tuple[str, str]:
...
def process_media(path: Path) -> tuple[Path, Path, str, str, str, bool]:
    ...
    clean_title, prompt = gpt_refine_and_describe(
        ocr_title, content_hint, has_typography,
    )
...
def main():
    ...
    ap.add_argument(
        "--yes", "-y", action="store_true", help="Skip confirmation when applying",
    )
    ap.add_argument(
        "--csv", default="output.csv", help="CSV output path (TAB-delimited)",
    )
    args = ap.parse_args()
