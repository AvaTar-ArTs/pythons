    ...
    if not title:
        for line in src.splitlines()[:15]:
            if line.strip().startswith("#"):
                ...
                if not title and t:
                    title = t[:120]
                elif t and len(desc) < 200:
                    desc += (" " if desc else "") + t
    return title, desc[:400]


def categorize(
    imports: Iterable[str], src: str, rules: dict[str, dict[str, list[str]]],
) -> str:
    text = (src or "").lower()
...
def llm_category(src: str, model: str = "gpt-4o-mini") -> str | None:
...
def main():
    ap = argparse.ArgumentParser(
        description="Catalog and (optionally) organize Python files safely.",
    )
    ...
    ap.add_argument(
        "--mode",
        choices=["index", "move", "copy", "link"],
        default="index",
        help="What to do with files",
    )
    ap.add_argument(
        "--apply",
        action="store_true",
    ...
    ap.add_argument(
        "--rules", type=Path, help="Path to categories JSON (merges with defaults)",
    )
    ap.add_argument(
        "--github-root",
        type=str,
        help="If set, MD links will point to this base + relative path",
    )
    ap.add_argument(
        "--use-llm",
        action="store_true",
        help="Use OpenAI to refine category (optional)",
    )
    args = ap.parse_args()
    ...
    if args.rules and args.rules.exists():
        try:
            ...
            for k, v in user_rules.items():
                ...
                rules[k]["imports"] = list(
                    set(rules[k].get("imports", []) + v.get("imports", [])),
                )
                rules[k]["keywords"] = list(
                    set(rules[k].get("keywords", []) + v.get("keywords", [])),
                )
    ...
    if args.mode != "index" and dest:
        for r in records:
            ...
            if not r.dest_path:
                continue
            if not args.apply:
                print(f"[DRY-RUN] {args.mode.upper()} {src_p} -> {dst_p}")
                continue
            ...
            print(f"{args.mode.upper()} {src_p} -> {dst_p}")
    ...
    def gh_link(rel_path: str) -> str | None:
    ...
    if args.mode != "index":
        print(f"Mode: {args.mode} | Apply: {args.apply} | Dest: {dest}")
