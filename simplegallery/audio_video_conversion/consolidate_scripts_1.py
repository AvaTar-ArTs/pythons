...
class ScriptConsolidator:
    def __init__(self, base_dir: str):
        ...
        self.duplicate_groups = {
            ...
            "generate_speech": [
                "generate_speech.py",
                "generate_speech 2.py",
                "generate_speech 2 (1).py",
                "generate_speech 2 2.py",
            ],
            "generate_csv": [
                "generate_song_csv.py",
                "generate_song_csv (1).py",
                "generate_songs_csv.py",
                "generate_songs_csv 1.py",
                "generate_songs_csv_1.py",
            ],
            "generate_album": [
            ...
            "cover_processing": ["cover.py", "cover2.py"],
            "quiz_variants": [
                "Quiz22s.py",
                "Quiz22sec.py",
                "QuizPrompts.py",
    ...
    def merge_analysis_scripts(self):
        ...
        for category_dir in self.sorted_dir.iterdir():
            if category_dir.is_dir() and category_dir.name == "analysis":
                for file_path in category_dir.glob("*.py"):
                    if (
                        file_path.name not in ["README.md"]
                        and file_path not in analysis_files
                    ):
                        analysis_files.append(file_path)

        # Create consolidated analysis script
        consolidated_analyzer = (
            self.consolidated_dir / "analysis" / "advanced_content_analyzer.py"
        )
...
def main():
    ...
    if args.text:
        ...
        if input_path.is_file():
            ...
            print(f"Analyzed {len(results)} files. Results saved to {output_dir}")
        else:
...
''',
    ...
    def merge_transcription_scripts(self):
        ...
        consolidated_transcriber = (
            self.consolidated_dir / "transcription" / "advanced_transcriber.py"
        )
...
def main():
    ...
    if input_path.is_file():
        ...
        print(f"Transcribed {len(results)} files. Results saved to {output_dir}")
...
''',
    ...
    def merge_generation_scripts(self):
        ...
        consolidated_generator = (
            self.consolidated_dir / "generation" / "advanced_content_generator.py"
        )
...
def main():
    ...
    if args.mode == "csv" and args.input and args.output:
        ...
        with open(args.output, 'w') as f:
            f.write(content)
        print(f"Video content generated: {args.output}")
...
''',
    ...
    def remove_duplicates(self):
        ...
        logger.info(
            f"Removed {removed_count} duplicate files (archived in {archive_dir})",
        )
    ...
    def create_consolidation_report(self):
        ...
        with open(report_path, "w") as f:
            ...
            f.write("## Summary\n\n")
            f.write(
                "This report documents the consolidation of Python music processing scripts.\n\n",
            )

            f.write("## Consolidated Scripts\n\n")
            f.write("### Analysis\n")
            f.write(
                "- `advanced_content_analyzer.py` - Comprehensive content analysis tool\n\n",
            )

            f.write("### Transcription\n")
            f.write(
                "- `advanced_transcriber.py` - Comprehensive transcription tool\n\n",
            )

            f.write("### Generation\n")
            f.write(
                "- `advanced_content_generator.py` - Comprehensive content generation tool\n\n",
            )

            f.write("## Duplicate Removal\n\n")
            f.write(
                "Duplicate files have been identified and archived in the `archived_duplicates` directory.\n\n",
            )

            f.write("## Usage\n\n")
            f.write(
                "Each consolidated script can be used independently or as part of a larger workflow.\n",
            )
            f.write(
