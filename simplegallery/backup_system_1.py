...
class AvaTarArTsBackupSystem:
    ...
    def create_backup_structure(self):
        ...
        print("✅ Backup structure created")
    ...
    def backup_project(self, project_name: str, project_info: dict):
        ...
        print(f"✅ {project_name} backed up: {len(backed_up_files)} files, {len(backed_up_dirs)} directories")
        return manifest
...
"""
    ...
    def run_backup(self):
        ...
        print(f"📊 Total Projects: {len(manifests)}")
        print(f"📄 Total Files: {sum(m['total_files'] for m in manifests)}")
        print(f"📁 Total Directories: {sum(m['total_directories'] for m in manifests)}")
        print(f"\n🔄 To update: python3 {self.backup_dir}/update_backup.py")

def main():
    """Main function"""
    backup_dir = "/Users/steven/AvaTarArTs_Backup_20251014_143236"
