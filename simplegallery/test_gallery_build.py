...
class SPGBuildTestCase(unittest.TestCase):
    ...
    @mock.patch("builtins.input", side_effect=["", "Default description of my gallery", "", ""])
    def test_index_html(self, input):
        with TempDirectory() as tempdir:
            ...
            files = os.listdir(os.path.join(tempdir.path, "public"))
            files = [f for f in files if f != '.DS_Store']
            self.assertEqual(set(files), set(["css", "images", "js", "index.html"]))
            ...
            with open(
                ...
                self.assertIn("<title>Λ∀ʌ†ʌʀ 🦄 ∆ʀ†s</title>", html)
                self.assertIn(f"<h2>{os.path.basename(tempdir.path)}</h2>", html)
                default_description = "Default description of my gallery"
                self.assertIn(f"<p>{default_description}</p>", html)
