...
class HTMLOrganizer:
    ...
    def find_html_files(self):
        ...
        for html_file in self.documents_dir.rglob("*.html"):
            ...
            if any(skip in str(html_file) for skip in ['site-packages', '.venv', '__pycache__', '.git']):
                continue
    ...
    def find_site_folders(self):
        ...
        for pattern in site_patterns:
            for folder in self.documents_dir.rglob(pattern):
                if folder.is_dir() and not any(skip in str(folder) for skip in ['site-packages', '.venv', '__pycache__']):
    ...
    def analyze_html_content(self, html_file):
        ...
        try:
            ...
            indicators = {
                'portfolio': ['portfolio', 'about me', 'resume', 'cv', 'skills', 'experience'],
    ...
    def suggest_site_folder(self, html_file):
        ...
        for site_folder in self.site_folders:
            ...
            if any(pattern in site_name for pattern in ['html', 'web', 'site', 'portfolio', 'gallery']):
    ...
    def organize_html_files(self):
        ...
        for html_file in self.html_files:
            try:
                ...
                if any(site_folder in html_file.parents for site_folder in self.site_folders):
                    continue
                ...
                if suggested_folder:
                    ...
                    if not dest_path.exists():
                        ...
                        logger.info(f"Moved {html_file.name} to {suggested_folder.name}")
                else:
                    # Move to appropriate category folder
                    category = content_type if content_type in site_categories else 'misc'
