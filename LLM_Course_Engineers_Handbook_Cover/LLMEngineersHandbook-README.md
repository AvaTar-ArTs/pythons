...
The course is now mirrored in plain `.py` files so you can lint, diff, and orchestrate everything outside of Google Colab. Each script was exported with `jupyter nbconvert --to python` and lives next to its source notebook for easy auditing.

### Script catalog

| Script | Focus | Highlights |
|--------|-------|------------|
| `4_bit_LLM_Quantization_with_GPTQ.py` | Quantization | 4-bit GPTQ workflow for open-weight models, including config hints for AutoGPTQ. |
| `Decoding_Strategies_in_Large_Language_Models.py` | Generation | Side-by-side decoding demos (greedy, beam, nucleus, etc.) to visualize sampling trade-offs. |
| `Fine_tune_LLMs_with_Axolotl.py` | Fine-tuning | Axolotl-powered QLoRA recipe with config generation, training, merging, and HF upload calls. |
| `Fine_tune_Llama_2_in_Google_Colab.py` | Fine-tuning | Classic SFT walkthrough for Llama 2 that still works when you swap in your own dataset/model IDs. |
| `Fine_tune_a_Mistral_7b_model_with_DPO.py` | Alignment | Direct Preference Optimization loop plus launch commands for TRL + accelerator. |
| `Introduction_to_Weight_Quantization.py` | Quantization | Gentle ramp from FP16 to int4/int8 along with code to profile accuracy vs. compression. |
| `Mergekit.py` | Merging | Minimal MergeKit CLI examples (DARE, SLERP, TIES) so you can remix checkpoints offline. |
| `Quantize_Llama_2_models_using_GGUF_and_llama_cpp.py` | Deployment | GGUF export plus `llama.cpp` invocations for CPU/GPU-friendly inference binaries. |
| `Quantize_models_with_ExLlamaV2.py` | Serving | EXL2 quantization + runtime harness to benchmark throughput on gamer GPUs. |

### Running a script locally

1. Create an isolated environment: `python3 -m venv .venv && source .venv/bin/activate`.
2. Install the common stack (huggingface_hub, transformers, accelerate, bitsandbytes, peft, ipython) and any extra packages mentioned inside the script comments.
3. Execute with IPython so cell magics keep working: `ipython Fine_tune_LLMs_with_Axolotl.py`.
4. Swap model names, dataset IDs, or Hugging Face tokens via environment variables instead of editing secrets into the files.

> ⚠️ Hot tip: the exports still contain `get_ipython()` calls, shell magics, and Colab helpers. They run best with `ipython` or `jupyter console`. Refactor to pure functions before embedding them inside production services.

### Staying in sync with the notebooks

- Regenerate any script after editing the notebook: `jupyter nbconvert --to python Fine_tune_Llama_2_in_Google_Colab.ipynb`.
- If you need two-way sync, pair the files with [Jupytext](https://github.com/mwouts/jupytext) (`jupytext --set-formats ipynb,py:percent notebook.ipynb`) so every save updates both representations.
- Check diffs into version control only for the `.py` files to keep reviews readable, and treat the notebooks as canonical references when screenshots or rich outputs matter.

Once you're comfortable running everything headlessly, jump into the roadmap below to choose what to learn/build next. 
