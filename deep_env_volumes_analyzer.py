...
class DeepEnvVolumesAnalyzer:
    ...
    def analyze_volumes(self) -> Dict[str, VolumeAnalysis]:
        ...
        volumes = [v for v in self.volumes_dir.iterdir() 
    ...
    def analyze_volume(self, volume_path: Path) -> VolumeAnalysis:
        ...
        if 'mp3' in file_types or 'wav' in file_types or 'flac' in file_types:
            opportunities.append("Audio files detected - music management system integration")
        if 'jpg' in file_types or 'png' in file_types or 'svg' in file_types:
            opportunities.append("Image files detected - art gallery or print-on-demand opportunities")
        if 'mp4' in file_types or 'mov' in file_types:
    ...
    def categorize_file(self, filepath: Path, ext: str) -> str:
        ...
        # Images
        if ext_lower in ['.jpg', '.jpeg', '.png', '.gif', '.svg', '.webp', '.bmp', '.tiff']:
            return 'image'
        ...
        # Code
        if ext_lower in ['.py', '.js', '.ts', '.html', '.css', '.json', '.xml', '.yaml', '.yml']:
            return 'code'
    ...
    def generate_insights(
        ...
        # Resource Mapping
        resource_mapping = {
            'api_services': sorted(list(all_services)),
        ...
        for vol_name, vol_analysis in volumes_analysis.items():
            resource_mapping['content_distribution'][vol_name] = vol_analysis.content_categories
        ...
        # Integration
        if all_services:
            recommendations.append(f"Leverage {len(all_services)} API services for multi-AI orchestration")
...
"""
        for vol_name, categories in analysis.resource_mapping.get('content_distribution', {}).items():
            report += f"#### {vol_name}\n"
...
def main():
    ...
    parser.add_argument('--output', default='~/analysis_reports',
