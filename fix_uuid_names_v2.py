#!/usr/bin/env python3
"""
Script to rename UUID-named directories and files to proper song titles
    ...
    for item in Path(root_dir).iterdir():
        if item.is_dir():
            ...
            if re.match(uuid_pattern, dir_name.lower()):
                # Normalize the directory name to lowercase and convert underscores to hyphens for lookup
                lookup_key = dir_name.lower().replace("_", "-")
                ...
                if lookup_key in uuid_mapping:
                    ...
                    lookup_key_underscore = dir_name.lower().replace("-", "_")
                    if lookup_key_underscore in uuid_mapping:
                        new_name = uuid_mapping[lookup_key_underscore]
                    else:
