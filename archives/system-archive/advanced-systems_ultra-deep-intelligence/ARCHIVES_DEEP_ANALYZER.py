...
class ArchivesDeepAnalyzer:
    ...
    def scan_archives(self):
        ...
        print(
            f"{C.MAGENTA}   💾 Total Size: {self.stats['total_size'] / (1024**3):.2f} GB{C.END}\n"
        )
    ...
    async def analyze_markdown_files(self):
        ...
        print(
            f"{C.YELLOW}Analyzing {len(self.markdown_files)} markdown files...{C.END}\n"
        )
    ...
    def analyze_archive_contents(self):
        ...
        for archive in sorted(self.zip_files, key=lambda x: x["size"], reverse=True)[
            :20
        ]:
    ...
    def detect_duplicates(self):
        ...
        print(
            f"{C.YELLOW}   Total duplicate files: {self.stats['duplicates_found']}{C.END}\n"
        )
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            if duplicates:
                ...
                for i, (file_hash, files) in enumerate(
                    list(duplicates.items())[:20], 1
                ):
                    f.write(f"### Duplicate Group #{i}\n")
                    f.write(f"**Hash:** `{file_hash[:16]}...`\n\n")
                    for file_info in files:
                        f.write(
                            f"- `{file_info['relative_path']}` ({file_info['size'] / 1024:.1f} KB)\n"
                        )
                    f.write("\n")
    ...
    async def run_complete_analysis(self):
        ...
        print(
            "║        Specialized Analysis for Archives Directory                            ║"
        )
        print(
            "║                                                                               ║"
        )
