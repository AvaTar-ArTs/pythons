...
class HomeDirectoryScanner:
    ...
    def scan_home_directory(self, max_depth: int = 4):
                    ...
                    try:
                        ...
                        if item.is_dir():
                            ...
                            if project:
                                ...
                                print(
                                    f"{C.GREEN}📦 Project found: {item.relative_to(self.home_path)}{C.END}"
                                )

                            # Progress
                            if self.stats["total_dirs"] % 100 == 0:
                                print(
                                    f"{C.CYAN}  Scanned {self.stats['total_dirs']} dirs, {self.stats['total_files']:,} files...{C.END}"
                                )
        ...
        print(
            f"{C.MAGENTA}   Size: {self.stats['total_size'] / (1024**3):.2f} GB{C.END}\n"
        )
    ...
    def categorize_file(self, file_path: Path, size: int):
        ...
        if file_path.name in [
            ...
            self.inventory["configs"].append(
                {"path": str(file_path), "name": file_path.name, "size": size}
            )
    ...
    async def analyze_key_content(self):
        ...
        py_scripts_sorted = sorted(py_scripts, key=lambda x: x["size"], reverse=True)[
            :20
        ]
    ...
    def discover_api_keys(self):
        ...
        for config in api_configs[:10]:
            print(
                f"{C.CYAN}  🔑 {Path(config['path']).relative_to(self.home_path)}{C.END}"
            )
    ...
    def generate_master_map(self):
        ...
        report_path = Path(
            f"/Users/steven/advanced-systems/ultra-deep-intelligence/reports/HOME_DIRECTORY_DEEP_SCAN_{timestamp}.md"
        )
        ...
        with open(report_path, "w") as f:
            ...
            if self.inventory["projects"]:
                ...
                sorted_projects = sorted(
                    self.inventory["projects"], key=lambda x: x["size"], reverse=True
                )
            ...
            if self.inventory["scripts"]:
                ...
                for script_type, scripts in sorted(
                    scripts_by_type.items(), key=lambda x: len(x[1]), reverse=True
                ):
                    ...
                    largest = sorted(scripts, key=lambda x: x["size"], reverse=True)[
                        :10
                    ]
            ...
            if self.inventory["data"]:
                ...
                for data_type, count in sorted(
                    data_by_type.items(), key=lambda x: x[1], reverse=True
                ):
            ...
            f.write(
                f"1. Found {self.stats['projects_found']} projects - consider consolidating similar ones\n"
            )
            f.write(
                f"2. Found {self.stats['configs_found']} config files - centralize API keys\n"
            )
            f.write(
                f"3. Found {self.stats['scripts_found']} scripts - organize by function\n"
            )
            ...
            f.write(
                f"- Review {len(self.inventory['media'])} media files for duplicates\n"
            )
            f.write("- Archive old projects to external storage\n")
            f.write(
                f"- Consider removing unused node_modules (skipped {self.stats['skipped_dirs']} dirs)\n\n"
            )
    ...
    async def run(self):
        ...
        print(
            "║        Complete Content-Aware Analysis of Entire Home Directory               ║"
        )
        print(
            "║                                                                               ║"
        )
