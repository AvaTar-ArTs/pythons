...
class FolderDeepDive:
    ...
    def __init__(self, folder_path: str, api_manager: APIKeyManager):
        ...
        self.vector_manager = VectorDatabaseManager(
            api_manager.available_apis["vector"]
        )
    ...
    def scan_folder(self):
                ...
                if item.is_file():
                    ...
                    if item.parent == self.folder_path:  # Direct subdirectory
                        ...
                        self.inventory["subdirs"].append(
                            {"name": item.name, "path": str(item), "size": dir_size}
                        )
        ...
        print(
            f"{C.GREEN}✅ Scanned: {self.stats['total_files']:,} files in {self.stats['total_dirs']:,} directories{C.END}"
        )
        print(
            f"{C.MAGENTA}   Total size: {self.stats['total_size'] / (1024**2):.2f} MB{C.END}\n"
        )
    ...
    async def analyze_content(self, max_files: int = 20):
        ...
        for i, file_info in enumerate(to_analyze, 1):
            try:
                ...
                if len(content) > 100:
                    print(
                        f"{C.CYAN}  [{i}/{len(to_analyze)}] {file_info['name'][:50]}...{C.END}"
                    )
                    ...
                    analysis = {"file": file_info, "results": results}
    ...
    def generate_insights(self):
        ...
        if self.stats["total_size"] > 1_000_000_000:
            self.insights.append(
                f"⚠️ Large folder ({self.stats['total_size'] / (1024**3):.2f} GB) - consider archiving"
            )

        # File type insights
        if self.stats["code_files"] > 100:
            self.insights.append(
                f"💻 Code repository with {self.stats['code_files']} code files"
            )

        if self.stats["media_files"] > 100:
            self.insights.append(
                f"🎨 Media collection with {self.stats['media_files']} media files"
            )

        # Subdirectory insights
        if len(self.inventory["subdirs"]) > 50:
            self.insights.append(
                f"📁 Complex structure with {len(self.inventory['subdirs'])} subdirectories"
            )
    ...
    def generate_folder_report(self) -> str:
        ...
        report_path = Path(
            f"/Users/steven/advanced-systems/ultra-deep-intelligence/reports/folders/FOLDER_{self.folder_name}_{timestamp}.md"
        )
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                f"| **Total Size** | {self.stats['total_size'] / (1024**2):.2f} MB |\n"
            )
            ...
            if self.inventory["subdirs"]:
                ...
                sorted_subdirs = sorted(
                    self.inventory["subdirs"], key=lambda x: x["size"], reverse=True
                )
                ...
                for subdir in sorted_subdirs[:30]:
                    f.write(
                        f"| `{subdir['name']}` | {subdir['size'] / (1024**2):.2f} MB |\n"
                    )
            ...
            sorted_types = sorted(
                self.inventory["by_type"].items(), key=lambda x: len(x[1]), reverse=True
            )
            ...
            for ext, files in sorted_types[:20]:
                ...
                f.write(
                    f"| `{ext_display}` | {len(files):,} | {total_size / (1024**2):.2f} MB |\n"
                )
            f.write("\n")
            ...
            if self.inventory["content_analyzed"]:
                ...
                for i, analysis in enumerate(
                    self.inventory["content_analyzed"][:10], 1
                ):
    ...
    async def analyze(self):
        ...
        if self.inventory["subdirs"]:
            ...
            for subdir in sorted_subdirs:
                print(
                    f"{C.CYAN}📂 {subdir['name']:<40} {subdir['size'] / (1024**2):>8.2f} MB{C.END}"
                )
        ...
        for ext, files in sorted_types:
            ...
            print(
                f"{C.CYAN}{ext_display:<15} {len(files):>6,} files  {total_size / (1024**2):>8.2f} MB{C.END}"
            )
...
class FolderByFolderAnalyzer:
    ...
    async def analyze_all(self):
        ...
        print(
            "║        Systematic Deep-Dive of Each Folder in ~/Documents                     ║"
        )
        print(
            "║                                                                               ║"
        )
        ...
        for i, folder in enumerate(folders, 1):
            ...
            if report:
                self.folder_reports.append(
                    {"folder": folder.name, "report": str(report)}
                )
    ...
    def generate_master_index(self):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        index_path = Path(
            f"/Users/steven/advanced-systems/ultra-deep-intelligence/reports/MASTER_FOLDER_INDEX_{timestamp}.md"
        )
