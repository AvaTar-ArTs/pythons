...
class DuplicateFixer:
    ...
    def find_all_duplicates(self):
        ...
        print(
            f"{C.GREEN}✅ Found {self.stats['groups_found']} duplicate groups{C.END}\n"
        )
    ...
    def display_duplicates_report(self):
        ...
        for i, (file_hash, files) in enumerate(sorted_groups[:50], 1):
            ...
            print(
                f"{C.YELLOW}Size:{C.END} {analysis['size']:,} bytes ({size_mb:.2f} MB)"
            )
            ...
            print(
                f"   Modified: {datetime.fromtimestamp(analysis['original'].stat().st_mtime).strftime('%Y-%m-%d %H:%M')}"
            )
            print()

            # Show duplicates (to remove)
            print(
                f"{C.RED}🗑️  REMOVE ({len(analysis['duplicates'])} duplicates):{C.END}"
            )
            for dup in analysis["duplicates"]:
                mod_time = datetime.fromtimestamp(dup.stat().st_mtime).strftime(
                    "%Y-%m-%d %H:%M"
                )
            ...
            self.stats["space_to_free"] += analysis["size"] * len(
                analysis["duplicates"]
            )
    ...
    def generate_fix_script(self):
        ...
        script_path = Path(
            f"/Users/steven/advanced-systems/ultra-deep-intelligence/fix_duplicates_{timestamp}.sh"
        )
        ...
        with open(script_path, "w") as f:
            ...
            backup_dir = (
                f"/Users/steven/Documents/Backups/duplicate_fix_backup_{timestamp}"
            )
    ...
    def generate_detailed_report(self):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = Path(
            f"/Users/steven/advanced-systems/ultra-deep-intelligence/reports/DUPLICATES_TO_FIX_{timestamp}.md"
        )
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                f"- **Space to Free:** {self.stats['space_to_free'] / (1024**2):.2f} MB\n\n"
            )
            ...
            for i, (file_hash, files) in enumerate(sorted_groups, 1):
                ...
                f.write(
                    f"**Size:** {analysis['size']:,} bytes ({analysis['size'] / 1024 / 1024:.2f} MB)\n"
                )
                ...
                mod_time = datetime.fromtimestamp(
                    analysis["original"].stat().st_mtime
                ).strftime("%Y-%m-%d %H:%M")
    ...
    def run(self):
        ...
        print(
            f"  💾 Space to free: {C.BOLD}{self.stats['space_to_free'] / (1024**2):.2f} MB{C.END}"
        )
        print()
