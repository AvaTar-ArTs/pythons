...
"""
            ...
            print(f"{Colors.YELLOW}{Emojis.WARN} AI analysis failed: {e}{Colors.END}")
            return self._fallback_analysis(code_snippet, filepath)
        ...
        try:
            ...
            response = openai.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert code analyzer. Always respond with valid JSON.",
                    },
            ...
            print(f"{Colors.RED}{Emojis.ERROR} OpenAI error: {e}{Colors.END}")
    ...
    def _query_gemini(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"{Colors.RED}{Emojis.ERROR} Gemini error: {e}{Colors.END}")
            return self._fallback_analysis(prompt, "")
    ...
    def _query_anthropic(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"{Colors.RED}{Emojis.ERROR} Anthropic error: {e}{Colors.END}")
            return self._fallback_analysis(prompt, "")
    ...
    def get_code_embedding(self, code: str, identifier: str) -> Optional[List[float]]:
        ...
        try:
            ...
            response = openai.embeddings.create(
                model="text-embedding-3-small", input=code_sample
            )
            ...
            print(f"{Colors.YELLOW}{Emojis.WARN} Embedding failed: {e}{Colors.END}")
            return None
...
class AIDeepIntelligentAnalyzer:
    ...
    def extract_python_metadata(self, filepath: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if metadata["loc"] > 10 and metadata["loc"] < 1000:  # Reasonable size
                if self.stats["ai_analyzed"] < 100:  # Limit AI calls
                    print(
                        f"{Colors.CYAN}{Emojis.BRAIN} AI analyzing: {filepath.name}{Colors.END}"
                    )
                    ai_result = self.ai_analyzer.analyze_code_with_ai(
            ...
            if metadata["loc"] > 50 and self.stats["ai_analyzed"] < 50:
                embedding = self.embedding_analyzer.get_code_embedding(
                    content, str(filepath)
                )
                if embedding:
    ...
    def scan_directory(self, directory: str, current_depth: int = 0):
        ...
        try:
            ...
            if not dir_path.exists():
                print(f"{Colors.RED}{Emojis.ERROR} Not found: {directory}{Colors.END}")
                return

            print(
                f"{Colors.CYAN}{Emojis.FOLDER} Scanning: {directory} (depth: {current_depth}/{self.max_depth}){Colors.END}"
            )
            ...
            print(f"{Colors.RED}{Emojis.ERROR} Error: {directory}: {e}{Colors.END}")
    ...
    def process_file(self, filepath: Path, source_dir: str):
        ...
        try:
            ...
            if file_info["is_python"]:
                ...
                file_info["category"] = file_info["metadata"].get(
                    "category", "uncategorized"
                )
    ...
    def analyze_duplicates(self):
        ...
        self.stats["exact_dupes"] = sum(
            len(files) - 1 for files in exact_dupes.values()
        )
        self.stats["unique_files"] = (
            self.stats["total_files"] - self.stats["exact_dupes"]
        )

        print(
            f"{Colors.GREEN}{Emojis.CHECK} Exact Duplicates: {len(exact_dupes)} groups{Colors.END}"
        )
        print(
            f"{Colors.BLUE}   Duplicate files: {self.stats['exact_dupes']}{Colors.END}"
        )
        ...
        print(
            f"{Colors.GREEN}{Emojis.CHECK} Semantic Groups: {self.stats['semantic_dupes']}{Colors.END}"
        )

        # Category analysis
        print(f"\n{Colors.MAGENTA}{Emojis.SPARKLES} AI-Powered Categories:{Colors.END}")
        for category, files in sorted(
            self.category_groups.items(), key=lambda x: len(x[1]), reverse=True
        )[:10]:
            print(f"  {Colors.CYAN}{category}{Colors.END}: {len(files)} files")
    ...
    def create_detailed_report(self, exact_dupes: Dict, semantic_dupes: Dict):
        ...
        with open(report_file, "w", encoding="utf-8") as report:
            ...
            report.write(
                f"| **Total Size** | {self.stats['total_size'] / (1024**3):.2f} GB |\n"
            )
            ...
            report.write(
                f"| **Semantic Groups** | {self.stats['semantic_dupes']:,} |\n"
            )
            ...
            for category, files in sorted(
                ...
                for file_path in files[:5]:
                    ...
                    if ai_analysis:
                        report.write(
                            f"  - Purpose: {ai_analysis.get('purpose', 'N/A')}\n"
                        )
                        report.write(
                        ...
                        if patterns:
                            report.write(
                                f"  - Patterns: {', '.join([p[0] for p in patterns[:2]])}\n"
                            )
            ...
            for category in sorted(self.category_groups.keys())[:10]:
                ...
                report.write(
                    f"- **{category}**: {count} files → Move to `{category}/`\n"
                )
        ...
        print(f"{Colors.GREEN}{Emojis.CHECK} Diffs: {diff_dir}{Colors.END}")
    ...
    def create_json_export(self):
        ...
        print(f"{Colors.GREEN}{Emojis.CHECK} JSON: {json_file}{Colors.END}")
        return json_file
    ...
    def run_complete_analysis(self):
        ...
        print(
            "║        Advanced AI-Driven Code Analysis & Pattern Recognition System         ║"
        )
        print(
            "║                                                                               ║"
        )
        ...
        print(
            f"{Colors.YELLOW}{Emojis.GEAR} Max Depth: {self.max_depth} levels{Colors.END}\n"
        )
        ...
        print(
            f"  {Emojis.FILE} Files: {Colors.CYAN}{self.stats['total_files']:,}{Colors.END}"
        )
        print(
            f"  {Emojis.PYTHON} Python: {Colors.CYAN}{self.stats['py_files']:,}{Colors.END}"
        )
        print(
            f"  {Emojis.BRAIN} AI Analyzed: {Colors.CYAN}{self.stats['ai_analyzed']:,}{Colors.END}"
        )
        print(
            f"  {Emojis.FIRE} Patterns: {Colors.CYAN}{self.stats['patterns_detected']:,}{Colors.END}"
        )
        print(
            f"  💾 Size: {Colors.CYAN}{self.stats['total_size'] / (1024**3):.2f} GB{Colors.END}\n"
        )

        print(f"{Colors.BOLD}📝 OUTPUTS:{Colors.END}\n")
        print(f"  {Emojis.FILE} Report: {Colors.BLUE}{report_file}{Colors.END}")
        print(f"  {Emojis.FOLDER} Diffs: {Colors.BLUE}{diff_dir}{Colors.END}")
        print(f"  {Emojis.CHART} Data: {Colors.BLUE}{json_file}{Colors.END}\n")
...
def main():
    ...
    if Path(env_file).exists():
        print(f"{Colors.CYAN}{Emojis.GEAR} Loading API keys...{Colors.END}")
    ...
    directories = [
        "/Users/steven/Documents/python-repo",
