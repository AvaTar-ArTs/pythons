...
from ULTRA_DEEP_INTELLIGENCE_ORCHESTRATOR import C, E, APIKeyManager, MultiLLMAnalyzer
    ...
    async def analyze_file_content(self, file_path: Path) -> dict:
        ...
        try:
            ...
            if file_path.stat().st_size > 500000:
                return {
                    "category": f"large_{file_path.suffix[1:]}_files",
                    "confidence": 0.5,
                }
...
"""
            ...
            if any(
                word in content_lower for word in ["api", "code", "function", "class"]
            ):
                return {"category": "technical", "confidence": 0.6}
            elif any(
                word in content_lower
                for word in ["business", "price", "service", "client"]
            ):
                return {"category": "business", "confidence": 0.6}
            elif any(
                word in content_lower for word in ["story", "creative", "art", "design"]
            ):
    ...
    def execute_plan(self):
        ...
        for item in self.move_plan:
            ...
            print(
                f"{C.CYAN}  {conf_emoji} {file.name[:50]:.<50} → {target_folder}/{C.END}"
            )
    ...
    async def run(self):
        ...
        print(
            "║        AI Reads Content to Understand Purpose, Not Just File Type             ║"
        )
        print(
            "║                                                                               ║"
        )
        ...
        print(
            f"  🗂️  Categories created: {C.BOLD}{self.stats['categories_created']}{C.END}\n"
        )
