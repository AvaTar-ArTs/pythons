...
from utils.queue_manager import AsyncBatchQueue
from cli import cli
...
class EnhancedContentIntelligence:
    ...
    async def analyze_file_with_cache(:
        ...
        content_hash = content[:1000].decode(
            "utf-8", errors="ignore"
        )  # First 1KB for hash
        ...
        if not force:
            cached = self.cache_manager.get(
                content_hash, operation, content_type=content_type
            )
    ...
    async def _analyze_audio(self, path: Path) -> dict:
        ...
        return {"quality_score": 85, "metadata": {"title": path.stem}}

    async def _analyze_video(self, path: Path) -> dict:
        """Placeholder for video analysis"""
        return {"quality_score": 85, "duration": 0}

    async def _analyze_text(self, path: Path) -> dict:
        """Placeholder for text analysis"""
        with open(path, "r") as f:
            text = f.read()
        return {"word_count": len(text.split()), "quality_score": 85}

    async def batch_analyze_directory(:
        self, directory: Path, recursive: bool = True, force: bool = False
    ) -> dict:
        """
        Analyze entire directory with progress tracking
        """
        cli.print_header("📁 Batch Analysis", f"Directory: {directory}")
    ...
    def _print_batch_statistics(self, results: list):
        ...
        if RICH_AVAILABLE:
            ...
            cli.print_table(
                "📊 Analysis Summary", table_data, ["Content Type", "Count"]
            )
            ...
            cli.print_info("\n💾 Cache Statistics:")
            cli.console.print(
                f"   Cache Hit Rate: [green]{cache_stats['cache_hit_rate']}%[/green]"
            )
            cli.console.print(
                f"   Cost Saved: [green]${cache_stats['total_cost_saved']:.2f}[/green]"
            )
        else:
            print("\n📊 Analysis Summary:")
            for ctype, count in sorted(by_type.items()):
    ...
    def show_api_capabilities(self):
        ...
        if RICH_AVAILABLE:
            self.console.print(
                Panel(report, border_style="green", title="API Capabilities")
            )
        else:
            print(report)
    ...
    if len(sys.argv) < 2:
        ...
        if command == "apis":
            ...
            results = await system.batch_analyze_directory(
                directory, recursive=recursive, force=force
            )
    ...
    while True:
        ...
        if choice == "1":
            ...
            await system.batch_analyze_directory(
                Path(directory), recursive=recursive, force=force
            )
            ...
            print("\n✅ Analysis complete!")
            print(
                f"Quality Score: {result.get('analysis', {}).get('quality_score', 'N/A')}"
            )
