...
class PythonOrganizer:
    ...
    def _analyze_class(self, node: ast.ClassDef, file_path: Path) -> PythonClass:
        ...
        for item in node.body:
            if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                method.is_static_method = any(
                    "staticmethod" in dec for dec in method.decorators
                )
    ...
    def _calculate_metrics(self):
        ...
        avg_function_length = (
            sum(func.lines_of_code for func in self.functions) / total_functions
            if total_functions > 0
            else 0
        )
        avg_complexity = (
            sum(func.complexity for func in self.functions) / total_functions
            if total_functions > 0
            else 0
        )
    ...
    def _find_duplicate_functions(self) -> List[Tuple[str, List[str]]]:
        ...
        return [
            (sig, files) for sig, files in function_signatures.items() if len(files) > 1
        ]
    ...
    def _generate_organization_suggestions(self) -> List[Dict[str, Any]]:
        ...
        small_modules = [
            m for m in self.modules if m.lines_of_code < 50 and len(m.functions) < 3
        ]
        if small_modules:
            suggestions.append(
                {
    ...
    def _calculate_quality_score(self) -> int:
        ...
        docstring_ratio = self.code_metrics["functions_with_docstrings"] / max(
            self.code_metrics["total_functions"], 1
        )
    ...
    def _get_top_recommendations(self) -> List[str]:
        ...
        if (
            self.code_metrics["functions_with_docstrings"]
            / max(self.code_metrics["total_functions"], 1)
            < 0.5
        ):
