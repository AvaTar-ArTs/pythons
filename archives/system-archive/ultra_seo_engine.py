...
"""
    ...
    def _audit_on_page(:
        self, content: str, target_keyword: Optional[str]
    ) -> Dict[str, Any]:
        ...
        if title:
            if (
                self.seo_standards["title_min"]
                <= len(title)
                <= self.seo_standards["title_max"]
            ):
                score += 6
            else:
        ...
        if headers >= self.seo_standards["min_headers"]:
            ...
            issues.append(
                f"Headers: {headers} (minimum: {self.seo_standards['min_headers']})"
            )
        ...
        if target_keyword:
            ...
            if (
                self.seo_standards["keyword_density_min"]
                <= keyword_data["density"]
                <= self.seo_standards["keyword_density_max"]
            ):
                score += 5
            else:
                issues.append(
                    f"Keyword density: {keyword_data['density']:.2f}% (optimal: 1-3%)"
                )
        ...
        if (
            meta_desc
            and self.seo_standards["meta_desc_min"]
            <= len(meta_desc)
            <= self.seo_standards["meta_desc_max"]
        ):
            score += 5
    ...
    def _audit_content_quality(:
        ...
        # Word count
        if (
            self.seo_standards["optimal_word_count_min"]
            <= word_count
            <= self.seo_standards["optimal_word_count_max"]
        ):
            score += 10
...
async def demo():
    ...
    audit = await engine.ultra_seo_audit(
        content=sample_content, target_keyword="AI content creation"
    )
