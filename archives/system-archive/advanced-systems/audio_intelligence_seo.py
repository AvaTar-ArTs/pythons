...
class AudioIntelligence:
        ...
        try:
            if MUTAGEN_AVAILABLE:
                ...
                if audio:
                    # Basic info
                    technical["format"] = (
                        audio.mime[0] if hasattr(audio, "mime") else "unknown"
                    )
                    technical["duration_seconds"] = getattr(audio.info, "length", 0)
                    technical["duration_formatted"] = str(
                        timedelta(seconds=int(technical["duration_seconds"]))
                    )

                    # Audio quality
                    technical["bitrate"] = getattr(audio.info, "bitrate", 0)
                    technical["bitrate_kbps"] = (
                        round(technical["bitrate"] / 1000, 1)
                    ...
                    # Quality indicators
                    technical["is_lossless"] = audio_path.suffix.lower() in [
                        ".flac",
                        ".wav",
                    ]
                    technical["is_high_quality"] = (
                        technical["bitrate_kbps"] >= 320
                        if technical["bitrate_kbps"]
                        else False
                    )
        ...
        try:
            if MUTAGEN_AVAILABLE:
                ...
                if audio:
                    # Common tags
                    metadata["title"] = (
                        audio.get("title", [""])[0] if "title" in audio else ""
                    )
                    metadata["artist"] = (
                        audio.get("artist", [""])[0] if "artist" in audio else ""
                    )
                    metadata["album"] = (
                        audio.get("album", [""])[0] if "album" in audio else ""
                    )
                    metadata["genre"] = (
                        audio.get("genre", [""])[0] if "genre" in audio else ""
                    )
                    metadata["date"] = (
                        audio.get("date", [""])[0] if "date" in audio else ""
                    )
                    metadata["comment"] = (
                        audio.get("comment", [""])[0] if "comment" in audio else ""
                    )
                    ...
                    metadata["composer"] = (
                        audio.get("composer", [""])[0] if "composer" in audio else ""
                    )
                    metadata["bpm"] = (
                        audio.get("bpm", [""])[0] if "bpm" in audio else ""
                    )
        ...
        try:
            if MUTAGEN_AVAILABLE:
                ...
                if isinstance(audio, MP3):
                    return (
                        "APIC:" in audio
                        or len([k for k in audio.keys() if k.startswith("APIC")]) > 0
                    )
    ...
    async def _transcribe_audio(self, audio_path: Path) -> Optional[str]:
        ...
        try:
            ...
            if file_size_mb > 25:
                logger.warning(
                    f"File too large for transcription: {file_size_mb:.1f}MB"
                )
                return None
...
Format as JSON with keys: content_type, topics, genre, mood, audience, keywords, description"""
                ...
                return {"description": content[:500], "provider": "anthropic"}
    ...
    async def _generate_seo_metadata(:
        self, audio_path: Path, analysis: Dict
    ) -> Dict[str, Any]:
        ...
        seo["suggested_filename"] = self._generate_seo_filename(
            audio_path, metadata, ai
        )
    ...
    def _generate_audio_schema(:
        self, audio_path: Path, seo: Dict, analysis: Dict
    ) -> Dict[str, Any]:
    ...
    def _generate_recommendations(self, analysis: Dict) -> List[str]:
        ...
        if bitrate < 192:
            recommendations.append(
                "⚠️ Quality: Consider re-encoding at 256-320 kbps for better quality"
            )

        file_size_mb = technical.get("file_size_mb", 0)
        if file_size_mb > 50:
            recommendations.append(
                "🗜️ Optimization: Large file size. Consider compression for web streaming"
            )
        ...
        if not metadata.get("has_cover_art"):
            recommendations.append(
                "🎨 Cover Art: Add album/cover art for better presentation"
            )
        ...
        if len(seo.get("keywords", [])) < 5:
            recommendations.append(
                "🔍 SEO: Add more relevant keywords for discoverability"
            )

        desc = seo.get("description", "")
        if len(desc) < 50:
            recommendations.append(
                "📝 Description: Add detailed description (100-300 characters)"
            )
        ...
        if suggested and suggested != Path(analysis["file_name"]).name:
            recommendations.append(
                f"📁 Filename: Rename to '{suggested}' for better SEO"
            )

        if not recommendations:
            recommendations.append(
                "✅ Audio is well-optimized! No major improvements needed."
            )
    ...
    async def analyze_directory(:
        self, directory: Path, recursive: bool = True, skip_transcription: bool = False
    ) -> Dict[str, Any]:
        ...
        for i, audio_path in enumerate(audio_files, 1):
            ...
            try:
                ...
                results.append({"file_path": str(audio_path), "error": str(e)})
        ...
        output_file = (
            directory
            / f"audio_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        print("\n💡 Usage:")
        print(
            "   python audio_intelligence_seo.py <audio_file>    # Analyze single file"
        )
        print("   python audio_intelligence_seo.py <directory>     # Analyze directory")
        print(
            "   python audio_intelligence_seo.py <directory> -r  # Recursive analysis"
        )
        print("\n📝 Example:")
