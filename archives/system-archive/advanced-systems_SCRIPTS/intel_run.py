...
def iter_python_files(root: Path) -> Iterable[Path]:
    for path in root.rglob("*.py"):
        ...
        if "__pycache__" in path.parts:
            continue
        if any(
            part in {".venv", "venv", ".git", "node_modules"} for part in path.parts
        ):
            continue
        yield path
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run intelligent content-aware analysis over the clean project."
    )
    parser.add_argument(
        "paths", nargs="*", help="Optional specific files or directories to analyze"
    )
    parser.add_argument(
        "--report", default="reports", help="Directory for markdown reports"
    )
    parser.add_argument(
        "--model",
