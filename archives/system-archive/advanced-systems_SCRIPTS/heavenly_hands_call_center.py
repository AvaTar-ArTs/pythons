...
class HeavenlyHandsCallCenterAgent:
    ...
    def process_customer_message(self, call_id: str, message: str) -> str:
        ...
        relevant_knowledge = self.knowledge_base.find_relevant_knowledge(
            message, top_k=3
        )
    ...
    def analyze_intent(self, conversation_history: List[Dict[str, str]]) -> tuple:
        ...
        intent_scores["pricing"] = sum(
            1 for keyword in pricing_keywords if keyword in recent_text.lower()
        )
        ...
        intent_scores["testimonial"] = sum(
            1 for keyword in testimonial_keywords if keyword in recent_text.lower()
        )
    ...
    def generate_response(:
        self, message: str, relevant_knowledge: List[tuple], session: CallSession
    ) -> str:
        """Generate response using GPT with real business context"""
...
def main():
    ...
    for i, message in enumerate(test_messages, 1):
        ...
        print(
            f"📊 Intent: {summary['intent_classification']} | Confidence: {summary['confidence_score']:.2f}"
        )
