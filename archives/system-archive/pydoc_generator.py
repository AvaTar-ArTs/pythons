...
class PyDocGenerator:
    ...
    def _analyze_function(:
        ...
        func_doc = FunctionDoc(
            ...
            is_async=is_async,
            is_generator=isinstance(node, ast.AsyncFunctionDef)
            or any(isinstance(n, ast.Yield) for n in ast.walk(node)),
            is_class_method=any(
    ...
    def _analyze_class(self, node: ast.ClassDef, file_path: Path) -> ClassDoc:
        ...
        class_doc.is_abstract = any(
            "abstract" in dec for dec in [str(d) for d in node.decorator_list]
        )
        class_doc.is_dataclass = any(
            "dataclass" in dec for dec in [str(d) for d in node.decorator_list]
        )
        ...
        for item in node.body:
            if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                method_doc.is_static_method = any(
                    "staticmethod" in dec for dec in method_doc.decorators
                )
    ...
    def _generate_all_documentation(self):
        ...
        for module in self.modules:
            self.documentation[f"module_{module.name}"] = self._generate_module_doc(
                module
            )

        # Generate function documentation
        for func in self.functions:
            self.documentation[f"function_{func.name}"] = self._generate_function_doc(
                func
            )
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "documentation_coverage": {
                "modules": f"{modules_with_docs}/{total_modules} ({modules_with_docs / total_modules * 100:.1f}%)"
                if total_modules > 0
                else "0/0 (0%)",
                "functions": f"{functions_with_docs}/{total_functions} ({functions_with_docs / total_functions * 100:.1f}%)"
                if total_functions > 0
                else "0/0 (0%)",
                "classes": f"{classes_with_docs}/{total_classes} ({classes_with_docs / total_classes * 100:.1f}%)"
                if total_classes > 0
                else "0/0 (0%)",
            },
            "average_complexity": sum(f.complexity for f in self.functions)
            / max(total_functions, 1),
            "most_complex_functions": sorted(
                self.functions, key=lambda x: x.complexity, reverse=True
            )[:5],
            "largest_functions": sorted(
                self.functions, key=lambda x: x.lines_of_code, reverse=True
            )[:5],
        }
...
def main():
    ...
    print(
        f"   📝 Documentation Coverage: {results['summary']['documentation_coverage']}"
    )
