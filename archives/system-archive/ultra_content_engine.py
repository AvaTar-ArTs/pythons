...
"""
    ...
    async def generate_ultra_quality_content(:
        ...
        for iteration in range(1, max_iterations + 1):
            ...
            if iteration == 1:
                ...
                content = await self._generate_improved_version(
                    topic, best_result["content"], critique
                )
            ...
            logger.info(
                f"   Breakdown: R:{quality_score['readability']}, E:{quality_score['engagement']}, SEO:{quality_score['seo']}, S:{quality_score['structure']}"
            )
        ...
        final_package = await self._create_complete_package(
            best_result, topic, content_type
        )

        logger.info(f"\n{'=' * 80}")
        logger.info("🎉 ULTRA GENERATION COMPLETE!")
...
Make this the definitive technical resource.""",
    ...
    def _score_seo_ultra(self, content: str) -> float:
        ...
        link_worthy = content.lower().count("learn more") + content.lower().count(
            "read about"
        )
    ...
    def _score_structure_ultra(self, content: str, content_type: str) -> float:
        ...
        if any(
            ...
            for word in ["imagine", "discover", "what if", "?", "did you know"]
        ):
            score += 7
        ...
        if any(
            word in closing
            for word in ["conclusion", "summary", "finally", "ultimately", "in short"]
        ):
            score += 6
    ...
    async def _generate_improved_version(:
        self, topic: str, previous_content: str, critique: str
    ) -> str:
...
Make it EXCEPTIONAL."""
    ...
    async def _create_complete_package(:
        self, result: Dict, topic: str, content_type: str
    ) -> Dict[str, Any]:
...
Provide ONLY the 5 titles, one per line."""
            ...
            titles = [
                t.strip() for t in response.content[0].text.split("\n") if t.strip()
            ]
            return titles[:5]
    ...
    def _generate_schema(:
        self, topic: str, description: str, keywords: List[str]
    ) -> Dict:
