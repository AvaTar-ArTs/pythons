        ...
        for file_path in self.documents_root.rglob("*"):
            if file_path.is_file():
                ...
                if file_type:
                    ...
                    self.statistics["total_size_mb"] += file_path.stat().st_size / (
                        1024 * 1024
                    )
...
class DocumentIntelligence:
    ...
    async def index_all_documents(:
        self, max_files: int = None, analyze_content: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        output_file = (
            Path.home()
            / f"document_index_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    async def _quick_analyze_document(self, doc_path: Path) -> Dict[str, Any]:
        ...
        analysis = {"keywords": [], "summary": "", "category": ""}
    ...
    async def semantic_search(:
        ...
        for doc in self.scanner.file_catalog[:1000]:  # Limit for demo
            if "analysis" in doc:
                ...
                if overlap > 0:
                    results.append(
                        {**doc, "relevance_score": overlap / len(query_keywords)}
                    )
    ...
    def get_insights(self) -> Dict[str, Any]:
        ...
        if stats["total_files"] > 10000:
            insights["recommendations"].append(
                "Large document collection - consider vector search indexing"
            )

        if stats["by_type"]["markdown"] > 1000:
            insights["recommendations"].append(
                "Significant markdown content - perfect for knowledge base"
            )

        if stats["by_type"]["html"] > 1000:
            insights["recommendations"].append(
                "Many HTML files - consider static site consolidation"
            )
...
async def main():
    ...
    results = await system.index_all_documents(max_files=1000, analyze_content=True)
