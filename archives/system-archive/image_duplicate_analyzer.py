...
class ImageAnalyzer:
    ...
    def find_similar_images(self, threshold=5):
        ...
        if len(image_files) > 1000:
            print(
                f"⚠️  Found {len(image_files)} images, analyzing first 1000 for performance"
            )
    ...
    def analyze_with_imagga(self, sample_size=20):
        ...
        image_files = [
            f
            for f in image_files
            if not self.should_skip(f) and f.suffix.lower() in [".jpg", ".jpeg", ".png"]
        ]
        ...
        for img_path in image_files:
            try:
                ...
                if response.status_code == 200:
                    ...
                    tags = [
                        tag["tag"]["en"]
                        for tag in result.get("result", {}).get("tags", [])[:5]
                    ]
...
def main():
    ...
    exclude_dirs = [
        "etsy",
        "cLeanShoT",
    ]  # Exclude etsy and cLeanShoT folders from analysis
