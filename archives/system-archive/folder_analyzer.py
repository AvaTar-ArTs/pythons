...
class FolderAnalyzer:
    ...
    def analyze(self) -> Dict:
                ...
                try:
                    if file_path.is_file():
                        ...
                        if file_info:
                            ...
                            structure["size_by_category"][file_info.category] += (
                                file_info.size
                            )
                            ...
                            structure["largest_files"].append(
                                (str(file_path), file_info.size)
                            )
                            structure["oldest_files"].append(
                                (str(file_path), file_info.modified_time)
                            )
                            structure["newest_files"].append(
                                (str(file_path), file_info.modified_time)
                            )
        ...
        structure["largest_files"] = sorted(
            structure["largest_files"], key=lambda x: x[1], reverse=True
        )[:100]
        structure["oldest_files"] = sorted(
            structure["oldest_files"], key=lambda x: x[1]
        )[:100]
        structure["newest_files"] = sorted(
            structure["newest_files"], key=lambda x: x[1], reverse=True
        )[:100]
        ...
        print(
            f"   ✅ Analysis complete: {structure['total_files']:,} files in {structure['total_directories']:,} directories"
        )
