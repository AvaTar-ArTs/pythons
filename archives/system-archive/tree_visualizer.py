...
class TreeVisualizer:
    ...
    def _prepare_tree_data(self, result: Any) -> Dict:
        ...
        if hasattr(result, "files") and result.files:
            tree_data["children"] = self._build_tree_from_files(
                result.files, result.root_path
            )
        else:
    ...
    def _build_tree_from_files(self, files: List, root_path: str) -> List[Dict]:
        ...
        for file_info in files:
            try:
                ...
                self._update_parent_metadata(
                    tree_nodes, path_parts[:-1], file_info.size
                )
    ...
    def _update_parent_metadata(:
        self, tree_nodes: Dict, path_parts: List[str], file_size: int
    ):
