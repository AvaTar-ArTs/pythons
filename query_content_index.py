    ...
    for func in functions:
        ...
        if search_type == 'name':
            ...
            try:
                ...
                if pattern.search(func.get('name', '')) or \
                   pattern.search(func.get('docstring', '')) or \
                   pattern.search(func.get('body_preview', '')):
                    match = True
            except re.error:
                return []
...
def search_classes(query, classes, search_type='name'):
    ...
    for cls in classes:
        ...
        if search_type == 'name':
            ...
            try:
                pattern = re.compile(query, re.IGNORECASE)
                if pattern.search(cls.get('name', '')) or \
                   pattern.search(cls.get('docstring', '')):
                    match = True
            except re.error:
                return []
...
def search_files(query, files, search_type='name'):
    ...
    for file_info in files:
        ...
        if search_type == 'name':
            ...
            try:
                pattern = re.compile(query, re.IGNORECASE)
                if pattern.search(file_info.get('file_name', '')) or \
                   pattern.search(file_info.get('content_preview', '')):
                    match = True
            except re.error:
                return []
...
def filter_results(results, filters):
    ...
    if 'min_lines' in filters:
        filtered = [r for r in filtered if int(r.get('line_count', 0) or 0) >= filters['min_lines']]
    
    if 'max_lines' in filters:
        filtered = [r for r in filtered if int(r.get('line_count', 0) or 0) <= filters['max_lines']]
    
    if 'min_size_kb' in filters:
        filtered = [r for r in filtered if float(r.get('size_kb', 0) or 0) >= filters['min_size_kb']]
    
    if 'max_size_kb' in filters:
        filtered = [r for r in filtered if float(r.get('size_kb', 0) or 0) <= filters['max_size_kb']]
    
    if 'language' in filters:
        filtered = [r for r in filtered if r.get('language', '').lower() == filters['language'].lower()]
    
    if 'min_functions' in filters:
        filtered = [r for r in filtered if int(r.get('function_count', 0) or 0) >= filters['min_functions']]
...
def print_function_results(results, limit=None, detailed=False):
    ...
    for i, func in enumerate(results, 1):
        ...
        if func.get('args'):
            args_str = ', '.join(func['args']) if isinstance(func['args'], list) else func['args']
            print(f"   Args: {args_str}")
        
        if func.get('docstring'):
            doc = func['docstring'][:150] + '...' if len(func['docstring']) > 150 else func['docstring']
            print(f"   Doc: {doc}")
        
        if detailed and func.get('body_preview'):
            body = func['body_preview'][:200] + '...' if len(func['body_preview']) > 200 else func['body_preview']
...
def print_class_results(results, limit=None, detailed=False):
    ...
    for i, cls in enumerate(results, 1):
        ...
        if cls.get('methods'):
            methods_str = ', '.join(cls['methods']) if isinstance(cls['methods'], list) else cls['methods']
            print(f"   Methods: {methods_str}")
        
        if cls.get('docstring'):
            doc = cls['docstring'][:150] + '...' if len(cls['docstring']) > 150 else cls['docstring']
            print(f"   Doc: {doc}")
...
def print_file_results(results, limit=None, detailed=False):
    ...
    for i, file_info in enumerate(results, 1):
        ...
        print(f"   Path: {file_info.get('relative_path', file_info.get('file_path', 'unknown'))}")
        ...
        print(f"   Functions: {file_info.get('function_count', 0)}, Classes: {file_info.get('class_count', 0)}")
        
        if detailed and file_info.get('content_preview'):
            content = file_info['content_preview'][:200] + '...' if len(file_info['content_preview']) > 200 else file_info['content_preview']
...
def get_statistics(functions, classes, files):
    ...
    files_by_funcs = sorted(files, key=lambda x: int(x.get('function_count', 0) or 0), reverse=True)
    ...
    stats['common_function_names'] = dict(sorted(func_names.items(), key=lambda x: x[1], reverse=True)[:20])
...
def main():
    ...
    parser.add_argument('--type', choices=['function', 'class', 'file', 'all'], default='all',
                       help='Type of search (default: all)')
    parser.add_argument('--search-in', choices=['name', 'file', 'content', 'methods', 'path', 'language', 'regex'],
                       default='name', help='What to search in (default: name)')
    ...
    parser.add_argument('--stats', action='store_true', help='Show statistics instead of searching')
    ...
    parser.add_argument('--min-functions', type=int, help='Filter: minimum function count')
    ...
    print(f"Loaded: {len(functions)} functions, {len(classes)} classes, {len(files)} files\n")
    ...
    # Search
    if not args.query:
        print("No query provided. Use --stats to see statistics or provide a search query.")
        return
