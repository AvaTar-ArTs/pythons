...
SKIP_DIRS = {
    ...
    'Library', 'Applications', 'System',  # System directories
    '.Trash', '.cache', '.local/share',  # Cache/trash
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    ...
                    decorators = [ast.unparse(d) if hasattr(ast, 'unparse') else d.id if isinstance(d, ast.Name) else 'decorator' for d in node.decorator_list]
                    ...
                    functions.append({
                        ...
                        'docstring': docstring[:200] if docstring else "",  # Limit length
                    ...
                    try:
                        if isinstance(node, ast.Import):
                            ...
                            imports.append(f"{module}.{', '.join([a.name for a in node.names])}")
                    except:
                        pass
...
def extract_functions_regex(content, file_path):
    ...
    for match in re.finditer(func_pattern, content):
        ...
        docstring = ""
        doc_match = re.search(r'""".*?"""', content[start_pos:start_pos+500], re.DOTALL)
        if doc_match:
...
def main():
    ...
    for i, file_path in enumerate(code_files, 1):
        ...
        try:
            ...
            for func in functions:
                ...
                func['relative_path'] = str(file_path.relative_to(home_dir)) if file_path.is_relative_to(home_dir) else str(file_path)
                all_functions.append(func)
            
            for cls in classes:
                cls['file_path'] = str(file_path)
                cls['file_name'] = file_path.name
                cls['relative_path'] = str(file_path.relative_to(home_dir)) if file_path.is_relative_to(home_dir) else str(file_path)
            ...
            file_index.append({
                ...
                'relative_path': str(file_path.relative_to(home_dir)) if file_path.is_relative_to(home_dir) else str(file_path),
    ...
    if all_functions:
        ...
        with open(func_csv, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=[
                'name', 'file_path', 'file_name', 'relative_path',
                'args', 'decorators', 'docstring', 'body_preview', 'line_start'
            ])
            writer.writeheader()
            for func in all_functions:
                ...
                func_copy['decorators'] = ', '.join(func['decorators']) if func['decorators'] else ''
    ...
    if all_classes:
        ...
        with open(class_csv, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=[
                'name', 'file_path', 'file_name', 'relative_path',
                'methods', 'docstring', 'line_start'
            ])
            writer.writeheader()
            for cls in all_classes:
                cls_copy = cls.copy()
                cls_copy['methods'] = ', '.join(cls['methods']) if cls['methods'] else ''
    ...
    with open(file_csv, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=[
            'file_path', 'file_name', 'relative_path', 'directory',
            'size_kb', 'line_count', 'function_count', 'class_count',
            'import_count', 'language', 'content_preview'
        ])
        writer.writeheader()
    ...
    for file_info in top_files:
        print(f"  {file_info['file_name']}: {file_info['function_count']} functions, {file_info['line_count']} lines")
