...
class PythonScriptsOrganizer:
    ...
    def scan_scripts(self):
        ...
        for script_path in all_scripts:
            ...
            if any(keyword in str(script_path).lower() for keyword in ['organize', 'reorganize']):
                self.proposed_structure["core_tools"]["organization"].append(str(script_path))
            elif any(keyword in str(script_path).lower() for keyword in ['cleanup', 'clean']):
                self.proposed_structure["core_tools"]["cleanup"].append(str(script_path))
            elif any(keyword in str(script_path).lower() for keyword in ['duplicate', 'similar']):
                self.proposed_structure["core_tools"]["duplicate_management"].append(str(script_path))
            elif any(keyword in str(script_path).lower() for keyword in ['analyze', 'analysis']):
                self.proposed_structure["core_tools"]["analysis"].append(str(script_path))
            elif any(keyword in str(parts[0]).lower() for keyword in ['ai', 'ml', 'openai', 'claude', 'anthropic']):
                if any(keyword in str(script_path).lower() for keyword in ['generate', 'text']):
                    self.proposed_structure["ai_ml_tools"]["content_generation"].append(str(script_path))
                elif any(keyword in str(script_path).lower() for keyword in ['analyze', 'analysis']):
                    self.proposed_structure["ai_ml_tools"]["text_analysis"].append(str(script_path))
                else:
                    self.proposed_structure["ai_ml_tools"]["automation"].append(str(script_path))
            elif any(keyword in str(parts[0]).lower() for keyword in ['audio', 'video', 'image', 'media']):
                if any(keyword in str(parts[0]).lower() for keyword in ['audio']):
                    self.proposed_structure["media_tools"]["audio"].append(str(script_path))
                elif any(keyword in str(parts[0]).lower() for keyword in ['video']):
                    self.proposed_structure["media_tools"]["video"].append(str(script_path))
                else:  # image and general media
                    self.proposed_structure["media_tools"]["image"].append(str(script_path))
            elif any(keyword in str(parts[0]).lower() for keyword in ['automation', 'bot', 'social']):
                if 'youtube' in str(script_path).lower():
                    self.proposed_structure["automation_tools"]["youtube"].append(str(script_path))
                elif 'instagram' in str(script_path).lower():
                    self.proposed_structure["automation_tools"]["instagram"].append(str(script_path))
                else:
                    self.proposed_structure["automation_tools"]["general"].append(str(script_path))
            elif any(keyword in str(parts[0]).lower() for keyword in ['data', 'csv', 'json']):
                if any(keyword in str(script_path).lower() for keyword in ['analyze', 'analysis']):
                    self.proposed_structure["data_tools"]["analysis"].append(str(script_path))
                elif any(keyword in str(script_path).lower() for keyword in ['convert', 'transform']):
                    self.proposed_structure["data_tools"]["conversion"].append(str(script_path))
                else:
                    self.proposed_structure["data_tools"]["utilities"].append(str(script_path))
            else:
                # Default to utilities based on subdirectory
                if any(keyword in str(parts[0]).lower() for keyword in ['file', 'utility', 'util']):
                    if any(keyword in str(script_path).lower() for keyword in ['file', 'op']):
                        self.proposed_structure["utilities"]["file_ops"].append(str(script_path))
                    elif any(keyword in str(script_path).lower() for keyword in ['web', 'http', 'request']):
                        self.proposed_structure["utilities"]["web"].append(str(script_path))
                    else:
                        self.proposed_structure["utilities"]["misc"].append(str(script_path))
                else:
...
def main():
    ...
    implementation_script = organizer.generate_implementation_script(structure, duplicates)
