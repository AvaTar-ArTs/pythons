...
class MusicRecategorizer:
    ...
    def __init__(self):
        ...
        self.stats = {"total_checked": 0, "moved_to_suno": 0, "stayed_other": 0}
    ...
    def recategorize(self):
            ...
            for item in yours:
                try:
                    ...
                    if self.stats["moved_to_suno"] % 50 == 0:
                        print(
                            f"      Moved {self.stats['moved_to_suno']}/{len(yours)}...",
                            end="\r",
                        )
        ...
        # New totals
        suno_count = (
            len(list(self.suno_dir.glob("*.mp3")))
            + len(list(self.suno_dir.glob("*.wav")))
            + len(list(self.suno_dir.glob("*.m4a")))
        )
        other_count = (
            len(list(self.other_dir.glob("*.mp3")))
            + len(list(self.other_dir.glob("*.wav")))
            + len(list(self.other_dir.glob("*.m4a")))
        )
        ...
        log_file = (
            ...
            / "workspace"
            / "music-empire"
            / f"RECATEGORIZATION_LOG_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
