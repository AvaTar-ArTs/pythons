...
class InVideoProofSearcher:
    ...
    def categorize_file(self, filepath: Path) -> str:
        ...
        if any(
            ...
            for term in ["2025", "project 2025", "trump", "disaster"]
        ):
            if any(
                term in filename_lower
                for term in ["project", "proof", "american", "disaster"]
            ):
                return "project_2025"
        ...
        if any(
            term in filename_lower
            for term in ["ai-generated", "ai-480", "runway", "gen-", "leonardo"]
        ):
            return "ai_videos"
    ...
    def search_documents(self):
        ...
        for root, dirs, files in os.walk(self.docs_dir):
            ...
            if scanned % 100 == 0:
                print(
                    f"   Scanned {scanned} directories, found {self.total_found} files...",
                    end="\r",
                )
                            ...
                            self.findings[category].append(
                                {
                                    ...
                                    "path": str(filepath),
                                    "size_mb": round(
                                        filepath.stat().st_size / (1024**2), 2
                                    ),
                                    "location": str(
                                        Path(root).relative_to(self.docs_dir)
                                    ),
    ...
    def generate_report(self):
        ...
        report_file = (
            self.workspace
            / f"INVIDEO_PROOFS_SEARCH_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )

        with open(report_file, "w", encoding="utf-8") as f:
            f.write("# ?? InVideo & 2025 Proofs Search Results\n\n")
            f.write(f"**Date:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            f.write("**Searched:** ~/Documents\n\n")
            f.write("---\n\n")
            ...
            for category, files in self.findings.items():
                if files:
                    ...
                    f.write(
                        f"- **{category.replace('_', ' ').title()}:** {len(files)} files ({total_size / 1024:.2f} GB)\n"
                    )
            ...
            for category, files in self.findings.items():
                if files:
                    ...
                    for location, loc_files in sorted(
            ...
            if self.findings["invideo"]:
                f.write(
                    f"### InVideo Content ({len(self.findings['invideo'])} files)\n"
                )
            ...
            if self.findings["project_2025"]:
                f.write(
                    f"### Project 2025 Content ({len(self.findings['project_2025'])} files)\n"
                )
    ...
    def print_summary(self):
        ...
        for category, files in sorted(
            self.findings.items(), key=lambda x: len(x[1]), reverse=True
        ):
            if files:
