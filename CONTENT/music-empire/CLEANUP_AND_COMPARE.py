...
class CleanupAndCompare:
    ...
    def __init__(self):
        ...
        self.organized_dir = (
            self.home / "Music" / "nocTurneMeLoDieS" / "FINAL_ORGANIZED"
        )
        ...
        for file_info in self.organized_files:
            ...
            for song_id, song in self.catalog.items():
                if song_id in filename:
                    self.matches.append({"file": file_info, "song": song})
    ...
    def generate_comparison_report(self):
        ...
        report_file = (
            self.workspace
            / f"CLEANUP_COMPARISON_REPORT_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- **You already have:** {len(self.have)} songs ({len(self.have) / len(self.catalog) * 100:.1f}%)\n"
            )
            f.write(
                f"- **Need to download:** {len(self.need)} songs ({len(self.need) / len(self.catalog) * 100:.1f}%)\n\n"
            )
            ...
            f.write("4. ?? **Upload to DistroKid** and start earning!\n\n")
    ...
    def export_download_list(self):
        ...
        with open(download_file, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(
                f, fieldnames=["id", "title", "tags", "audio_url", "url"]
            )
            writer.writeheader()
    ...
    def print_summary(self):
        ...
        print(
            f"   ? You have: {len(self.have)} songs ({len(self.have) / len(self.catalog) * 100:.1f}%)"
        )
        print(
            f"   ? Need: {len(self.need)} songs ({len(self.need) / len(self.catalog) * 100:.1f}%)"
        )
