...
class ASTAnalyzer:
    ...
    def analyze_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"file_path": file_path, "error": str(e), "confidence_score": 0.0}
    ...
    def _detect_voice_agent_patterns(:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                if isinstance(node.func, ast.Attribute):
                    if (
                        "openai" in str(node.func.value).lower()
                        or "client" in str(node.func.value).lower()
                    ):
                        patterns.append(
                            CodePattern(
    ...
    def _detect_business_logic_patterns(:
        self, tree: ast.AST, content: str
    ) -> List[CodePattern]:
...
class SemanticAnalyzer:
    ...
    def analyze_semantic_patterns(:
        self, code_content: str, comments: str = ""
    ) -> Dict[str, Any]:
        ...
        # Classify intent
        analysis["intent_classification"] = self._classify_intent(
            code_content, comments
        )

        # Analyze complexity indicators
        analysis["complexity_indicators"] = self._analyze_complexity_indicators(
            code_content
        )

        # Detect domain-specific patterns
        analysis["domain_specific_patterns"] = self._detect_domain_patterns(
            code_content
        )
    ...
    def _classify_intent(self, code_content: str, comments: str) -> Dict[str, float]:
        ...
        voice_score = sum(
            1 for indicator in voice_indicators if indicator in combined_text
        )
        ...
        data_score = sum(
            1 for indicator in data_indicators if indicator in combined_text
        )
        ...
        business_score = sum(
            1 for indicator in business_indicators if indicator in combined_text
        )
    ...
    def _analyze_complexity_indicators(self, code_content: str) -> Dict[str, Any]:
        ...
        cognitive_score = sum(
            1 for pattern in complex_patterns if pattern in code_content.lower()
        )
        indicators["cognitive_load"] = min(cognitive_score / len(complex_patterns), 1.0)

        # Maintainability indicators
        maintainable_patterns = [
            "function",
            "class",
            "module",
            "test",
        ...
        maintainable_score = sum(
            1 for pattern in maintainable_patterns if pattern in code_content.lower()
        )
        indicators["maintainability"] = min(
            maintainable_score / len(maintainable_patterns), 1.0
        )

        # Readability indicators
        readable_patterns = ["clear", "simple", "readable", "documented", "explained"]
        readable_score = sum(
            1 for pattern in readable_patterns if pattern in code_content.lower()
        )
        ...
        testable_score = sum(
            1 for pattern in testable_patterns if pattern in code_content.lower()
        )
    ...
    def _detect_domain_patterns(self, code_content: str) -> Dict[str, Any]:
        ...
        if any(
            ...
            for keyword in ["openai", "voice", "speech", "conversation"]
        ):
        ...
        if any(
            keyword in content_lower
            for keyword in ["lead", "google maps", "business", "contact"]
        ):
        ...
        if any(
            keyword in content_lower
            for keyword in ["requests", "api", "http", "endpoint"]
        ):
        ...
        if any(
            keyword in content_lower
            for keyword in ["csv", "json", "process", "transform"]
        ):
        ...
        if any(
            keyword in content_lower
            for keyword in ["revenue", "scoring", "analytics", "metrics"]
        ):
...
class IntelligentCategorizer:
    ...
    def categorize_code(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        category_scores = self._calculate_category_scores(
            content, patterns, semantic_features
        )
        if category_scores:
            categorization["primary_category"] = max(
        ...
        categorization["intelligence_level"] = self._determine_intelligence_level(
            complexity, confidence
        )

        # Generate recommendations
        categorization["recommendations"] = self._generate_recommendations(
        ...
        # Calculate overall confidence
        categorization["confidence_score"] = self._calculate_categorization_confidence(
            categorization
        )
    ...
    def _calculate_category_scores(:
        ...
        for category, config in self.categories.items():
            ...
            # Pattern matching
            pattern_matches = sum(
                1 for pattern in config["patterns"] if pattern.lower() in content_lower
            )
            if config["patterns"]:
                score += (pattern_matches / len(config["patterns"])) * 0.4

            # Pattern detection from AST analysis
            pattern_type_matches = sum(
                1
                for p in patterns
                if category.replace("_", "") in p.pattern_type.lower()
            )
            if patterns:
    ...
    def _determine_intelligence_level(:
        self, complexity: float, confidence: float
    ) -> str:
        ...
        for level, ranges in self.intelligence_levels.items():
            ...
            if (
                complexity_min <= complexity < complexity_max
    ...
    def _generate_recommendations(:
        self, analysis: Dict[str, Any], categorization: Dict[str, Any]
    ) -> List[str]:
        ...
        if complexity > 20:
            ...
            recommendations.append(
                "Add comprehensive error handling for better robustness"
            )

        if complexity < 5:
            recommendations.append(
                "Code is well-structured with low complexity - good for maintainability"
            )

        # Confidence-based recommendations
        if confidence < 0.6:
            recommendations.append(
                "Add more documentation and comments to improve code clarity"
            )
            recommendations.append(
                "Consider adding type hints for better code understanding"
            )

        # Category-specific recommendations
        if primary_category == "voice_agents":
            recommendations.append(
                "Consider implementing conversation state management"
            )
            recommendations.append("Add voice quality metrics and monitoring")

        elif primary_category == "lead_generation":
            recommendations.append(
                "Implement lead scoring algorithms for better prioritization"
            )
        ...
        if intelligence_level == "beginner":
            recommendations.append(
                "Great foundation! Consider adding more advanced features"
            )
        elif intelligence_level == "expert":
            recommendations.append(
                "Excellent code quality! Consider creating reusable components"
            )
...
class CodeIntelligenceEngine:
    ...
    def analyze_project(self, project_path: str) -> Dict[str, Any]:
        ...
        for py_file in python_files:
            ...
            analysis_results["files_analyzed"].append(
                {
                    "file": str(py_file),
                    "category": file_analysis.get("categorization", {}).get(
                        "primary_category", "unknown"
                    ),
                    "intelligence_level": file_analysis.get("categorization", {}).get(
                        "intelligence_level", "beginner"
                    ),
                    "confidence": file_analysis.get("categorization", {}).get(
                        "confidence_score", 0.0
                    ),
                }
            )
        ...
        analysis_results["overall_intelligence"] = self._generate_overall_intelligence(
            file_analyses
        )

        # Detect architectural patterns
        analysis_results["architectural_patterns"] = (
            self._detect_architectural_patterns(file_analyses)
        )

        # Generate project-level recommendations
        analysis_results["recommendations"] = self._generate_project_recommendations(
            file_analyses
        )
    ...
    def _analyze_single_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            # Semantic Analysis
            semantic_analysis = self.semantic_analyzer.analyze_semantic_patterns(
                content
            )
    ...
    def _generate_overall_intelligence(:
    ...
    def _detect_architectural_patterns(:
        ...
        if all(
            cat in ["voice_agents", "api_integration", "data_processing"]
    ...
    def _generate_project_recommendations(:
        ...
        if avg_confidence < 0.6:
            recommendations.append(
                "Overall project confidence is low - consider improving documentation and code clarity"
            )
        ...
        if "voice_agents" in categories and "lead_generation" in categories:
            recommendations.append(
                "Consider implementing a unified business logic layer"
            )

        if "data_processing" in categories:
            recommendations.append(
                "Add data validation and error handling across data processing components"
            )

        # Check for testing
        test_files = [
            f for f in file_analyses if "test" in f.get("file_path", "").lower()
        ]
        if not test_files:
    ...
    def _assess_project_maturity(self, file_analyses: List[Dict[str, Any]]) -> str:
        ...
        advanced_count = sum(
            1 for level in intelligence_levels if level in ["advanced", "expert"]
        )
