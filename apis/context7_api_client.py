...
class Context7APIClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError(
                "CONTEXT7_API_KEY not found. Add it to ~/.env.d/llm-apis.env"
            )
...
class Context7CodebaseIntegrator:
    ...
    def analyze_and_fetch_docs(self):
        ...
        top_libraries = sorted(
            libraries.items(), key=lambda x: x[1].get("usage_count", 0), reverse=True
        )[:20]  # Top 20 libraries
        ...
        with open(docs_file, "w") as f:
            json.dump(
                {"generated_at": datetime.now().isoformat(), "libraries": library_docs},
                f,
                indent=2,
            )
...
def main():
    ...
    parser.add_argument(
        "--integrate", action="store_true", help="Integrate with codebase"
    )
    ...
    if args.integrate:
        ...
        for result in results:
            ...
            print(
                f"    Trust: {result.get('trustScore')}, Snippets: {result.get('totalSnippets')}"
            )
    else:
        print("Usage:")
