...
def extract_from_html(html_path: Path) -> List[Dict]:
    ...
    if next_data:
        try:
            ...
            clips = (
                props.get("clips")
                or props.get("playlist", {}).get("clips")
                or props.get("songs")
                or props.get("library", {}).get("clips")
                or []
            )
                ...
                songs.append(
                    {
                        ...
                        or f"https://cdn1.suno.ai/{song_id}.mp3",
                        "imageUrl": clip.get("image_url")
                        or clip.get("image_large_url", ""),
                        ...
                        "authorLink": f"https://suno.com/@{clip.get('handle', '')}"
                        if clip.get("handle")
                        else "",
    ...
    for link in song_links:
        ...
        # Get title
        title = link.get("title") or link.get_text(strip=True) or "Untitled"
...
def main():
    ...
    parser.add_argument(
        "--dir", help="Directory to search for HTML files (default: ~/Documents/HTML)"
    )
    parser.add_argument("--file", help="Single HTML file to process")
    parser.add_argument(
        "--output", "-o", help="Output CSV file (default: auto-generated)"
    )
    parser.add_argument(
