...
def scrapeVideos(pexelsApiKey: str):
    ...
    if results == 0:
        logger.info(
            "No video results for your query: ",
            parameters["query"],
            Path("\nExiting..."),
        )
        return None
...
def videoIntro(introText, videoNumber) -> CompositeVideoClip:
    ...
    intro_clip = VideoFileClip("intro_clip/glitch.mp4").resize(
        (CONSTANT_1080, CONSTANT_1920)
    )
    ...
    intro_final = CompositeVideoClip([intro_clip, text_with_bg]).set_duration(
        intro_clip_duration
    )
    return intro_final
...
def createVideo(:
    quoteText: str, bgMusic: str, bgVideo: str, videoNumber: int, ttsAudio: bool
):
    ...
    for idx, sentence in enumerate(quoteArray):
        ...
        color_clip = ColorClip(
            size=(tc_width + CONSTANT_100, tc_height + 50), color=(0, 0, 0)
        ).set_opacity(0.6)

        text_together = (
            CompositeVideoClip([color_clip, text_clip])
            .set_duration(time)
            .set_position("center")
        )
...
def launchImageMagicksInstaller():
    ...
    subprocess.run(
        ["imageMagicksInstaller/ImageMagick-7.1.0-52-Q16-HDRI-x64-dll.exe"],
...
def checkIfImageMagicksIsInstalled() -> bool:
    ...
    if "ImageMagick" in terminalOutput:
        ...
        logger.info(
            "Checking if you selected 'Install legacy utilities(e.g. Convert)' "
        )
        try:
            TextClip(txt="text")  # trying to make a textclip
            logger.info(
                f"{bcolors.OKGREEN}ImageMagicks is installed correctly{changes}{bcolors.ENDC}"
            )
            return True
...
Options menu:
    ...
    Enter your choice: """
        ...
        match int(choice):
            case 1:
                changeJsonValue(
                    "Amount of videos to create: ", data, "amountOfVideosToMake"
                )
                changes = "updated video's to create successfully"
            case 2:
                changeJsonValue("Your Pexels API key: ", data, "pexelsAPIKey")
                changes = (
                    f"updated your API key successfully to - {data['pexelsAPIKey']}"
                )
            case 3:
                verifyData(data)
