...
def paginate(url, headers, params=None):
    ...
    while url:
        ...
        if r.status_code != CONSTANT_200:
            raise SystemExit(
                f"❌ GitHub API error {r.status_code}: {r.text[:CONSTANT_300]}"
            )
        yield r.json()
...
def fetch_repos(username, token):
    ...
    if login and login.lower() == username.lower():
        ...
        logger.info(
            "🔐 Authenticated as target user → using /user/repos (includes private)."
        )
    else:
...
def main():
    ...
    if args.delete:
        if args.confirm != "I UNDERSTAND":
            logger.info(
                '❌ Refusing to delete; pass --confirm "I UNDERSTAND" to proceed.'
            )
            sys.exit(1)
