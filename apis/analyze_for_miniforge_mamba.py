...
def generate_environment_yml(imports_dict, directory_name, output_path):
    ...
    sorted_packages = sorted(
        third_party.items(), key=lambda x: x[1]["files"], reverse=True
    )
    ...
    with open(output_path, "w") as f:
        ...
        for conda_name, info in sorted_packages[:50]:  # Top 50
            f.write(
                f"  - {conda_name}  # {info['module']} (used in {info['files']} files)\n"
            )
...
packages = generate_environment_yml(
    combined_imports, "combined_all_projects", combined_env
)
...
with open(report_file, "w") as f:
    ...
    f.write(
        f"- **Total Python Files:** {sum(r['file_count'] for r in all_results.values())}\n"
    )
    ...
    for dir_name, results in all_results.items():
        ...
        sorted_imports = sorted(
            results["imports"].items(), key=lambda x: len(x[1]), reverse=True
        )
    ...
    sorted_combined = sorted(
        combined_imports.items(), key=lambda x: len(x[1]), reverse=True
    )
...
print("  - Individual environment files for each directory")
print("  - Combined environment file")
