...
class InsightGenerator:
    ...
    def generate(:
        self, filepath: Path, content: str, context: FileContext
    ) -> List[Insight]:
    ...
    def _security_insights(:
        ...
        if context.purpose == "config":
            sensitive_keys = re.findall(
                r"(password|secret|key|token|credential)\s*[:=]", content, re.IGNORECASE
            )
            if sensitive_keys:
                insights.append(
                    Insight(
    ...
    def _documentation_insights(:
        ...
        if "readme" in filepath.name.lower():
            ...
            has_install = bool(
                re.search(r"##?\s+(install|installation|setup)", content, re.IGNORECASE)
            )
...
class CodeAnalyzer:
    ...
    def _analyze_directory(self, dirpath: Path) -> AnalysisResult:
        ...
        for i, filepath in enumerate(
            files_to_analyze[:CONSTANT_100], 1
    ...
    def _create_summary(:
        self, contexts: List[FileContext], insights: List[Insight]
    ) -> Dict:
    ...
    def print_report(result: AnalysisResult, show_details: bool = True):
        ...
        if summary.get("frameworks"):
            ...
            for fw, count in sorted(
                summary["frameworks"].items(), key=lambda x: x[1], reverse=True
            ):
