...
def extract_code_signature(content: str) -> Dict:
    ...
    try:
        ...
        signature["imports"] = set(
            re.findall(r"^(?:import|from)\s+(\w+)", content, re.MULTILINE)
        )
        signature["functions"] = set(
            re.findall(r"^def\s+(\w+)\s*\(", content, re.MULTILINE)
        )
    ...
    if any(
        ...
        for term in ["openai", "claude", "anthropic", "gpt", "llm"]
    ):
    ...
    if any(
        term in content_lower
        for term in ["requests", "beautifulsoup", "selenium", "scrap"]
    ):
    ...
    # Database
    if any(
        term in content_lower for term in ["sqlite", "mongodb", "postgres", "mysql"]
    ):
    ...
    if any(
        term in content_lower
        for term in ["subprocess", "automation", "bot", "selenium"]
    ):
...
def calculate_content_similarity(sig1: Dict, sig2: Dict) -> float:
    ...
    import_sim = len(sig1["imports"] & sig2["imports"]) / max(
        len(sig1["imports"] | sig2["imports"]), 1
    )
    similarity += import_sim * 0.3
    total_weight += 0.3

    # Function similarity (high weight)
    func_sim = len(sig1["functions"] & sig2["functions"]) / max(
        len(sig1["functions"] | sig2["functions"]), 1
    )
    similarity += func_sim * 0.25
    total_weight += 0.25

    # Class similarity (high weight)
    class_sim = len(sig1["classes"] & sig2["classes"]) / max(
        len(sig1["classes"] | sig2["classes"]), 1
    )
    similarity += class_sim * 0.2
    total_weight += 0.2

    # Pattern similarity (medium weight)
    pattern_sim = len(sig1["key_patterns"] & sig2["key_patterns"]) / max(
        len(sig1["key_patterns"] | sig2["key_patterns"]), 1
    )
...
def find_functional_duplicates(:
    data: List[Dict], similarity_threshold: float = 0.8
) -> List[Dict]:
...
def main():
    ...
    space_saved = sum(
        int(d.get("file_size", 0))
        for d in all_duplicates
        if d.get("file_size", "").isdigit()
    )
    ...
    print(
        "📄 Check 'content_based_duplicates_to_remove.csv' for detailed recommendations"
    )
    print("\n🔍 Analysis focused on:")
