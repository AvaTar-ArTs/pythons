...
class FunctionalDuplicateScanner:
    ...
    def analyze_file_purpose(self, filepath):
        ...
        try:
            ...
            purpose["purpose_signature"] = (
                f"{actions_str}|{targets_str}|{apis_str}|{ops_str}"
            )
    ...
    def print_summary(self, functional_dupes):
        ...
        if functional_dupes:
            ...
            for i, group in enumerate(functional_dupes[:30], 1):
                ...
                targets_str = (
                    ", ".join(group["targets"]) if group["targets"] else "general"
                )
    ...
    if functional_dupes:
        ...
        if confirm == "FUNCTIONAL":
            ...
            final = len(
                [
                    f
                    for f in scanner.pythons_dir.rglob("*.py")
                    if "_archive" not in str(f)
                    and "2T-Xx-python" not in str(f)
                    and ".venv" not in str(f)
                    and ".history" not in str(f)
