...
class DeepContentScanner:
    ...
    def scan_file(self, filepath):
        ...
        try:
            ...
            result["last_modified"] = datetime.fromtimestamp(stats.st_mtime).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
    ...
    def save_detailed_csv(self):
        ...
        print("💾 Saving detailed CSV report...\n")
    ...
    def save_api_usage_csv(self):
        ...
        with open(csv_file, "w", newline="", encoding="utf-8") as f:
            ...
            for api in sorted(
                api_counts.keys(), key=lambda x: len(api_counts[x]), reverse=True
            ):
    ...
    def print_statistics(self):
        ...
        if api_counts:
            ...
            for api, count in sorted(
                api_counts.items(), key=lambda x: x[1], reverse=True
            )[:10]:
