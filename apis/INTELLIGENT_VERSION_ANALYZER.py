...
class IntelligentVersionAnalyzer:
    ...
    def compare_versions(self, root_file, organized_file):
        ...
        if root_analysis["lines"] > org_analysis["lines"] * 1.2:
            factors.append(
                (
                    "root",
                    10,
                    f"Root has {root_analysis['lines']} lines vs {org_analysis['lines']}",
                )
            )
            ...
            factors.append(
                (
                    "org",
                    10,
                    f"Organized has {org_analysis['lines']} lines vs {root_analysis['lines']}",
                )
            )
        ...
        if root_funcs > org_funcs:
            factors.append(
                ("root", 5, f"Root has {root_funcs} functions vs {org_funcs}")
            )
        elif org_funcs > root_funcs:
            factors.append(
                ("org", 5, f"Organized has {org_funcs} functions vs {root_funcs}")
            )
        ...
        if root_classes > org_classes:
            factors.append(
                ("root", 8, f"Root has {root_classes} classes vs {org_classes}")
            )
        elif org_classes > root_classes:
            factors.append(
                ("org", 8, f"Organized has {org_classes} classes vs {org_classes}")
            )
        ...
        if root_analysis["complexity_score"] > org_analysis["complexity_score"] * 1.1:
            factors.append(
                (
                    "root",
                    15,
                    f"Root complexity: {root_analysis['complexity_score']:.1f} vs {org_analysis['complexity_score']:.1f}",
                )
            )
            ...
            factors.append(
                (
                    "org",
                    15,
                    f"Organized complexity: {org_analysis['complexity_score']:.1f} vs {root_analysis['complexity_score']:.1f}",
                )
            )
        ...
        if root_analysis["error_handling"] > org_analysis["error_handling"]:
            factors.append(
                (
                    "root",
                    5,
                    f"Root has better error handling ({root_analysis['error_handling']} blocks)",
                )
            )
            ...
            factors.append(
                (
                    "org",
                    5,
                    f"Organized has better error handling ({org_analysis['error_handling']} blocks)",
                )
            )
        ...
        if unique_root_apis:
            factors.append(
                ("root", 7, f"Root has additional APIs: {', '.join(unique_root_apis)}")
            )
        if unique_org_apis:
            factors.append(
                (
                    "org",
                    7,
                    f"Organized has additional APIs: {', '.join(unique_org_apis)}",
                )
            )
    ...
    def _generate_recommendation(self, comparison):
        ...
        if winner == "root":
            if confidence > 70:
                ...
                return "MERGE: Combine features from both versions"
            else:
                return "REVIEW: Root slightly better, manual review needed"

        elif winner == "organized":
            if confidence > 70:
                return "DELETE: Remove root version (organized is better)"
            elif confidence > 40:
                return "ARCHIVE: Move root to _archive/ (organized is better)"
            else:
                return "REVIEW: Organized slightly better, manual review needed"
    ...
    def show_top_findings(self, limit=20):
        ...
        sorted_analyses = sorted(
            self.analyses, key=lambda x: x["confidence"], reverse=True
        )

        for i, analysis in enumerate(sorted_analyses[:limit], 1):
            print(f"{i}. {analysis['filename']}")
            print(
                f"   Winner: {analysis['winner'].upper()} (confidence: {analysis['confidence']}%)"
            )
