...
class Context7CodebaseIndexer:
    ...
    def generate_index(self):
        ...
        index = {
            ...
            "libraries": {
                ...
                for lib, files in sorted(
                    self.libraries.items(), key=lambda x: len(x[1]), reverse=True
                )[:50]
            },
            "top_imports": dict(self.imports.most_common(30)),
            "code_patterns": {
                pattern: len(examples)
                for pattern, examples in self.code_patterns.items()
            },
    ...
    def generate_library_report(self):
        ...
        report = {
            "most_used_libraries": [
                {"library": lib, "usage_count": len(files), "files": files[:5]}
                for lib, files in sorted(
                    self.libraries.items(), key=lambda x: len(x[1]), reverse=True
                )[:20]
            ],
    ...
    def generate_markdown_summary(self, report: Dict):
        ...
        with open(md_file, "w") as f:
            ...
            f.write(
                f"- **Total Python Files:** {len(list(self.pythons_dir.rglob('*.py')))}\n"
            )
            ...
            for item in report["most_used_libraries"][:20]:
                ...
                f.write(
                    f"| `{item['library']}` | {item['usage_count']} | {files_str} |\n"
                )
            f.write("\n")
            ...
            f.write(
                "These libraries are good candidates for Context7 documentation:\n\n"
            )
            for lib in report["context7_ready_libraries"]:
                f.write(
                    f"- `{lib}` - Used in {len(self.libraries.get(lib, []))} files\n"
                )
            f.write("\n")

            f.write("## 💡 Next Steps\n\n")
            f.write(
                "1. Use Context7 MCP server to fetch latest docs for these libraries\n"
            )
