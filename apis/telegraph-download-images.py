...
def print_percent(last_percent=-1):
    ...
    while not stop:
        ...
        if not stop and percent != CONSTANT_100:
            if this_failed == 0:
                logger.info(
                    f"     Скачивание... {percent}%", end=Path("\r"), flush=True
                )
            else:
                print(
                    f"     Скачивание... {percent}% (есть ошибки)",
                    end=Path("\r"),
                    flush=True,
                )
        sleep(0.01)
        ...
        try:
            if metadataPath:
                with open(metadataPath, "w", encoding="UTF-8") as metadata:
                    metadata.write(
                        """В данный момент выполняется скачивание изображений... 
...
В первом случае просто запусти скрипт ещё раз, а во втором случае нужно отправить разработчику файл "error_log.txt", который находится в папке "Загрузки"."""
        ...
        # SECURITY FIX: Replaced dangerous exec() calls with proper thread management.
        # The original code used:
        #   exec(f"img{imgNumber} = threading.Thread(target=download, args=(imgNumber, imgUrl))")
        #   exec(f"img{imgNumber}.start()")
        #   exec(f"img{n + 1}.join()")
        # This allowed arbitrary code execution if any part of imgUrl or imgNumber could
        # be influenced by user input. Replaced with a simple list of Thread objects.
        download_threads = []
        ...
        for imgUrl in imgs:
            ...
            # Safe: create thread directly without exec()
        ...
        # Safe: join all threads from the list
