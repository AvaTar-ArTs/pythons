...
class DeepContentAnalyzer:
        ...
        try:
            ...
            try:
                ...
                if any(
                    ...
                    for word in ["trend", "seo", "optimize", "keyword"]
                ):
                    result["purpose"] = "SEO/Content Optimization"
                elif any(
                    word in content_lower
                    for word in ["extract", "scrape", "grab", "download"]
                ):
                    result["purpose"] = "Content Extraction"
                elif any(
                    word in content_lower
                    for word in ["orchestrat", "route", "multi", "llm"]
                ):
                    result["purpose"] = "AI Orchestration"
                elif any(
                    word in content_lower
                    for word in ["audio", "music", "transcribe", "whisper"]
                ):
                    result["purpose"] = "Audio Processing"
                elif any(
                    word in content_lower
                    for word in ["image", "photo", "gallery", "visual"]
                ):
                    result["purpose"] = "Image Processing"
                elif any(
                    word in content_lower
                    for word in ["automate", "workflow", "pipeline"]
                ):
                    result["purpose"] = "Automation"
                elif any(
                    word in content_lower
                    for word in ["customer", "retention", "churn", "lifetime"]
                ):
                    result["purpose"] = "CRM/Customer Management"
                elif any(
                    word in content_lower
                    for word in ["analyze", "analyze", "intelligence", "insight"]
                ):
                    result["purpose"] = "Analysis/Intelligence"
                elif any(
                    word in content_lower
                    for word in ["youtube", "video", "description", "tags"]
                ):
    ...
    def _analyze_html(self, filepath: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(
                word in content_lower
                for word in ["gallery", "image", "photo", "portfolio"]
            ):
                result["is_gallery"] = True
            if any(
                word in content_lower for word in ["landing", "hero", "cta", "sign up"]
            ):
    ...
    def _analyze_javascript(self, filepath: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(
                word in content
                for word in ["document.", "window.", "navigator.", "console."]
            ):
...
def generate_markdown_report(results: Dict, output_file: Path):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        for volume_path, volume_data in results["volumes"].items():
            ...
            f.write(
                f"- **Total Size:** {volume_data['total_size'] / (1024**3):.2f} GB\n"
            )
            ...
            for dir_path, dir_info in list(volume_data["directories"].items())[:20]:
                ...
                if dir_info.get("technologies"):
                    f.write(
                        f"- Technologies: {', '.join(dir_info['technologies'][:5])}\n"
                    )
