...
head0 = {
    "User-Agent": "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/CONSTANT_537.36 (KHTML, like Gecko) "
...
def Erf(hvostfile):
    print(
        f"\CONSTANT_33[31;1mНе могу найти_прочитать файл: '{hvostfile}'.\CONSTANT_33[0m \CONSTANT_33[36m\n "
...
def module3():
    while True:
            ...
            with progressYa:
                ...
                for reqY, login in zip(azS, listlogin):
                    ...
                    if rdict.get("display_name") == "-No-":
                        ...
                        table1 = Table(
                            title=f"\n{Style.DIM}{Fore.CYAN}demo_func{Style.RESET_ALL}",
                            style="green",
                        )
                        ...
                        logger.info(
                            "\CONSTANT_33[32;1mЯ.Маркет:\CONSTANT_33[0m", market
                        )
                        ...
                        logger.info(
                            "\CONSTANT_33[32;1mЯ.Avatar:\CONSTANT_33[0m", avatar_cli
                        )
                    ...
                    for webopen in yalist:
                        if webopen == music and Ya == "3":
                            ...
                            if "arm" in platform.platform(
                                aliased=True, terse=0
                            ) or "aarch64" in platform.platform(aliased=True, terse=0):
                                pass
                            else:
        ...
        # Указать login.
            ...
            logger.info(
                "\CONSTANT_33[36m└──Введите login/email разыскиваемого пользователя, например,\CONSTANT_33[0m\CONSTANT_33[32;1m bobbimonov\CONSTANT_33[0m\n"
            )
        ...
        # Указать ссылку на Я.Диск.
            ...
            logger.info(
                "\CONSTANT_33[36m└──Введите публичную ссылку на Яндекс.Диск, например,\CONSTANT_33[0m\CONSTANT_33[32;1m https://yadi.sk/d/7C6Z9q_Ds1wXkw\CONSTANT_33[0m\n"
            )
        ...
        # Указать идентификатор Яндекс пользователя.
            ...
            logger.info(
                "\CONSTANT_33[36m└──Введите идентификатор пользователя Яндекс, например,\CONSTANT_33[0m\CONSTANT_33[32;1m tr6r2c8ea4tvdt3xmpy5atuwg0\CONSTANT_33[0m\n"
            )
            ...
            if len(login) != 26:
                logger.info(
                    Style.BRIGHT
                    + Fore.RED
                    + "└──Неверно указан идентификатор пользователя"
                    + Style.RESET_ALL
                )
                ravno()
            else:
        ...
        # Указать файл с логинами.
            ...
            logger.info(
                "\CONSTANT_33[31;1m└──В demo version этот метод плагина недоступен\CONSTANT_33[0m\n"
            )
            snoopbanner.donate()
        else:
...
def module2():
    if Android:
        logger.info(
            Style.BRIGHT
            + Fore.RED
            + "└──Плагин Reverse Vgeocoder 'сложен' и не поддерживается (по умолчанию) "
            + "в Snoop for Termux\n\nВыход\n"
            + Style.RESET_ALL
        )
        sys.exit()
...
[\CONSTANT_33[0m\CONSTANT_33[31;1mq\CONSTANT_33[0m\CONSTANT_33[36m] --> Выход\n""")
        ...
        # выбрать файл с геокоординатами.
            ...
            while True:
                logger.info(
                    "\CONSTANT_33[36m└──Введите \CONSTANT_33[0m\CONSTANT_33[32;1mабсолютный путь\CONSTANT_33[0m \CONSTANT_33[36mк файлу (кодировка файла -> utf-8) с данными: \n\
        [геокоординаты] или перетащите файл в окно терминала\CONSTANT_33[0m\n"
                ...
                try:
                    ...
                    print(
                        "\CONSTANT_33[31;1m└──Указан неверный путь. "
            ...
            while True:
                ...
                if rGeo == "q" or rGeo == "1" or rGeo == "2":
                    ...
                    logger.info(
                        Style.BRIGHT + Fore.RED + "└──Неверный выбор" + Style.RESET_ALL
                    )
                    ravno()
...
def module1():
    ...
    def treads_dr(fun, args, dr):
        ...
        if domain is None or (dr == "res" and res4 is None and res6 is None):
            ...
            os.kill(
                os.getpid(), signal.SIGBREAK
            ) if sys.platform == "win32" else os.kill(os.getpid(), signal.SIGKILL)
    ...
    # одиночный запрос.
        ...
        def ip_check(url_api, dip, res4, err):
            if dip == "":
                ...
                url_api = f"{url_api}{dip}"
            else:
                url_api = f"{url_api}{res4}"

            try:
                r = my_session.get(url=url_api, headers=head0, timeout=3)
                dip_dic = json.loads(r.text)
                T1 = (
                    dip_dic["country_code"]
                    if err is False
                    else dip_dic["country"]["isoCode"]
                )
                T2 = dip_dic["region"] if err is False else dip_dic["city"]["name"]
                T3 = (
                    dip_dic["latitude"]
                    if err is False
                    else dip_dic["location"]["latitude"]
                )
                T4 = (
                    dip_dic["longitude"]
                    if err is False
                    else dip_dic["location"]["longitude"]
                )
                ...
                try:
                    if err is True:
                        if dip == "":
                            ...
                            dip + "/"
                        console.log(
                            "[bold yellow]--> Внимание! Последний доступный url_ip[/bold yellow]"
                        )
                        T1 = my_session.get(
                            url=f"https://ipinfo.io/{p}country", timeout=3
                        ).text
                        T2 = my_session.get(
                            url=f"https://ipinfo.io/{p}region", timeout=3
                        ).text
                        T5 = my_session.get(
                            url=f"https://ipinfo.io/{p}ip", timeout=3
                        ).text
                    ...
                    #                        raise Exception("")
                    return ip_check(
                        "https://ipdb.ipcalc.co/ipdata/", dip, res4, err=True
                    )
                except (
...
| \|(_)| |(/_\CONSTANT_33[0m""")
        ...
        if (
            ...
            and ":" not in dip
            and dip != ""
            or (dip != "" and len(dip) <= 4)
            or ".." in dip
        ):
            logger.info(
                Style.BRIGHT + Fore.RED + "└──Неверный ввод\n" + Style.RESET_ALL
            )
            return module1()
        else:
        ...
        with console.status("[cyan]работаю[/cyan]", spinner="earth"):
            ...
            try:
                if ipaddress.IPv4Address(dip):
                    res4 = dip
            except Exception:
                pass
            try:
                if ipaddress.IPv6Address(dip):
                    res6 = dip
            except Exception:
                pass
            ...
            if T3 == "stop" and T4 == "stop":
                ...
                logger.info(
                    Style.BRIGHT + Fore.BLACK + f"{URL_GEO2}\n" + Style.RESET_ALL
                )
