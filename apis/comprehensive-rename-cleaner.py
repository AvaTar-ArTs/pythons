...
class ComprehensiveRenameCleaner:
    ...
    def infer_purpose_from_code(self, content, filename):
        ...
        patterns = {
            "shuffle-csv-rows.py": (
                "shuffle_csv" in content and "random.shuffle" in content
            ),
            "gpt-script-categorizer.py": (
                "get_openai_category" in content and "script_content" in content
            ),
            "spotify-track-model.py": (
                "spotify_data" in content and "class Track" in content
            ),
            "backup-documents-csv.py": (
                "~/documents" in content_lower
                and "csv" in content_lower
                and "backup" in content_lower
            ),
            "reddit-comment-formatter.py": (
                "reddit" in content_lower
                and "html" in content_lower
                and "comment" in content_lower
            ),
            "instagram-bot-core.py": (
                filename == "instagram.py" and len(content) < 3000
            ),
            "shuffle-csv-from-url.py": (
                "requests.get(url)" in content
                and "shuffle" in content
                and "csv" in content
            ),
            "telegram-file-downloader.py": (
                "telegram" in content_lower and "download" in content_lower
            ),
            "snoop-osint-tool.py": (
                "snoop" in content_lower and "osint" in content_lower
            ),
    ...
    def suggest_accurate_name(self, filepath, info, current_name):
        ...
        specific_renames = {
            ...
            "categorizer.py": (
                "gpt-script-categorizer.py",
                "Categorizes scripts using GPT-3.5",
            ),
            "documents.py": (
                "backup-documents-to-csv.py",
                "Backs up ~/documents to CSV",
            ),
            "reddit.py": (
                "reddit-to-html-formatter.py",
                "Formats Reddit comments to HTML",
            ),
            ...
            "compile.py": (
                "compile-image-info-csv.py",
                "Compiles image info to master CSV",
            ),
            "converts.py": (
                "batch-convert-upscale-images.py",
                "Converts and upscales images in batch",
            ),
            "convertupscale.py": (
                "convert-upscale-images.py",
                "Single convert + upscale operation",
            ),
            "createimages.py": (
            ...
            "csvsort.py": (
                "csv-download-sort-images.py",
                "Downloads images from CSV and sorts",
            ),
            "denoiser.py": ("ffdnet-image-denoiser.py", "Denoises images using FFDNet"),
            "lexica.py": (
                "lexica-art-downloader.py",
                "Downloads images from Lexica.art",
            ),
            "mydesigner.py": (
                "batch-process-images.py",
                "Batch processes and downloads images",
            ),
            "nativetypes.py": (
                "jinja2-native-types.py",
                "Jinja2 native type converter",
            ),
            ...
            "setuptools.py": (
                ...
                "Setuptools bootstrap script",
            ),
            "upscalecreateimages.py": (
                "dalle-generate-upscale.py",
                "Generates with DALL-E and upscales",
            ),
            "cover.py": (
                "dalle-typography-generator.py",
                "Creates typography covers with DALL-E",
            ),
            "vision.py": (
                "gpt-vision-describer.py",
                "Describes images with GPT-4 Vision",
            ),
            "textgenerator.py": ("gpt-text-generator.py", "Generates text with GPT"),
            "smart.py": (
                "generate-organization-plan.py",
                "Generates smart organization plan",
            ),
            "sorts.py": (
                "sort-images-exclude-patterns.py",
                "Sorts images with exclusion patterns",
            ),
            "rename.py": ("batch-rename-images.py", "Batch renames image files"),
            "renamer.py": (
                "intelligent-script-renamer.py",
                "Renames Python scripts intelligently",
            ),
            "scrape.py": (
                "reddit-scraper-cleaner.py",
                "Scrapes Reddit and cleans text",
            ),
            "organize.py": ("ai-outputs-organizer.py", "Organizes AI output files"),
            "implement.py": (
                "execute-organization-plan.py",
                "Executes smart organization system",
            ),
            "pyorganize.py": (
                "extract-functions-classes.py",
                "Extracts top-level functions/classes",
            ),
        ...
        if current_name.isupper() or (
            current_name.replace("_", "").replace(".py", "").isupper()
        ):
    ...
    def analyze_all_files(self):
        ...
        for i, filepath in enumerate(py_files, 1):
            ...
            suggested_name, reason = self.suggest_accurate_name(
                filepath, info, current_name
            )
