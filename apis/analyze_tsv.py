...
def analyze_tsv(input_file):
    ...
    if valid_dates:
        ...
        print(
            f"Date Range: {earliest.strftime('%Y-%m-%d %H:%M:%S')} to {latest.strftime('%Y-%m-%d %H:%M:%S')}"
        )
    ...
    for value, count in sorted(
        values.items(), key=lambda x: int(x[0]) if x[0].isdigit() else 999
    ):
        percentage = (count / len(entries)) * 100
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("\nExample:")
        print(
            "  python analyze_tsv.py htu_autobackup_20260103_incremental_combined.tsv"
        )
        sys.exit(1)
