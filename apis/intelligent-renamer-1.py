...
class IntelligentRenamer:
    ...
    def deep_analyze_file(self, filepath: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.use_ai and (
                self.openai_key or self.gemini_key or self.anthropic_key
            ):
                ai_suggestion = self.get_ai_suggestion(
                    filepath, analysis, content[:CONSTANT_2000]
                )
                if ai_suggestion:
            ...
            if analysis["suggested_name"]:
                analysis["suggested_name"] = self.clean_filename(
    ...
    def detect_patterns(self, analysis: Dict) -> List[str]:
        ...
        if any(
            lib in all_text
            for lib in ["instagram", "instapy", "discord", "telegram", "tweepy"]
        ):
            patterns.append("bot")
        ...
        if any(
            lib in imports
            for lib in ["beautifulsoup", "bs4", "selenium", "scrapy", "playwright"]
        ):
        ...
        if any(
            lib in all_text
            for lib in ["openai", "anthropic", "leonardo", "dalle", "stable_diffusion"]
        ):
    ...
    def extract_keywords(self, analysis: Dict, content: str) -> List[str]:
        ...
        for func in analysis["functions"][:10]:
            ...
            if not func.startswith("__") and func.lower() not in [
                "main",
                "run",
                "init",
            ]:
    ...
    def get_ai_suggestion(:
        self, filepath: Path, analysis: Dict, content_preview: str
    ) -> Optional[Dict]:
...
"""
    ...
    def generate_pattern_name(self, analysis: Dict) -> str:
        ...
        if pattern == "youtube_automation":
            ...
            service = (
                keywords[0]
                if keywords and keywords[0] in ["openai", "leonardo", "dalle"]
                else "ai"
            )
    ...
    def scan_and_analyze(self) -> List[Dict]:
        ...
        self.print_header(
            "SCANNING AND ANALYZING FILES", Colors.CYAN, Emojis.MICROSCOPE
        )
        ...
        for idx, filepath in enumerate(python_files, 1):
            ...
            if (
                analysis["suggested_name"]
                and analysis["suggested_name"] != analysis["original_name"]
            ):
    ...
    def create_rename_plan(self, analyses: List[Dict]) -> None:
        ...
        self.print_header(
            "CREATING INTELLIGENT RENAME PLAN", Colors.YELLOW, Emojis.BRAIN
        )
        ...
        for analysis in analyses:
            ...
            if any(
                bad in new_name for bad in ["usr-bin", "import-", "from-", "for-the"]
            ):
            ...
            if not has_capitals and (
                len(new_name) < 10
                or (new_name.count("_") == 0 and new_name.count("-") == 0)
            ):
                self.stats["ambiguous_names"] += 1
                logger.info(
                    f"{Colors.YELLOW}⚠️  Ambiguous: {new_name} (from {analysis['original_name']}){Colors.END}"
                )
    ...
    def execute_rename_plan(self) -> None:
        ...
        sorted_plan = sorted(
            self.rename_plan, key=lambda x: x["confidence"], reverse=True
        )
        ...
        for idx, item in enumerate(sorted_plan, 1):
            ...
            logger.info(
                f"Confidence: {Colors.YELLOW}{item['confidence']:.2f}{Colors.END}"
            )
            if item["patterns"]:
            ...
            try:
                if not self.dry_run:
                    ...
                    logger.info(
                        f"{Colors.GREEN}{Emojis.CHECK} Renamed successfully{Colors.END}"
                    )

                    if item.get("category"):
                        logger.info(
                            f"{Colors.CYAN}  → Categorized to: {item['category']}{Colors.END}"
                        )
                else:
                    logger.info(f"{Colors.YELLOW}[DRY RUN] Would rename{Colors.END}")
                    if item.get("category"):
                        logger.info(
                            f"{Colors.YELLOW}[DRY RUN] Would move to: {item['category']}{Colors.END}"
                        )
    ...
    def run(self) -> None:
        ...
        logger.info(
            "║           AI-Powered Deep Analysis and Smart File Renaming                   ║"
        )
        logger.info(
            "║                                                                               ║"
        )
        ...
        logger.info(
            f"{Colors.CYAN}AI Enabled: {'Yes' if self.use_ai else 'No'}{Colors.END}"
        )

        if self.dry_run:
            logger.info(
                f"{Colors.YELLOW}{Emojis.WARNING} MODE: DRY RUN (no files will be renamed){Colors.END}"
            )
        else:
            logger.info(
                f"{Colors.RED}{Emojis.WARNING} MODE: LIVE RENAME (files will be renamed!){Colors.END}"
            )
        ...
        if not analyses:
            logger.info(
                f"{Colors.GREEN}{Emojis.CHECK} All files already have good names!{Colors.END}"
            )
            return
        ...
        logger.info(
            f"  {Emojis.RENAME} Renamed: {Colors.CYAN}{self.stats['files_renamed']:,}{Colors.END}"
        )
        logger.info(
            f"  {Emojis.BRAIN} AI Suggestions: {Colors.CYAN}{self.stats['ai_suggestions']:,}{Colors.END}"
        )
        logger.info(
            f"  {Emojis.FIRE} Pattern Matches: {Colors.CYAN}{self.stats['pattern_matches']:,}{Colors.END}"
        )
        logger.info(
            f"  📁 Categorized: {Colors.CYAN}{self.stats['files_categorized']:,}{Colors.END}"
        )
        logger.info(
            f"  ⚠️  Ambiguous: {Colors.CYAN}{self.stats['ambiguous_names']:,}{Colors.END}"
        )
        logger.info(
        ...
        logger.info(
            f"  {Emojis.FILE} JSON Log: {Colors.BLUE}{self.rename_log_file}{Colors.END}"
        )
        if self.undo_script.exists():
            logger.info(
                f"  🔄 Undo Script: {Colors.BLUE}{self.undo_script}{Colors.END}"
            )
        print()
        ...
        if self.dry_run:
            ...
            logger.info(
                f"{Colors.GREEN}{Emojis.CHECK} Renaming complete!{Colors.END}\n"
            )
