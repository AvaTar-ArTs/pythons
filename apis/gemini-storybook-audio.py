...
async def download_storybook(url: str, out_root: Path) -> Path:
    async with async_playwright() as p:
        ...
        async def classify_and_fetch(u: str) -> Optional[DownloadRecord]:
            ...
            try:
                absu = (
                    page.url if u.startswith("#") else urllib.parse.urljoin(page.url, u)
                )
            except (requests.RequestException, urllib.error.URLError, ConnectionError):
                pass
            ...
            if any(
                lower.endswith(x)
                for x in [".png", ".jpg", ".jpeg", ".webp", ".gif", ".svg"]
            ):
                kind = "image"
            elif any(lower.endswith(x) for x in [".mp4", ".webm", ".mov", ".m4v"]):
                kind = "video"
            elif any(
                lower.endswith(x) for x in [".mp3", ".wav", ".m4a", ".ogg", ".aac"]
            ):
                kind = "audio"
...
async def main():
    ap = argparse.ArgumentParser(
        description="Download Gemini Storybook share pages and assets."
    )
    ap.add_argument("urls", nargs="*", help="One or more Gemini share URLs.")
    ap.add_argument(
        "--file", "-f", dest="file", help="Path to a text file with one URL per line."
    )
    ap.add_argument(
        "--out", "-o", default="downloads", help="Output folder (default: downloads)"
    )
    args = ap.parse_args()
