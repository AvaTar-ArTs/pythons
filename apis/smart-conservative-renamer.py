...
class SmartConservativeRenamer:
    ...
    def __init__(:
        self, target_dir: str, dry_run: bool = True, csv_output: Optional[str] = None
    ):
        ...
        try:
            ...
            if analysis["docstring"]:
                ...
                analysis["keywords"] = [
                    w
                    for w in words
                    if w not in ["python", "script", "function", "class", "import"]
                ][:5]
    ...
    def analyze_file(self, filepath: Path) -> Optional[str]:
        ...
        if simplified != filename:
            ...
            if self.is_good_name(simplified) or (
                len(simplified) >= 10 and "_" in simplified
            ):
                return simplified
    ...
    def scan_and_plan(self) -> None:
        ...
        logger.info(
            f"{Colors.YELLOW}   (Content-aware + Parent-folder context){Colors.END}\n"
        )
        ...
        for root, dirs, files in os.walk(self.target_dir):
            dirs[:] = [
                d
                for d in dirs
                if not d.startswith((".", "__pycache__", "node_modules"))
            ]
        ...
        logger.info(
            f"{Colors.GREEN}✅ Found {len(self.rename_plan)} files to rename{Colors.END}"
        )
    ...
    def execute_plan(self) -> None:
        ...
        for parent_dir in sorted(by_parent.keys()):
            ...
            for item in items:
                ...
                logger.info(
                    f"      {Colors.CYAN}Context: {item.get('parent_context', 'none')}{Colors.END}"
                )
    ...
    def run(self) -> None:
        ...
        logger.info(
            "║      (Keeps good names, fixes bad ones - YOUR style!)            ║"
        )
        logger.info(
            "╚═══════════════════════════════════════════════════════════════════╝"
        )
        ...
        if not self.rename_plan:
            logger.info(
                f"{Colors.GREEN}✅ All files have good names already!{Colors.END}"
            )
            return
...
if __name__ == "__main__":
    ...
    target = (
        sys.argv[1]
        if len(sys.argv) > 1
        else Path(str(Path.home()) + "/Documents/python")
    )
