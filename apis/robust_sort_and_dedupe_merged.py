...
- /Users/steven/Music/nocTurneMeLoDieS/python/FINAL_ORGANIZED/core_analysis/robust_sort_and_dedupe.py
- /Users/steven/Music/nocTurneMeLoDieS/python/CLEAN_ORGANIZED/core_analysis/robust_sort_and_dedupe.py
...
Robust Sort and Deduplication Tool

This script comprehensively sorts, deduplicates, and organizes the Python music processing directory.
It handles already-moved files and creates a clean, organized structure.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('robust_sort_and_dedupe.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class RobustSortAndDedupe:
    ...
    def safe_move_file(self, src: Path, dst: Path) -> bool:
        """Safely move a file, handling errors gracefully."""
        try:
            if not src.exists():
    ...
    def process_duplicates(self, duplicates: Dict[str, List[Path]]):
        ...
        for file_hash, files in duplicates.items():
            if len(files) > 1:
                ...
                file_analyses = [self.analyze_file_content(f) for f in existing_files]
                ...
                for duplicate in duplicate_files:
                    if duplicate.exists():
                        ...
                        if self.safe_move_file(duplicate, archive_path):
                            total_duplicates += 1
    ...
    def process_similar_files(self, similar_groups: Dict[str, List[Path]]):
        ...
        for group_name, files in similar_groups.items():
            ...
            logger.info(f"Processing {group_name} group with {len(existing_files)} files")
            ...
            for similar in similar_files:
                if similar.exists():
                    ...
                    self.safe_move_file(similar, archive_path)
    ...
    def organize_remaining_files(self):
        ...
        for file_path in self.base_dir.rglob("*.py"):
            if (file_path.is_file() and 
                ...
                self.clean_dir not in file_path.parents and 
                self.duplicates_dir not in file_path.parents and
    ...
    def run_complete_sort_and_dedupe(self):
        ...
        print(f"🗂️  Files organized: {total_organized}")
...
def main():
    ...
    sorter = RobustSortAndDedupe(base_dir)
