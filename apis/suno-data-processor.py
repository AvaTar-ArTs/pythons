...
class SunoDataCleaner:
    ...
    def clean_song(song: Dict) -> Dict:
        ...
        if "tags" in cleaned and isinstance(cleaned["tags"], str):
            cleaned["tagsList"] = [
                tag.strip() for tag in cleaned["tags"].split(",") if tag.strip()
            ]
    ...
    def validate(songs: List[Dict]) -> Dict:
        ...
        print(
            f"  Valid songs: {report['valid']} ({report['valid'] / report['total'] * 100:.1f}%)"
        )
...
class SunoAnalytics:
    ...
    def analyze(songs: List[Dict]) -> Dict:
        ...
        # Calculate averages
        durations = [
            s.get("durationSeconds", 0) for s in songs if s.get("durationSeconds")
        ]
        if durations:
    ...
    def print_stats(stats: Dict):
        ...
        print(
            f"📝 With Lyrics: {stats['with_lyrics']} ({stats['with_lyrics'] / stats['total_songs'] * 100:.1f}%)"
        )
...
class SunoExporter:
    ...
    def export_markdown(songs: List[Dict], output_path: Path, stats: Dict):
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- **Total Duration:** {stats['total_duration_seconds'] / 3600:.1f} hours\n"
            )
            f.write(
                f"- **Average Duration:** {stats['avg_duration_seconds'] / 60:.1f} minutes\n\n"
            )
...
class SunoProcessor:
    ...
    def export_outputs(self):
        ...
        if self.args.output or self.args.all_formats:
            output_format = (
                self.args.output or self.input_file.suffix[1:]
            )  # csv or json
    ...
    def download_audio(self):
        ...
        for i, song in enumerate(self.songs, 1):
            ...
            try:
                print(
                    f"Downloading {i}/{len(self.songs)}: {song.get('title', 'Untitled')}",
                    end="... ",
                )
...
Examples:
  ...
  %(prog)s songs.csv --all-formats             # Export all formats
    ...
    parser.add_argument(
        "--distrokid", action="store_true", help="Create DistroKid-compatible CSV"
    )

    parser.add_argument(
        "--playlist", action="store_true", help="Create M3U playlist file"
    )

    parser.add_argument("--report", action="store_true", help="Create Markdown report")

    parser.add_argument("--all-formats", action="store_true", help="Export all formats")

    parser.add_argument(
        "--download-audio", action="store_true", help="Download audio files"
    )
    ...
    parser.add_argument(
        "--keep-duplicates", action="store_true", help="Keep duplicate songs"
    )
    ...
    parser.add_argument("--openai-key", type=str, help="OpenAI API key for analysis")
