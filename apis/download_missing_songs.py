...
def main():
    ...
    download_dir = (
        home / "Music/nocTurneMeLoDieS/FINAL_ORGANIZED/YOUR_SUNO_SONGS/DOWNLOADED"
    )
    ...
    suno_cookie = (
        config.get("SUNO_COOKIE") or config.get("suno_cookie") or config.get("cookie")
    )
    ...
    for i, song in enumerate(need_download[:10]):
        print(
            f"  {i + 1}. {song.get('Title', 'Unknown')} - {song.get('Artist', 'Unknown')}"
        )
    ...
    for i, song in enumerate(need_download):
        ...
        if "suno.com/song/" in song_url:
            ...
            print("  ??  Suno song page URL - need API/scraping")
            results["skipped"].append(
                {"title": title, "url": song_url, "reason": "Need Suno API or scraping"}
            )
            continue
        ...
        if output_path.exists():
            ...
            results["success"].append({"title": title, "path": str(output_path)})
            continue
        ...
        if download_with_curl(song_url, output_path, suno_cookie):
            ...
            results["failed"].append(
                {"title": title, "url": song_url, "reason": "Download failed"}
            )
    ...
    with open(report_path, "w", newline="", encoding="utf-8") as f:
        ...
        for item in results["failed"]:
            writer.writerow(
                [item["title"], "FAILED", f"{item['reason']}: {item['url']}"]
            )

        for item in results["skipped"]:
            writer.writerow(
                [item["title"], "SKIPPED", f"{item['reason']}: {item['url']}"]
            )
