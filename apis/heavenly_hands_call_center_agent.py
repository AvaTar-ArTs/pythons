...
class ArchitecturalPatternDetector:
    ...
    def _identify_call_stages(self, conversation_history: List[Dict]) -> List[str]:
        ...
        for message in conversation_history:
            ...
            if role == "assistant":
                if any(
                    word in content for word in ["hello", "hi", "thank you for calling"]
                ):
                    stages.append("greeting")
                elif any(
                    word in content
                    for word in ["what", "how can i help", "tell me about"]
                ):
                    stages.append("qualification")
                elif any(word in content for word in ["price", "cost", "quote"]):
                    stages.append("pricing")
                elif any(
                    word in content for word in ["schedule", "book", "appointment"]
                ):
                    stages.append("booking")
                elif any(
                    word in content for word in ["confirm", "perfect", "scheduled"]
                ):
    ...
    def _calculate_pattern_match_score(:
        self, actual_stages: List[str], expected_stages: List[str]
    ) -> float:
...
class AICategorizationEngine:
    ...
    def __init__(self):
        self.categorization_rules = {
            "customer_tier": {
                "premium": {
                    "keywords": ["deep clean", "weekly", "recurring", "premium"],
                    "threshold": 0.6,
                },
                "standard": {
                    "keywords": ["standard", "monthly", "one-time"],
                    "threshold": 0.5,
                },
                "budget": {
            ...
            "service_type": {
                "residential": {
                    "keywords": ["house", "home", "apartment", "residential"],
                    "threshold": 0.7,
                },
                "commercial": {
                    "keywords": ["office", "business", "commercial", "retail"],
                    "threshold": 0.7,
                },
                "airbnb": {
                    "keywords": ["airbnb", "rental", "turnover", "vacation"],
                    "threshold": 0.8,
                },
            },
            "urgency_level": {
                ...
                "soon": {
                    "keywords": ["soon", "this week", "quickly"],
                    "threshold": 0.6,
                },
                "flexible": {
                    "keywords": ["flexible", "whenever", "no rush"],
                    "threshold": 0.6,
                },
            },
        }
    ...
    def categorize_customer(:
        self, conversation_history: List[Dict], customer_info: Dict[str, Any]
    ) -> CustomerProfile:
    ...
    def _apply_categorization_rules(:
        self, text: str, rules: Dict[str, Any]
    ) -> Dict[str, Any]:
...
class ContentAwarenessIntelligence:
    ...
    def analyze_context(:
        self, conversation_history: List[Dict], customer_profile: CustomerProfile
    ) -> Dict[str, Any]:
        ...
        context_analysis["recommendations"] = self._generate_context_recommendations(
            context_analysis
        )

        # Calculate overall confidence
        context_analysis["confidence_score"] = self._calculate_context_confidence(
            context_analysis
        )
    ...
    def _analyze_conversation_context(:
        ...
        sentiment_score = sum(
            1 for word in positive_words if word in last_message
        ...
        urgency = (
            "high" if any(word in last_message for word in urgent_words) else "low"
        )
    ...
    def _generate_context_recommendations(:
        ...
        if conversation_context["stage"] == "greeting":
            recommendations.append(
                "Warm greeting with business name and value proposition"
            )
...
class HeavenlyHandsCallCenterAgent:
    ...
    def process_call(:
        self, user_message: str, customer_info: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        conversation_analysis = self.ast_analyzer.analyze_conversation_intent(
            self.conversation_history
        )
        ...
        # Architectural pattern detection
        architectural_analysis = self.architectural_detector.detect_call_architecture(
            self.conversation_history
        )
    ...
    def _generate_intelligent_response(:
        ...
        try:
            response = client.chat.completions.create(
                model="gpt-4o-mini", messages=messages, temperature=0.7, max_tokens=300
            )
        ...
        if semantic_patterns:
            ...
            for pattern_type, pattern_data in semantic_patterns.items():
                if (
                    ...
                    and "dominant_pattern" in pattern_data
                ):
...
"""
    ...
    def _update_call_metrics(self, response: str):
        ...
        if any(
            ...
            for word in ["scheduled", "booked", "appointment", "confirmed"]
        ):
...
def main():
    ...
    for i, message in enumerate(demo_messages, 1):
        ...
        if i == len(demo_messages):  # Show analysis for last message
            ...
            print(
                f"  Call Architecture: {result['architectural_analysis']['detected_pattern']}"
            )
            print(
                f"  Customer Tier: {result['customer_profile'].customer_tier if result['customer_profile'] else 'N/A'}"
            )
            print(
                f"  Confidence Score: {result['context_analysis']['confidence_score']:.2f}"
            )
