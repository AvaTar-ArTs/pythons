...
class ComprehensiveScriptAnalyzer:
    ...
    def categorize_script(self, filepath, description, apis):
        ...
        # Image-related
        if any(
            word in filename_lower + desc_lower
            for word in ["image", "photo", "picture", "gallery", "vision", "pil"]
        ):
        ...
        # Video-related
        if any(
            word in filename_lower + desc_lower for word in ["video", "mp4", "movie"]
        ):
        ...
        if any(
            word in filename_lower + desc_lower
            for word in ["code", "complexity", "quality", "review", "ast", "lint"]
        ):
            return "Code Analysis"

        # AI/LLM tools
        if any(
            word in filename_lower + desc_lower + apis_lower
            for word in ["gpt", "claude", "gemini", "llm", "orchestrat"]
        ):
        ...
        if any(
            word in filename_lower + desc_lower
            for word in ["docs", "documentation", "readme", "catalog"]
        ):
        ...
        if any(
            word in filename_lower + desc_lower
            for word in ["scrap", "crawl", "spider", "selenium", "playwright"]
        ):
            return "Web Scraping"
        ...
        if any(
            word in filename_lower + desc_lower
            for word in ["csv", "json", "data", "process", "convert"]
        ):
    ...
    def analyze_all(self):
        ...
        for i, filepath in enumerate(py_files, 1):
            ...
            if should_del:
                ...
                suggested_name, action = self.suggest_rename(
    ...
    def save_csv(self):
        ...
        with open(output_path, "w", newline="") as f:
            ...
            sorted_results = sorted(
                self.results, key=lambda x: (x["category"], x["current_name"])
            )
    ...
    def print_summary(self):
        ...
        for action, count in sorted(by_action.items()):
            emoji = {"DELETE": "🗑️", "KEEP": "✅", "REVIEW": "📝", "RENAME": "🏷️"}.get(
                action, "📌"
            )
