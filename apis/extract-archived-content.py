...
class ArchivedContentExtractor:
    ...
    def __init__(self, dry_run: bool = True):
        ...
        self.archive_dir = Path(
            "/Users/steven/Music/_ARCHIVED_DIRECTORIES/Song-origins-html"
        )
        ...
        self.extraction_report_dir = (
            Path.home() / "Music" / f"ARCHIVE_EXTRACTION_{self.timestamp}"
        )
        ...
        for csv_file in csv_files:
            ...
            try:
                ...
                if not self.dry_run:
                    ...
                    print(
                        f"  {Colors.YELLOW}[DRY RUN] Would preserve to: {target}{Colors.END}"
                    )
            ...
            try:
                with open(md_file, "r", encoding="utf-8") as f:
                    ...
                    if "prompt" in first_lines.lower():
                        ...
                        "professional" in first_lines.lower()
                        or "career" in first_lines.lower()
                    ):
        ...
        for html_file in html_files:
            ...
            try:
                with open(html_file, "r", encoding="utf-8") as f:
                    ...
                    soup.find_all(text=re.compile(r"\d+:\d+"))
                    ...
                    for song_title in songs_found:
                        self.html_songs.append(
                            {"title": song_title, "source": html_file.name}
                        )
    ...
    def scan_music_collection(self):
        ...
        print(
            f"{Colors.GREEN}✅ Found {self.stats['songs_in_collection']} songs in collection{Colors.END}"
        )
    ...
    def compare_and_find_gaps(self):
        ...
        print(
            f"{Colors.YELLOW}⚠️  Missing from collection: {len(csv_missing)}{Colors.END}\n"
        )
    ...
    def save_reports(self, csv_found: List, csv_missing: List):
        ...
        with open(found_file, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["Title", "Match_Type", "Similarity", "Found_In", "Duration", "Genre"]
            )
        ...
        with open(summary_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"Found {len(csv_missing)} songs in CSV that are NOT in your current collection.\n"
            )
            ...
            f.write(
                "3. Verify `FOUND_SONGS.csv` for fuzzy matches that might need renaming\n"
            )
            f.write(
                "4. Review preserved markdown files in ~/Documents/PRESERVED_ARCHIVES\n\n"
            )
    ...
    def run(self):
        ...
        print(
            "║           🔍 ARCHIVED CONTENT EXTRACTION & PRESERVATION 🔍                    ║"
        )
        print(
            "║                                                                               ║"
        )
        print(
            "║              Extract → Analyze → Compare → Report                            ║"
        )
        print(
            "║                                                                               ║"
        )
        ...
        print(
            f"Mode: {Colors.YELLOW}{'DRY RUN' if self.dry_run else 'LIVE EXTRACTION'}{Colors.END}"
        )
        ...
        print(
            f"  Songs in HTML: {Colors.CYAN}{self.stats['songs_in_html']}{Colors.END}"
        )
        print(
            f"  Songs in collection: {Colors.CYAN}{self.stats['songs_in_collection']}{Colors.END}"
        )
        print(
            f"  Missing songs: {Colors.YELLOW}{self.stats['missing_songs']}{Colors.END}\n"
        )
        ...
        if self.stats["missing_songs"] > 0:
            print(
                f"{Colors.YELLOW}⚠️  IMPORTANT: {self.stats['missing_songs']} songs from CSV are NOT in your collection!{Colors.END}"
            )
            print(
                f"{Colors.YELLOW}Review MISSING_SONGS.csv and use the Suno links to download them.{Colors.END}\n"
            )

        if self.dry_run:
            print(
                f"{Colors.YELLOW}⚠️  This was a DRY RUN. Run with --live to actually preserve files.{Colors.END}\n"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="🔍 Extract & Preserve Archived Content"
    )
    parser.add_argument(
        "--dry-run", action="store_true", default=True, help="Dry run mode (default)"
    )
    parser.add_argument(
        "--live", action="store_true", help="Live mode (actually preserve files)"
    )
