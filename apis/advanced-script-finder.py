...
class AdvancedScriptFinder:
        ...
        try:
            ...
            for pattern_name, pattern in self.advanced_patterns.items():
                if re.search(pattern, content, re.MULTILINE):
                    ...
                    stats["score"] += 5
                    self.results["advanced_patterns"][pattern_name].append(
                        str(filepath)
                    )
    ...
    def print_report(self, analyzed_files):
        ...
        for i, script in enumerate(analyzed_files[:20], 1):
            ...
            print(
                f"   Score: {script['score']} | Lines: {script['lines']:,} | Classes: {script['classes']} | Functions: {script['functions']}"
            )
            if script["patterns"]:
        ...
        for pattern, files in sorted(
            self.results["advanced_patterns"].items(),
        ...
        for api, files in sorted(
            self.results["api_usage"].items(), key=lambda x: len(x[1]), reverse=True
        ):
        ...
        print("-" * 80)
        for practice, files in sorted(
            self.results["best_practices"].items(),
            key=lambda x: len(x[1]),
            reverse=True,
        ):
    ...
    def save_report(self, analyzed_files):
        ...
        report_path = (
            self.home / "pythons" / f"advanced_scripts_analysis_{timestamp}.json"
        )
        ...
        report = {
            ...
            "patterns": {
                k: len(v) for k, v in self.results["advanced_patterns"].items()
            },
            "apis": {k: len(v) for k, v in self.results["api_usage"].items()},
            "best_practices": {
                k: len(v) for k, v in self.results["best_practices"].items()
            },
            "categories": {
