...
class ProductionConfig:
    ...
    RECEPTIONIST_DB_PATH: str = os.getenv(
        "RECEPTIONIST_DB_PATH", "databases/ai_receptionist.db"
    )
    REVENUE_DB_PATH: str = os.getenv(
        "REVENUE_DB_PATH", "databases/revenue_dashboard.db"
    )
    ...
    AFFILIATE_HELLOFRESH_CODE: str = os.getenv(
        "AFFILIATE_HELLOFRESH_CODE", "your-hellofresh-code"
    )
    AFFILIATE_VITAMIX_CODE: str = os.getenv(
        "AFFILIATE_VITAMIX_CODE", "your-vitamix-code"
    )
    ...
    def validate(self) -> bool:
        ...
        required_fields = ["OPENAI_API_KEY", "RECIPE_DB_PATH", "RECEPTIONIST_DB_PATH"]
...
try:
        ...
        if validate_config():
            ...
            print(
                f"🔗 Affiliate Programs: {len([k for k, v in config.get_affiliate_config().items() if v['enabled']])}"
            )
        else:
