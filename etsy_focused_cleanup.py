...
class FocusedEtsyCleanup:
    ...
    def quick_duplicate_cleanup(self, dry_run=True):
        ...
        print(f"  🔄 Duplicate groups found: {len([g for g in pattern_groups.values() if len(g) > 1])}")
    ...
    def quick_archive_cleanup(self, dry_run=True):
        ...
        for file_path in files_only:
            try:
                ...
                # System files
                if any(pattern in name_lower for pattern in ['.ds_store', 'thumbs.db', '.tmp', '~']):
                    removal_candidates.append({
                        'path': file_path,
                        'size': file_size,
                        'reason': 'System file',
                ...
                # Temporary files
                if any(pattern in name_lower for pattern in ['.tmp', '.temp', '.bak', '~']):
                    removal_candidates.append({
                        'path': file_path,
                        'size': file_size,
        ...
        print(f"  💾 Total potential space saved: {self.format_size(sum(c['size'] for c in removal_candidates))}")
    ...
    def generate_cleanup_report(self):
        ...
        report_file = self.etsy_dir / "00_archives" / f"focused_cleanup_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def run_focused_cleanup(self, dry_run=True):
        ...
        total_space = sum(c['size'] for c in duplicate_removals) + sum(c['size'] for c in archive_removals)
