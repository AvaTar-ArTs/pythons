...
class FlattenAlbumStructure:
    ...
    def __init__(self, dry_run: bool = True):
        ...
        self.stats = {"albums_flattened": 0, "files_moved": 0, "folders_removed": 0}

        # Subfolders to flatten
        self.subfolders_to_flatten = [
            "files",
            "lyrics",
            "analysis",
            "metadata",
            ...
            for file in files:
                ...
                if not self.dry_run:
                    try:
                        ...
                        print(
                            f"  {Colors.RED}Error moving {file.name}: {e}{Colors.END}",
                        )
                else:
                    self.stats["files_moved"] += 1
                    flattened_any = True
    ...
    def flatten_all_albums(self):
        ...
        for item in sorted(self.nocturne_dir.iterdir()):
            ...
            has_subfolders = any(
                (item / subfolder).exists() for subfolder in self.subfolders_to_flatten
            )
            ...
            for mp3 in mp3_files:
                ...
                if not self.dry_run:
                    ...
                    if self.stats["files_moved"] % 100 == 0:
                        print(
                            f"{Colors.GREEN}✅ Moved {self.stats['files_moved']} files...{Colors.END}",
                        )
                else:
                    self.stats["files_moved"] += 1
    ...
    def run(self):
        ...
        print(
            f"  Albums flattened: {Colors.CYAN}{self.stats['albums_flattened']}{Colors.END}",
        )
        print(f"  Files moved: {Colors.CYAN}{self.stats['files_moved']}{Colors.END}")
        print(
            f"  Folders removed: {Colors.CYAN}{self.stats['folders_removed']}{Colors.END}\n",
        )
