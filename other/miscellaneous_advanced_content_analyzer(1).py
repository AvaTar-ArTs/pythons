...
def infer_domain(imports: List[str]) -> Tuple[str, List[str]]:
    ...
    if {"requests", "beautifulsoup", "bs4", "selenium", "scrapy", "lxml"}.intersection(imp):
        domain = "Web Scraping"
    ...
    if {"os", "subprocess", "sys", "pathlib", "shutil"}.intersection(imp) and domain == "General":
        domain = "Automation"
...
def analyse_python_file(filepath: str) -> Dict[str, Union[str, Dict[str, Union[int, bool, List[str], Dict[str, List[str]]]], float, List[str]]]:
    """
    ...
    try:
        with open(filepath, "r", encoding="utf-8") as f:
            source = f.read()
        metrics.num_lines = len(source.splitlines())
    ...
    if not metrics.has_type_hints:
        suggestions.append("Add type annotations to function signatures to improve type safety.")
    # Suggest refactoring recursive functions if many
    if metrics.num_recursive_funcs > 1:
        suggestions.append("Consider refactoring recursive functions to iterative implementations if possible to reduce complexity.")
    ...
    if not patterns and metrics.num_classes > 3:
        suggestions.append("Consider applying appropriate design patterns (e.g., Factory, Strategy) to organise classes.")
    ...
    if metrics.num_loops > 5:
        suggestions.append("Reduce the number of loops by using comprehensions or vectorised operations where possible.")
...
def analyse_directory(root_dir: str) -> List[Dict[str, Union[str, float, Dict[str, Union[int, bool, List[str], Dict[str, List[str]]]], List[str]]]]:
    """
    Walk through the given directory and analyse all Python files.
    Returns a list of per‑file analysis dictionaries.
    """
    results: List[Dict[str, Union[str, float, Dict[str, Union[int, bool, List[str], Dict[str, List[str]]]], List[str]]]] = []
...
def main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="Analyse a Python project for content‑aware metrics and suggestions.")
    parser.add_argument("directory", help="Path to the root directory of the Python project")
    parser.add_argument("--output", "-o", default="analysis_report.json", help="Path to write the JSON report")
