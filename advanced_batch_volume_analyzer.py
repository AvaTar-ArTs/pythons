...
class AdvancedPythonAnalyzer:
    ...
    def __init__(self):
        ...
        self.purpose_patterns = {
            'SEO/Content Optimization': ['seo', 'optimize', 'keyword', 'trend', 'trending', 'aeo'],
            'Content Extraction': ['extract', 'scrape', 'grab', 'download', 'crawl'],
            'AI Orchestration': ['orchestrat', 'route', 'multi', 'llm', 'model', 'ai_'],
            'Audio Processing': ['audio', 'music', 'transcribe', 'whisper', 'ffmpeg', 'mp3'],
            'Image Processing': ['image', 'photo', 'gallery', 'visual', 'pil', 'pillow'],
            'Automation': ['automate', 'workflow', 'pipeline', 'task', 'job'],
            'CRM/Customer Management': ['customer', 'retention', 'churn', 'lifetime', 'crm'],
            'Analysis/Intelligence': ['analyze', 'analysis', 'intelligence', 'insight', 'report'],
            'YouTube/Video Content': ['youtube', 'video', 'description', 'tags', 'thumbnail'],
            'File Management': ['file', 'organize', 'move', 'copy', 'rename', 'duplicate'],
    ...
    def analyze_python_file(self, filepath: Path) -> Dict[str, Any]:
        ...
        try:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ClassDef):
                        ...
                        result['classes'].append({
                            ...
                            'bases': [self._ast_to_string(b) for b in node.bases][:5],
                        })
            ...
            main_functions = [f['name'] for f in result['functions'] if any(keyword in f['name'].lower() 
                            for keyword in ['main', 'run', 'execute', 'process', 'analyze', 'generate'])]
            if main_functions:
...
class AdvancedBatchVolumeAnalyzer:
    ...
    def analyze_volume(self, volume_path: str, max_depth: int = 5) -> Dict[str, Any]:
        ...
        for i, script_path in enumerate(python_scripts[:200], 1):  # Limit to 200 scripts
            if i % 20 == 0:
                print(f"   Progress: {i}/{min(200, len(python_scripts))} scripts analyzed...")
        ...
        for root, dirs, files in os.walk(volume):
            ...
            for file in files:
                if file.endswith('.py'):
                    filepath = Path(root) / file
                    try:
                        if filepath.stat().st_size > 0 and filepath.stat().st_size < 5 * 1024 * 1024:  # < 5MB
...
def generate_report(results: Dict, output_file: Path, stats: Dict):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"- **Python Scripts Analyzed:** {stats.get('python_scripts_analyzed', 0)}\n")
        f.write(f"- **Unique APIs Found:** {len(stats.get('apis_found', set()))}\n")
        f.write(f"- **Unique Technologies:** {len(stats.get('technologies_found', set()))}\n\n")
        ...
        for volume_path, volume_data in results.items():
            ...
            f.write(f"- **Python Scripts:** {len(volume_data.get('python_scripts', []))}\n")
            f.write(f"- **APIs Integrated:** {len(volume_data.get('apis_integrated', []))}\n")
