...
class UndoAndMergeCollection:
    ...
    def __init__(self, dry_run: bool = True):
        ...
        self.stats = {"moved_back": 0, "distributed": 0, "total_files": 0}
    ...
    def step1_move_all_mp3s_to_collection(self):
        ...
        for album_folder in self.nocturne_dir.iterdir():
            ...
            if any(
                skip in album_folder.name.upper()
                for skip in ["ANALYSIS", "LOG", "SCAN", "BACKUP"]
            ):
                continue
            ...
            if mp3_files:
                print(
                    f"{Colors.CYAN}{album_folder.name}{Colors.END}: {len(mp3_files)} MP3s"
                )
        ...
        for mp3 in mp3_files:
            ...
            if not self.dry_run:
                ...
                if self.stats["distributed"] % 100 == 0:
                    print(
                        f"{Colors.GREEN}✅ Moved {self.stats['distributed']} files...{Colors.END}"
                    )
            else:
                self.stats["distributed"] += 1
    ...
    def step3_remove_collection_folder(self):
        ...
        if not self.dry_run:
            try:
                ...
                print(
                    f"{Colors.GREEN}✅ Removed Steven_Chaplinski_Collection folder{Colors.END}"
                )
    ...
    def run(self):
        ...
        print(
            "║         🔄 UNDO COLLECTION & MERGE INTO NOCTURNE 🔄                           ║"
        )
        print(
            "╚═══════════════════════════════════════════════════════════════════════════════╝"
        )
        ...
        print(
            f"  Files moved back to collection: {Colors.CYAN}{self.stats['moved_back']}{Colors.END}"
        )
        print(
            f"  Files distributed to nocturne: {Colors.CYAN}{self.stats['distributed']}{Colors.END}"
        )
        print(
            f"  Total processed: {Colors.CYAN}{self.stats['total_files']}{Colors.END}\n"
        )

        if self.dry_run:
            print(
                f"{Colors.YELLOW}⚠️  DRY RUN - Run with --live to execute{Colors.END}\n"
            )
        else:
            print(f"{Colors.GREEN}✅ All songs merged into nocturne root!{Colors.END}")
            print(
                f"{Colors.GREEN}✅ Steven_Chaplinski_Collection removed!{Colors.END}\n"
            )
