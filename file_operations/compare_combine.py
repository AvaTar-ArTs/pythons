...
def compare_combine_tsv(input_file, output_file=None):
    ...
    if output_file is None:
        output_file = (
            input_path.parent / f"{input_path.stem}_combined{input_path.suffix}"
        )
    ...
    with open(input_path, "r", encoding="utf-8") as f:
        for line_num, line in enumerate(f, 1):
            ...
            if len(parts) != 4:
                print(
                    f"Warning: Line {line_num} has {len(parts)} columns (expected 4), skipping"
                )
                continue
            entries.append(
                {
    ...
    if duplicates:
        ...
        print(
            f"Found {len(duplicates)} URLs with duplicates ({total_duplicates} duplicate entries)"
        )
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        for entry in combined_entries:
            f.write(
                f"{entry['url']}\t{entry['timestamp']}\t{entry['value']}\t{entry['title']}\n"
            )
    ...
    print(
        f"  Reduction: {((len(entries) - len(combined_entries)) / len(entries) * 100):.1f}%"
    )
