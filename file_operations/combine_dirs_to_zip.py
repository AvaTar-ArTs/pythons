...
def add_directory_to_zip(:
    zip_file, dir_path, base_path=None, seen_files=None, file_stats=None
):
...
def combine_dirs_to_zip(input_dirs, output_zip):
    ...
    try:
        ...
        print(f"\n✅ Created: {output_zip}")
        print(
            f"   Output size: {final_size / 1024 / 1024:.2f} MB ({final_size / 1024 / 1024 / 1024:.2f} GB)"
        )
        print(f"   Total files: {final_files}")
        print("   Files from each source:")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Usage: python combine_dirs_to_zip.py <output.zip> <dir1> <dir2> [dir3] ..."
        )
        print("\nExample:")
