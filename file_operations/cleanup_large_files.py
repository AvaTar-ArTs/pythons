...
def compare_zip_files_detailed(zip1_path, zip2_path):
    ...
    try:
        ...
        similarity = (
            len(common) / max(len(files1), len(files2))
            if max(len(files1), len(files2)) > 0
            else 0
        )
...
def cleanup_large_files(root_path, dry_run=True):
    ...
    for zip_file in root_path.rglob("*.zip"):
        if (
            zip_file.is_file() and zip_file.stat().st_size > 100 * 1024 * 1024
        ):  # > 100MB
                ...
                for zip2 in files[i + 1 :]:
                    ...
                    if comparison and "error" not in comparison:
                        if comparison["similarity"] > 0.7:  # Lower threshold
                            ...
                            print(
                                f"        Similarity: {comparison['similarity'] * 100:.1f}%"
                            )
                            print(f"        Common files: {comparison['common_files']}")
                            print(
                                f"        Only in {zip1.name}: {comparison['only_in_1']}"
                            )
                            print(
                                f"        Only in {zip2.name}: {comparison['only_in_2']}"
                            )

                            # If one has significantly more files, keep that one
                            if (
                                comparison["only_in_2"] == 0
                                and comparison["only_in_1"] > 0
                            ):
                                # zip2 is subset of zip1
                                zip_to_remove.append(zip2)
                                print(
                                    f"        → {zip2.name} is subset of {zip1.name}, removing {zip2.name}"
                                )
                            elif (
                                comparison["only_in_1"] == 0
                                and comparison["only_in_2"] > 0
                            ):
                                # zip1 is subset of zip2
                                zip_to_remove.append(zip1)
                                print(
                                    f"        → {zip1.name} is subset of {zip2.name}, removing {zip1.name}"
                                )
                                ...
                                if zip1.stat().st_size >= zip2.stat().st_size:
                                    zip_to_remove.append(zip2)
                                    print(
                                        f"        → Keep: {zip1.name}, Remove: {zip2.name}"
                                    )
                                else:
                                    zip_to_remove.append(zip1)
                                    print(
                                        f"        → Keep: {zip2.name}, Remove: {zip1.name}"
                                    )
    ...
    for date_key, files in date_groups.items():
        if date_key == "20251225" and len(files) > 1:
            ...
            print(
                f"     Keeping largest: {files_sorted[0].name} ({files_sorted[0].stat().st_size / 1024 / 1024 / 1024:.2f} GB)"
            )
            # Mark smaller ones for potential removal (but be conservative)
            for smaller_zip in files_sorted[1:]:
                size_diff = (
                    files_sorted[0].stat().st_size - smaller_zip.stat().st_size
                ) / files_sorted[0].stat().st_size
                if size_diff < 0.1:  # Less than 10% difference - likely same content
                    if smaller_zip not in zip_to_remove:
                        ...
                        print(
                            f"     → Marking for removal: {smaller_zip.name} (similar size, likely duplicate)"
                        )
    ...
    for i, file1 in enumerate(large_files):
        for file2 in large_files[i + 1 :]:
            ...
            if file1.name == file2.name or (
                "ChatGPT" in file1.name and "ChatGPT" in file2.name
            ):
    ...
    if zip_to_remove:
        for zip_file in zip_to_remove:
            ...
            print(f"     - {zip_file.name}: {size_gb:.2f} GB")
        print(
            f"   Total ZIP size to remove: {total_zip_size / 1024 / 1024 / 1024:.2f} GB"
        )
    else:
    ...
    if html_to_remove:
        ...
        print(
            f"   Total HTML/MD size to remove: {total_html_size / 1024 / 1024 / 1024:.2f} GB"
        )
    else:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print(
            "  python cleanup_large_files.py /path/to/dir --execute # Actually remove"
        )
        sys.exit(1)
