...
def save_organization_state(root_dir):
    ...
    # Collect statistics
    defaultdict(lambda: {"count": 0, "files": 0, "size": 0})
    depth_dist = Counter()
    file_types = Counter()
    ...
    snapshot = {
        ...
        "top_categories": sorted(
            category_stats.items(), key=lambda x: x[1]["dirs"], reverse=True
        )[:20],
    }

    # Save JSON snapshot
    snapshot_file = (
        root / "organization_reports" / f"organization_snapshot_{timestamp}.json"
    )
