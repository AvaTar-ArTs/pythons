...
class QuickLookDiagnostic:
                ...
                try:
                    ...
                    if target.is_absolute():
                        ...
                        if target.exists() and target.is_symlink():
                            ...
                            if (
                                target_target == item
                                or target_target.resolve() == item.resolve()
                            ):
                                self.issues.append(
                                    {
    ...
    def check_homebrew_plugins(self):
        ...
        try:
            result = subprocess.run(
                ["brew", "list", "--cask"], capture_output=True, text=True, timeout=10
            )
            ...
            self.warnings.append({"type": "homebrew_timeout", "severity": "warning"})
            ...
            self.warnings.append(
                {"type": "homebrew_check_error", "error": str(e), "severity": "warning"}
            )
    ...
    def check_quicklook_service(self):
        ...
        try:
            result = subprocess.run(
                ["qlmanage", "-r"], capture_output=True, text=True, timeout=5
            )
            ...
            self.warnings.append(
                {"type": "service_check_error", "error": str(e), "severity": "warning"}
            )
    ...
    def find_actual_plugin_locations(self):
        ...
        if found_plugins:
            self.info.append({"type": "found_plugins", "plugins": dict(found_plugins)})
    ...
    def print_report(self):
        ...
        if self.info:
            ...
            for info in self.info:
                if info["type"] == "directory_info":
                    print(
                        f"  {info['directory']} plugins: {info['plugin_count']} found"
                    )
                    print(f"    Path: {info['path']}")
                elif info["type"] == "found_plugins":
                    print("  Found actual plugin locations:")
        ...
        if self.issues:
            ...
            if circular:
                ...
                print(
                    "     - Reinstall plugins using: brew reinstall --cask <cask-name>"
                )
