...
def is_backup_file(path: Path) -> bool:
    ...
    if name.endswith(".bak"):
        return True
    if ".bak" in name and any(
        char.isdigit() for char in name.split(".bak")[-1].split(".")[0][:5]
    ):
...
def find_backup_files(root_dir: Path, dry_run: bool = True) -> list:
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        dirs[:] = [
            d
            for d in dirs
            if d not in EXCLUDE_DIRS and not d.startswith(".") or d in [".git", ".venv"]
        ]
...
def remove_backup_files(backup_files: list, dry_run: bool = True) -> dict:
    """Remove backup files."""
    results = {"removed": 0, "failed": 0, "total_size": 0, "errors": []}
...
Examples:
  ...
  python cleanup_backup_files.py --dir pythons --dry-run
    ...
    if not dry_run:
        response = input(
            "⚠️  This will permanently delete backup files. Continue? (yes/no): "
        )
    ...
    print("=" * 70)
    print(
        f"Mode: {'DRY RUN (preview only)' if dry_run else 'EXECUTE (will delete files)'}"
    )
    print()
