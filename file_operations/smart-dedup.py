...
class SmartDeduplicator:
    ...
    def __init__(:
        ...
        self.backup_dir = backup_dir or Path(
            str(Path.home()) + "/Documents/python/dedup_backup"
        )
    ...
    def find_duplicates(self, directories: List[str]) -> Dict[str, List[str]]:
        ...
        for directory in directories:
            logger.info(
                f"{Colors.CYAN}{Emojis.FOLDER} Scanning: {directory}{Colors.END}"
            )
                    ...
                    try:
                        ...
                        if not file_hash.startswith("ERROR"):
                            ...
                            if file_count % CONSTANT_100 == 0:
                                logger.info(
                                    f"{Colors.YELLOW}  Processed: {file_count} files...{Colors.END}",
                                    end="\r",
                                )
                    except Exception:
                        pass
        ...
        logger.info(
            f"{Colors.GREEN}{Emojis.FIRE} Found {len(duplicates)} duplicate groups ({total_dupes} duplicate files){Colors.END}"
        )
    ...
    def choose_file_to_keep(self, files: List[str]) -> Tuple[str, List[str]]:
        ...
        for filepath in files:
            ...
            if any(
                word in name_lower for word in ["backup", "old", "temp", "copy", "bak"]
            ):
                score -= CONSTANT_100
    ...
    def create_backup(self, filepath: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"{Colors.RED}{Emojis.WARNING} Backup failed for {filepath}: {e}{Colors.END}"
            )
            return False
        ...
        try:
            if backup:
                if not self.create_backup(filepath):
                    logger.info(
                        f"{Colors.RED}{Emojis.WARNING} Skipping deletion (backup failed): {filepath}{Colors.END}"
                    )
                    return False
            ...
            logger.info(
                f"{Colors.RED}{Emojis.WARNING} Deletion failed: {filepath}: {e}{Colors.END}"
            )
            return False

    def process_duplicate_group(:
        self, file_hash: str, files: List[str], group_num: int, total_groups: int
    ) -> None:
        ...
        if self.interactive:
            ...
            if choice == "q":
                logger.info(
                    f"{Colors.YELLOW}{Emojis.WARNING} Aborted by user{Colors.END}"
                )
                sys.exit(0)
            elif choice == "n":
        ...
        for filepath in delete_files:
            if self.delete_file(filepath, backup=True):
                ...
                if not self.dry_run:
                    logger.info(
                        f"  {Colors.GREEN}{Emojis.CHECK} Deleted: {filepath}{Colors.END}"
                    )
                else:
                    logger.info(
                        f"  {Colors.YELLOW}[DRY RUN] Would delete: {filepath}{Colors.END}"
                    )
    ...
    def generate_report(self) -> str:
        ...
        report_file = (
            fstr(Path.home()) + "/Documents/python/DEDUPLICATION_REPORT_{timestamp}.md"
        )
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"| **Space Saved** | {self.stats['space_saved'] / (CONSTANT_1024**2):.2f} MB |\n"
            )
            ...
            for entry in self.kept_log:
                f.write(
                    f"- `{entry['file']}` (removed {entry['duplicates_removed']} duplicates)\n"
                )
    ...
    def run(self, directories: List[str]) -> None:
        ...
        logger.info(
            "║                    🧹 SMART DEDUPLICATION TOOL 🛡️                            ║"
        )
        logger.info(
            "║                                                                               ║"
        )
        logger.info(
            "║                  AI-Powered Safe Duplicate Removal                           ║"
        )
        logger.info(
            "║                                                                               ║"
        )
        ...
        if self.dry_run:
            logger.info(
                f"{Colors.YELLOW}{Emojis.SHIELD} MODE: DRY RUN (no files will be deleted){Colors.END}"
            )
        else:
            logger.info(
                f"{Colors.RED}{Emojis.WARNING} MODE: LIVE DELETION (files will be removed!){Colors.END}"
            )

        logger.info(
            f"{Colors.CYAN}{Emojis.BACKUP} Backup Directory: {self.backup_dir}{Colors.END}"
        )
        logger.info(
        ...
        if not duplicates:
            logger.info(
                f"{Colors.GREEN}{Emojis.CHECK} No duplicates found! All clean!{Colors.END}"
            )
            return
        ...
        logger.info(
            f"  {Emojis.FIRE} Groups Processed: {Colors.CYAN}{self.stats['groups_processed']:,}{Colors.END}"
        )
        logger.info(
            f"  {Emojis.TRASH} Files Deleted: {Colors.CYAN}{self.stats['files_deleted']:,}{Colors.END}"
        )
        logger.info(
            f"  {Emojis.KEEP} Files Kept: {Colors.CYAN}{self.stats['files_kept']:,}{Colors.END}"
        )
        logger.info(
            f"  💾 Space Saved: {Colors.CYAN}{self.stats['space_saved'] / (CONSTANT_1024**2):.2f} MB{Colors.END}\n"
        )

        logger.info(f"{Colors.BOLD}📝 OUTPUTS:{Colors.END}\n")
        logger.info(f"  {Emojis.FILE} Report: {Colors.BLUE}{report_file}{Colors.END}")
        logger.info(
            f"  {Emojis.BACKUP} Backups: {Colors.BLUE}{self.backup_dir}{Colors.END}\n"
        )

        if self.dry_run:
            logger.info(
                f"{Colors.YELLOW}{Emojis.WARNING} This was a DRY RUN. Re-run with --live to actually delete files.{Colors.END}\n"
            )
        else:
            logger.info(
                f"{Colors.GREEN}{Emojis.CHECK} Deduplication complete! Backups saved for safety.{Colors.END}\n"
            )
...
Examples:
  ...
  python smart_deduplication_tool.py --dry-run --batch
    ...
    parser.add_argument(
        "--live", action="store_true", help="Live mode (actually deletes files)"
    )
    parser.add_argument(
        "--interactive",
        action="store_true",
    ...
    parser.add_argument(
        "--dirs", nargs="+", help="Custom directories to scan (default: python dirs)"
    )
    ...
    dedup = SmartDeduplicator(
        dry_run=dry_run, interactive=interactive, backup_dir=args.backup_dir
    )
