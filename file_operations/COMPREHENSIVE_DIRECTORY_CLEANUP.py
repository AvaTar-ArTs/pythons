...
class DirectoryOrganizer:
    def __init__(self, pythons_dir="/Users/steven/pythons"):
        ...
        self.consolidation_map = {
            ...
            "audio_transcription/tools": ["AutoTranscribe"],
            ...
            # Docs → documentation/
            "documentation": ["Comprehensive-setup-docs", "md"],
            # Adobe/Image tools → MEDIA_PROCESSING/image_tools/
            "MEDIA_PROCESSING/image_tools": ["Adobe Python Scripts"],
            # Lyrics/Typography → content_creation/
            "content_creation/typography": ["AutoTypographyh - lyrics"],
    ...
    def print_plan(self, stats):
        ...
        if by_action["review"]:
            ...
            for action in by_action["review"]:
                size = sum(
                    f.stat().st_size for f in action["source"].rglob("*") if f.is_file()
                ) / (1024 * 1024)
                file_count = len(list(action["source"].rglob("*")))
                print(
                    f"  • {action['source'].name} ({size:.1f} MB, {file_count} files)"
                )
    ...
    def organize_root_files(self):
        ...
        for f in root_files:
            ...
            if (
                name.startswith("deep_")
                or name.startswith("intelligent_")
                or name.startswith("smart_")
                or name.startswith("cleanup_")
                or name.startswith("comprehensive_")
            ):
                continue
...
""")
    ...
    # Analyze root files
    organizer.organize_root_files()

    print("\n" + "=" * 70)
    print("🎯 EXPECTED RESULT")
    print("=" * 70)
    print("BEFORE:  131 directories + 77 root files (MESS!)")
    print("AFTER:   ~30 clean directories + ~10 root files")
    print("         Everything organized and consolidated!")
    print("=" * 70)
