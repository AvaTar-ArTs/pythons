...
def get_package_size(package_name, python_version):
    ...
    if python_version == "3.12":
        path = (
            Path.home() / "Library/Python/3.12/lib/python/site-packages" / package_name
        )
    else:
        path = (
            Path.home() / "Library/Python/3.11/lib/python/site-packages" / package_name
        )
...
def main():
    ...
    if total_savings > 0:
        print(
            f"💾 Potential additional savings: {total_savings:.2f} MB ({total_savings / 1024:.2f} GB)"
        )
        print()
