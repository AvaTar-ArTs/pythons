...
zip_files = [
    f for f in zip_files if f.is_file() and f.stat().st_size > 100 * 1024 * 1024
]  # > 100MB
...
for zip_file in zip_files:
    ...
    if "error" not in info:
        print(
            f"   - {info['name']}: {info['size'] / 1024 / 1024 / 1024:.2f} GB, {info['file_count']} files, date: {info['date']}"
        )
    ...
    for zip2 in zip_infos[i + 1 :]:
        ...
        if comparison and comparison["similarity"] > 0.5:
            zip_comparisons.append(
                {"zip1": zip1["name"], "zip2": zip2["name"], "comparison": comparison}
            )
...
if html_comparisons:
    ...
    for comp in html_comparisons:
        print(
            f"     {comp['file1']} <-> {comp['file2']} ({comp['size'] / 1024 / 1024:.2f} MB each)"
        )
...
print("     - Review ZIP files - many appear to be backups of the same content")
print("     - Consider keeping only the most recent backup")
print("     - Archive old backups to external storage if needed")
...
if html_comparisons:
    ...
    print(
        f"   Potential space savings: {potential_savings / 1024 / 1024 / 1024:.2f} GB"
    )
    print("   Recommendations:")
    print("     - Remove duplicate large HTML/MD files")
    print("     - Keep only one copy (preferably in organized subdirectory)")
