...
def check_empty(root_path):
    ...
    return {"empty_files": empty_files, "empty_dirs": empty_dirs}
...
def remove_empty(root_path, empty_files, empty_dirs, dry_run=True):
    ...
    if dry_run:
        ...
        print("   Use --execute to actually remove")
    ...
    if empty_dirs:
        ...
        sorted_dirs = sorted(
            empty_dirs,
            key=lambda d: len(str(d.relative_to(root_path)).split("/")),
            reverse=True,
        )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print(
            "  python check_empty.py /path/to/dir --remove --execute # Actually remove"
        )
        sys.exit(1)
    ...
    if should_remove and results:
        remove_empty(
            Path(root_path), results["empty_files"], results["empty_dirs"], dry_run
        )
