    ...
    def find_similar_content(self, threshold=80):
        ...
        for i, fp1 in enumerate(self.files_data):
            ...
            for j, fp2 in enumerate(self.files_data[i + 1 :], start=i + 1):
                ...
                size_ratio = min(fp1["size"], fp2["size"]) / max(
                    fp1["size"], fp2["size"], 1
                )
                ...
                if len(fp1["content"]) > 1000 and len(fp2["content"]) > 1000:
                    ...
                    similarity = self.compare_content_similarity(
                        fp1["content"], fp2["content"]
                    )
            ...
            if i % 100 == 0:
                print(
                    f"   ... compared {i}/{len(self.files_data)} files (found {len(similar_groups)} groups)"
                )
        ...
        if groups:
            ...
            for i, group in enumerate(sorted_groups[:20], 1):
                ...
                if group[0]["func_names"]:
                    ...
                    if common_funcs:
                        print(
                            f"    Common functions: {', '.join(list(common_funcs)[:5])}"
                        )
                print()
