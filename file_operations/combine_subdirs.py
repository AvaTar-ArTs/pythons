...
def combine_subdirectories(source1, source2, output_dir=None):
    ...
    if duplicates:
        print("  (These will be skipped - keeping files from first directory)")
    ...
    if merge_into_first:
        ...
        print(
            f"   (Files from {source2_path.name} will be copied to {source1_path.name})"
        )
    else:
    ...
    print(
        f"   Total size: {final_size / 1024 / 1024:.2f} MB ({final_size / 1024 / 1024 / 1024:.2f} GB)"
    )
    print(f"   Files from {source1_path.name}: {len(files1):,}")
    print(
        f"   Files from {source2_path.name}: {len(files2) - len(duplicates):,} (unique)"
    )
