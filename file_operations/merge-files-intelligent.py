...
class IntelligentMergeTool:
    ...
    def compare_files(self, source_file: Path, target_file: Path) -> Dict[str, Any]:
        ...
        if diff_stats.get("total_changes", 0) > 10:
            ...
            if source_newer and source_larger:
                ...
                comparison["reason"] = (
                    "Source is newer and larger - likely more complete"
                )
            elif source_newer:
                comparison["recommendation"] = "replace"
                comparison["reason"] = "Source is newer"
            elif comparison.get("source_analysis", {}).get(
                "functions", []
                ...
                comparison["reason"] = (
                    "Significant differences - manual review recommended"
                )
        else:
            # Minor differences
            if source_newer:
    ...
    def scan_and_compare(self) -> Dict[str, Any]:
        ...
        self.print_header(
            "SCANNING AND ANALYZING FILES (PARENT-FOLDER AWARE)",
            Colors.CYAN,
            Emojis.MICROSCOPE,
        )
        ...
        logger.info(
            f"{Colors.CYAN}{Emojis.FOLDER} Scanning target: {self.target_dir}{Colors.END}"
        )
        target_files = {}
        target_dirs = set()
        ...
        logger.info(
            f"{Colors.GREEN}  Found {len(target_files)} files in target{Colors.END}"
        )
        logger.info(
            f"{Colors.GREEN}  Found {len(target_dirs)} directories in target{Colors.END}"
        )
        ...
        for source_idx, source_dir in enumerate(self.source_dirs, 1):
            ...
            for root, dirs, files in os.walk(source_dir):
                dirs[:] = [
                    d
                    for d in dirs
                    if not d.startswith((".git", ".", "__pycache__", "node_modules"))
                ]
            ...
            logger.info(
                f"{Colors.GREEN}  Found {source_count} files in {source_name}{Colors.END}"
            )
            logger.info(
                f"{Colors.GREEN}  Found {len(source_dirs_found)} subdirectories{Colors.END}"
            )
    ...
    def analyze_and_merge(self, file_map: Dict) -> None:
        ...
        self.print_header(
            "DEEP ANALYSIS AND COMPARISON (PARENT-AWARE)", Colors.YELLOW, Emojis.BRAIN
        )
        ...
        for rel_path, info in sorted_files:
            ...
            if comparison["conflict"]:
                diff_file = (
                    self.merge_report_dir / f"{rel_path.replace('/', '_')}_diff.txt"
                )
    ...
    def execute_merge_plan(self, merge_plan: List[Dict]) -> None:
        ...
        self.print_header(
            "EXECUTING MERGE PLAN (PRESERVING DIRECTORY STRUCTURE)",
            Colors.GREEN,
            Emojis.MERGE,
        )
        ...
        logger.info(
            f"{Colors.CYAN}Mode: {'DRY RUN' if self.dry_run else 'LIVE MERGE'}{Colors.END}\n"
        )
        ...
        for idx, action in enumerate(merge_plan, 1):
            ...
            if action["conflict"]:
                logger.info(
                    f"{Colors.RED}{Emojis.WARNING} CONFLICT DETECTED{Colors.END}"
                )
            ...
            try:
                ...
                if not self.dry_run:
                    ...
                    logger.info(
                        f"{Colors.CYAN}  → Ensuring parent dir: {target.parent.relative_to(self.target_dir)}{Colors.END}"
                    )
                ...
                if target.exists() and not self.dry_run:
                    ...
                    logger.info(
                        f"{Colors.BLUE}  → Backed up to: {backup_path.relative_to(self.backup_dir)}{Colors.END}"
                    )
                ...
                if not self.dry_run:
                    ...
                    logger.info(
                        f"{Colors.GREEN}{Emojis.CHECK} Merged successfully → {target.relative_to(self.target_dir)}{Colors.END}"
                    )
                else:
                    logger.info(
                        f"{Colors.YELLOW}[DRY RUN] Would create: {target.relative_to(self.target_dir)}{Colors.END}"
                    )
                    if action.get("parent_dir"):
                        logger.info(
                            f"{Colors.YELLOW}[DRY RUN] Parent structure: {action['parent_dir']}/{Colors.END}"
                        )
    ...
    def generate_report(self) -> str:
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"| **Identical Files Skipped** | {self.stats['identical_files']:,} |\n"
            )
            ...
            for parent in sorted(by_parent.keys()):
                ...
                for action in actions:
                    ...
                    f.write(
                        f"- **Source Parent:** `{action.get('source_parent', 'unknown')}`\n"
                    )
    ...
    def run(self) -> None:
        ...
        logger.info(
            "║              AI-Powered Deep Analysis and Smart Merging                      ║"
        )
        logger.info(
            "║                                                                               ║"
        )
        ...
        logger.info(
            f"{Colors.CYAN}{Emojis.FOLDER} Target: {self.target_dir}{Colors.END}"
        )
        ...
        if self.dry_run:
            logger.info(
                f"{Colors.YELLOW}{Emojis.SHIELD} MODE: DRY RUN (no files will be modified){Colors.END}"
            )
        else:
            logger.info(
                f"{Colors.RED}{Emojis.WARNING} MODE: LIVE MERGE (files will be modified!){Colors.END}"
            )
        ...
        logger.info(
            f"  {Emojis.MICROSCOPE} Analyzed: {Colors.CYAN}{self.stats['files_analyzed']:,}{Colors.END}"
        )
        logger.info(
            f"  {Emojis.MERGE} Merged: {Colors.CYAN}{self.stats['files_merged']:,}{Colors.END}"
        )
        logger.info(
            f"  {Emojis.SPARKLES} New Files: {Colors.CYAN}{self.stats['new_files']:,}{Colors.END}"
        )
        logger.info(
            f"  {Emojis.CHECK} Updated: {Colors.CYAN}{self.stats['updated_files']:,}{Colors.END}"
        )
        logger.info(
            f"  {Emojis.WARNING} Conflicts: {Colors.CYAN}{self.stats['conflicts_found']:,}{Colors.END}"
        )
        logger.info(
            f"  ⏭️  Skipped: {Colors.CYAN}{self.stats['files_skipped']:,}{Colors.END}\n"
        )

        logger.info(f"{Colors.BOLD}📝 OUTPUTS:{Colors.END}\n")
        logger.info(f"  {Emojis.FILE} Report: {Colors.BLUE}{report_file}{Colors.END}")
        logger.info(
            f"  {Emojis.FOLDER} Backups: {Colors.BLUE}{self.backup_dir}{Colors.END}"
        )
        logger.info(
            f"  {Emojis.DIFF} Diffs: {Colors.BLUE}{self.merge_report_dir}{Colors.END}\n"
        )

        if self.dry_run:
            logger.info(
                f"{Colors.YELLOW}{Emojis.WARNING} This was a DRY RUN. Use --live to actually merge files.{Colors.END}\n"
            )
        else:
            logger.info(
                f"{Colors.GREEN}{Emojis.CHECK} Merge complete! Review the report and diffs.{Colors.END}\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--live", action="store_true", help="Live mode (actually merges files)"
    )
    parser.add_argument(
        "--interactive",
        action="store_true",
    ...
    parser.add_argument(
        "--batch", action="store_true", help="Batch mode (no confirmation)"
    )
