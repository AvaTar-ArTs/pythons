...
def get_zip_info(zip_path):
    ...
    try:
        ...
        return {"name": zip_path.name, "path": str(zip_path), "error": str(e)}
...
def compare_zips(zip_paths):
    ...
    for info in zip_infos:
        ...
        print(f"  {info['name']}")
        print(
            f"    Size: {info['file_size']:,} bytes ({info['file_size'] / 1024 / 1024:.2f} MB)"
        )
        print(f"    Files inside: {info['total_files']}")
        print(
            f"    Total uncompressed: {info['total_size']:,} bytes ({info['total_size'] / 1024 / 1024:.2f} MB)"
        )
        print(
            f"    Total compressed: {info['total_compressed']:,} bytes ({info['total_compressed'] / 1024 / 1024:.2f} MB)"
        )
        compression_ratio = (
            (1 - info["total_compressed"] / info["total_size"]) * 100
    ...
    files_in_multiple = {
        f: zips for f, zips in all_files.items() if 1 < len(zips) < len(zip_infos)
    }
    print(
    ...
    if files_in_multiple:
        ...
        for filename, zips in sorted(
            files_in_multiple.items(), key=lambda x: len(x[1]), reverse=True
        )[:10]:
            zip_names = [z[0] for z in zips]
            print(
                f"     - {filename} (in {len(zips)} ZIPs: {', '.join(zip_names[:3])}{'...' if len(zip_names) > 3 else ''})"
            )
    ...
    if size_differences:
        ...
        for filename, sizes in sorted(
            size_differences, key=lambda x: max(x[1].values()), reverse=True
        )[:10]:
            print(f"  {filename}")
            for zip_name, size in sorted(
    ...
    if crc_differences:
        ...
        for filename, crcs in sorted(
            crc_differences, key=lambda x: len(x[1]), reverse=True
        )[:10]:
            print(f"  {filename}")
