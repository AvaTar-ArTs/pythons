...
def compare_directories(dir1, dir2, check_content=False):
    ...
    print(
        f"  {dir1_info['name']}: {dir1_info['file_count']:,} files, {dir1_info['total_size'] / 1024 / 1024 / 1024:.2f} GB"
    )
    print(
        f"  {dir2_info['name']}: {dir2_info['file_count']:,} files, {dir2_info['total_size'] / 1024 / 1024 / 1024:.2f} GB"
    )
    ...
    print(
        f"  Difference: {abs(dir1_info['file_count'] - dir2_info['file_count']):,} files"
    )
    ...
    print(
        f"  {dir2_info['name']}: {dir2_info['total_size'] / 1024 / 1024 / 1024:.2f} GB"
    )
        ...
        for filename in common_files:
            ...
            if size1 != size2:
                size_mismatches.append(
                    {"file": filename, "size1": size1, "size2": size2}
                )
    ...
    if check_content and common_files:
        ...
        for filename in list(common_files)[:100]:  # Limit to first 100 for performance
            ...
            if checked % 10 == 0:
                print(
                    f"    Checking {checked}/{min(100, len(common_files))} files...",
                    end="\r",
                )
        ...
        if content_mismatches:
            ...
            print("  ✅ All checked files have identical content")
    ...
    for ext, data in sorted(ext1.items(), key=lambda x: x[1]["count"], reverse=True)[
        :10
    ]:
        print(
            f"    {ext:20s} {data['count']:6,} files  ({data['size'] / 1024 / 1024 / 1024:6.2f} GB)"
        )

    print(f"\n  {dir2_info['name']} - Top 10 file types:")
    for ext, data in sorted(ext2.items(), key=lambda x: x[1]["count"], reverse=True)[
        :10
    ]:
        print(
            f"    {ext:20s} {data['count']:6,} files  ({data['size'] / 1024 / 1024 / 1024:6.2f} GB)"
    ...
    similarity = (
        (len(common_files) / max(len(files1), len(files2)) * 100)
