...
def merge_into_destination(source_dir, dest_dir, remove_source=True):
        ...
        for file_path in dir_path.rglob("*"):
            if file_path.is_file():
                try:
                    ...
                    files[rel_path] = {"path": file_path, "size": size}
                    total_size += size
                except Exception:
                    continue
    ...
    print(
        f"  Source ({source_path.name}): {len(source_files):,} files, {source_size / 1024 / 1024 / 1024:.2f} GB"
    )
    print(
        f"  Destination ({dest_path.name}): {len(dest_files):,} files, {dest_size / 1024 / 1024 / 1024:.2f} GB"
    )
    ...
    if duplicates:
        print("  (These will be skipped - keeping files already in destination)")
    ...
    print(
        f"  Final destination: {len(final_files):,} files, {final_size / 1024 / 1024 / 1024:.2f} GB"
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Usage: python merge_and_remove.py <source_dir> <dest_dir> [--keep-source]"
        )
        print("\nExample:")
