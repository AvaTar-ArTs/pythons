...
def fix_insane_tools(root_dir, dry_run=True):
    ...
    for item in tools_dir.iterdir():
        if item.is_dir():
            ...
            if (
                name.startswith("refs_")
                or name.startswith("objects_")
                or name.startswith("logs_")
                or name.startswith("hooks_")
                or name.startswith("info_")
                or name.startswith("AUTOMATION_BOTS_logs")
            ):
    ...
    for artifact in git_artifacts:
        try:
            ...
            if removed <= 20:
                ...
                print(
                    f"   {'Would remove' if dry_run else 'Removed'}: {artifact.name} ({size_mb:.2f} MB)"
                )
        ...
        for auto_dir in automation_dirs:
            ...
            try:
                ...
                if moved > 0:
                    ...
                    if consolidated <= 20:
                        print(
                            f"   ✅ {auto_dir.name} → AUTOMATION_BOTS ({moved} items)"
                        )
        ...
        if target.exists() and target != long_dir:
            ...
            try:
                ...
                if moved > 0:
                    ...
                    if flattened <= 15:
                        print(
                            f"   ✅ {long_dir.name[:50]}... → {new_name} ({moved} items)"
                        )
