...
def remove_duplicates(dry_run=True, backup=True, confirm_each=False):
    ...
    print(f"\n{'='*60}")
    print(f"DUPLICATE FILE REMOVAL - DOWNLOADS")
    print(f"{'='*60}")
    ...
    if backup and not dry_run:
        backup_dir = Path(f'/Users/steven/Downloads_Duplicates_Backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
    ...
    if dry_run:
        ...
        print("To actually remove files, run with: python3 remove_duplicates_downloads.py --live")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        if '--live' in sys.argv or '--remove' in sys.argv:
            ...
            if '--force' not in sys.argv:
                ...
                try:
                    ...
                    print("Use --force flag to skip confirmation in non-interactive mode.")
                    sys.exit(1)
        ...
        if '--force' in sys.argv and ('--live' in sys.argv or '--remove' in sys.argv):
            print("WARNING: Running in FORCE mode - files will be deleted without confirmation!")
