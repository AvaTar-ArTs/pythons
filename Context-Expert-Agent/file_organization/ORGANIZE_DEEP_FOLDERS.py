                ...
                candidates.append(
                    {
                        ...
                        "can_flatten": total_sub_files
                        < file_count * 2,  # If subfolders don't have many more files
                    },
                )
...
def create_organization_plan(candidates):
    ...
    plan = {"high_priority": [], "medium_priority": [], "low_priority": [], "skip": []}

    for candidate in candidates:
        # High priority: depth > 4, severity > 500, can flatten
        if (
            candidate["depth"] > 4
            and candidate["severity"] > 500
            and candidate["can_flatten"]
        ):
...
def flatten_folder(source_path, target_depth=2):
    ...
    try:
        ...
        new_name = "_".join(
            parts[target_depth:],
        )  # Combine deeper parts with underscore
        ...
        with open(backup_log, "a") as log:
            log.write(
                f"{source_path},{new_path.relative_to(documents_dir)},{current_depth},{target_depth}\n",
            )
...
for i, folder in enumerate(plan["high_priority"][:20], 1):
    print(
        f"{i:2d}. Depth: {folder['depth']} | Files: {folder['files']:,} | "
