...
class CodexAnalyzer:
    ...
    def analyze(self) -> Dict:
        ...
        print(f"   ✅ Codex analysis complete: {len(self.codex_patterns)} patterns found")
    ...
    def _analyze_python_functions(self, file_path: Path, lines: List[str], structure: Dict):
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith('def ') and '(' in line:
                ...
                ai_relevance = self._calculate_ai_relevance(func_name, params, file_path)
    ...
    def _analyze_python_classes(self, file_path: Path, lines: List[str], structure: Dict):
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith('class ') and '(' in line:
                ...
                pattern = CodexPattern(
                    ...
                    ai_relevance=self._calculate_ai_relevance(class_name, methods, file_path)
                )
                
                self.codex_patterns.append(pattern)
                structure['patterns_found'].append(pattern)
    
    def _analyze_python_imports(self, file_path: Path, lines: List[str], structure: Dict):
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith('class ') and '{' in line:
                ...
                pattern = CodexPattern(
                    ...
                    return_type=None,
                    complexity=self._calculate_js_complexity(lines, i),
                    ai_relevance=self._calculate_ai_relevance(class_name, [], file_path)
                )
        ...
        for file_path in config_files:
            try:
                ...
                pattern = CodexPattern(
                    ...
                    file_path=str(file_path),
                    line_number=1,
                    parameters=list(config_data.keys()) if isinstance(config_data, dict) else [],
                    return_type=None,
                    complexity=3,
    ...
    def _calculate_ai_relevance(self, name: str, params: List[str], file_path: Path) -> int:
        """Calculate AI relevance score for a code pattern."""
        relevance = 5  # Base score
        
        # High relevance for common patterns
        if any(keyword in name.lower() for keyword in ['api', 'endpoint', 'handler', 'service', 'util', 'helper']):
            relevance += 3
    ...
    def _find_class_methods(self, lines: List[str], class_start: int) -> List[str]:
        ...
        for i in range(class_start + 1, len(lines)):
            ...
            if indent_level is not None and line.strip():
                ...
                if current_indent <= indent_level and not line.strip().startswith('def '):
                    break
    ...
    def _extract_doc_patterns(self, content: str) -> List[str]:
        ...
        if any(keyword in content.lower() for keyword in ['api', 'endpoint', 'route', 'method']):
            patterns.append('api_documentation')
        
        # Extract configuration examples
        if any(keyword in content.lower() for keyword in ['config', 'setting', 'parameter']):
