...
class JSONExporter:
    ...
    def _export_duplicate_analysis(self, result: Any, output_dir: Path):
        ...
        for i, (content_hash, file_paths) in enumerate(result.duplicate_groups.items()):
            duplicate_data.append({
                ...
                'wasted_space_bytes': (len(file_paths) - 1) * self._get_file_size(file_paths[0]) if file_paths else 0
            })
        
        duplicate_analysis = {
            'metadata': {
                'total_groups': len(duplicate_data),
                'total_duplicate_files': sum(group['group_size'] - 1 for group in duplicate_data),
                'total_wasted_space_bytes': sum(group['wasted_space_bytes'] for group in duplicate_data),
                'export_timestamp': datetime.now().isoformat()
            },
    ...
    def _export_github_analysis(self, result: Any, output_dir: Path):
        ...
        github_analysis = {
            'metadata': {
                ...
                'repository_health_score': github_structure.get('repository_health_score', 0)
            },
            'repository_structure': {
                ...
                'has_github_workflows': github_structure.get('has_github_workflows', False),
                'has_issue_templates': github_structure.get('has_issue_templates', False),
                'has_pr_template': github_structure.get('has_pr_template', False),
                'has_security_policy': github_structure.get('has_security_policy', False)
            },
            'file_counts': {
                ...
                'documentation_files': len(github_structure.get('documentation_files', [])),
                'asset_files': len(github_structure.get('asset_files', []))
            },
            'recommendations': {
                'missing_essentials': github_structure.get('missing_essentials', []),
                'suggested_improvements': github_structure.get('suggested_improvements', [])
            }
        }
    ...
    def _prepare_category_analysis_data(self, result: Any) -> Dict:
        ...
        return {
            ...
            'size_by_category': {k: v for k, v in result.size_by_category.items()},
            'top_categories': sorted(result.categories.items(), key=lambda x: x[1], reverse=True)[:10]
        }
    
    def _prepare_duplicate_analysis_data(self, result: Any) -> Dict:
        """Prepare duplicate analysis data for JSON export."""
        total_duplicates = sum(len(files) - 1 for files in result.duplicate_groups.values())
    ...
    def _prepare_depth_analysis_data(self, result: Any) -> Dict:
        ...
        return {
            ...
            'depth_with_most_files': max(result.depth_distribution.items(), key=lambda x: x[1])[0] if result.depth_distribution else 0
        }
    ...
    def _calculate_average_depth(self, depth_distribution: Dict[int, int]) -> float:
        ...
        weighted_depth = sum(depth * count for depth, count in depth_distribution.items())
