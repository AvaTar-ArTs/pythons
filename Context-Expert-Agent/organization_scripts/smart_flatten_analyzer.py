...
def analyze_patterns(analysis):
    ...
    for cat, count in cat_sizes[:15]:
        ...
        print(f"   {cat:30} {count:4,} dirs | {small_count:3} small | {deep_count:3} deep")
    ...
    merge_opportunities = [(parent, dirs) for parent, dirs in small_by_parent.items()
                          if len(dirs) >= 2]

    print(f"   Merge opportunities (≥2 small dirs per parent): {len(merge_opportunities):,}")
    ...
    print(f"   Deep nesting by category:")
...
def suggest_improvements(analysis, patterns):
    ...
    for cat, count in cat_sizes[:10]:
        ...
        if count > 200:
            ...
            if deep_count > 50:
                strategy.append(f"   → Flatten deep structures (could reduce ~{deep_count // 2} dirs)")
                potential_reduction += deep_count // 2
            if small_count > 30:
                strategy.append(f"   → Merge small dirs (could reduce ~{small_count * 0.7:.0f} dirs)")
                potential_reduction += int(small_count * 0.7)

        elif count > 50:
            strategy.append(f"🔸 Moderate consolidation ({count} dirs)")
            if deep_count > 20:
                strategy.append(f"   → Flatten depth > 3 (could reduce ~{deep_count // 2} dirs)")
                potential_reduction += deep_count // 2
            if small_count > 10:
                strategy.append(f"   → Merge small dirs (could reduce ~{small_count * 0.6:.0f} dirs)")
        ...
        if strategy:
            ...
            if potential_reduction > 0:
                print(f"      💰 Potential reduction: ~{potential_reduction} directories")
    ...
    small_count = len([d for d in analysis['dirs'] if d['files'] < 5 and d['depth'] > 2])

    print(f"\n   1. Aggressive Depth Flattening:")
    print(f"      Current: {deep_count:,} directories at depth > 3")
    print(f"      Strategy: Flatten all depth > 3 to depth 3")
    ...
    print(f"      Strategy: Merge into parents or combine similar")
    ...
    print(f"      Strategy: Merge directories with same/similar names")
    print(f"      Potential reduction: ~{len(merge_ops) * 2:,} directories")

    # Calculate total potential
    total_potential = (deep_count // 3 * 2) + int(small_count * 0.65) + (len(merge_ops) * 2)
...
def generate_improved_strategy(analysis, patterns):
    ...
    print(f"   • Group by parent and merge when beneficial")
...
def main():
    ...
    report = {
        'current_state': {
            ...
            'categories': {cat: len(dirs) for cat, dirs in analysis['categories'].items()}
        },
        'opportunities': {
            'deep_nesting_count': len(analysis['deep_nesting']),
            'small_dirs_count': sum(len(dirs) for dirs in analysis['small_dirs'].values()),
