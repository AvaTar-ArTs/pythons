...
def get_file_category_mapping():
    ...
    for script_path in all_scripts:
        ...
        elif any(keyword in str_path.lower() for keyword in ['cleanup', 'clean', 'final', 'complete']):
            file_mapping[str_path] = 'core_tools/cleanup'
        elif any(keyword in str_path.lower() for keyword in ['duplicate', 'similarity', 'similar', 'dedupe']):
            file_mapping[str_path] = 'core_tools/duplicate_management'
        elif any(keyword in str_path.lower() for keyword in ['analyze', 'analysis', 'scanner']):
            file_mapping[str_path] = 'core_tools/analysis'
        elif any(keyword in str_path.lower() for keyword in ['openai', 'claude', 'anthropic', 'gemini', 'groq']):
            if any(keyword in str_path.lower() for keyword in ['generate', 'text', 'content']):
                file_mapping[str_path] = 'ai_ml_tools/content_generation'
            else:
                file_mapping[str_path] = 'ai_ml_tools/automation'
        elif any('audio' in part.lower() for part in relative_path.parts) or 'audio' in str_path.lower():
            file_mapping[str_path] = 'media_tools/audio'
        elif any('video' in part.lower() for part in relative_path.parts) or 'video' in str_path.lower():
            file_mapping[str_path] = 'media_tools/video'
        elif any('image' in part.lower() for part in relative_path.parts) or 'image' in str_path.lower():
        ...
        elif any(keyword in str_path.lower() for keyword in ['tiktok', 'twitter', 'social']):
            file_mapping[str_path] = 'automation_tools/general'
        elif any(keyword in str_path.lower() for keyword in ['data', 'csv', 'json', 'pandas']):
        ...
        elif any('file' in part.lower() for part in relative_path.parts) or 'file' in str_path.lower():
            if any(keyword in str_path.lower() for keyword in ['op', 'operation', 'move', 'copy']):
                file_mapping[str_path] = 'utilities/file_ops'
            else:
                file_mapping[str_path] = 'utilities/misc'
        elif any('web' in part.lower() for part in relative_path.parts) or 'web' in str_path.lower():
