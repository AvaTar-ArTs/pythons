...
class SingleZipProcessor:
    def __init__(self, source_dir, output_dir, temp_dir=None):
        ...
        self.temp_dir = Path(temp_dir) if temp_dir else Path('/tmp/canva_single_processing')
    ...
    def analyze_zip(self, zip_path):
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as zip_file:
                ...
                logger.info(f"✅ Analyzed {zip_path.name}: {analysis['file_count']} files, {analysis['duplicates_in_archive']} internal duplicates")
    ...
    def create_clean_zip(self, zip_path, analysis):
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as source_zip:
                with zipfile.ZipFile(clean_zip_path, 'w', zipfile.ZIP_DEFLATED, compresslevel=9) as dest_zip:
                    ...
                    logger.info(f"✅ Cleaned {zip_path.name}: removed {files_skipped} internal duplicates")
    ...
    def compress_zip(self, clean_zip_path):
        ...
        try:
            ...
            # Compress with 7zip
            compressed_path = self.temp_dir / f"compressed_{clean_zip_path.name.replace('.zip', '.7z')}"
            cmd = [
                self.seven_zip_cmd, 'a', '-t7z', '-mx9', '-m0=lzma2',
                str(compressed_path), str(extract_dir / '*')
            ]
            ...
            if result.returncode == 0:
                ...
                logger.info(f"✅ Compressed {clean_zip_path.name}: {ratio:.1f}% reduction")
    ...
    def organize_zip(self, zip_path, analysis, clean_zip_path=None, compressed_path=None):
    ...
    def generate_zip_report(self, zip_path, analysis):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"- **File Size:** {analysis['file_size'] / (1024*1024):.1f} MB\n")
            f.write(f"- **Status:** {'✅ Success' if analysis['success'] else '❌ Failed'}\n")
            
            if analysis['success']:
                f.write(f"- **Files Count:** {analysis['file_count']}\n")
                f.write(f"- **Content Size:** {analysis['total_content_size'] / (1024*1024):.1f} MB\n")
                f.write(f"- **Internal Duplicates:** {analysis['duplicates_in_archive']}\n")
            else:
    ...
    def process_all_zips(self, interactive=True):
        ...
        logger.info(f"🚀 Starting single ZIP processing")
        ...
        for i, zip_path in enumerate(zip_files, 1):
            ...
            if interactive:
                response = input(f"Process {zip_path.name}? (y/n/s to skip remaining): ").lower()
                if response == 'n':
    ...
    def generate_final_summary(self):
        ...
        with open(summary_path, 'w') as f:
            ...
            f.write(f"- **Space Saved:** {self.stats['space_saved'] / (1024*1024):.1f} MB\n\n")
            ...
            f.write("├── Cleaned/            # Cleaned versions (internal deduplication)\n")
...
def main():
    ...
    parser.add_argument('--temp', default='/tmp/canva_single_processing',
                       help='Temporary directory for processing')
    parser.add_argument('--non-interactive', action='store_true',
                       help='Process all files without prompting')
    parser.add_argument('--zip-file', type=str,
                       help='Process only a specific ZIP file')
