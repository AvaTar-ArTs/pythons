...
class DeepFolderAnalyzer:
    ...
    def get_file_category(self, filepath):
        ...
        # Web
            ...
            return 'web'
        # Data/Config
        elif ext in ['.json', '.yaml', '.yml', '.toml', '.xml', '.csv', '.tsv', '.ini', '.conf']:
            return 'config'
        # Documents
        elif ext in ['.pdf', '.doc', '.docx', '.txt', '.md', '.rtf', '.odt']:
            return 'document'
        # Images
        elif ext in ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.svg', '.webp', '.ico', '.tiff', '.psd']:
            return 'image'
        # Video
    ...
    def analyze(self, max_depth=None):
        ...
        for root, dirs, files in os.walk(self.root_path):
            try:
                ...
                if not dirs and not files:
                    self.stats['empty_dirs'].append(str(root_path.relative_to(self.root_path)))
                ...
                for filename in files:
                    ...
                    try:
                        ...
                        self.stats['errors'].append(f"Error accessing file {filepath}: {e}")
        ...
        top_level_dirs = [d for d in self.root_path.iterdir() if d.is_dir() and not d.is_symlink()]
        for dirpath in top_level_dirs:
            try:
                ...
                self.stats['errors'].append(f"Error calculating size for {dirpath}: {e}")
    ...
    def print_report(self):
        ...
        print("-"*80)
        for category, count in sorted(self.stats['file_types'].items(), key=lambda x: x[1], reverse=True):
        ...
        for ext, count in self.stats['file_extensions'].most_common(20):
            ...
            percentage = (count / self.stats['total_files'] * 100) if self.stats['total_files'] > 0 else 0
            print(f"{ext:20} {count:10,} files ({percentage:5.2f}%) - {self.format_size(size):>12}")
        
        print("\n" + "-"*80)
        print("💾 TOP 30 LARGEST DIRECTORIES")
        print("-"*80)
        for i, item in enumerate(self.stats['largest_dirs'][:30], 1):
            print(f"{i:2}. {item['path']:50} {self.format_size(item['size']):>12} ({item['size_gb']:.2f} GB)")
    ...
    def save_json_report(self, output_path):
        ...
        report = {
            ...
            'size_by_extension': {k: v for k, v in self.stats['size_by_extension'].items()},
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Deep Folder Analyzer - Comprehensive directory analysis')
    parser.add_argument('path', nargs='?', default='~/', help='Path to analyze (default: ~/)')
