...
class WorkspaceOptimizer:
    ...
    def find_archived_node_modules(self):
                ...
                if 'node_modules' in dirs:
                    ...
                    try:
                        ...
                        self.report['node_modules_found'].append({
                            ...
                            'parent_project': str(Path(root).relative_to(self.home))
                        })
        ...
        print(f"✅ Found {len(self.report['node_modules_found'])} node_modules directories")
        print(f"💾 Potential savings: {self.format_size(self.report['potential_savings'])}")
    ...
    def find_python_projects(self):
        ...
        self.report['python_projects'].sort(key=lambda x: x['py_files_count'], reverse=True)
    ...
    def print_report(self):
        ...
        if self.report['node_modules_found']:
            ...
            for i, nm in enumerate(self.report['node_modules_found'][:20], 1):
                print(f"{i:2}. {nm['path']}")
                print(f"    Size: {self.format_size(nm['size'])} ({nm['size_mb']:.1f} MB)")
        ...
        if self.report['large_videos']:
            ...
            for i, video in enumerate(self.report['large_videos'][:10], 1):
                ...
                print(f"    Size: {self.format_size(video['size'])} ({video['size_gb']:.2f} GB)")
                print()
        ...
        if self.report['python_projects']:
            ...
            for i, proj in enumerate(self.report['python_projects'][:15], 1):
                ...
                deps = []
                if proj['has_requirements']: deps.append("requirements.txt")
                if proj['has_pyproject']: deps.append("pyproject.toml")
                if proj['has_setup']: deps.append("setup.py")
                if proj['has_pipfile']: deps.append("Pipfile")
        ...
        if self.report['js_projects']:
            ...
            for i, proj in enumerate(self.report['js_projects'][:15], 1):
                print(f"{i:2}. {proj['path']}")
                print(f"    Files: {proj['js_files']} JS, {proj['ts_files']} TS, "
                      f"{proj['jsx_files']} JSX, {proj['tsx_files']} TSX")
                print(f"    node_modules: {'✓ Present' if proj['has_node_modules'] else '✗ Missing'}")
                print()
        
        print("\n" + "="*80)
        print("💾 POTENTIAL SPACE SAVINGS")
        print("="*80)
        print(f"Removing archived node_modules: {self.format_size(self.report['potential_savings'])}")
        print("\n✅ ANALYSIS COMPLETE")
        print("="*80)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Workspace Optimizer - Clean and organize your development workspace')
    parser.add_argument('--home', default='~/', help='Home directory path (default: ~/)')
    parser.add_argument('--min-video-size', type=int, default=500, help='Minimum video size in MB (default: 500)')
    parser.add_argument('--json', help='Save JSON report to specified file')
    parser.add_argument('--generate-script', help='Generate cleanup script to specified file')
