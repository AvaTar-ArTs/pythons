...
class ContentAwareAnalyzer:
    ...
    def classify_code_type(self, code, language):
        ...
        if "userscript" in code_lower or "tampermonkey" in code_lower:
            return "userscript"
        if "function" in code_lower and "javascript" in language.lower():
            return "javascript_function"
        if "class " in code_lower or "def " in code_lower:
            return "class_definition"
        if "import " in code_lower or "from " in code_lower:
            return "imports"
        if "html" in language.lower() or "<html" in code_lower:
            return "html_template"
        if "css" in language.lower() or "style" in code_lower:
            return "styling"
        if "json" in language.lower() or ("{" in code and "}" in code):
            return "configuration"
        if "bash" in language.lower() or "shell" in language.lower():
            return "shell_script"
        return "general_code"
    ...
    def generate_master_analysis_report(self):
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.writelines(
                ...
                for category, files in self.analysis_results[
                    "content_categories"
                ].items()
            )
    ...
    def run_complete_analysis(self):
        ...
        print("📋 Check MASTER_ANALYSIS_REPORT.md for overview")
