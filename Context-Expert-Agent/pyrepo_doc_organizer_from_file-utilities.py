...
<p style="color:#555;">Generated by <code>pyrepo_doc_organizer.py</code></p>
{''.join(items)}
</body></html>"""
        ...
        rows.append(
            {
                ...
                "docstring": (mi.docstring or "").replace("\n", "\\n")[:2000],
            }
        )

    csv_path = write_csv(rows, out_dir)
    print(f"[OK] Wrote CSV index: {csv_path}")

    if emit_html:
        html_path = write_html_from_csv(csv_path, out_dir)
        print(f"[OK] Wrote HTML index: {html_path}")
