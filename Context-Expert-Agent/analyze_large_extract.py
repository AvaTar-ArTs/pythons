...
def analyze_csv(filepath: Path) -> Dict[str, Any]:
    ...
    records = []
    field_stats = defaultdict(lambda: {'total': 0, 'filled': 0, 'empty': 0, 'examples': []})
...
def compare_with_json(csv_path: Path, json_path: Path) -> Dict[str, Any]:
    ...
    for field in json_field_stats:
        ...
        json_field_stats[field]['fill_rate'] = (filled / total * 100) if total > 0 else 0
...
def generate_report(filepath: Path) -> str:
    ...
    critical_fields = ['id', 'title', 'audio', 'href', 'lyrics', 'tags', 'duration', 'author', 'imageUrl']
    other_fields = [f for f in csv_data['field_stats'].keys() if f not in critical_fields]
    sorted_fields = [f for f in critical_fields if f in csv_data['field_stats']] + other_fields
    ...
    for field in sorted_fields:
        ...
        report_lines.append(f"- **Fill Rate**: {fill_rate:.1f}% ({stats['filled']}/{stats['total']})")
        ...
        if field in json_stats:
            ...
            if abs(fill_rate - json_fill) > 1:
                report_lines.append(f"- **⚠️ JSON Comparison**: CSV={fill_rate:.1f}%, JSON={json_fill:.1f}%")
    ...
    avg_critical = sum(critical_fill_rates) / len(critical_fill_rates) if critical_fill_rates else 0
    avg_valuable = sum(valuable_fill_rates) / len(valuable_fill_rates) if valuable_fill_rates else 0
    ...
    report_lines.append(f"- **Critical Fields** (ID, Title, Audio): {avg_critical:.1f}% average fill rate")
    report_lines.append(f"- **Valuable Fields** (Lyrics, Tags, Duration, Author): {avg_valuable:.1f}% average fill rate")
    ...
    if avg_valuable < 50:
        ...
        if missing_fields:
            report_lines.append(f"**Missing Data**: {', '.join(missing_fields)} are {avg_valuable:.0f}% empty")
            report_lines.append("")
            ...
            report_lines.append("3. Consider scraping individual song detail pages for missing fields")
            report_lines.append("")
