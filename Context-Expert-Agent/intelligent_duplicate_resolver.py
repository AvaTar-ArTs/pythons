...
class ContentAwareDuplicateResolver:
    ...
    def _extract_metadata(self, file: Path, content: str) -> Dict:
        ...
        if file.suffix == '.md':
            ...
            for pattern in date_patterns:
                ...
                if match:
                    metadata['date'] = match.group(1) if match.lastindex else match.group(0)
                    break
        ...
        updated_match = re.search(r'(last\s+updated|updated|modified)[:\s]+([^\n]+)', content, re.IGNORECASE)
        if updated_match:
    ...
    def _resolve_exact_duplicates(self, hash_groups: Dict) -> Dict:
        ...
        for hash_val, files in hash_groups.items():
            ...
            resolution = {
                ...
                'reason': f'Exact duplicate (same hash). Keeping most recent/complete version.',
                'hash': hash_val[:16] + '...'
            }
            ...
            print(f"     Reason: Most recent/complete")
    ...
    def _find_similar_content(self, file_infos: List[FileInfo], threshold: float = 0.8) -> List[List[FileInfo]]:
    ...
    def _calculate_similarity(self, info1: FileInfo, info2: FileInfo) -> float:
        ...
        if info1.metadata and info2.metadata:
            ...
            if common_keys:
                matching = sum(1 for k in common_keys 
                             if info1.metadata[k] == info2.metadata[k])
    ...
    def _resolve_similar_files(self, similar_groups: List[List[FileInfo]]) -> Dict:
        ...
        for group in similar_groups:
            ...
            similarities = []
            for alt in alternatives:
                sim = self._calculate_similarity(best, alt)
            ...
            resolution = {
                ...
                'avg_similarity': round(sum(sim for _, sim in similarities) / len(similarities) * 100, 1)
            }
    ...
    def _resolve_different_versions(self, file_infos: List[FileInfo]) -> Dict:
        ...
        # Determine best version
        best = max(file_infos, key=lambda f: (
            f.modified,  # Most recent
            f.size,  # Largest (most complete)
            len(f.metadata),  # Best metadata
            f.line_count  # Most lines
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Content-Aware Duplicate Resolver'
    )
