#!/usr/bin/env python3
"""
Debugging script to check UUID mappings and rename directories and files.
"""
...
def parse_suno_csv_for_mappings(csv_file_path):
    ...
    # Pattern to match the specific format from the grep results:
    # UUID,,,,title,,,,,,,,
    ...
    for line in lines:
        ...
        # Match UUID followed by 4 commas and then a title
    ...
    # Also look for the pattern where title appears in the second column
    # Pattern: UUID,title,,,,,,
    for line in lines:
        if not line.strip():
            continue

        # Match UUID followed immediately by a title
        pattern = r'([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}),([^,\n"]+?)(?:,|$)'
        matches = re.findall(pattern, line)
...
def debug_uuid_directories_and_files(root_dir, uuid_mapping):
    """
    Debug: Check directories and files with UUID names to see if they match mappings.
    """
    print("Checking directories and files for UUID matches...")
                ...
                if re.match(uuid_pattern, file.lower()):
                    ...
                    if uuid_match:
                        ...
                        if uuid_part in uuid_mapping:
                            print(f"  -> Found mapping: {uuid_mapping[uuid_part]}")
                        else:
                            print(f"  -> No mapping found for: {uuid_part}")
...
def main():
    ...
    # Debug: Check what UUIDs exist and if they match
    print("\nDebug: Checking existing UUID directories and files...")
    debug_uuid_directories_and_files(music_root_dir, uuid_mapping)
