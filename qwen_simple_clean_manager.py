#!/usr/bin/env python3
"""
Simple Clean Script to extract Qwen conversations without duplicates
"""
...
class SimpleCleanQwenManager:
    def __init__(self):
        self.base_dir = Path.home() / ".qwen" / "projects"
        self.db_path = Path.home() / ".simple_clean_qwen_conversations.db"
        self.init_db()
    ...
    def store_in_database(self, conversation_data, session_id, project_name):
        """Store conversation data in the SQLite database, avoiding duplicates."""
    ...
    def generate_simple_clean_html_report(self):
        """Generate a simple clean HTML report focusing only on unique conversations."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
...
<head>
    ...
    <title>Qwen Conversations - Simple Clean Report</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Qwen Conversations - Simple Clean Report</h1>
        <div class="summary">
            <p><strong>Total Sessions:</strong> ''' + str(len(sessions)) + '''</p>
            <p><strong>Generated:</strong> ''' + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '''</p>
            <p><em>This report contains only unique conversations with duplicates removed.</em></p>
        </div>'''
        ...
        <div class="session">
            <div class="session-header">
                Session: {session_id[:12]}... | Project: {project_name} | {msg_count} unique messages
            </div>'''
    ...
    def run_extraction(self):
        """Main function to extract Qwen conversations and generate clean HTML report."""
            ...
            for jsonl_file in jsonl_files:
                ...
                try:
                    ...
                    if conversation_data:
                        ...
                        # Store in database (duplicates automatically handled by UNIQUE constraint)
                        unique_count = self.store_in_database(conversation_data, session_id, project_name)
                        total_unique_messages += unique_count
                        
                except Exception as e:
                    print(f"Error processing {jsonl_file}: {str(e)}")
        
        # Generate the simple clean HTML report
        html_report = self.generate_simple_clean_html_report()
        
        # Write the HTML report to a file
        output_file = Path.home() / "qwen_conversations_simple_clean.html"
        
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(html_report)
        
        print(f"\nSimple clean HTML export completed! Conversations saved to: {output_file}")
...
def main():
    manager = SimpleCleanQwenManager()
