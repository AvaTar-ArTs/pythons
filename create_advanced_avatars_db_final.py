...
def create_advanced_summary_views(conn):
    ...
    # Project summary view - Fixed to handle NULL values properly
    cursor.execute('''
    ...
    # File category distribution - Fixed to handle NULL values properly
    cursor.execute('''
        ...
        SELECT 
            ...
            COUNT(f.id) as file_count,
            COALESCE(SUM(f.size_bytes), 0) as total_size,
            COALESCE(AVG(f.size_bytes), 0) as avg_size
...
def main():
    ...
    # Show project summary - Fixed to handle NULL values
    print(f"\nProject Summary:")
    cursor.execute("""
    ...
    for row in cursor.fetchall():
        size_mb = (row[2] or 0) / (1024*1024)  # Handle NULL size
        print(f"  {row[0]}: {row[1]:,} files, {size_mb:.2f} MB, ${row[3]:,.0f} potential")
    
    # Show category distribution - Fixed to handle NULL values
    ...
    for row in cursor.fetchall()[:10]:  # Top 10
        size_mb = (row[2] or 0) / (1024*1024)  # Handle NULL size
        print(f"  {row[0]}: {row[1]:,} files, {size_mb:.2f} MB")
