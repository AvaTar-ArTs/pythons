...
class MassiveTextAnalyzer:
        ...
        for filename, info in self.text_files.items():
            ...
            if stats:
                ...
                for type_name, count in sorted(
                    ...
                    if count > 0:
                        pct = (
                            (count / stats["file_paths"] * 100)
                            if stats["file_paths"] > 0
                            else 0
                        )
    ...
    def save_consolidated_analysis(self, output_dir=None):
        ...
        output_file = (
            Path(output_dir)
            / f"text_inventories_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def extract_md_html_files(self):
        ...
        with open(csv_file, "w", newline="") as f:
            writer = csv.DictWriter(
                f, fieldnames=["path", "extension", "source_file", "name"]
            )
            writer.writeheader()
...
def main():
    ...
    analyzer.extract_md_html_files()
