...
def is_user_web_file(file_path):
    ...
    if (file_name.startswith('.') or
        file_name.endswith('.map') or
        file_name.endswith('.lock') or
        file_name.endswith('.sample') or
        '.git' in str(file_path) or
...
def analyze_user_python_content(file_path):
    ...
    ai_keywords = [
        ...
        'youtube', 'video', 'audio', 'channel', 'automation'
    ]
    ...
    tool_keywords = [
        ...
        'file', 'organize', 'manage', 'helper', 'util', 'convert'
    ]
...
def clean_user_files():
    ...
    current_folders = [
        "Python_AI", "Python_General", "Python_Tools", "Python_Web", "Python_Utils",
        "HTML_Files", "CSS_Files", "JavaScript_Files", "Web_HTML", "Web_CSS", "Web_JS", "Web_Projects",
        "Assets_Docs", "Assets_Images", "Documentation", "Config_Files", "Images", "Media_Files", "Archives",
        "Scripts_All", "Shell_Scripts", "Compiled_Python", "Cython_Files", "Binaries_Libraries",
        "Node_Modules", "Git_Files", "Other_Files"
    ]
...
def process_user_folder(source_folder, base_dir):
    ...
    print(f"  ✅ Moved {user_count} user files, {system_count} system files")

if __name__ == "__main__":
    print("🚀 Starting clean user file organization...")
    print("📊 Separating user-created files from system files...")
    clean_user_files()
    print("\n✅ Clean user file organization complete!")
    print("🗑️ System files moved to System_Files/ for cleanup")