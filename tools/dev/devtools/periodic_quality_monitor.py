...
class PeriodicQualityMonitor:
    ...
    def _check_alerts(self, metrics: QualityMetrics) -> None:
        ...
        docstring_coverage = (
            (metrics.files_with_docstrings / metrics.total_files * 100)
        ...
        type_hint_coverage = (
            (metrics.files_with_type_hints / metrics.total_files * 100)
            if metrics.total_files > 0
            else 0
        )
        ...
        error_handling_coverage = (
            (metrics.files_with_error_handling / metrics.total_files * 100)
            if metrics.total_files > 0
            else 0
        )
        ...
        logging_coverage = (
            (metrics.files_with_logging / metrics.total_files * 100)
            if metrics.total_files > 0
            else 0
        )
    ...
    def _generate_report(self, metrics: QualityMetrics) -> None:
        ...
        with open(report_file, "w") as f:
            ...
            f.write(
                f"- **Maintainability Score:** {metrics.maintainability_score:.1f}/100\n"
            )
            f.write(
                f"- **Performance Potential:** {metrics.performance_potential:.1f}/100\n\n"
            )
