...
class AlbumSorter:
    def __init__(self, root_dir, dry_run=True):
        ...
        if not self.root_dir.exists():
            logger.error(f"{Colors.RED}Error: Directory not found: {self.root_dir}{Colors.END}")
            sys.exit(1)
    ...
    def load_csv_metadata(self):
        ...
        if not METADATA_CSV_PATH.exists():
            logger.warning(f"{Colors.YELLOW}Warning: Metadata CSV not found at {METADATA_CSV_PATH}{Colors.END}")
            return meta_map
        ...
        try:
            with open(METADATA_CSV_PATH, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    if path:
                        meta_map[path] = row
            logger.info(f"{Colors.CYAN}Loaded metadata for {len(meta_map)} items from CSV.{Colors.END}")
            ...
            for stem in [base_name, clean_stem]:
                ...
                if txt_path.exists():
                    try:
                        ...
                        if title_match and meta['title'] == base_name: # Only override if generic
    ...
    def organize_library(self):
        """Main execution loop"""
        logger.info(f"{Colors.BOLD}Starting Album Sorting & Organization...{Colors.END}")
        ...
        for file_path in self.root_dir.rglob('*.mp3'):
            ...
            if safe_artist == 'Unknown_Artist': safe_artist = 'Misc'
    ...
    def move_file(self, src, dst):
        ...
        try:
            if self.dry_run:
                logger.info(f"  {Colors.YELLOW}[DRY RUN] Would move:{Colors.END} {src.name}")
                logger.info(f"       -> {dst.relative_to(self.root_dir)}")
            else:
                dst.parent.mkdir(parents=True, exist_ok=True)
                shutil.move(str(src), str(dst))
                logger.info(f"  {Colors.GREEN}Moved:{Colors.END} {src.name} -> {dst.name}")
    ...
    def move_associated_files(self, audio_src, audio_dst):
        ...
        for sibling in audio_src.parent.glob(f"{re.escape(base_name)}*"):
            if sibling == audio_src: continue 
            if sibling.suffix.lower() == '.mp3': continue # Don't move other mp3s, let the loop handle them
            ...
            if sibling.resolve() != sibling_dst.resolve():
                if self.dry_run:
                    logger.info(f"    {Colors.YELLOW}[DRY RUN] Would move sibling:{Colors.END} {sibling.name}")
                else:
                    try:
                        shutil.move(str(sibling), str(sibling_dst))
                        logger.info(f"    {Colors.GREEN}Moved sibling:{Colors.END} {sibling.name}")
                    except Exception as e:
                        logger.error(f"    {Colors.RED}Error moving sibling: {e}{Colors.END}")

def main():
    parser = argparse.ArgumentParser(description="Organize Music Library by Artist/Album")
    parser.add_argument("--live", action="store_true", help="Execute changes (default is dry-run)")
    parser.add_argument("--dir", type=str, default="/Users/steven/Music/nocTurneMeLoDieS", help="Root directory to process")
    
    args = parser.parse_args()
    
    if not HAS_MUTAGEN:
        logger.warning(f"{Colors.YELLOW}Warning: 'mutagen' library not found. ID3 tag parsing will be disabled.{Colors.END}")
