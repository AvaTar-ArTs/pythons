...
class SmartConsolidator:
    def __init__(self, pythons_dir="/Users/steven/pythons"):
        ...
        # Find latest report
        reports = sorted(self.pythons_dir.glob('VERSION_ANALYSIS_REPORT_*.csv'), reverse=True)
        if reports:
    ...
    def load_and_categorize(self):
        ...
        with open(self.report_file, 'r') as f:
            ...
            for row in reader:
                ...
                root_comp = float(row['Root Complexity'])
                org_comp = float(row['Org Complexity'])
                ...
                if root_lines > org_lines * 1.5 and root_lines > 50:
                    action['reason'] = f"Root is expanded: {root_lines} lines vs {org_lines}"
                ...
                # 2. Organized is significantly expanded
                    ...
                    action['reason'] = f"Organized is better: {org_lines} lines vs {root_lines}"
                    self.actions['delete_root'].append(action)

                # 3. Minor differences (< 10 lines diff)
                elif abs(root_lines - org_lines) < 10:
                    action['reason'] = f"Minor edit: {abs(root_lines - org_lines)} line difference"
                    self.actions['archive_root'].append(action)

                # 4. Moderate changes - need review
                else:
                    action['reason'] = f"Moderate change: Root {root_lines} vs Org {org_lines}"
    ...
    def execute_consolidation(self, dry_run=True):
        ...
        if dry_run:
            ...
            archive_dir = self.pythons_dir / '_archive' / f'version-consolidation-{timestamp}'
        ...
        for action in self.actions['replace_with_root']:
            ...
            print(f"     Root: {action['root_lines']} lines → {action['parent_folder']}/")
            print(f"     Replacing organized version...")
        ...
        for action in self.actions['archive_root']:
            print(f"  📦 {action['filename']}")
            print(f"     Minor difference: {abs(action['root_lines'] - action['org_lines'])} lines")
...
""")
    ...
    if '--execute' in sys.argv:
        ...
        if confirm == 'CONSOLIDATE':
            stats = consolidator.execute_consolidation(dry_run=False)
