...
class UltimateReorganizer:
    ...
    def create_backup(self):
        ...
        backup_dir = self.base_path / 'archive' / 'backups' / f'before_ultimate_{timestamp}'
    ...
    def execute_hierarchical(self, dry_run: bool = True):
        ...
        for parent, data in sorted(hierarchy.items(), key=lambda x: x[1]['total_files'], reverse=True):
        ...
        if dry_run:
            ...
            total_subs = sum(len(d.get('subcategories', {})) for d in hierarchy.values())
            if total_subs:
    ...
    def show_summary(self):
        ...
        for i, (cat, data) in enumerate(
            sorted(hierarchy.items(), key=lambda x: x[1]['total_files'], reverse=True)[:10], 1
        ):
            conf = data.get('confidence', 0)
            subs = len(data.get('subcategories', {}))
            sub_text = f" ({subs} subcategories)" if subs else ""
            print(f"  {i:2d}. {cat:30s} {data['total_files']:4d} files "
...
def main():
    ...
    parser.add_argument(
        '--flat',
        action='store_true',
        help='Flat structure (no subcategories)'
    )
