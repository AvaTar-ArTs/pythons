...
STDLIB = {
    'json', 'random', 'datetime', 'collections', 'os', 'sys', 'pathlib',
    're', 'subprocess', 'shutil', 'glob', 'itertools', 'functools',
    'operator', 'copy', 'pickle', 'csv', 'io', 'string', 'textwrap',
    'argparse', 'logging', 'time', 'threading', 'multiprocessing',
    'urllib', 'http', 'socket', 'ssl', 'hashlib', 'base64', 'zlib',
    'gzip', 'tarfile', 'zipfile', 'tempfile', 'fileinput', 'linecache',
    'pprint', 'reprlib', 'enum', 'dataclasses', 'typing', 'abc',
    'contextlib', 'weakref', 'gc', 'inspect', 'traceback', 'warnings',
    'unittest', 'doctest', 'pdb', 'profile', 'cProfile', 'trace',
    '__future__', '__main__', 'builtins', 'importlib', 'pkgutil',
    'site', 'sysconfig', 'platform', 'errno', 'signal', 'atexit',
    'math', 'cmath', 'decimal', 'fractions', 'statistics', 'array',
    'struct', 'codecs', 'unicodedata', 'stringprep', 'readline',
    'rlcompleter', 'getpass', 'curses', 'termios', 'tty', 'pty',
    'pwd', 'spwd', 'grp', 'crypt', 'termios', 'tty', 'select',
    'selectors', 'asyncio', 'concurrent', 'queue', 'sched', 'socketserver',
    'xml', 'html', 'email', 'mimetypes', 'base64', 'binascii', 'quopri',
    'uu', 'binhex', 'plistlib', 'configparser', 'netrc', 'xdrlib',
    'mailcap', 'mailbox', 'mmap', 'dbm', 'sqlite3', 'zlib', 'gzip',
    'bz2', 'lzma', 'zipfile', 'tarfile', 'shutil', 'filecmp', 'stat',
    'fileinput', 'linecache', 'tempfile', 'glob', 'fnmatch', 'pathlib',
    'os.path', 'shutil', 'pickle', 'copy', 'marshal', 'dbm', 'sqlite3',
    'zlib', 'gzip', 'bz2', 'lzma', 'zipfile', 'tarfile', 'csv', 'configparser',
    'netrc', 'xdrlib', 'plistlib', 'logging', 'getopt', 'argparse',
    'shlex', 'configparser', 'logging', 'getpass', 'curses', 'platform',
    'errno', 'io', 'codecs', 'unicodedata', 'stringprep', 'readline',
    'rlcompleter', 'getpass', 'curses', 'termios', 'tty', 'pty', 'pwd',
    'spwd', 'grp', 'crypt', 'termios', 'tty', 'select', 'selectors',
    'asyncio', 'concurrent', 'queue', 'sched', 'socketserver', 'xml',
    'html', 'email', 'mimetypes', 'base64', 'binascii', 'quopri', 'uu',
    'binhex', 'plistlib', 'configparser', 'netrc', 'xdrlib', 'mailcap',
    'mailbox', 'mmap', 'dbm', 'sqlite3', 'zlib', 'gzip', 'bz2', 'lzma',
    'zipfile', 'tarfile', 'shutil', 'filecmp', 'stat', 'fileinput',
    'linecache', 'tempfile', 'glob', 'fnmatch', 'pathlib', 'os.path'
}
...
def generate_environment_yml(modules, env_name, output_path):
    ...
    for module in third_party:
        ...
        if conda_name in ['numpy', 'pandas', 'matplotlib', 'scipy', 'scikit-learn',
                         'jupyter', 'ipython', 'notebook', 'pytest', 'requests',
                         'flask', 'django', 'tensorflow', 'pytorch', 'opencv',
                         'pillow', 'pyyaml', 'beautifulsoup4', 'lxml', 'selenium']:
...
def quick_scan(directory, max_files=1000):
    ...
    for py_file in directory.rglob('*.py'):
        if any(x in str(py_file) for x in ['venv', '__pycache__', '.git', 'site-packages']):
            continue
        if count >= max_files:
            break
        try:
            with open(py_file, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    if line.startswith('import ') or line.startswith('from '):
                        match = re.match(r'(?:import\s+(\S+)|from\s+(\S+)\s+import)', line)
                        if match:
                            module = (match.group(1) or match.group(2)).split('.')[0].split(' as ')[0]
                            if module and not module.startswith('_') and module not in STDLIB:
                                imports.add(module)
        except:
            pass
        count += 1
...
print(f"  - pythons_environment.yml")
print(f"  - pythons_sort_environment.yml")
print(f"  - combined_environment.yml")
print(f"  - mamba_analysis_report.md")
