...
CSV_FILE = Path(
    "/Users/steven/Documents/Archives/duplicate_analysis/duplicates_report.csv"
)
OUTPUT_SUMMARY = Path(
    "/Users/steven/Documents/Archives/duplicate_analysis/actionable_summary.csv"
)
OUTPUT_DETAILED = Path(
    "/Users/steven/Documents/Archives/duplicate_analysis/important_duplicates.csv"
)
...
def analyze_duplicates():
    ...
    for group_id, rows in important_groups:
        ...
        rows_sorted = sorted(
            rows,
            key=lambda x: (x["action"] == "REMOVE", float(x["size_mb"])),
            reverse=True,
        )
            ...
            actionable_rows.append(
                {
                    ...
                    "remove_files": " | ".join(
                        [r["file_name"] for r in remove_files[:3]]
    ...
    # Sort by size
    actionable_rows_sorted = sorted(
        actionable_rows, key=lambda x: float(x["remove_size_mb"]), reverse=True
    )
    for i, row in enumerate(actionable_rows_sorted[:10], 1):
        print(
            f"  {i}. {row['keep_name']} - {row['remove_size_mb']} MB ({row['remove_count']} duplicates)"
        )
