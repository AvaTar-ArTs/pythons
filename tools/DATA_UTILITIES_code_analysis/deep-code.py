...
"""
            ...
            logger.info(
                f"{Colors.YELLOW}{Emojis.WARN} AI analysis failed: {e}{Colors.END}",
            )
            return self._fallback_analysis(code_snippet, filepath)
...
class VectorEmbeddingAnalyzer:
    ...
    def get_code_embedding(self, code: str, identifier: str) -> list[float] | None:
        ...
        try:
            ...
            logger.info(
                f"{Colors.YELLOW}{Emojis.WARN} Embedding failed: {e}{Colors.END}",
            )
            return None
...
class AIDeepIntelligentAnalyzer:
    ...
    def extract_python_metadata(self, filepath: Path) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                metadata["loc"] > 10 and metadata["loc"] < CONSTANT_1000
            ):  # Reasonable size
    ...
    def scan_directory(self, directory: str, current_depth: int = 0):
        ...
        try:
            ...
            if not dir_path.exists():
                logger.info(
                    f"{Colors.RED}{Emojis.ERROR} Not found: {directory}{Colors.END}",
                )
                return
            ...
            logger.info(
                f"{Colors.RED}{Emojis.ERROR} Error: {directory}: {e}{Colors.END}",
            )
    ...
    def create_detailed_report(self, exact_dupes: dict, semantic_dupes: dict):
        ...
        report_file = (
            fstr(Path.home()) + "/Documents/python/AI_DEEP_ANALYSIS_{timestamp}.md"
        )
        ...
        with open(report_file, "w", encoding="utf-8") as report:
            ...
            report.write(
                f"| **Total Size** | {self.stats['total_size'] / (CONSTANT_1024**3):.2f} GB |\n",
            )
    ...
    def run_complete_analysis(self):
        ...
        logger.info(
            f"  💾 Size: {Colors.CYAN}{self.stats['total_size'] / (CONSTANT_1024**3):.2f} GB{Colors.END}\n",
        )
