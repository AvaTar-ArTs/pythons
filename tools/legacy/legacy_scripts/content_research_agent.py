...
class ContentResearchAgent:
    ...
    def __init__(self):
        self.base_path = Path("/Users/steven/Documents")
    ...
    def _load_knowledge_base(self) -> Dict:
        ...
        kb_file = Path("/Users/steven/content_research_knowledge.json")
        if kb_file.exists():
    ...
    def _load_pattern_database(self) -> Dict:
        ...
        pattern_file = Path("/Users/steven/content_patterns.json")
    ...
    def _load_content_index(self) -> Dict:
        ...
        index_file = Path("/Users/steven/content_index.json")
...
def main():
    ...
    print("🔍 Content Research Agent - Intelligent Content Analysis")
    print("=" * 60)
    
    agent = ContentResearchAgent()
    
    # Analyze content libraries
    print("📚 Analyzing content libraries...")
    ...
    print("-" * 30)
    for content_type, analyses in results.items():
        print(f"{content_type.upper()}: {len(analyses)} files analyzed")
        total_patterns = sum(len(a.patterns_found) for a in analyses)
        print(f"  - {total_patterns} patterns found")
        ...
        print(f"  - Average prompt potential: {sum(a.prompt_potential for a in analyses) / len(analyses) if analyses else 0:.2f}/10")
    ...
    for category, framework_data in frameworks.items():
        if framework_data:
            print(f"  - {category}: {len(framework_data.get('alternative_frameworks', []))} alternatives")
    
    # Save results
    output_file = Path("/Users/steven/content_research_results.json")
    ...
    print("✅ Content research complete!")
