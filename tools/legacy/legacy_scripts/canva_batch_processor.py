...
class CanvaBatchProcessor:
    def __init__(self, source_dir, output_dir, temp_dir=None, batch_size=3):
        ...
        self.temp_dir = (
            Path(temp_dir) if temp_dir else Path("/tmp/canva_batch_processing")
        )
        ...
        # File tracking
        self.global_file_hashes = defaultdict(
            list
        )  # hash -> [(archive, file_path, size)]
    ...
    def get_archive_batches(self):
        ...
        logger.info(
            f"📦 Split {len(all_archives)} archives into {len(batches)} batches of {self.batch_size}"
        )
        return batches
    ...
    def process_single_archive(self, archive_path):
        ...
        try:
            with zipfile.ZipFile(archive_path, "r") as zip_file:
                ...
                logger.info(
                    f"✅ Processed {archive_path.name}: {archive_info['file_count']} files"
                )
    ...
    def find_duplicates_in_batch(self, batch_results):
        ...
        logger.info("🔍 Finding duplicates in batch...")
    ...
    def create_deduplicated_archives_batch(self, batch_results, batch_duplicates):
        ...
        logger.info("📦 Creating deduplicated archives for batch...")
        ...
        for result in batch_results:
            ...
            logger.info(
                f"📦 Deduplicating {archive_name}: removing {len(files_to_skip)} files"
            )
            ...
            try:
                ...
                logger.error(
                    f"❌ Error creating deduplicated archive {archive_name}: {e}"
                )

    def compress_archives_batch(self, batch_results):
        """Compress archives in this batch using 7zip"""
        logger.info("🗜️ Compressing archives in batch...")
        ...
        for result in batch_results:
            ...
            try:
                ...
                compressed_archive = (
                    self.temp_dir / f"compressed_{archive_name.replace('.zip', '.7z')}"
                )
                cmd = [
                    self.seven_zip_cmd,
                    "a",
                ...
                if result_cmd.returncode == 0:
                    ...
                    logger.error(
                        f"7zip compression failed for {archive_name}: {result_cmd.stderr}"
                    )
    ...
    def organize_archives_batch(self, batch_results):
        ...
        logger.info("📁 Organizing archives in batch...")
    ...
    def generate_batch_report(self, batch_num, batch_results, batch_duplicates):
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                f"- **Successful Archives:** {sum(1 for r in batch_results if r['success'])}\n"
            )
            f.write(
                f"- **Total Files:** {sum(r['file_count'] for r in batch_results if r['success'])}\n"
            )
            ...
            for result in batch_results:
                ...
                if result["success"]:
                    ...
                    f.write(
                        f"- **Size:** {result['total_size'] / (1024 * 1024):.1f} MB\n"
                    )
                else:
    ...
    def process_all_batches(self):
        ...
        try:
            for batch_num, batch_archives in enumerate(batches, 1):
                ...
                # Update global stats
                self.stats["duplicate_files"] += sum(
                    len(dup["duplicates"]) for dup in batch_duplicates.values()
                )
            ...
            logger.info(
                f"💾 Total space saved: {self.stats['space_saved'] / (1024 * 1024):.1f} MB"
            )
    ...
    def generate_final_report(self, all_duplicates):
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                f"- **Total Duplicate Files Found:** {self.stats['duplicate_files']}\n"
            )
            f.write(
                f"- **Total Space Saved:** {self.stats['space_saved'] / (1024 * 1024):.1f} MB\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--temp",
        default="/tmp/canva_batch_processing",
    ...
    if args.dry_run:
        ...
        processor = CanvaBatchProcessor(
            args.source, args.output, args.temp, args.batch_size
        )
        batches = processor.get_archive_batches()
        logger.info(
            f"Would process {len(batches)} batches of {args.batch_size} archives each"
        )
        return
