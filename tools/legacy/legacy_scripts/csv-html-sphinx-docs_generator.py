                ...
                for row in reader:
                    ...
                    if timestamp:
                        try:
                            ...
                            if (
                                not conv["date_range"]["first"]
                                or date < conv["date_range"]["first"]
                            ):
                                conv["date_range"]["first"] = date
                            if (
                                not conv["date_range"]["last"]
                                or date > conv["date_range"]["last"]
                            ):
...
"""
    ...
    def _generate_topic_pages(self):
        ...
        for topic, conversations in self.topics.items():
            ...
            with open(
                self.output_dir / "topics" / f"{topic_file}.html", "w", encoding="utf-8"
            ) as f:
                f.write(html_content)
    ...
    def _generate_conversation_pages(self):
        ...
        for conv in self.conversations:
            ...
            with open(
                self.output_dir / "conversations" / conv_file, "w", encoding="utf-8"
            ) as f:
                f.write(html_content)
    ...
    def _generate_conversation_list(self, conversations: List[Dict]) -> str:
        ...
        for conv in conversations:
            date_str = (
                conv["date_range"]["last"].strftime("%Y-%m-%d")
                if conv["date_range"].get("last")
                else "Unknown"
            )
            conv_file = conv["filename"].replace(".csv", ".html")
    ...
    def _generate_sitemap(self):
        ...
        sitemap = {
            ...
            "search": "search.html",
            "topics": {
                topic: f"topics/{topic.lower().replace(' ', '_')}.html"
                for topic in self.topics.keys()
            },
            "conversations": {
                conv[
                    "title"
                ]: f"conversations/{conv['filename'].replace('.csv', '.html')}"
                for conv in self.conversations
            },
        }
    ...
    def _get_base_html(self, title: str, content: str) -> str:
        ...
        for topic in sorted(self.topics.keys())[:10]:
            nav_items.append(
                (f"topics/{topic.lower().replace(' ', '_')}.html", topic.title())
            )

        nav_html = "\n".join(
            f'<li><a href="{url}">{label}</a></li>' for url, label in nav_items
        )
