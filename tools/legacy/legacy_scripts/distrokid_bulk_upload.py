...
def generate_metadata(track):
    ...
    album = (
        f"{track['directory']} Collection"
        if track["directory"] != "Music"
        else f"Singles {datetime.now().year}"
    )
...
def create_upload_csv(tracks, output_path=None):
    ...
    if output_path is None:
        output_path = (
            OUTPUT_DIR
            / f"distrokid_upload_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
        )
...
def create_release_checklist(tracks, output_path=None):
    ...
    if output_path is None:
        output_path = (
            OUTPUT_DIR
            / f"distrokid_checklist_{datetime.now().strftime('%Y%m%d_%H%M')}.md"
        )
    ...
    for genre, items in sorted(by_genre.items()):
        ...
        for track, meta in items:
            ...
            content.append("- [ ] Uploaded to DistroKid\n")
            content.append("- [ ] Artwork attached\n")
            content.append("- [ ] Metadata verified\n")
            content.append("- [ ] Pre-save link created\n")
            content.append("- [ ] Social posts scheduled\n\n")
    ...
    content.extend(
        [
            ...
            "## 📊 Expected Revenue\n\n",
            f"- **Tracks:** {len(tracks)}\n",
            "- **Per Track (conservative):** $5-20/month after 6 months\n",
    ...
    for genre in sorted(by_genre.keys()):
        ...
        content.append("- Submit to user-curated playlists via SubmitHub\n")
...
def create_marketing_plan(tracks, output_path=None):
    ...
    if output_path is None:
        output_path = (
            OUTPUT_DIR
            / f"distrokid_marketing_{datetime.now().strftime('%Y%m%d_%H%M')}.md"
        )
