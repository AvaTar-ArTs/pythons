...
class FinalCategorizer:
    ...
    def deep_analyze_file(self, filepath):
        ...
        try:
            ...
            try:
                ...
                if any(
                    x in all_text for x in ["convert", "transform", "change", "modify"]
                ):
                    signals["converters"] += 10

                if any(
                    x in all_text for x in ["organize", "sort", "categorize", "arrange"]
                ):
                ...
                if any(
                    x in all_text for x in ["csv", "excel", "spreadsheet", "dataframe"]
                ):
                ...
                if any(
                    x in all_text for x in ["image", "photo", "picture", "thumbnail"]
                ):
    ...
    def recategorize_general_folders(self):
        ...
        for general_path in general_folders:
            ...
            for cat in sorted(
                new_categories.keys(),
                key=lambda x: len(new_categories[x]),
                reverse=True,
            )[:15]:
