...
def render_adaptive(:
    ...
    for idx, (title, body) in enumerate(chapters):
        ...
        final = (
            normalize_audio(combined, TARGET_DBFS_DEFAULT)
            .fade_in(CONSTANT_300)
            .fade_out(CONSTANT_500)
        )
...
def render_asmr(:
    ...
    seg = (
        normalize_audio(seg, target_dbfs=-16.0)
        .fade_in(CONSTANT_1500)
        .fade_out(CONSTANT_1500)
    )
...
def render_cinematic(:
    ...
    for idx, (title, body) in enumerate(chapters):
        ...
        if out_file.exists():
            logger.info(f"✅ Skipping (exists): {out_file.name}")
            master += AudioSegment.from_mp3(out_file) + AudioSegment.silent(
                CONSTANT_1000
            )
            continue
        ...
        combined = (
            normalize_audio(combined, TARGET_DBFS_DEFAULT)
            .fade_in(CONSTANT_1200)
            .fade_out(CONSTANT_1200)
        )
    ...
    master = (
        normalize_audio(master, TARGET_DBFS_DEFAULT)
        .fade_in(CONSTANT_1600)
        .fade_out(CONSTANT_1800)
    )
