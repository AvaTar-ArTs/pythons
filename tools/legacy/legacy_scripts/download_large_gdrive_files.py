...
def download_files(:
    remote: str, files: List[Dict], dest_dir: Path, dry_run: bool = False
):
    """
    ...
    # Confirm download
    response = input(
        f"Download {len(files)} files ({human_readable_size(total_size)})? [y/N]: "
    )
...
Examples:
  ...
  %(prog)s --remote "gdrive:My Drive/Videos"
    ...
    parser.add_argument(
        "--remote", default="gdrive:", help="rclone remote path (default: gdrive:)"
    )
