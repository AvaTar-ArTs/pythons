...
class AIWebhookProcessor:
    ...
    def __init__(self):
        self.base_path = Path(
            "/Users/steven/Documents/python/LLM_APPLICATIONS/content_generation"
        )

    def process_content_generation(self, data):
        """Process content generation request"""
        try:
            data.get("content_type", "blog_post")
            ...
            script_path = (
                self.base_path
                / "AI_CONTENT_GENERATION_AI_CONTENT_AI_CONTENT_content_creation_agent.py"
            )
            ...
            thread = threading.Thread(
                target=self._run_content_generation, args=(process_id, args, data)
            )
            thread.start()
    ...
    def process_voice_synthesis(self, data):
        ...
        try:
            ...
            script_path = (
                self.base_path
                / "AI_CONTENT_GENERATION_AI_CONTENT_AI_CONTENT_voice_synthesis_scripty_tts_unified_v2.py"
            )
            ...
            thread = threading.Thread(
                target=self._run_voice_synthesis, args=(process_id, args, data)
            )
            thread.start()
    ...
    def _run_content_generation(self, process_id, args, data):
        ...
        try:
            result = subprocess.run(
                args, capture_output=True, text=True, cwd=self.base_path
            )
    ...
    return jsonify(
        {
            ...
            "active_processes": len(
                [p for p in processing_status.values() if p["status"] == "processing"]
            ),
        }
    )
