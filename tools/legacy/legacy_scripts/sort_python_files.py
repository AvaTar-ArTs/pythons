...
class PythonFileSorter:
    ...
    def analyze_file_content(self, file_path: Path) -> Dict[str, any]:
        ...
        if category_scores:
            ...
            confidence = (
                category_scores[best_category] / sum(category_scores.values())
    ...
    def classify_file(self, file_path: Path) -> str:
        ...
        logger.info(
            f"Content analysis: {filename} -> {category} (confidence: {confidence:.2f})"
        )
        ...
        for file_path in python_files:
            try:
                ...
                if any(
                    ...
                    for part in ["sora-video-generator", "suno-analytics"]
                ):
                    continue
    ...
    def create_category_readmes(self, sorted_files: Dict[str, List[Dict]]):
        ...
        for category, files in sorted_files.items():
            ...
            with open(readme_path, "w") as f:
                f.write(f"# {self.categories[category]}\n\n")
                f.write(
                    f"This directory contains {len(files)} Python files related to {self.categories[category].lower()}.\n\n"
                )
...
def main():
    ...
    print(f"📁 Sorted files are in: {sorter.sorted_dir}")
    print("📊 Check SORTING_REPORT.md for detailed information")
