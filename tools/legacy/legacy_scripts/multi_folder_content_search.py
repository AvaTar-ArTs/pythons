...
class MultiFolderContentSearch:
    ...
    def search_patterns(self):
        ...
        print(
            f"✅ Found content patterns in {len(list(self.base_dir.rglob('*.py')))} Python files"
        )
    ...
    def search_functions(self):
        ...
        common_functions = {
            name: files
            for name, files in self.function_patterns.items()
            if len(files) > 1
        }

        print(
            f"✅ Found {len(common_functions)} functions that appear in multiple files"
        )
            ...
            if item.is_dir() and not item.name.startswith("."):
                ...
                if py_files:
                    ...
                    folder_content = {
                        ...
                        "files": [
                            str(f.relative_to(self.base_dir)) for f in py_files[:10]
                        ],  # First 10 files
                    ...
                    for py_file in py_files:
                        try:
                            ...
                            if re.search(
                                r"load_env_d|\.env\.d|load_dotenv",
                                content,
                                re.IGNORECASE,
                            ):
                                folder_content["pattern_counts"]["env_loading"] += 1
                            if re.search(
                                r"duplicate|hashlib|md5|similarity",
                                content,
                                re.IGNORECASE,
                            ):
                                folder_content["pattern_counts"]["deduplication"] += 1
                            if re.search(
                                r"analyze|analysis|report", content, re.IGNORECASE
                            ):
                                folder_content["pattern_counts"]["analysis"] += 1
                            if re.search(
                                r"pandas|DataFrame|csv", content, re.IGNORECASE
                            ):
                                folder_content["pattern_counts"]["data_processing"] += 1
                            if re.search(
                                r"openai|anthropic|gemini|api_key",
                                content,
                                re.IGNORECASE,
                            ):
    ...
    def find_cross_directory_functionality(self):
        ...
        for py_file in all_py_files:
            ...
            try:
                ...
                if re.search(
                    r"hashlib|md5|duplicate|similarity|compare", content, re.IGNORECASE
                ):
                    cross_dir_patterns["deduplication"].append(file_path)
                if re.search(
                    r"analyze|analysis|report|metrics|summary", content, re.IGNORECASE
                ):
                    cross_dir_patterns["analysis"].append(file_path)
                if re.search(
                    r"os\.path|pathlib|shutil|open\(|\.read\(|\.write\(",
                    content,
                    re.IGNORECASE,
                ):
                    cross_dir_patterns["file_operations"].append(file_path)
                if re.search(
                    r"openai|anthropic|gemini|api_key|load_env_d",
                    content,
                    re.IGNORECASE,
                ):
        ...
        print("✅ Cross-directory functionality mapping complete")
    ...
    def generate_report(self):
        ...
        with open(report_file, "w") as f:
            f.write("# Multi-Folder Content Analysis Report\n\n")
            f.write(
            ...
            common_functions = {
                ...
                for name, files in self.function_patterns.items()
                if len(files) > 5
            }  # Only show functions in more than 5 files
...
def main():
    ...
    searcher.analyze_folder_contents()

    # Find cross-directory functionality
    print("\n4. Finding cross-directory functionality...")
    searcher.find_cross_directory_functionality()
