...
class DeepHiddenFolderAnalyzer:
    ...
    def __init__(self, home_dir: Path = None):
        ...
        self.target_folders = [
            ...
            ".oh-my-zsh",
            ".ollama",
            ".package_manager_backup_20251106_070741",
            ".pdf-filler-profiles",
            ".postman",
            ".putty",
            ".pytest_cache",
    ...
    def analyze_content(self, folder_path: Path) -> Dict[str, Any]:
        ...
        try:
                    ...
                    if item.is_file():
                        ...
                        if ext in self.content_patterns["scripts"]:
                            ...
                            analysis["configs"].append(
                                str(item.relative_to(folder_path))
                            )
            ...
            elif analysis["has_system_scripts"] and analysis["category"] in [
                "development_tools",
                "automation",
            ]:
    ...
    def generate_report(self, results: Dict[str, Any]) -> str:
        ...
        report.append(
            f"- **Folders Analyzed:** {results['folders_analyzed']}/{results['total_folders']}"
        )
        ...
        report.append(
            f"- **Folders with User Scripts:** {stats['folders_with_user_scripts']}\n"
        )
        ...
        for category, folders in sorted(results["categories"].items()):
            ...
            for folder in sorted(folders):
                ...
                if folder_data.get("exists"):
                    ...
                    report.append(
                        f"- **`{folder}`** - {size:.1f} MB, {files:,} files, {scripts} scripts"
                    )
            report.append("")
        ...
        script_folders = [
            (name, data)
            for name, data in results["folders"].items()
        ...
        if script_folders:
            ...
            for folder_name, folder_data in script_folders[:15]:
                ...
                report.append(
                    f"- **Type:** {'User scripts' if folder_data.get('has_user_scripts') else 'System scripts'}"
                )
                if folder_data["scripts"][:5]:
                    report.append("- **Sample scripts:**")
                    for script in folder_data["scripts"][:5]:
                        report.append(
                            f"  - `{script['path']}` ({script['size'] / 1024:.1f} KB)"
                        )
                report.append("")
        ...
        if large_folders:
            ...
            for folder_name, folder_data in large_folders:
                report.append(
                    f"- **`{folder_name}`** - {folder_data['size_mb']:.1f} MB ({folder_data['file_count']:,} files)"
                )
            report.append("")
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick scan (skip large folders)"
    )
