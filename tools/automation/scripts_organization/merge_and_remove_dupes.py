...
def remove_duplicate_directory(dir_to_remove):
    ...
    if dir_path.exists():
        ...
        if len(non_ds_store_items) == 0:
            ...
            print(
                f"  Directory {dir_to_remove} still has {len(non_ds_store_items)} items, not removing"
            )
...
def main():
    ...
    if duplicate_path.exists() and primary_path.exists():
        ...
        if success:
            ...
            # Verify the merge
            primary_size = sum(
                f.stat().st_size for f in primary_path.rglob("*") if f.is_file()
            )
            print("\n✅ Merge completed!")
            print(
                f"Primary directory ({primary_dir}) size: {primary_size / 1024 / 1024:.2f} MB"
            )
