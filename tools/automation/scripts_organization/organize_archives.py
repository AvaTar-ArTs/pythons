...
def analyze_harbor():
    ...
    for item in harbor_path.iterdir():
        if item.is_dir() and not item.name.startswith("."):
            ...
            docker_files = [
                f for f in files if f.name in ["Dockerfile", "docker-compose.yml"]
            ]
        ...
        recommendations.append(
            {
                ...
                "remove": [
                    f
                    for f in duplicates["ollama_configs"]
                    if f != "ollama-setup-kit-Intel-macOS.zip"
                ],
            }
        )
