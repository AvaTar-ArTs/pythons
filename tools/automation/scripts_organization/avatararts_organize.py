...
class AvatarArtsOrganizer:
                ...
                try:
                    ...
                    if (
                        any(
                            ...
                            for p in self.config["temp_patterns"]
                            if "*" in p
                        )
                        or file in self.config["temp_patterns"]
                    ):
                        stats["temp_files"].append(filepath)

                    # Find old files (for potential archival)
                    age_threshold = datetime.now() - timedelta(
                        days=self.config["archive_age_days"]
                    )
                    if file_mtime < age_threshold and any(
                        x in str(filepath) for x in ["analysis", "report", "summary"]
                    ):
    ...
    def _print_analysis(self, stats: Dict):
        ...
        print(
            f"   Total Size: {Colors.BOLD}{self._format_size(stats['total_size'])}{Colors.ENDC}"
        )
        print(f"   Total Files: {Colors.BOLD}{stats['total_files']:,}{Colors.ENDC}")
        print(
            f"   Total Directories: {Colors.BOLD}{stats['total_dirs']:,}{Colors.ENDC}\n"
        )
        ...
        if duplicate_groups:
            ...
            print("   Run 'avatararts_organize.py dedupe' for details\n")
        ...
        if stats["temp_files"]:
            ...
            print("   Run 'avatararts_organize.py cleanup' to remove\n")

        # Old files
        if stats["old_files"]:
            print(f"{Colors.YELLOW}📦 Old Files (>90 days){Colors.ENDC}")
            print(f"   Count: {len(stats['old_files'])}")
            print("   Run 'avatararts_organize.py archive' to archive\n")
                ...
                if should_remove:
                    try:
                        ...
                        if dry_run:
                            ...
                            filepath.unlink()
                            print(
                                f"   {Colors.GREEN}✓{Colors.ENDC} Removed: {filepath.name}"
                            )
                        removed_count += 1
                        freed_space += size
                    except Exception as e:
                        print(
                            f"   {Colors.RED}✗{Colors.ENDC} Error removing {filepath}: {e}"
                        )
            ...
            if "__pycache__" in dirs:
                ...
                try:
                    if dry_run:
                        ...
                        print(
                            f"   {Colors.GREEN}✓{Colors.ENDC} Removed: {pycache_path}"
                        )
                    removed_count += 1
                except Exception as e:
    ...
    def dedupe(self, interactive: bool = True):
        ...
        for hash_val, files in duplicate_groups.items():
            ...
            print(
                f"{Colors.CYAN}Duplicate group ({len(files)} files, {self._format_size(file_size)} each):{Colors.ENDC}"
            )
            for f in files:
                print(f"   - {f.relative_to(self.workspace_root)}")
            print(
                f"   Potential savings: {Colors.YELLOW}{self._format_size(savings)}{Colors.ENDC}\n"
            )

        print(
            f"{Colors.BOLD}Total potential savings: {self._format_size(total_savings)}{Colors.ENDC}\n"
        )

        if interactive:
            print(
                f"{Colors.YELLOW}💡 Tip: Review duplicates manually before deleting{Colors.ENDC}"
            )
            print(
                "   Consider keeping files in different locations if they serve different purposes"
            )
...
Examples:
  ...
  %(prog)s organize             # Context-aware organization
    ...
    parser.add_argument(
        "--dir", type=Path, help="Directory to operate on (default: current)"
    )
    ...
    if args.command == "analyze":
        ...
        print(
            "Will implement context-aware file organization based on your existing scripts"
        )
