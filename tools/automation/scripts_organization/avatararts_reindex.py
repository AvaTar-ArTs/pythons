...
class WorkspaceIndexer:
    ...
    def __init__(self, workspace_root: Path, db_path: Path = None):
        ...
        self.db_path = db_path or (
            workspace_root / "UTILITIES_TOOLS" / "workspace_index.db"
        )
    ...
    def categorize_file(self, filepath: Path, content_preview: str) -> Tuple[str, str]:
        ...
        project_context = (
            max(context_scores, key=context_scores.get) if context_scores else "general"
        )
    ...
    def _print_stats(self, stats: Dict):
        ...
        print(
            f"   Total files scanned: {Colors.BOLD}{stats['total_files']:,}{Colors.ENDC}"
        )
        print(
            f"   Files indexed: {Colors.BOLD}{Colors.GREEN}{stats['indexed_files']:,}{Colors.ENDC}"
        )
...
Examples:
  ...
  %(prog)s stats             # Show index statistics
    ...
    parser.add_argument(
        "command", choices=["reindex", "search", "stats"], help="Command to execute"
    )
    ...
    if args.command == "reindex":
        ...
        print(
            f"{Colors.CYAN}🔍 Searching for: {Colors.BOLD}{args.query}{Colors.ENDC}\n"
        )
        ...
        print(
            f"   Total files indexed: {Colors.BOLD}{stats['total_files']:,}{Colors.ENDC}"
        )
