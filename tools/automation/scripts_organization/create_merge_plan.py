...
def main():
    ...
    for group_id, rows in important:
        ...
        rows_sorted = sorted(
            rows, key=lambda x: (x["action"] != "KEEP", -float(x["size_mb"]))
        )
            ...
            merge_rows.append(
                {
                    ...
                    "remove_files": "; ".join(
                        [f"{r['file_name']} ({r['size_mb']}MB)" for r in remove]
                    ),
                }
            )
    ...
    with open(SUMMARY, "w") as f:
        ...
        for i, row in enumerate(merge_rows[:20], 1):
            ...
            f.write(
                f"   Remove {row['remove_count']} files ({row['size_savings_mb']} MB)\n"
            )
            f.write(
                f"   Files: {row['remove_files'][:100]}...\n"
                if len(row["remove_files"]) > 100
                else f"   Files: {row['remove_files']}\n"
            )
