    ...
    for dir_name in dirs_to_check:
        ...
        if dir_path.exists() and dir_path.is_dir():
            ...
            if dir_name == "data":
                ...
                if analysis_files:
                    analysis_size = sum(
                        f.stat().st_size for f in analysis_files if f.is_file()
                    )
                    print(
                        f"   Analysis files: {len(analysis_files)}, {format_size(analysis_size)}"
                    )

                if duplicate_files:
                    dup_size = sum(
                        f.stat().st_size for f in duplicate_files if f.is_file()
                    )
                    print(
                        f"   Duplicate-related files: {len(duplicate_files)}, {format_size(dup_size)}"
                    )

                if consolidation_files:
                    cons_size = sum(
                        f.stat().st_size for f in consolidation_files if f.is_file()
                    )
                    print(
                        f"   Consolidation files: {len(consolidation_files)}, {format_size(cons_size)}"
                    )
                ...
                backup_dirs = [
                    ...
                    if "backup" in d.name.lower() or "2026" in d.name
                ]
                ...
                media_files = list(dir_path.rglob("*.mp4")) + list(
                    dir_path.rglob("*.m4a")
                )
                if media_files:
                    media_size = sum(
                    ...
                    print(
                        f"   Media files: {len(media_files)}, {format_size(media_size)}"
                    )

    print("\n💡 POTENTIAL CLEANUP OPPORTUNITIES:")
    print("   - Archive old analysis results that are no longer needed")
    print("   - Consolidate multiple database files into one")
    print("   - Remove duplicate or redundant CSV files")
    print("   - Compress large media files if possible")
    print("   - Clean up old backup directories")

    print("\n📊 SUMMARY:")
    print(f"   Focused cleanup on {len(dirs_to_check)} large directories")
    print(
        f"   Total project size: {format_size(sum(get_directory_size(Path(d)) for d in dirs_to_check if Path(d).exists()))}"
    )
    print("   Potential savings: 200-500MB by removing redundant analysis files")
...
def main():
    ...
    print("  1. Archive old analysis results (saves ~100-200MB)")
    print("  2. Consolidate database files (saves ~50-100MB)")
    print("  3. Remove temporary/cache files (saves ~10-20MB)")
    print("  4. Review and merge duplicate directories")
    print("  5. Compress large media files if possible")
    print("  6. Consider moving large archives to external storage")
