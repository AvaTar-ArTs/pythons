...
def analyze_duplicate_directories():
    ...
    if dir1_path.exists() and dir2_path.exists():
        ...
        print("\n💡 RECOMMENDATION:")
        print(
            f"  Since {dir1} is significantly larger ({size1 / 1024 / 1024:.2f} MB vs {size2 / 1024 / 1024:.2f} MB),"
        )
...
def main():
    ...
    if dir1 and dir2:
        ...
        print("  Option 3: Create new combined directory")

        print(f"\n🧪 DRY RUN - Testing merge of {dir2} into {dir1}:")
        merge_directories(dir2, dir1, dry_run=True)

        print("\n⚠️  IMPORTANT:")
        print("  • Always backup before merging directories")
        print("  • Review conflicts before proceeding")
        print("  • Consider if you want to keep both directories")

        print("\n🎯 NEXT STEPS:")
        print("  1. Review the conflicts identified above")
        print("  2. Decide which directory to keep as primary")
        print("  3. Manually resolve any conflicts if needed")
        print("  4. Run the merge with dry_run=False when ready")
