...
class ContentAwareAnalyzer:
    ...
    def get_parent_context(self, filepath):
        ...
        for part in parts:
            if any(
                x in part.lower() for x in ["complete", "empire", "suite", "project"]
            ):
    ...
    def analyze_python_files(self):
        ...
        for py_file in self.root.rglob("*.py"):
            ...
            if any(
                x in str(py_file) for x in [".venv", "_build", "__pycache__", ".git"]
            ):
                continue
    ...
    def generate_report(self, exact_duplicates, file_contexts, similar_groups):
        ...
        report_path = (
            ...
            / "docs"
            / "reports"
            / f"CONTENT_AWARE_DUPLICATES_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
        ...
        with open(report_path, "w") as f:
            ...
            f.write("- **Analysis Method:** Content-based (not filename-based)\n")
            f.write("- **Context Awareness:** Parent directory analysis included\n\n")
            ...
            if exact_duplicates:
                ...
                for i, (hash_val, files) in enumerate(
                    ...
                    f.write(
                        f"- **Potential Savings:** {total_duplicate_size / 1024:.1f} KB\n\n"
                    )
                    ...
                    if not keep_file:
                        ...
                        for file_path in file_paths:
                            if "tools/" in str(file_path) and "archive" not in str(
                                file_path
                            ):
                                keep_file = file_path
                                break
            ...
            if similar_groups:
                ...
                f.write(
                    "Review to determine if they should be merged or kept separate.\n\n"
                )
                ...
                for i, group in enumerate(similar_groups[:20], 1):  # Top 20
                    ...
                    f.write("\n")
                    f.write(
                        "**Action:** Review manually to determine if merge is appropriate\n\n"
                    )
                    f.write("---\n\n")
...
def main():
    ...
    if exact_duplicates:
        ...
        for hash_val, files in sorted(
            exact_duplicates.items(), key=lambda x: x[1][0][1] * len(x[1]), reverse=True
        )[:20]:  # Top 20
            file_paths = [f[0] for f in files]
            size = files[0][1]
            ...
            print(
                f"     Copies: {len(files)} | Size: {size / 1024:.1f} KB | Savings: {savings / 1024:.1f} KB"
            )
    ...
    report_path = analyzer.generate_report(
        exact_duplicates, file_contexts, similar_groups
    )
