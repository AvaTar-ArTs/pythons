...
def find_duplicate_directories(root_path):
    ...
    for normalized, dirs in name_groups.items():
        if len(dirs) > 1:
            ...
            if processed % 10 == 0:
                print(
                    f"  Processing group {processed}/{len([g for g in name_groups.values() if len(g) > 1])}..."
                )
                    ...
                    potential_duplicates.append(
                        {
                            ...
                            "similarity": 1.0
                            if content_hash
                            else size_ratio,  # 100% if same hash
                            "content_hash": content_hash,
                        }
                    )
        ...
        for filepath, size in batch:
            ...
            if file_hash:
                file_hashes[file_hash].append(
                    {"path": str(filepath.relative_to(root)), "size": size}
                )
...
def main():
    ...
    total_waste_mb = 0
    for dup_group in sorted(
        dup_dirs,
        key=lambda x: max(d["size_bytes"] for d in x["directories"]),
        reverse=True,
    )[:20]:
