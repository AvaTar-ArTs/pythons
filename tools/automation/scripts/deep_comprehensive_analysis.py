...
class DeepWorkspaceAnalyzer:
            ...
            for item in sorted(items):
                ...
                if item.is_dir():
                    ...
                    if (
                        ...
                        and item.name not in [".claude", ".history", ".playwright-mcp"]
                    ):
                        continue
    ...
    def generate_report(self, output_file=None):
        ...
        self.stats["large_directories"].sort(
    ...
    def print_summary(self):
        ...
        for i, dir_info in enumerate(self.stats["large_directories"][:10], 1):
            print(
                f"   {i:2}. {dir_info['path'][:60]:60} {dir_info['size_mb']:>8.1f} MB ({dir_info['file_count']:,} files)"
            )
        ...
        sorted_dupes = sorted(
            self.stats["duplicate_names"].items(), key=lambda x: len(x[1]), reverse=True
        )[:10]
