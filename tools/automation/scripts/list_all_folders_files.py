...
class FolderFileLister:
    ...
    def _list_tree(self, output_file=None):
            ...
            try:
                ...
                for i, item in enumerate(items):
                    ...
                    if item.is_dir():
                        ...
                        lines.append(
                            f"{prefix}{current_prefix}{Colors.BLUE}📁 {item.name}/{Colors.ENDC} ({size_str})"
                        )
                        ...
                        try:
                            ...
                            lines.append(
                                f"{prefix}{current_prefix}{Colors.CYAN}{item.name}{Colors.ENDC} ({size_str})"
                            )
                        except:
        ...
        for ext, count in sorted(
            self.stats["files_by_type"].items(), key=lambda x: x[1], reverse=True
        )[:10]:
            ...
            try:
                for item in path.iterdir():
                    ...
                    if item.is_dir():
                        self.stats["total_dirs"] += 1
                        all_items.append(
                            {"type": "directory", "path": str(rel_path), "depth": depth}
                        )
    ...
    def _list_json(self, output_file=None):
                ...
                for item in path.iterdir():
                    ...
                    if item.is_dir():
                        ...
                        data["directories"].append(
                            {"path": str(rel_path), "depth": depth, "size": dir_size}
                        )
        ...
        output_file = (
            output_file
            or f"all_folders_files_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def _list_csv(self, output_file=None):
        ...
        output_file = (
            output_file
            or f"all_folders_files_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )

        with open(output_file, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(
                f, fieldnames=["type", "path", "name", "size", "ext", "depth"]
            )
            writer.writeheader()
