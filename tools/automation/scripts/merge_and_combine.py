...
class MergeAndCombine:
    ...
    def load_merge_opportunities(self, mapping_file="dedupe_mapping.csv"):
        ...
        print(
            f"{Colors.CYAN}📂 Loading merge opportunities from {mapping_file}...{Colors.ENDC}"
        )
    ...
    def show_merge_plan(self, limit=None):
        ...
        for target, items in sorted_groups:
            ...
            for item in items[:5]:
                print(
                    f"      - {item['source']} ({item['size_mb']:.2f} MB, {item['similarity']})"
                )
            if len(items) > 5:
        ...
        print(
            f"{Colors.BOLD}Total merge size: {total_size:.2f} MB ({total_size / 1024:.2f} GB){Colors.ENDC}"
        )
    ...
    def execute_merge_plan(self, strategy="copy_unique", limit=None):
        ...
        for target, items in sorted_groups:
            ...
            if not target_path.exists():
                print(
                    f"{Colors.YELLOW}⚠️  Target not found: {target}, skipping group{Colors.ENDC}"
                )
                continue
            ...
            for item in items:
                ...
                try:
                    if (self.root / source_path).is_dir():
                        ...
                        print(
                            f"{Colors.YELLOW}⚠️  File merge not yet implemented: {source_path}{Colors.ENDC}"
                        )
                except Exception as e:
                    errors.append((source_path, str(e)))
                    print(
                        f"{Colors.RED}❌ Error merging {source_path}: {e}{Colors.ENDC}"
                    )
    ...
    def consolidate_duplicate_dirs(self, dir_patterns=None):
        ...
        for name, dirs in sorted(
            duplicates.items(), key=lambda x: sum(d["size"] for d in x[1]), reverse=True
        )[:20]:
            total_size = sum(d["size"] for d in dirs) / (1024**2)
            print(
                f"{Colors.CYAN}{name}{Colors.ENDC} ({len(dirs)} copies, {total_size:.2f} MB):"
            )
            for d in dirs:
                print(f"   - {d['path']}")
            print()
...
Examples:
  ...
  python3 merge_and_combine.py consolidate
    ...
    parser.add_argument(
        "--strategy",
        choices=["copy_unique", "overwrite", "skip", "rename"],
        default="copy_unique",
        help="Merge strategy",
    )
    ...
    execute_parser.add_argument(
        "--execute", action="store_true", help="Actually merge (default is dry-run)"
    )
    ...
    combine_parser.add_argument(
        "--files", nargs="+", required=True, help="Files to combine"
    )
    combine_parser.add_argument("--output", required=True, help="Output file")
    combine_parser.add_argument(
        "--strategy",
        choices=["concatenate", "unique_lines", "newest"],
        default="concatenate",
        help="Combine strategy",
    )
    combine_parser.add_argument(
        "--execute", action="store_true", help="Actually combine"
    )

    # Consolidate command
    consolidate_parser = subparsers.add_parser(
        "consolidate", help="Find duplicate directories"
    )
    consolidate_parser.add_argument(
        "--patterns", nargs="+", help="Directory name patterns to match"
    )
