...
"""
        ...
        for name, stats in sorted(
            ...
            top_ext = (
                stats["extensions"].most_common(1)[0]
                if stats["extensions"]
                else ("(none)", 0)
            )
            top_dir = (
                stats["top_level_dirs"].most_common(1)[0]
...
"""
        ...
        for dir_name, count in self.analysis["global_stats"]["directories"].most_common(
            20
        ):
            md += f"| `{dir_name}` | {count:,} |\n"
        ...
        for name, stats in sorted(self.analysis["by_inventory"].items()):
            ...
            for location, files in sorted(
                stats["by_location"].items(), key=lambda x: len(x[1]), reverse=True
            ):
        ...
        for location, count in sorted(
            ...
            pct = (
                (count / self.analysis["global_stats"]["total_files"] * 100)
                if self.analysis["global_stats"]["total_files"] > 0
                else 0
            )
...
"""
    ...
    def run(self):
        ...
        print("\n📊 Summary:")
        print(
            f"   Total files catalogued: {self.analysis['global_stats']['total_files']:,}"
        )
        print(
            f"   Unique extensions: {len(self.analysis['global_stats']['extensions'])}"
        )
        print(
            f"   Unique directories: {len(self.analysis['global_stats']['directories'])}"
        )
        print()
