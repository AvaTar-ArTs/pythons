...
class MergeDiffDedupe:
    ...
    def load_dedupe_mapping(self, mapping_file="dedupe_mapping.csv"):
        ...
        if not Path(mapping_file).exists():
            ...
            print("   Run dedupe_analysis.py first to generate mapping")
            return False
    ...
    def show_summary(self):
        ...
        if not self.duplicates and not self.merge_opportunities:
            print(
                f"{Colors.YELLOW}No duplicates or merge opportunities found{Colors.ENDC}"
            )
            return
    ...
    def diff_files(self, file1, file2, output_file=None):
        ...
        try:
            ...
            print(
                f"{Colors.YELLOW}⚠️  Files differ ({len(diff)} diff lines){Colors.ENDC}"
            )
            ...
            result = {"identical": False, "diff_lines": len(diff), "diff": diff}
    ...
    def diff_directories(self, dir1, dir2, output_file=None):
        ...
        for rel_path in in_both:
            ...
            if hash1 == hash2 and hash1 is not None:
                ...
                similarity = self._content_similarity(
                    file1_info["path"], file2_info["path"]
                )
                if similarity > 0.8:
        ...
        print(
            f"{Colors.GREEN}✅ Files in both (identical): {len(identical)}{Colors.ENDC}"
        )
        print(
            f"{Colors.YELLOW}⚠️  Files only in {path1.name}: {len(only_in_1)}{Colors.ENDC}"
        )
        print(
            f"{Colors.YELLOW}⚠️  Files only in {path2.name}: {len(only_in_2)}{Colors.ENDC}"
        )
        print(
            f"{Colors.RED}❌ Files in both (different): {len(different)}{Colors.ENDC}"
        )
        if similar_content:
            print(
                f"{Colors.CYAN}📊 Files with similar content (>80%): {len(similar_content)}{Colors.ENDC}"
            )
        if different_by_size:
            print(
                f"{Colors.YELLOW}📏 Files different by size only: {len(different_by_size)}{Colors.ENDC}"
            )
        ...
        if identical:
            ...
            for f in identical[:5]:
                print(
                    f"      {f['path']} ({f['size'] / 1024:.1f} KB) [parent: {f['parent']}]"
                )
            if len(identical) > 5:
        ...
        if similar_content:
            ...
            for f in sorted(
                similar_content, key=lambda x: x["similarity"], reverse=True
            )[:10]:
                print(
                    f"   {f['path']} ({f['similarity'] * 100:.1f}% similar) [parent: {f['parent']}]"
                )
        ...
        if different_by_size:
            ...
            for f in sorted(different_by_size, key=lambda x: x["diff"], reverse=True)[
                :10
            ]:
                size_diff_mb = f["diff"] / (1024**2)
                print(
                    f"   {f['path']} (diff: {size_diff_mb:.2f} MB) [parent: {f['parent']}]"
                )
        ...
        print(
            f"   Merge potential: {len(only_in_1) + len(only_in_2)} unique files could be merged"
        )
        ...
        result = {
            ...
            "only_in_1_by_parent": {
                k: [f["path"] for f in v] for k, v in only_in_1_by_parent.items()
            },
            "only_in_2_by_parent": {
                k: [f["path"] for f in v] for k, v in only_in_2_by_parent.items()
            },
            "stats": {
        ...
        try:
            ...
            with open(output, "w", encoding="utf-8") as outfile:
                for i, file_path in enumerate(files):
                    ...
                    if not path.exists():
                        print(
                            f"{Colors.YELLOW}⚠️  Skipping {path} (not found){Colors.ENDC}"
                        )
                        continue
    ...
    def dedupe(self, limit=None, min_size_mb=0):
        ...
        if not self.duplicates:
            print(
                f"{Colors.YELLOW}No duplicates found. Load dedupe_mapping.csv first.{Colors.ENDC}"
            )
            return
        ...
        if not self.dry_run:
            response = input(
                f"{Colors.RED}⚠️  Are you sure? Type 'yes' to continue: {Colors.ENDC}"
            )
        ...
        for dup in sorted(filtered, key=lambda x: x["size_mb"], reverse=True):
            ...
            if not path.exists():
                print(
                    f"{Colors.YELLOW}⚠️  {dup['path']} (not found, skipping){Colors.ENDC}"
                )
                continue
...
Examples:
  ...
  python3 merge_diff_dedupe.py dedupe --min-size 10 --execute
    ...
    parser.add_argument(
        "--mapping", default="dedupe_mapping.csv", help="Dedupe mapping file"
    )

    subparsers = parser.add_subparsers(dest="command", help="Command")

    # Summary command
    subparsers.add_parser("summary", help="Show summary of duplicates")
    ...
    merge_parser.add_argument(
        "--files", nargs="+", required=True, help="Files to merge"
    )
    merge_parser.add_argument("--output", required=True, help="Output file")
    merge_parser.add_argument(
        "--strategy", choices=["concatenate", "unique_lines"], default="concatenate"
    )

    # Dedupe command
    dedupe_parser = subparsers.add_parser("dedupe", help="Remove duplicates")
    dedupe_parser.add_argument(
        "--execute", action="store_true", help="Actually remove (default is dry-run)"
    )
    dedupe_parser.add_argument(
        "--limit", type=int, help="Limit number of items to remove"
    )
    dedupe_parser.add_argument(
        "--min-size", type=float, default=0, help="Minimum size in MB"
    )
