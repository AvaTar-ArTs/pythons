...
def calculate_metrics(df):
    ...
    # Growth rates
    df["Y1_Y2_Growth"] = (
        (df["Potential Revenue (Year 2)"] - df["Potential Revenue (Year 1)"])
        / df["Potential Revenue (Year 1)"].replace(0, 1)
    ) * 100
    df["Y2_Y3_Growth"] = (
        (df["Potential Revenue (Year 3)"] - df["Potential Revenue (Year 2)"])
        / df["Potential Revenue (Year 2)"].replace(0, 1)
    ) * 100
    df["Y1_Y3_CAGR"] = (
        (
            (
                df["Potential Revenue (Year 3)"]
                / df["Potential Revenue (Year 1)"].replace(0, 1)
            )
            ** (1 / 2)
        )
    ...
    df["Quick_Win_Score"] = (
        df["Potential Revenue (Year 1)"] / df["Implementation_Difficulty"]
    )
    ...
    df["Risk_Score"] = df["Implementation_Difficulty"] * (
        df["Priority"].map({"High": 1, "Medium": 2, "Low": 3}).fillna(2)
    )
...
def analyze_roi_leaders(df):
    ...
    print_section_header(
        "💰 TOP ROI OPPORTUNITIES (Best Return on Implementation Effort)"
    )
...
def analyze_growth_leaders(df):
    ...
    growing = (
        df[df["Potential Revenue (Year 1)"] > 0]
        .nlargest(20, "Y1_Y3_CAGR")[
            [
                "Revenue Stream",
                "Status",
                "Priority",
...
def analyze_risk_profile(df):
    ...
    low_risk = df[
        (df["Status"].isin(["Ready", "Active"])) & (df["Priority"] == "High")
    ...
    high_risk = df[df["Risk_Score"] >= 10].sort_values(
...
def analyze_by_implementation_readiness(df):
    ...
    ready_sorted = df.sort_values(
        ["Status_Order", "Potential Revenue (Year 3)"], ascending=[True, False]
    )[
        [
            "Revenue Stream",
            "Status",
...
def analyze_passive_income_breakdown(df):
    ...
    passive = df[df["Type"] == "Passive"].sort_values(
        "Potential Revenue (Year 3)", ascending=False
    )
...
def generate_improvement_recommendations(df):
    ...
    missing_priority = df[
        (df["Status"] == "Not Found") & (df["Priority"].isin(["High", "Medium"]))
    ]
    ...
    available_high = df[
        (df["Status"] == "Available") & (df["Priority"].isin(["High", "Medium"]))
    ]
    ...
    top_5_pct = df.nlargest(5, "Potential Revenue (Year 3)")[
        "Revenue_Contribution_Pct"
    ].sum()
    if top_5_pct > 70:
        recommendations.append(
...
def generate_prioritized_action_plan(df):
    ...
    print("\n🚀 PHASE 1: QUICK WINS (0-3 months)")
    print("Focus: Immediate revenue from ready opportunities")
    ...
    phase2 = df[df["Status"].isin(["In Development", "Available"])].nlargest(
        10, "ROI_Score"
    )[["Revenue Stream", "Status", "Potential Revenue (Year 3)", "ROI_Score"]]

    print("\n⚡ PHASE 2: HIGH ROI DEVELOPMENT (3-6 months)")
    print("Focus: Complete high-value in-development projects")
    ...
    print("\n🔍 PHASE 3: EXPLORATION & DIVERSIFICATION (6-12 months)")
    print("Focus: Explore untapped opportunities")
    print(
        f"Target: ${phase3['Potential Revenue (Year 3)'].sum():,.0f} Year 3 revenue potential"
    )
...
def run_analysis():
    ...
    ready_active_y3 = df[df["Status"].isin(["Ready", "Active"])][
        "Potential Revenue (Year 3)"
    ].sum()
    print(
    ...
    # Run all analyses
    analyze_quick_wins(df)
    analyze_roi_leaders(df)
    analyze_growth_leaders(df)
    analyze_revenue_concentration(df)
    low_risk, high_risk = analyze_risk_profile(df)
    analyze_passive_income_breakdown(df)
    analyze_by_implementation_readiness(df)
    generate_improvement_recommendations(df)
