...
def missing_opportunities(df):
    ...
    print(
        f"Year 3 Potential (if implemented): ${missing['Potential Revenue (Year 3)'].sum():,.0f}"
    )
...
def generate_summary_stats(df):
    ...
    ready_y3 = df[df["Status"].isin(["Ready", "Active"])][
        "Potential Revenue (Year 3)"
    ].sum()
    ...
    print("\nBy Status:")
    print(
        f"  Ready/Active (Year 3): ${ready_y3:,.0f} ({ready_y3 / total_y3 * 100:.1f}%)"
    )
    print(
        f"  Not Found (Year 3): ${missing_y3:,.0f} ({missing_y3 / total_y3 * 100:.1f}%)"
    )

    print("\nBy Type:")
    print(
        f"  Passive (Year 3): ${passive_y3:,.0f} ({passive_y3 / total_y3 * 100:.1f}%)"
    )
