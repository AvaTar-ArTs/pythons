...
class UnlimitedDepthSearch:
    ...
    def search_by_type(self, extensions: List[str]) -> List[Dict]:
        ...
        print(
            f"{Colors.CYAN}Searching for file types: {', '.join(extensions)}{Colors.ENDC}"
        )
        ...
        for root, dirs, files in os.walk(self.root_path):
            ...
            for file in files:
                ...
                if file_extensions:
                    ext = file_path.suffix.lower().lstrip(".")
                    if (
                        ext not in file_extensions
                        and (ext or "no-ext") not in file_extensions
                    ):
                        continue
    ...
    def find_duplicates(:
        self, by_name: bool = False, by_content: bool = False
    ) -> Dict[str, List[Dict]]:
        ...
        if by_content:
            print(
                f"{Colors.CYAN}Finding duplicate file content (hashing)...{Colors.ENDC}"
            )
    ...
    def find_large_dirs(self, min_size: int = 100 * 1024 * 1024) -> List[Dict]:
        ...
        print(
            f"{Colors.CYAN}Finding large directories (> {self.format_size(min_size)})...{Colors.ENDC}"
        )
            ...
            for file in files:
                try:
                    ...
                    largest_files.append(
                        {"path": str(file_path), "size": size, "ext": ext}
                    )
                except:
                    pass
    ...
    def export_results(:
        self, results: List[Dict], output_file: str, format: str = "json"
    ):
    ...
    def print_results(self, results: List[Dict], limit: int = 50):
        ...
        for i, result in enumerate(results[:limit], 1):
            ...
            if "size" in result:
                size_str = result.get(
                    "size_formatted", self.format_size(result["size"])
                )
...
def main():
    ...
    parser.add_argument(
        "--pattern", type=str, help="Search pattern (regex) for file/directory names"
    )
    parser.add_argument(
        "--type", type=str, nargs="+", help="File extensions to search (e.g., py js md)"
    )
    parser.add_argument(
        "--min-size", type=str, help="Minimum file size (e.g., 100MB, 1GB)"
    )
    parser.add_argument(
        "--max-size", type=str, help="Maximum file size (e.g., 100MB, 1GB)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="File extensions to search content in",
    )
    parser.add_argument(
        "--duplicates", action="store_true", help="Find duplicate files"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--empty-dirs", action="store_true", help="Find empty directories"
    )
    parser.add_argument(
        "--large-dirs", type=str, help="Find large directories (min size, e.g., 100MB)"
    )
    parser.add_argument(
        "--analyze", action="store_true", help="Complete structure analysis"
    )
    ...
    parser.add_argument(
        "--limit", type=int, default=50, help="Limit number of results to display"
    )
    parser.add_argument(
    ...
    if args.analyze:
        ...
        for ext, count in sorted(
            ...
            print(
                f"  .{ext}: {count:,} files ({analysis['type_sizes'].get(ext, '0 B')})"
            )
        ...
        for item in analysis["largest_dirs"][:10]:
            print(
                f"  {searcher.format_size(item['size']):>12} - {item['path']} ({item['file_count']} files)"
            )
    ...
    if args.content:
        ...
        results = searcher.search_by_content(
            args.content, content_ext, args.case_sensitive
        )
    ...
    if args.duplicates_by_content:
        ...
        for hash_key, files in list(dupes.items())[: args.limit]:
            print(
                f"{Colors.YELLOW}{hash_key}{Colors.ENDC} ({len(files)} identical files):"
            )
            for f in files[:5]:
    ...
    if results:
        ...
        print(
            f"{Colors.YELLOW}No search criteria specified. Use --help for options.{Colors.ENDC}"
        )
