...
class WorkspaceOrganizer:
    ...
    def create_structure(self):
        ...
        structure = {
            ...
            "scripts": {"root-utilities": None},
            "docs": {"reports": None, "indexes": None},
        }
    ...
    def categorize_csv(self, filename):
        ...
        if any(x in name_lower for x in ["inventory", "index", "mapping", "reindex"]):
            ...
            x in name_lower
            for x in ["duplicate", "cleanup", "scattered", "consolidation"]
        ):
            return "analysis"
        else:
            return "active"
                ...
                for csv_file in files:
                    try:
                        ...
                        if dest.exists():
                            ...
                            dest = (
                                dest_dir
                                / f"{csv_file.stem}_{timestamp}{csv_file.suffix}"
                            )
    ...
    def organize_root_scripts(self, dry_run=True):
        ...
        if not dry_run:
            ...
            print(
                f"  🔍 DRY RUN - {len(root_scripts)} scripts would move to scripts/root-utilities/"
            )
    ...
    def generate_organization_log(self):
        ...
        log_path = (
            ...
            / "docs"
            / "reports"
            / f"ORGANIZATION_LOG_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
    ...
    def generate_master_index(self):
        ...
        with open(index_path, "w") as f:
            ...
            f.write(
                "- See `AVATARARTS_ANALYSIS_AND_RECOMMENDATIONS.md` for recommendations\n"
            )
            f.write("\n")
            ...
            f.write(
                "- **Inventory CSV:** `data/csv/inventories/AVATARARTS_INVENTORY.csv`\n"
            )
            f.write("\n")
