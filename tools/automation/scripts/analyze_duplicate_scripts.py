    ...
    for name, files in sorted(
        ...
        if len(hashes) == 1:
            ...
            if len(files) >= 2:
                ...
                if similarity > 0.95:
                    print(
                        "  💡 Recommendation: Very similar - likely same script, different versions"
                    )
                    print("     Review manually to determine which is most current")
                elif similarity > 0.70:
                    print(
                        "  💡 Recommendation: Similar but different - may be variations"
                    )
                    print(
                        "     Review to see if they can be merged or should be renamed"
                    )
                else:
                    print("  💡 Recommendation: Different scripts with same name")
                    print("     Rename to reflect their different purposes")
