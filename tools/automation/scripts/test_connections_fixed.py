...
try:
    ...
    if "401" in error_msg or "Incorrect API key" in error_msg:
        ...
        print("   ??  OpenAI: No credits (add billing at platform.openai.com)")
        warnings += 1
    else:
...
try:
    ...
    print("   Model: llama-3.1-8b-instant (FREE!)")
    passed += 1
except Exception as e:
    error_msg = str(e)
    if "401" in error_msg:
        ...
        print("   Note: May need to update groq package")
        warnings += 1
print()
...
try:
    ...
    if "401" in error_msg or "20003" in error_msg:
        print("   ? Twilio: Authentication failed")
        print("   Check credentials in ~/.env.d/notifications.env")
        failed += 1
    elif "403" in error_msg:
        print("   ??  Twilio: Trial account (upgrade for production)")
        warnings += 1
    else:
...
try:
    ...
    try:
        ...
        print("   ? ElevenLabs CONNECTED (v1.x)")
        ...
        print("   ? ElevenLabs API KEY SET (v0.x)")
        print("   Ready for voice generation")
        passed += 1
except Exception as e:
    print(f"   ??  ElevenLabs: {str(e)[:100]}")
    print("   May need package update, but API key is set")
    warnings += 1
print()
...
try:
    ...
    try:
        ...
        print("   Model: gemini-1.5-flash (FREE!)")
        passed += 1
    except:
...
if failed > 0 or warnings > 0:
    ...
    print(
        "   pip install --upgrade openai anthropic groq google-generativeai elevenlabs twilio"
    )
    print()
