            ...
            for filename in files:
                ...
                if filepath.exists():
                    try:
                        ...
                        found.append(
                            {"file": filename, "size": size, "path": str(filepath)}
                        )
                    except:
                        pass
    ...
    def scan_env_directory(self):
        ...
        try:
                ...
                if file.is_file() and file.suffix == ".env":
                    ...
                    try:
                        with open(file, "r") as f:
                            ...
                            api_inventory.append({"file": file.name, "keys": keys})
                    except:
                        pass
            ...
            self.analysis["api_keys"] = {
                ...
                "management_scripts": [
                    str(f.name) for f in env_dir.iterdir() if f.suffix in [".sh", ".py"]
                ],
            }
            ...
            print(
                f"   ✓ Management scripts: {len(self.analysis['api_keys']['management_scripts'])}"
            )
        ...
        for root_name in project_roots:
            ...
            try:
                ...
                self.analysis["project_directories"][root_name] = {
                    ...
                    "projects": sorted(
                        projects, key=lambda x: x["size_mb"], reverse=True
                    )[:20],
                }
...
"""
        ...
        for name, data in sorted_dirs:
            percentage = (
                data["size_gb"] / self.analysis["overview"]["total_size_gb"]
            ) * 100
