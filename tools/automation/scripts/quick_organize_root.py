...
def organize_root_files(dry_run=True):
    ...
    for md_file in md_files:
        ...
        if any(
            x in name_lower for x in ["guide", "how-to", "tutorial", "intelligence"]
        ):
            dest = docs_guides / md_file.name
            category = "guides"
        elif any(
            x in name_lower
            for x in ["strategy", "suggestions", "improvements", "organization"]
        ):
            dest = docs_strategies / md_file.name
            category = "strategies"
        elif any(
            x in name_lower for x in ["session", "handoff", "complete", "verification"]
        ):
    ...
    for json_file in json_files:
        ...
        if any(
            x in name_lower
            for x in ["analysis", "complete", "reindex", "cleanup", "inventory"]
        ):
