...
if not os.path.isdir(AUDIO_DIR):
    logger.info(
        colored("❌ Invalid directory path. Please check and try again.", "red"),
    )
    sys.exit(1)
...
def transcribe_audio(file_path, max_attempts=3):
    if not os.path.isfile(file_path) or os.path.getsize(file_path) == 0:
        logger.info(
            colored(f"❌ {os.path.basename(file_path)} is invalid or empty.", "red"),
        )
        return None
    ...
    if transcript:
        ...
        if analysis:
            ...
            with open(analysis_file_path, "w") as f:
                f.write(
                    "# Analysis of " + filename_no_ext + Path("\n\n") + linked_analysis,
                )
            print(
...
def process_audio_directory(audio_dir):
    ...
    if not audio_files:
        logger.info(
            colored(
                "📂 No MP3 files found in the directory. Please check your path.",
                "red",
            ),
        )
        return
    ...
    logger.info(
        colored(
            f"🎶 Found {len(audio_files)} audio files. Starting processing...",
            "green",
        ),
    )
