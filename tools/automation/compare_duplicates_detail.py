...
def compare_functions(func1_source, func2_source):
    ...
    diff = list(
        unified_diff(
            lines1, lines2, fromfile="Function 1", tofile="Function 2", lineterm="", n=3
        )
    )
...
def extract_functions_from_file(filepath):
    ...
    try:
        ...
        return {"file": str(filepath), "functions": extractor.functions}
    except Exception as e:
...
def analyze_and_compare():
    ...
    if json_files:
        ...
        print(
            "❌ No existing analysis found. Please run compare_content_and_functions.py first."
        )
        return
    ...
    for hash_val, func_list in top_duplicates:
        ...
        if len(func_details) >= 2:
            ...
            if (
                len(func_details) >= 2
                and func_details[0]["source"]
                and func_details[1]["source"]
            ):
                comparison = compare_functions(
                    func_details[0]["source"], func_details[1]["source"]
                )
    ...
    # Print summary
    identical = sum(
        1 for comp in comparison_results if comp["comparison"]["similarity"] == 1.0
    )
    similar = sum(
        1
        for comp in comparison_results
        if 0.9 <= comp["comparison"]["similarity"] < 1.0
    )
    different = sum(
        1 for comp in comparison_results if comp["comparison"]["similarity"] < 0.9
    )
