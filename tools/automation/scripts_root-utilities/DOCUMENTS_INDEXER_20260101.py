...
class DocumentsIndexer:
    ...
    def generate_inventory_csv(self) -> str:
        ...
        sorted_data = sorted(
            self.files_data, key=lambda x: x["size_bytes"], reverse=True
        )
    ...
    def generate_directory_summary_csv(self) -> str:
        ...
        sorted_dirs = sorted(
            self.stats["directory_sizes"].items(), key=lambda x: x[1], reverse=True
        )

        with open(output_file, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["Directory", "File Count", "Total Size (MB)", "Total Size (GB)"]
            )
    ...
    def find_duplicates(self) -> str:
        ...
        with open(output_file, "w", newline="", encoding="utf-8") as f:
            ...
            for (size, name), files in sorted(
                duplicates.items(), key=lambda x: x[0][0], reverse=True
            ):
                paths = " | ".join([f["relative_path"] for f in files])
                writer.writerow(
                    [name, round(size / (1024 * 1024), 2), len(files), paths]
                )
    ...
    def generate_summary_report(self) -> str:
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            f.write("Documents Directory Analysis Summary\n")
            f.write("=" * 60 + "\n\n")
            ...
            for i, (ext, count) in enumerate(sorted_types[:10], 1):
                ...
                f.write(
                    f"{i:2}. {ext or '(no ext)':15} - {count:6,} files ({size_mb:10.2f} MB)\n"
                )
            ...
            for i, file_info in enumerate(sorted_files[:10], 1):
                f.write(
                    f"{i:2}. {file_info['name']:40} - {file_info['size_mb']:10.2f} MB\n"
                )
    ...
    def run_full_analysis(self) -> Dict[str, str]:
        ...
        print(f"\n{'=' * 60}")
        print("Documents Directory Indexer")
        print(f"{'=' * 60}\n")
