...
def calculate_similarity(sig1, sig2):
    ...
    line_diff = abs(sig1["line_count"] - sig2["line_count"]) / max(
        sig1["line_count"], sig2["line_count"]
    )
...
def generate_reports(duplicate_groups):
    ...
    with open(plan_file, "w", encoding="utf-8") as f:
        ...
        total_size_kb = sum(
            sum(item["size_kb"] for item in group[1:]) for group in duplicate_groups
        )
        total_lines = sum(
            sum(item["code_lines"] for item in group[1:]) for group in duplicate_groups
        )
        ...
        for i, group in enumerate(sorted_groups[:20], 1):
            ...
            f.write(
                f"### {i}. {group[0]['filename']} ({len(group)} copies, {avg_sim:.0%} similar)\n\n"
            )
