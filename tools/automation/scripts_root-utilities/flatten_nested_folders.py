...
class FolderFlattener:
    ...
    def find_nested_patterns(self) -> dict:
        ...
        print(
            f"   Found {sum(len(v) for v in nested_patterns.values())} nested directories"
        )
        for pattern, dirs in nested_patterns.items():
            print(
                f"   - {pattern}: {len(dirs)} nested instances (max depth: {max(d['depth'] for d in dirs)})"
            )

        return nested_patterns

    def flatten_nested_directory(:
        self, nested_dir: Path, target_root: Path, pattern_name: str
    ):
        '\''
                    ...
                    for subitem in item.rglob("*"):
                        if subitem.is_file():
                            ...
                            self.mappings.append(
                                {
                                    "original_path": str(
                                        subitem.relative_to(self.workspace_root)
                                    ),
                                    "new_path": str(
                                        new_file_path.relative_to(self.workspace_root)
                                    ),
                                    "action": "MOVE",
                                    "reason": f"Flattening nested {pattern_name} structure",
                                }
                ...
                if target_item.exists():
                    ...
                    if item.stat().st_size == target_item.stat().st_size:
                        ...
                        self.mappings.append(
                            {
                                ...
                                "new_path": str(
                                    target_item.relative_to(self.workspace_root)
                                ),
                                "action": "DELETE",
                        ...
                        while target_item.exists():
                            target_item = (
                                target_dir / f"{item.stem}_{counter}{item.suffix}"
                            )
                            counter += 1
    ...
    def generate_mapping_csv(self) -> Path:
        ...
        output_file = (
            self.workspace_root / f"FOLDER_FLATTENING_MAPPING_{self.timestamp}.csv"
        )
