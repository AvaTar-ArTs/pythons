...
def consolidate_duplicates(recommendations):
    ...
    for rec in recommendations:
        ...
        if file_path not in consolidated:
            ...
            current_priority = conflict_resolution[
                consolidated[file_path]["analysis_source"]
            ]
...
def calculate_space_savings(recommendations):
    ...
    return {
        ...
        "largest_file": max(
            (int(rec.get("file_size", 0)), rec["filename"]) for rec in recommendations
        ),
    }
...
def generate_final_report():
    ...
    with open(report_filename, "w", newline="", encoding="utf-8") as csvfile:
        ...
        writer.writerow(
            ["Total space savings (KB)", plan["summary"]["total_space_saved_kb"]]
        )
        writer.writerow(
            [
                "Largest file to remove",
                f"{plan['space_analysis']['largest_file'][1]} ({plan['space_analysis']['largest_file'][0]} bytes)",
            ]
        )
        writer.writerow([""])
        ...
        for category, count in plan["summary"]["categories"].items():
            space_kb = (
                plan["space_analysis"]["by_category"].get(
                    category.replace("_", " ").title(), 0
                )
                // 1024
            )
        ...
        for phase_key, phase_files in plan["removal_phases"].items():
            if phase_files:
                ...
                for rec in sorted(
                    phase_files, key=lambda x: int(x.get("file_size", 0)), reverse=True
                ):
                    writer.writerow(
                        [
    ...
    print(
        f"   • Safe to remove immediately: {len(plan['removal_phases']['phase_1_safe'])} files"
    )
    print(f"   • Require review: {len(plan['removal_phases']['phase_2_review'])} files")
    print(
        f"   • Need careful consideration: {len(plan['removal_phases']['phase_3_careful'])} files"
    )
    ...
    if all_files:
        largest_files = sorted(
            all_files, key=lambda x: int(x.get("file_size", 0)), reverse=True
        )[:5]
        print("\n🏆 Top space-saving files:")
        for i, file in enumerate(largest_files, 1):
            size_kb = int(file.get("file_size", 0)) // 1024
            print(
                f"   {i}. {file['filename']} ({size_kb} KB) - {file.get('confidence', 'Medium')} confidence"
            )
