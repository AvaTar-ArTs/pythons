...
class AdvancedParentFolderOrganizer:
    ...
    def find_latest_index(self):
        ...
        ultra_deep_files = sorted(
            self.workspace_root.glob("ULTRA_DEEP_REINDEX_*.db"), reverse=True
        )
        if ultra_deep_files:
        ...
        db_files = sorted(self.workspace_root.glob("REINDEX_*.db"), reverse=True)
        if db_files:
    ...
    def analyze_parent_folder_context(self, db_path: Path):
        ...
        for row in cursor.fetchall():
            ...
            directory_analysis[parent_dir] = {
                ...
                "oldest": oldest,
                "newest": newest,
                "purpose": self._detect_directory_purpose(
                    parent_dir, types, file_count
                ),
                "project": self._detect_project(parent_dir),
                "should_move": self._should_move_directory(
                    parent_dir, file_count, total_mb
                ),
            }
        ...
        print(
            f"   Analyzed file relationships in {len(file_relationships)} directories\n"
        )

        return directory_analysis, file_relationships

    def _detect_directory_purpose(:
        self, parent_dir: str, types: list, file_count: int
    ) -> str:
    ...
    def _should_move_directory(:
        self, parent_dir: str, file_count: int, total_mb: float
    ) -> bool:
    ...
    def generate_intelligent_moves(:
        self, directory_analysis: dict, file_relationships: dict
    ):
                ...
                for file_info in files:
                    if file_info["file_type"] == "code_python":
                        new_path = self._determine_tools_location(
                            file_info["filename"], file_info["path"], parent_dir
                        )
                        moves.append(
                            {
    ...
    def _determine_tools_location(:
        self, filename: str, path: str, parent_dir: str
    ) -> str:
        ...
        if any(
            ...
            for term in ["analyze", "analysis", "scan", "inventory", "report"]
        ):
            return f"tools/analysis/{filename}"

        if any(
            term in filename_lower
            for term in ["automate", "automation", "bot", "scraper"]
        ):
            return f"tools/automation/{filename}"

        if any(
            term in filename_lower for term in ["util", "utility", "helper", "helper"]
        ):
    ...
    def _determine_data_location(:
        ...
        if (
            ...
            or "analysis" in filename_lower
            or "analysis" in parent_lower
        ):
    ...
    def _determine_docs_location(:
        ...
        # Guides
        if any(
            term in filename_lower for term in ["guide", "tutorial", "howto", "how-to"]
        ):
    ...
    def _determine_directory_location(self, parent_dir: str, purpose: str) -> str:
        ...
        if purpose == "python_utilities":
            ...
            if "analysis" in parent_dir.lower():
                return parent_dir.replace("tools/", "tools/analysis/").replace(
                    "scripts/", "tools/analysis/"
                )
            elif "automation" in parent_dir.lower():
                return parent_dir.replace("tools/", "tools/automation/").replace(
                    "scripts/", "tools/automation/"
                )
            else:
                return parent_dir.replace("tools/", "tools/utilities/").replace(
    ...
    def generate_report(:
        self, directory_analysis: dict, file_moves: list, directory_moves: list
    ):
        """Generate comprehensive organization report."""
        report_file = (
            self.workspace_root / f"ADVANCED_ORGANIZATION_PLAN_{self.timestamp}.md"
        )
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                "**Method:** Parent-folder content awareness with context analysis\n\n"
            )
            f.write("---\n\n")
            ...
            purpose_summary = defaultdict(
                lambda: {"count": 0, "files": 0, "size_mb": 0}
            )
            for parent_dir, dir_info in directory_analysis.items():
                purpose = dir_info["purpose"]
            ...
            for purpose, stats in sorted(
                purpose_summary.items(), key=lambda x: x[1]["files"], reverse=True
            ):
                f.write(
                    f"| `{purpose}` | {stats['count']} | {stats['files']:,} | {stats['size_mb']:.1f} |\n"
                )
            f.write("\n")
            ...
            for project, info in sorted(
                projects_found.items(), key=lambda x: x[1]["file_count"], reverse=True
            ):
                action = (
                    "Keep in place (project structure)"
                    if self.project_patterns.get(project, {}).get("should_stay")
                    else "Review"
                )
                f.write(
                    f"| `{project}` | {len(info['directories'])} | {info['file_count']:,} | {info['total_mb']:.1f} | {action} |\n"
                )
            f.write("\n")
            ...
            for category, moves in by_category.items():
                f.write(
                    f"### {category.replace('_', ' ').title()} ({len(moves)} files)\n\n"
                )
                f.write("| Original Path | New Path | Parent Context | Reason |\n")
                f.write("|---------------|----------|---------------|--------|\n")
                for move in moves[:30]:
                    f.write(
                        f"| `{move['original_path']}` | `{move['new_path']}` | `{move['parent_context']}` | {move['reason']} |\n"
                    )
                if len(moves) > 30:
            ...
            if directory_moves:
                ...
                f.write(
                    "| Original Directory | New Directory | Files | Size (MB) | Reason |\n"
                )
                f.write(
                    "|-------------------|---------------|------:|----------:|--------|\n"
                )
                for move in directory_moves:
                    f.write(
                        f"| `{move['original_directory']}` | `{move['new_directory']}` | {move['file_count']} | {move['total_mb']:.1f} | {move['reason']} |\n"
                    )
                f.write("\n")
            ...
            f.write(
                "1. **Respect Project Structure** - Keep project directories intact\n"
            )
            f.write(
                "2. **Context-Aware Placement** - Files moved based on parent folder context\n"
            )
    ...
    def generate_migration_csv(self, file_moves: list, directory_moves: list):
        ...
        csv_file = (
            self.workspace_root
            / f"ADVANCED_ORGANIZATION_MIGRATION_{self.timestamp}.csv"
        )
...
def main():
    ...
    directory_analysis, file_relationships = organizer.analyze_parent_folder_context(
        db_path
    )

    # Generate intelligent moves
    file_moves = organizer.generate_intelligent_moves(
        directory_analysis, file_relationships
    )
    ...
    print("   - Respects project structures")
    print("   - Context-aware file placement")
    print("   - Preserves file relationships")
    print()
