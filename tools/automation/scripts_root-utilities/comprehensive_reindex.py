...
class ComprehensiveReindexer:
    ...
    def calculate_file_hash(:
        self, file_path: Path, max_size: int = 100 * 1024 * 1024
    ) -> str:
        ...
        try:
            ...
            if size > 100 * 1024 * 1024:  # > 100MB
                self.stats["large_files"].append(
                    {"path": rel_path, "size_mb": size / (1024 * 1024)}
                )
            ...
            if days_old < 30:
                self.stats["recent_files"].append(
                    {"path": rel_path, "days_old": days_old}
                )
            elif days_old > 365:
                self.stats["old_files"].append({"path": rel_path, "days_old": days_old})
        ...
        try:
            ...
            dir_data = {
                ...
                "parent_directory": str(
                    dir_path.parent.relative_to(self.workspace_root)
                )
                ...
                "is_nested": len(parts) > 3
                and any(parts[i] == parts[i + 1] for i in range(len(parts) - 1)),
            }
        ...
        with open(stats_csv, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow(
                [
                    "General",
                    "Total Size (GB)",
                    f"{self.stats['total_size'] / (1024**3):.2f}",
                ]
            )
            writer.writerow(
                ["General", "Large Files (>100MB)", len(self.stats["large_files"])]
            )
            writer.writerow(
                ["General", "Recent Files (<30 days)", len(self.stats["recent_files"])]
            )
            writer.writerow(
                ["General", "Old Files (>1 year)", len(self.stats["old_files"])]
            )
            ...
            # By type
            for file_type, data in sorted(
                self.stats["by_type"].items(), key=lambda x: x[1]["count"], reverse=True
            ):
                writer.writerow(
                    [
                        "By Type",
                        file_type,
                        f"{data['count']} files, {data['size'] / (1024**3):.2f} GB",
                    ]
                )
    ...
    def generate_json_index(self):
        ...
        index_data = {
            ...
            "statistics": {
                ...
                "by_type": {
                    k: {"count": v["count"], "size_gb": v["size"] / (1024**3)}
        ...
        for dir_data in self.dir_index:
            ...
            """,
                (
                    ...
                    dir_data["depth"],
                    dir_data["file_count"],
                    dir_data["subdirectory_count"],
                    int(dir_data["is_empty"]),
                    int(dir_data["is_nested"]),
                ),
            )
    ...
    def generate_report(self):
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- **Recent Files (<30 days):** {len(self.stats['recent_files'])}\n"
            )
            ...
            for file_type, data in sorted(
                ...
                f.write(
                    f"| `{file_type}` | {data['count']:,} | {data['size'] / (1024**3):.2f} |\n"
                )
            f.write("\n")
            ...
            if self.stats["large_files"]:
                ...
                for file_info in sorted(
                    self.stats["large_files"], key=lambda x: x["size_mb"], reverse=True
                )[:50]:
