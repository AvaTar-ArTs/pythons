        ...
        with open(output_path, "w", newline="", encoding="utf-8") as f:
            ...
            for file_hash, files in sorted_groups:
                ...
                for idx, file_info in enumerate(files_sorted):
                    ...
                    names_match = all(
                        f["name"] == files_sorted[0]["name"] for f in files_sorted
                    )
                    note = (
                        "Content-identical"
                        if names_match
                        else "Content-identical, different names"
                    )
...
def main():
    ...
    print(
        f"Total files in duplicates: {sum(len(files) for files in generator.duplicates.values() if len(files) > 1):,}"
    )
    print("=" * 80)
