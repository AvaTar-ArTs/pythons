...
def analyze_home_directory():
    ...
    for dir_name in major_dirs:
        ...
        file_counts = {"code": 0, "data": 0, "docs": 0, "media": 0, "config": 0}
        ...
        py_count = (
            len(py_result.stdout.strip().split("\n")) if py_result.stdout.strip() else 0
        )
    ...
    print("=" * 80)
    print(
        f"{'Directory':<20} {'Size':>10} {'Python':>8} {'Code':>8} {'Data':>8} {'Docs':>8} {'Media':>8}"
    )
    print("-" * 80)

    for r in results:
        print(
            f"{r['directory']:<20} {r['size_formatted']:>10} {r['python_files']:>8,} "
    ...
    print("-" * 80)
    print(
        f"{'TOTAL':<20} {format_size(total_size):>10} {total_py:>8,} "
    ...
    output_file = (
        home
        / "AVATARARTS"
        / f"HOME_DIRECTORY_ANALYSIS_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
    ...
    print(
        f"  Media Storage:     {format_size(media_size)} ({media_size / total_size * 100:.1f}%)"
    )
    print(
        f"  Code Storage:      {format_size(code_size)} ({code_size / total_size * 100:.1f}%)"
    )
