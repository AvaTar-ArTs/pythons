...
def generate_statistics(all_files):
    ...
    stats = {
        ...
        "by_location": defaultdict(
            lambda: {"count": 0, "size_mb": 0, "lines": 0, "code_lines": 0}
        ),
    }
    ...
    stats["duplicate_file_count"] = sum(
        len(v) - 1 for v in stats["duplicate_groups"].values()
    )
...
def main():
    ...
    print("-" * 70)
    print(
        f"{'Location':<20} {'Files':>10} {'Size (MB)':>12} {'Lines':>12} {'Code Lines':>12}"
    )
    print("-" * 70)

    for loc, data in sorted(
        stats["by_location"].items(), key=lambda x: x[1]["count"], reverse=True
    ):
        print(
            f"{loc:<20} {data['count']:>10,} {data['size_mb']:>12.2f} "
    ...
    if stats["duplicate_groups"]:
        ...
        with open(dup_csv, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["md5_hash", "file_count", "locations", "filenames", "file_paths"]
            )

            for hash_val, files in sorted(
                stats["duplicate_groups"].items(), key=lambda x: len(x[1]), reverse=True
            ):
