...
def calculate_statistics(analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Top quality files
    top_quality = sorted(analyses, key=lambda a: a["code_quality_score"], reverse=True)[
        :10
    ]

    # Files needing attention
    needs_attention = sorted(analyses, key=lambda a: a["code_quality_score"])[:10]
...
def main():
    ...
    parser.add_argument("json_report", type=Path, help="Path to JSON analysis report")
    parser.add_argument(
        "--output",
        type=Path,
