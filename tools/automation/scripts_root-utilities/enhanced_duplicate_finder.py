...
def find_more_duplicates(data: list) -> list:
    ...
    for filename, files in filename_groups.items():
        if len(files) > 1:
            ...
            sorted_files = sorted(
                files, key=lambda x: int(x.get("file_size", 0)), reverse=True
            )
    ...
    small_files = [
        row for row in data if int(row.get("file_size", 0)) < 100
    ]  # Less than 100 bytes
...
def find_config_duplicates(data: list) -> list:
    ...
    if len(config_files) > 1:
        ...
        sorted_configs = sorted(
            config_files, key=lambda x: int(x.get("file_size", 0)), reverse=True
        )
...
def main():
    ...
    # Summary
    total_removals = (
        len(filename_duplicates) + len(empty_files) + len(config_duplicates)
    )
