...
def generate_mapping_csv(duplicates, workspace_root: Path):
    ...
    with open(output_csv, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["original_path", "new_path", "filename", "size_mb", "waste_mb"],
        )
        writer.writeheader()
    ...
    for i, dup in enumerate(duplicates[:20], 1):
        ...
        print(
            f"       {dup['duplicate_count']} duplicates, {dup['waste_mb']:.2f} MB waste"
        )
...
def main():
    ...
    print("💡 Use this CSV to remove duplicates")
    print()
