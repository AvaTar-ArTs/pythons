...
def verify_group(csv_path, group_id):
    ...
    for file_info in files:
        ...
        if calculated_hash:
            ...
            match = (
                "✅ MATCH" if calculated_hash == file_info["hash"] else "❌ MISMATCH"
            )
    ...
    if len(set(hashes)) == 1:
        ...
        print("   Safe to delete duplicates (keep one, delete others)")
        return True
    else:
        print("❌ WARNING: Files have DIFFERENT content!")
        print("   DO NOT DELETE - these are different files")
...
def find_numbered_copies(csv_path):
    ...
    for group_id, files in groups.items():
        if len(files) >= 2:
            ...
            print("  ✅ Same Group_ID = Same Content (verified identical)")
            print()
