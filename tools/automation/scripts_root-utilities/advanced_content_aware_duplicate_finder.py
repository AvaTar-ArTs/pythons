...
class ContentAwareDuplicateFinder:
    ...
    def find_duplicates(self) -> list:
        ...
        print(
            f"   Total duplicate files: {sum(d['duplicate_count'] for d in duplicates):,}"
        )
        print(
            f"   Total waste: {sum(d['total_waste_mb'] for d in duplicates) / 1024:.2f} GB\n"
        )
    ...
    def generate_mapping_csv(self, duplicates: list) -> Path:
        ...
        output_csv = (
            self.workspace_root / f"ADVANCED_DUPLICATES_MAPPING_{self.timestamp}.csv"
        )
                ...
                mappings.append(
                    {
                        ...
                        "keep_reason": self._get_keep_reason(
                            dup_group["keep_file"], dup_file
                        ),
                    }
                )
    ...
    def _get_keep_reason(self, keep_file: dict, dup_file: dict) -> str:
        ...
        if (
            "archive" in dup_file["path"].lower()
            or "backup" in dup_file["path"].lower()
        ):
    ...
    def generate_report(self, duplicates: list, mapping_csv: Path) -> Path:
        ...
        report_file = (
            self.workspace_root / f"ADVANCED_DUPLICATES_REPORT_{self.timestamp}.md"
        )
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                "- **Nested Detection:** Identifies and penalizes nested folder patterns\n"
            )
            f.write(
                "- **File Type Awareness:** Handles text, image, and binary files appropriately\n\n"
            )
            ...
            for i, dup in enumerate(duplicates[:30], 1):
                ...
                f.write(
                    f"  - Location Priority: {dup['keep_file']['location_priority']}\n"
                )
