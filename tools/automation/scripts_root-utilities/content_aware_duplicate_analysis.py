...
class ContentAwareAnalyzer:
    ...
    def _analyze_pattern(self, files):
        ...
        return {
            ...
            "likely_legitimate": len(dir_patterns)
            > len(files) * 0.5,  # If in many different projects
        }
    ...
    def analyze_all_common_names(self, db_path: Path):
        ...
        for filename in top_names:
            print(
                f"Analyzing: {filename} ({common_names[top_names.index(filename)][1]} copies)..."
            )
            ...
            if result:
                ...
                print(
                    f"   Likely legitimate: {'Yes' if pattern['likely_legitimate'] else 'Maybe duplicates'}"
                )
                print()
    ...
    def generate_report(self, results: dict):
        ...
        report_file = (
            self.workspace_root / f"CONTENT_AWARE_ANALYSIS_{self.timestamp}.md"
        )
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                "Files with the same name in different projects are likely legitimate.\n\n"
            )
            f.write("---\n\n")
            ...
            for filename, result in results.items():
                ...
                f.write(
                    f"**Distinct Projects:** {result['analysis']['distinct_projects']}\n"
                )
                f.write(
                    f"**Likely Legitimate:** {'✅ Yes' if result['analysis']['likely_legitimate'] else '⚠️ Review needed'}\n\n"
                )
                ...
                for project, count in sorted(
                    result["analysis"]["project_distribution"].items(),
                    key=lambda x: x[1],
                    reverse=True,
                ):
                ...
                if result["exact_duplicates"]:
                    ...
                    for i, dup_group in enumerate(result["exact_duplicates"], 1):
                        ...
                        for dup_file in dup_group["files"]:
                            f.write(
                                f"  - `{dup_file['path']}` ({dup_file['size_mb']:.2f} MB)\n"
                            )
                        f.write("\n")
                ...
                if result["similar_content"]:
                    ...
                    for similar in result["similar_content"][:10]:
                        f.write(
                            f"- `{similar['file1']}` ↔ `{similar['file2']}` ({similar['similarity'] * 100:.1f}% similar)\n"
                        )
                    f.write("\n")
                ...
                if result["exact_duplicates"] or result["similar_content"]:
                    ...
                    if result["exact_duplicates"]:
                        f.write(
                            "**Action:** Remove exact duplicates, keeping one copy per project.\n\n"
                        )
                    if result["similar_content"]:
                        f.write(
                            "**Action:** Review similar files - may be templates or need consolidation.\n\n"
                        )
                else:
                    f.write("### ✅ Conclusion\n\n")
                    f.write(
                        "**No duplicates found.** Files with this name serve different purposes in different projects.\n\n"
                    )
...
def main():
    ...
    print("=" * 80)
    print(f"\n📄 Report: {report.name}")
    print("💡 This analysis properly distinguishes between:")
    print("   - Legitimate files with same names in different projects")
    print("   - True duplicates that should be removed")
    print()
