...
def find_all_duplicates(workspace_root: Path):
    ...
    for file_path in workspace_root.rglob("*"):
        ...
        for i in range(len(parts) - 1):
            if parts[i] == parts[i + 1]:
                pattern = parts[i]
                nested_patterns[pattern].append({"path": rel_path, "depth": len(parts)})
                break
    ...
    for filename, files in name_groups.items():
        if len(files) > 1:
            ...
            for size, size_files in size_groups.items():
                if len(size_files) > 1:
                    size_files_sorted = sorted(
                        size_files, key=lambda x: x["path"].count("/")
                    )
    ...
    if nested_patterns:
        ...
        for pattern, files in sorted(
            nested_patterns.items(), key=lambda x: len(x[1]), reverse=True
        )[:10]:
            print(
                f"   - {pattern}: {len(files)} files (max depth: {max(f['depth'] for f in files)})"
            )
...
def generate_csv(duplicates, workspace_root: Path):
    ...
    with open(output_csv, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["original_path", "new_path", "filename", "method", "size_mb"]
        )
        writer.writeheader()
...
def main():
    ...
    if duplicates:
        ...
        print(f"{'=' * 80}")
        print(f"\n📄 CSV: {csv_file.name}")
        print("💡 Run: python3 execute_consolidation_auto.py to remove duplicates")
    else:
        print("\n✅ No duplicates found!")
