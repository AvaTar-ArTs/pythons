    ...
    try:
        ...
        if file1.suffix == ".zip" and file2.suffix == ".zip":
            result1 = subprocess.run(
                ["unzip", "-l", str(file1)], capture_output=True, text=True, timeout=10
            )
            result2 = subprocess.run(
                ["unzip", "-l", str(file2)], capture_output=True, text=True, timeout=10
            )
            ...
            with open(diff_file, "w") as f:
                ...
                f.write(
                    f"  Modified: {datetime.fromtimestamp(file1.stat().st_mtime)}\n"
                )
                f.write(f"\nFile 2: {file2}\n")
                f.write(f"  Size: {get_file_size(file2)} bytes\n")
                f.write(
                    f"  Modified: {datetime.fromtimestamp(file2.stat().st_mtime)}\n"
                )
                f.write(
                ...
                rows.append(
                    {
                        ...
                        "hash": get_file_hash(file)[:8]
                        if get_file_hash(file)
                        else "N/A",
                    }
                )
...
def main():
    ...
    if rows:
        ...
        print("   3. Decide which files to keep/remove")
    else:
