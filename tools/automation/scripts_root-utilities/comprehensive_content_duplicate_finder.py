...
class ComprehensiveContentDuplicateFinder:
    ...
    def find_all_duplicates(self, db_path: Path):
        ...
        print(
            f"   Total duplicate files: {sum(d['duplicate_count'] for d in all_duplicates):,}"
        )
    ...
    def generate_mapping_csv(self, duplicates: list) -> Path:
        ...
        output_csv = (
            self.workspace_root
            / f"COMPREHENSIVE_DUPLICATES_MAPPING_{self.timestamp}.csv"
        )
        ...
        for dup_group in duplicates:
            ...
            keep_score = self.calculate_location_score(
                keep_path, dup_group["keep_file"]["depth"]
            )
            ...
            for dup_file in dup_group["duplicate_files"]:
                ...
                if (
                    "backup" in dup_file["path"].lower()
                    or "archive" in dup_file["path"].lower()
                ):
    ...
    def generate_report(:
        self, duplicates: list, total_waste_mb: float, mapping_csv: Path
    ):
        """Generate detailed report."""
        report_file = (
            self.workspace_root / f"COMPREHENSIVE_DUPLICATES_REPORT_{self.timestamp}.md"
        )
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write("**Workspace:** `/Users/steven/AVATARARTS`\n\n")
            f.write(
            ...
            for file_type, stats in sorted(
                type_stats.items(), key=lambda x: x[1]["waste_mb"], reverse=True
            ):
                f.write(
                    f"| `{file_type}` | {stats['count']} | {stats['waste_mb']:.2f} |\n"
                )
            f.write("\n")
