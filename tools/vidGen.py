...
def renderVideo(video, rendering_screen):
    ...
    for i, clip in enumerate(clips):
        if clip.isUsed:
            ...
            if not clip.isInterval and not clip.isIntro:
                os.system(
                    f'ffmpeg -i "{mp4path}" -vf "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,setsar=1" "{settings.vid_finishedvids}/{mp4name}temp.mp4"',
                )
                os.system(
                    f'ffmpeg -i "{settings.vid_finishedvids}/{mp4name}temp.mp4" -filter:v fps=fps={chosenFps} "{settings.vid_finishedvids}/{mp4name}_finished.mp4"',
                )
    ...
    vid_concat.writelines(f"file {files}\n" for files in toCombine)
    vid_concat.close()

    os.system(
        f'ffmpeg -safe 0 -f concat -segment_time_metadata 1 -i concat.txt -vf select=concatdec_select -af aselect=concatdec_select,aresample=async=1 "{settings.final_video_path}/{videoName}_{current_date}.mp4"',
    )
