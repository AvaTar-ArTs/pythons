    ...
    def create_embedding(self, content: str) -> List[float]:
        ...
        try:
            response = self.openai.embeddings.create(
                model="text-embedding-3-large", input=content[:8000]
            )
    ...
    def semantic_search(self, query: str, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            results = self.qdrant.search(
                collection_name="documents", query_vector=query_embedding, limit=limit
            )
...
def main():
    ...
    parser.add_argument(
        "--analyze-all", action="store_true", help="Analyze all documents"
    )
    parser.add_argument(
        "--process-new", action="store_true", help="Process new/modified files"
    )
