...
class EnvDAnalyzer:
    ...
    def analyze_organization(self):
        ...
        for file_name, file_data in (
            self.results["file_analysis"].get("files", {}).items()
        ):
        ...
        self.results["organization_analysis"] = {
            ...
            "avg_file_size": sum(organization["file_sizes"].values()) / len(files)
            if files
            else 0,
            "avg_keys_per_file": sum(organization["key_distribution"].values())
            / len(files)
            if files
            else 0,
        }
    ...
    def generate_recommendations(self):
        ...
        large_files = [
            f
            for f, data in file_analysis.get("files", {}).items()
    ...
    def generate_summary_report(self, output_path: Path):
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- **Sensitive Keys Found:** {len(security.get('sensitive_keys', []))}\n"
            )
            f.write(
                f"- **Permission Issues:** {len(security.get('potential_issues', []))}\n\n"
            )
            ...
            f.write(
                f"- **Average File Size:** {org.get('avg_file_size', 0) / 1024:.2f} KB\n"
            )
            f.write(
                f"- **Average Keys per File:** {org.get('avg_keys_per_file', 0):.1f}\n\n"
            )
            ...
            if duplicates:
                ...
                f.write(
                    f"Found {len(duplicates)} keys appearing in multiple files:\n\n"
                )
