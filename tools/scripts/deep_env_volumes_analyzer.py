...
class DeepEnvVolumesAnalyzer:
    ...
    def analyze_volume(self, volume_path: Path) -> VolumeAnalysis:
        ...
        intelligence = {
            "dominant_file_type": max(file_types.items(), key=lambda x: x[1])[0]
            if file_types
            else None,
            "dominant_category": max(content_categories.items(), key=lambda x: x[1])[0]
    ...
    def generate_insights(:
        ...
        for vol_name, vol_analysis in volumes_analysis.items():
            resource_mapping["content_distribution"][vol_name] = (
