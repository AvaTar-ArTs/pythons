...
class CostTracker:
    ...
    def record_cost(self, api_name: str, cost: float):
        ...
        if self.daily_spend > max_daily * warn_threshold:
            self.logger.warning(
                f"Daily spend warning: ${self.daily_spend:.2f} / ${max_daily:.2f}"
            )
    ...
    def _parse_response(self, response) -> GoogleVisionResults:
        ...
        if response.label_annotations:
            results.labels = [
                ...
                if label.score
                >= self.config.get("google_vision", "confidence_threshold")
            ]
...
class OpenAIClient:
    ...
    def analyze_image(:
        ...
        image_data: str,
        mime_type: str,
        google_results: GoogleVisionResults = None,
    ) -> OpenAIResults:
        ...
        try:
            ...
            context = (
                self._build_context_from_google(google_results)
                if google_results
                else ""
            )
            ...
            messages = [
                {
                    "role": "system",
                    "content": self.config.get("prompts", "openai_system"),
                },
                {
                    "role": "user",
                    "content": [
...
class HybridImageProcessor:
        ...
        try:
            with Image.open(image_path) as img:
                ...
                metadata.aspect_ratio = (
                    round(img.width / img.height, 2) if img.height > 0 else None
                )
    ...
    def process_single_image(:
        ...
        try:
            ...
            combined_analysis = self._combine_results(
                google_results, openai_results, metadata
            )
        ...
        if self.config.get("image", "resize_large_images"):
            try:
                ...
                if max(img.width, img.height) > max_dim:
                    ...
                    self.logger.info(
                        f"Resized {image_path.name} to {new_width}x{new_height}"
                    )
    ...
    def _should_use_openai(:
        ...
        complexity_indicators = self.config.get(
            "analysis_rules", "complexity_indicators"
        )
        ...
        if metadata.aspect_ratio and (
            metadata.aspect_ratio < 0.5 or metadata.aspect_ratio > 2.0
        ):
    ...
    def _combine_results(:
        ...
        # From Google Vision
        if google_results.labels:
            analysis.primary_objects = [
                label["description"] for label in google_results.labels[:5]
            ]
        ...
        analysis.suggested_products = list(
            set(products[:5])
...
class CSVWriter:
    ...
    def write_results(:
        ...
        cost_file = (
            self.output_path.parent / f"{self.output_path.stem}_cost_summary.json"
        )
    ...
    def _result_to_row(self, result: ProcessingResult) -> Dict[str, Any]:
        ...
        row = {
            ...
            # Combined analysis
            "primary_objects": json.dumps(ca.primary_objects)
            if ca.primary_objects
            else "[]",
            ...
            "openai_used": bool(
                result.openai_results.style_description
                or result.openai_results.seo_title
            ),
            ...
            "openai_cost": result.api_costs.get("openai", 0.0)
            if result.api_costs
            else 0.0,
            "total_api_cost": sum(result.api_costs.values())
            if result.api_costs
            else 0.0,
        }
...
class HybridImagePipeline:
    ...
    def run(self, input_folder: Path, output_csv: Path) -> bool:
        ...
        try:
            ...
            if max_workers > 1:
                results = self._process_concurrent(
                    image_paths, input_folder, max_workers
                )
            else:
            ...
            self.logger.info("=" * 60)
            self.logger.info(
                f"Images processed: {successful}/{len(results)} successfully"
            )
            ...
            self.logger.info(
                f"Total time: {total_time:.1f}s, Average per image: {avg_time:.1f}s"
            )
    ...
    def _process_sequential(:
        ...
        with tqdm(image_paths, desc="Processing images", unit="image") as pbar:
            for image_path in pbar:
                ...
                pbar.set_postfix(
                    {"current": image_path.name, "cost": f"${cost_so_far:.3f}"}
                )
    ...
    def _process_concurrent(:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_path = {
                executor.submit(
                    self.processor.process_single_image, path, base_folder
                ): path
            ...
            with tqdm(
                ...
                for future in as_completed(future_to_path):
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Unexpected error processing {image_path.name}: {e}"
                        )
...
Environment Variables:
  ...
  GOOGLE_APPLICATION_CREDENTIALS: Path to Google Cloud credentials JSON
    ...
    parser.add_argument(
        "input_folder", type=Path, help="Directory containing images to analyze"
    )
    ...
    parser.add_argument(
        ...
        "--config",
        type=Path,
        help="Configuration file path (default: hybrid_config.json)",
    )

    parser.add_argument(
        "--google-only",
        action="store_true",
        help="Use only Google Vision API (faster, cheaper)",
    )
