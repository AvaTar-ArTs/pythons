#!/usr/bin/env python3
"""
Precise script to rename UUID-named directories and files to proper song titles
...
def parse_suno_csv_for_mappings(csv_file_path):
    ...
    Focus on the specific format seen in the grep results.
    """
    uuid_mapping = {}
    ...
    # Also look for the pattern where title appears after more commas
    # Pattern: UUID,,,,,title,,,,,,
    for line in lines:
        if not line.strip():
            continue

        # Match UUID followed by 5 commas and then a title
        pattern = r'([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}),*?,*?,*?,*?,*?([^,\n"]+?)(?:,|$)'
        matches = re.findall(pattern, line)
