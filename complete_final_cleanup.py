...
class CompleteFinalCleanup:
    ...
    def get_all_root_items(self):
        ...
        protected_dirs = {
            ...
            'MEDIA_PROCESSING', 'DOCUMENTATION',
            'experiments', 'notebooks', 'tests',
            '_archived_cleanup', '_organized_root_files',
        ...
        for item in all_items:
            if item not in self.cleanup_map:
                ...
                if item.endswith('.py'):
                    ...
                    if any(word in item_lower for word in ['bot', 'automation', 'scraper', 'reddit', 'instagram', 'twitch', 'youtube']):
                        self.cleanup_map[item] = 'AUTOMATION_BOTS/legacy_categories/' + item
                    elif any(word in item_lower for word in ['video', 'audio', 'media', 'music', 'spotify', 'mp3', 'voice']):
                        self.cleanup_map[item] = 'MEDIA_PROCESSING/legacy_categories/' + item
                    else:
