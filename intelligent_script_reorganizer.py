...
class IntelligentScriptReorganizer:
    ...
    def _determine_functional_cluster(self, desc: str, phrases: List[str], result) -> str:
        ...
        # Image processing
        if any(term in all_text for term in ['image', 'img', 'convert', 'resize', 'optimize']):
        ...
        if any(term in all_text for term in ['git', 'github', 'commit', 'push', 'pull']):
            return "git-operations"

        # Deployment
        if any(term in all_text for term in ['deploy', 'publish', 'release', 'production']):
            return "deployment"
        ...
        # Database operations
        if any(term in all_text for term in ['database', 'db', 'sql', 'postgres', 'mysql']):
            return "database"

        # API/web operations
        if any(term in all_text for term in ['api', 'curl', 'http', 'request', 'webhook']):
    ...
    def save_analysis_report(self, clusters: Dict, structure: Dict):
        ...
        report = {
            ...
            'cluster_summary': {
                name: len(files) for name, files in clusters.items()
            },
