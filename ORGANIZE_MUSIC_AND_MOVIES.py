...
if loose_videos or loose_audio or loose_images:
    ...
    if loose_other_movies:
        print(f"  - Create 'Loose_Files/' folder for {len(loose_other_movies)} other files")
    print()
        ...
        for script in loose_other:
            if script.suffix.lower() in ['.py', '.txt', '.md']:
                try:
                    ...
                    if new_path.exists():
                        new_path = scripts_dir / f"{script.stem}_{timestamp}{script.suffix}"
                    shutil.move(str(script), str(new_path))
                    with open(backup_log, 'a') as log:
                        log.write(f"{script},{new_path},script,{datetime.now().isoformat()}\n")
                    moved_count += 1
                except Exception as e:
    ...
    if loose_videos:
        ...
        for video in loose_videos:
            try:
                ...
                with open(backup_log, 'a') as log:
                    log.write(f"{video},{new_path},video,{datetime.now().isoformat()}\n")
                moved_count += 1
            except Exception as e:
    ...
    if loose_audio:
        ...
        for audio in loose_audio:
            try:
                ...
                with open(backup_log, 'a') as log:
                    log.write(f"{audio},{new_path},audio,{datetime.now().isoformat()}\n")
                moved_count += 1
            except Exception as e:
    ...
    if loose_other_movies:
        ...
        for other in loose_other_movies:
            try:
                ...
                with open(backup_log, 'a') as log:
                    log.write(f"{other},{new_path},other,{datetime.now().isoformat()}\n")
                moved_count += 1
            except Exception as e:
