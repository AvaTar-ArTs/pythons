...
class BlockchainChainOfCustody:
    ...
    def __init__(self, evidence_id: str):
        ...
        logger.info(
            f"Blockchain chain of custody initialized for evidence {evidence_id}"
        )
    ...
    def register_handler(:
        self, handler_id: str, name: str, role: str, biometric_data: bytes = None
    ):
        """
    ...
    def transfer_custody(:
        ...
        logger.info(
            f"Custody transfer recorded: {handler_from} → {handler_to} ({action})"
        )
    ...
    def _verify_biometric(self, handler_id: str, signature: BiometricSignature) -> bool:
    ...
    def verify_chain_integrity(self) -> Dict:
        ...
        if issues:
            logger.error(
                f"Chain integrity verification FAILED: {len(issues)} issues found"
            )
            return {"valid": False, "issues": issues, "chain_length": len(self.chain)}
        else:
            logger.info(
                f"Chain integrity verification PASSED: {len(self.chain)} blocks verified"
            )
            return {
                "valid": True,
                "issues": [],
            ...
            history.append(
                {
                    ...
                    "action": block.action,
                    "from": self.authorized_handlers.get(block.handler_from, {}).get(
                        "name", block.handler_from
                    ),
                    "to": self.authorized_handlers.get(block.handler_to, {}).get(
                        "name", block.handler_to
                    ),
                    "notes": block.notes,
...
def demo_blockchain_custody():
    ...
    for i, entry in enumerate(history, 1):
        ...
        print("   Verified: ✓" if entry["verified"] else "   Verified: ✗")
