...
class SmartContentAnalyzer:
    ...
    def _detect_duplicates(self):
        ...
        for content_hash, paths in hash_groups.items():
            if len(paths) > 1:
                ...
                for path in paths:
                    self.content_items[path].duplicates = [
        ...
        for i, path1 in enumerate(paths):
            for path2 in paths[i + 1 :]:
                similarity = self._calculate_similarity(
                    self.content_items[path1].content, self.content_items[path2].content
                )
    ...
    def _generate_report(self) -> Dict[str, Any]:
        ...
        # Top topics
        top_topics = sorted(
            self.topic_index.items(), key=lambda x: len(x[1]), reverse=True
        )[:10]
        ...
        quality_distribution = {
            ...
            "medium": sum(
                1 for score in self.quality_scores.values() if 0.4 <= score <= 0.7
            ),
...
def main():
    ...
    directories = ["Documents", "ai-sites", "claude", "reports"]
    ...
    total_duplicates = sum(
        r["summary"]["duplicate_files"] for r in all_results.values()
    )
