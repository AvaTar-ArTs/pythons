...
class SunoFolderCleaner:
        ...
        for file1 in self.folder1_files:
            ...
            if file1["hash"] and file1["hash"] in folder2_hashes:
                self.duplicates.append(
                    {"file1": file1, "file2": folder2_hashes[file1["hash"]]}
                )
            ...
            # Check if duplicate by name
                ...
                self.duplicates.append(
                    {"file1": file1, "file2": folder2_names[file1["filename"].lower()]}
                )
                self.stats["duplicates"] += 1
    ...
    def merge_folders(self):
        ...
        print(
            f"?? Moving {len(self.unique_to_folder1)} unique files from SUNO folder..."
        )
        print()
        ...
        for file_info in self.unique_to_folder1:
            try:
                ...
                if self.stats["moved"] % 10 == 0:
                    print(
                        f"   Moved {self.stats['moved']}/{len(self.unique_to_folder1)}...",
                        end="\r",
                    )
    ...
    def generate_report(self):
        ...
        report_file = (
            ...
            / "workspace"
            / "music-empire"
            / f"SUNO_FOLDERS_CLEANUP_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
