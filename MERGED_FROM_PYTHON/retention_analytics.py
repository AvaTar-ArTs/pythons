...
class RetentionAnalytics:
    ...
    def __init__(self, db_path: str = None):
        self.db_path = (
            db_path
            or "/Users/steven/ai-sites/retention-products-suite/analytics-tracking/retention_analytics.db"
        )
        self.setup_database()
    ...
    def generate_retention_report(:
        ...
        # Get top features
        top_features = sorted(
            feature_usage, key=lambda x: x["total_usage"], reverse=True
        )[:5]

        # Generate recommendations
        recommendations = self._generate_recommendations(
            retention_metrics, feature_usage
        )
    ...
    def _generate_recommendations(:
        self, retention_metrics: List[RetentionMetrics], feature_usage: List[Dict]
    ) -> List[str]:
        ...
        if avg_day_1_retention < 0.3:
            recommendations.append(
                "🚨 CRITICAL: Day 1 retention is below 30%. Focus on improving onboarding experience and first-time user value."
            )
        elif avg_day_1_retention < 0.5:
            recommendations.append(
                "⚠️ WARNING: Day 1 retention is below 50%. Consider improving welcome flow and immediate value delivery."
            )

        if avg_day_7_retention < 0.2:
            recommendations.append(
                "📈 Focus on improving Day 7 retention through engagement campaigns and habit formation."
            )

        if avg_day_30_retention < 0.1:
            recommendations.append(
                "🎯 Implement long-term retention strategies including community features and advanced functionality."
            )
        ...
        if feature_usage:
            ...
            if underused_features:
                recommendations.append(
                    f"🔧 Consider promoting or improving {len(underused_features)} underused features."
                )

        if not recommendations:
            recommendations.append(
                "✅ Retention metrics are performing well. Continue current strategies and monitor for optimization opportunities."
            )
...
def main():
    ...
    print(
        f"   - Average Day 1 Retention: {report['retention_metrics']['average_day_1_retention']}%"
    )
    print(
        f"   - Average Day 7 Retention: {report['retention_metrics']['average_day_7_retention']}%"
    )
    print(
        f"   - Average Day 30 Retention: {report['retention_metrics']['average_day_30_retention']}%"
    )
