...
from core.dna_matcher import DNAMatcher
...
def demo_dna_matching():
    ...
    for i, match in enumerate(matches[:5], 1):
        ...
        print(
            f"   Shared DNA: {match.shared_cm:.1f} cM ({match.num_segments} segments)"
        )
...
def demo_probability_simulation():
    ...
    db_result = simulator.simulate_database_search(
        query_relationship="3rd_cousin", database_size=1000000, num_simulations=500
    )
...
def demo_case_management():
    ...
    evidence.add_custody_entry(
        "Det. Johnson", "collected", "Initial collection at scene"
    )
    evidence.add_custody_entry(
        "Lab Tech Martinez", "received", "Delivered to crime lab"
    )
    evidence.add_custody_entry(
        "Lab Tech Martinez", "processed", "DNA extraction completed"
    )
