...
class MultiAIClient:
    ...
    def _create_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "anthropic": {
                ...
                "models": {"claude3": "claude-3-sonnet-20240229"},
            },
            "suno": {
    ...
    async def _generate_dalle3_images(:
        self, request: AIGenerationRequest
    ) -> List[AIGenerationResult]:
    ...
    async def _generate_suno_music(:
        ...
        try:
            ...
            async with self.session.post(
                f"{suno_config['base_url']}/music/generate", headers=headers, json=data
            ) as response:
                if response.status == 200:
                    result = await response.json()
    ...
    def _process_dalle3_response(:
        self, response: Dict[str, Any], request: AIGenerationRequest
    ) -> List[AIGenerationResult]:
    ...
    def get_generation_stats(self) -> Dict[str, Any]:
        ...
        avg_quality = (
            sum(result.quality_score for result in self.generation_history)
            / total_generations
        )
        ...
        return {
            ...
            "services_used": list(
                set(result.service for result in self.generation_history)
            ),
        }
