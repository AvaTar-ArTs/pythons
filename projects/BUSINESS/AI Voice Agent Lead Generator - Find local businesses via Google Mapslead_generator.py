...
def export_leads(leads, industry, location):
    ...
    csv_file = (
        OUTPUT_DIR
        / f"leads_{industry.replace(' ', '_')}_{location.replace(' ', '_')}_{timestamp}.csv"
    )
...
""")
        for industry, data in TARGET_INDUSTRIES.items():
            print(
                f"  - {industry:20s} (avg ticket: ${data['avg_ticket']}, priority: {data['priority']})"
            )
...
""")
    ...
    results = search_google_maps(
        industry, location, radius=radius_meters, max_results=20
    )
    ...
    for business in results:
        ...
        print(
            f"   Score: {score}/10 | Phone: {business['phone']} | Reviews: {business.get('reviews', 0)}"
        )
    ...
    pitches_file = (
        OUTPUT_DIR
        / f"pitches_{industry.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M')}.txt"
    )
    ...
    for i, lead in enumerate(leads[:3], 1):
        print(
            f"  {i}. {lead['name']} (Score: {lead['score']}/10, ${lead['potential_revenue']:,}/mo potential)"
        )
