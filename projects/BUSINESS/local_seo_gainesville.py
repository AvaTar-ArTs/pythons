...
"""
    ...
    for neighborhood in areas.keys():
        guide += (
            f"- **{neighborhood}:** Local market, less competition, community-focused\n"
        )
...
def create_pitch_template(:
    business_name, industry, city, phone="", website="", rating="N/A", reviews=0
):
...
Industries:'\'')
        for industry, data in FLORIDA_INDUSTRIES.items():
            print(
                f"  {industry:15s} ${data['avg_ticket']:4d} avg ticket, {data['priority']:6s} priority"
            )
...
""")
    ...
    if area not in ["gainesville", "ocala"]:
        print("⚠️  Area must be 'gainesville' or 'ocala'")
        sys.exit(1)
    ...
    print("   2. Click Google Maps links (or search manually)")
    print(f"   3. Fill out: {template_file}")
    print(f"   4. Review pitch: {pitch_file}")
    print("   5. Start outreach to top 20 leads")

    print("\n💡 Target: 50 leads → 10 contacted → 3-5 clients → $1,200-2,000/month!")

    print(f"\n📍 Focusing on {area.title()} gives you:")
    print("   - Local credibility (you're nearby)")
    print("   - Easy meetings (drive over)")
    print("   - Word-of-mouth referrals")
    print("   - Dominate local market first")
