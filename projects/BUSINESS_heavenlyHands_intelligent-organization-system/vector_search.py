...
try:
    ...
    print(
        "Warning: Vector database libraries not available. Some features will be limited."
    )
...
class AdvancedVectorSearch:
    ...
    def _initialize_chroma(self):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Loaded existing ChromaDB collection with {self.chroma_collection.count()} documents"
                )
            except:
    ...
    def _add_to_vector_db(:
        self, content_id: str, embedding: np.ndarray, metadata: Dict[str, Any]
    ):
    ...
    def _search_faiss(:
        self, query_embedding: np.ndarray, search_query: SearchQuery
    ) -> List[SearchResult]:
        ...
        try:
            # Search FAISS index
            k = min(
                search_query.limit * 2, self.faiss_index.ntotal
    ...
    def _search_chroma(:
        self, query: str, search_query: SearchQuery
    ) -> List[SearchResult]:
    ...
    def _apply_content_aware_ranking(:
    ...
    def _calculate_content_type_boost(:
        self, content_type: str, query_type: str
    ) -> float:
    ...
    def _generate_relevance_explanation(:
        self, query_embedding: Optional[np.ndarray], score: float
    ) -> str:
    ...
    def _apply_filters(:
        ...
        for result in results:
            ...
            if "path_patterns" in filters:
                if not any(
                    pattern in result.file_path for pattern in filters["path_patterns"]
                ):
                    continue
    ...
    def _log_search_query(:
        self, search_query: SearchQuery, results_count: int, execution_time: float
    ):
        ...
        try:
            ...
            for file_path in files:
                try:
                    ...
                    # Generate content ID
                    content_id = (
                        f"{project_path.name}_{file_path.relative_to(project_path)}"
                    )

                    # Add to index
                    if self.add_content(
                        content_id, str(file_path), content, content_type
                    ):
                        indexed_count += 1
