...
class AdvancedASTAnalyzer:
    ...
    def _detect_patterns(self, content: str, file_path: str) -> List[CodePattern]:
        ...
        for category, rules in self.pattern_rules.items():
            for rule in rules:
                matches = re.finditer(
                    rule["pattern"], content, re.MULTILINE | re.DOTALL
                )
    ...
    def _calculate_pattern_confidence(:
        self, match, rule: Dict, content: str, file_path: str
    ) -> float:
        ...
        if file_extension in [".py", ".js", ".ts"] and rule["name"] in [
            "MVC Pattern",
            "Repository Pattern",
        ]:
            context_bonus += 0.1
    ...
    def _calculate_architectural_significance(:
        self, pattern_name: str, content: str
    ) -> float:
    ...
    def _calculate_maintainability_score(:
        self, pattern_text: str, content: str
    ) -> float:
        ...
        comment_ratio = len(re.findall(r"#.*$", pattern_text, re.MULTILINE)) / max(
            1, pattern_text.count("\n")
        )
        comment_score = min(1, comment_ratio * 2)

        # Check for descriptive names
        descriptive_names = len(
            re.findall(r"[a-z][a-zA-Z]*[A-Z][a-zA-Z]*", pattern_text)
        )
    ...
    def _calculate_quality_metrics(:
        ...
        if tree and metrics["function_count"] > 0:
            ...
            total_function_lines = sum(
                len(ast.get_source_segment(content, node) or "").splitlines()
                for node in functions
            )
            metrics["avg_function_length"] = (
                total_function_lines / metrics["function_count"]
            )
        else:
    ...
    def analyze_project(self, project_path: str) -> ProjectIntelligence:
        ...
        recommendations = self._generate_recommendations(
            semantic_analyses, quality_metrics
        )
    ...
    def _calculate_project_quality_metrics(:
        self, analyses: List[SemanticAnalysis]
    ) -> Dict[str, float]:
        """Calculate project-wide quality metrics."""
        if not analyses:
            return {}
        ...
        # Aggregate metrics
        total_loc = sum(
            analysis.quality_metrics.get("lines_of_code", 0) for analysis in analyses
        )
        total_complexity = sum(analysis.ast_complexity for analysis in analyses)
        total_functions = sum(
            analysis.quality_metrics.get("function_count", 0) for analysis in analyses
        )
        total_classes = sum(
            analysis.quality_metrics.get("class_count", 0) for analysis in analyses
        )

        avg_comment_ratio = (
            sum(
                analysis.quality_metrics.get("comment_ratio", 0)
        ...
        avg_duplication = (
            sum(
                analysis.quality_metrics.get("duplication_ratio", 0)
                for analysis in analyses
            )
            / total_files
        )
    ...
    def _generate_recommendations(:
        self, analyses: List[SemanticAnalysis], quality_metrics: Dict[str, float]
    ) -> List[str]:
        ...
        if quality_metrics.get("average_comment_ratio", 0) < 0.1:
            recommendations.append(
                "Increase code documentation and comments for better maintainability"
            )
    ...
    def _identify_automation_opportunities(:
        ...
        low_doc_files = [
            a for a in analyses if a.quality_metrics.get("comment_ratio", 0) < 0.1
        ]
    ...
    def semantic_search(:
        self, query: str, project_path: str = None
    ) -> List[Dict[str, Any]]:
                ...
                for file_path, analysis_data in cursor.fetchall():
                    ...
                    if analysis.get("semantic_embedding"):
                        ...
                        file_embedding = np.frombuffer(
                            analysis["semantic_embedding"], dtype=np.float32
                        )
