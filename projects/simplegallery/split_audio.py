...
def split_file(:
    file_path: Path, out_dir: Path, chunk_ms: int, bitrate: Optional[str]
) -> list[Path]:
...
def main():
    ap = argparse.ArgumentParser(
        description="Split long audio into smaller MP3 chunks for transcription."
    )
    ap.add_argument("files", nargs="+", help="Input audio files (mp3/wav)")
    ap.add_argument(
        "-o", "--out-dir", default="output_chunks", help="Output directory root"
    )
    ap.add_argument(
        "--chunk-seconds", type=int, default=None, help="Fixed chunk length (seconds)"
    )
    ap.add_argument(
        "--target-size-mb",
        type=float,
        default=24.0,
        help="Approx target size per chunk (MB)",
    )
