...
class ConfigValidator:
    ...
    @staticmethod
    def validate_config(:
        config: Dict[str, Any], gallery_root: str
    ) -> tuple[bool, List[str]]:
        """
        ...
        if "templates_path" in config:
            if not os.path.exists(config["templates_path"]):
                errors.append(
                    f"Templates path does not exist: {config['templates_path']}"
                )
        ...
        if "thumbnail_height" in config:
            try:
                ...
                if height < 50 or height > 1000:
                    errors.append(
                        f"thumbnail_height must be between 50 and 1000, got {height}"
                    )
            except (ValueError, TypeError):
                errors.append(
                    f"thumbnail_height must be an integer, got {config['thumbnail_height']}"
                )
    ...
    def apply_defaults(config: Dict[str, Any], gallery_root: str) -> Dict[str, Any]:
        ...
        if not config.get("title") or config.get("title") == os.path.basename(
            gallery_root
        ):
        ...
        if (
            not config.get("description")
            or config.get("description") == "Default description of my gallery"
        ):
    ...
    @staticmethod
    def migrate_config(:
        config: Dict[str, Any], from_version: str = "2.0"
    ) -> Dict[str, Any]:
        """
