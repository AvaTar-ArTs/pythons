    ...
    while True:
        ...
        if status and isinstance(status, str):
            ...
            if status_lower in {"failed", "error", "cancelled"}:
                raise RuntimeError(
                    f"Job failed with status={status}: {json.dumps(data)[:400]}"
                )
        if time.time() - start > timeout:
            raise TimeoutError(
                f"Timeout waiting for job. Last response: {json.dumps(data)[:400]}"
            )
        time.sleep(interval)
...
def main():
    ...
    ap.add_argument(
        "--model-id",
        default="ac614f96-1082-45bf-be9d-757f2d31c174",
    ...
    if not generation_id:
        print("❌ No generationId in response.", file=sys.stderr)
        sys.exit(1)
    ...
    if not images:
        print("❌ No generated_images!", file=sys.stderr)
        sys.exit(1)
    ...
    if r.status_code >= 400:
        print(f"❌ Motion request failed: {r.status_code} {r.text}", file=sys.stderr)
        sys.exit(1)
    mv_req = r.json()
    ...
    if not mv_gen_id:
        print("❌ No motion generationId.", file=sys.stderr)
        sys.exit(1)
