...
def main():
    ...
    general_group.add_argument(
        "--dry_run", "-n", action="store_true", help=dry_run_help
    )
    general_group.add_argument(
        "--source_directory", default=os.getcwd(), help=source_directory_help
    )
    general_group.add_argument(
    ...
    general_group.add_argument(
        "--noninteractive", default=False, action="store_true", help=noninteractive_help
    )
    general_group.add_argument(
        "--upload_batch_limit", type=int, default=100, help=upload_batch_limit_help
    )

    # YouTube Options
    yt_group = parser.add_argument_group("YouTube Options")

    yt_client_secrets_file_help = "Mandatory: File path to youtube client secrets file. Example: --yt_client_secrets_file='/path/to/client_secret.json'"
    ...
    yt_desc_template_file_help = "Optional: File path to YouTube video description template. Example: --yt_desc_template_file='/path/to/description_template.txt'"
    ...
    yt_group.add_argument(
        "--yt_keywords", nargs="+", default=["music"], help=yt_keywords_help
    )
    ...
    thumb_file_prefix_help = (
        "Optional: Prefix for thumbnail filenames. Default: %(default)s"
    )
    thumb_file_suffix_help = (
        "Optional: Suffix for thumbnail filenames. Default: %(default)s"
    )
    ...
    thumbnail_group.add_argument(
        "--thumb_file_prefix", default=None, help=thumb_file_prefix_help
    )
    thumbnail_group.add_argument(
        "--thumb_file_suffix", default=None, help=thumb_file_suffix_help
    )
    thumbnail_group.add_argument(
    ...
    logger.info("YouTubeBulkUpload CLI beginning initialisation...")
    ...
    try:
        ...
        logger.error(
            f"An error occurred during bulk upload, see stack trace below: {e!s}"
        )
        raise e
