...
def analyze_file_type(file_path):
    ...
    if (
        file_name.startswith(".git")
        or file_name == ".gitignore"
        or file_name == ".gitattributes"
        or file_name.endswith(".git")
    ):
        return "Git_Files"
    ...
    if file_ext in [
        ...
        ".cfg",
        ".conf",
    ] or file_name in ["requirements.txt", "setup.py", "pyproject.toml", "Pipfile"]:
        return "Config_Files"

    # Documentation
    if file_ext in [".md", ".txt", ".rst", ".doc", ".docx"] or file_name in [
        "readme",
        "license",
        "changelog",
        "history",
