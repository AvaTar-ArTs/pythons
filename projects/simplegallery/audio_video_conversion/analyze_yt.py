    ...
    rows.append(
        {
            ...
            "docstring_preview": (
                (doc or "").strip().splitlines()[0][:160] if doc else ""
            ),
        }
    )
...
df = (
    pd.DataFrame(rows)
    .sort_values(["heuristic_category", "file"])
    .reset_index(drop=True)
)

# Save CSV and HTML
out_dir = Path("/mnt/data/_mini_audit")
...
<p style="color:#555;">Files analyzed from the uploaded workspace.</p>
{"".join(items)}
</body></html>"""
