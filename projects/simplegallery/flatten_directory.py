...
def flatten_directory(root_dir):
    """Flatten directory structure by moving all files to root level"""
    root_path = Path(root_dir)
    moved_files = []
    conflicts = []

    # First pass: collect all files and their target names
    file_mapping = {}
    ...
    for file_path in root_path.rglob("*"):
        if file_path.is_file() and file_path.parent != root_path:
            ...
            if any(
                skip in str(file_path) for skip in ["__pycache__", ".git", ".DS_Store"]
            ):
                continue
    ...
    for source, target in file_mapping.items():
        try:
            ...
            conflicts.append((str(source), str(e)))
...
def create_file_index(root_dir):
    ...
    index_content.append("# Python Directory - Flattened File Index\n")
    index_content.append(f"Generated: {os.popen('date').read().strip()}\n")
    index_content.append(
        f"Total files: {sum(len(files) for files in files_by_type.values())}\n\n"
    )
    ...
    # Write index file
    index_path = root_path / "FILE_INDEX.md"
...
if __name__ == "__main__":
    ...
    print("Starting directory flattening process...")
    print(f"Root directory: {root_directory}")

    # Flatten directory
    moved_files, conflicts = flatten_directory(root_directory)
