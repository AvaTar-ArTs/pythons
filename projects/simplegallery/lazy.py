        ...
        for line in f:
            ...
            if "/mp4/" in line:
                ...
                if line.endswith(".mp3") and albums[album_key]["mp3"] is None:
                    ...
                    if (
                        "transcript" in lower_line
                        and albums[album_key]["transcript"] is None
                    ):
                        try:
                            with open(
                                line, "r", encoding="utf-8", errors="ignore"
                            ) as tf:
                                albums[album_key]["transcript"] = tf.read().strip()
                        except IOError:
                            albums[album_key]["transcript"] = (
                                "Transcript not available."
                            )
                    elif (
                        "analysis" in lower_line
                        and albums[album_key]["analysis"] is None
                    ):
                        try:
                            with open(
                                line, "r", encoding="utf-8", errors="ignore"
...
def main():
    ...
    with open("discography.html", "w", encoding="utf-8") as outf:
        ...
        outf.write(
            "    .toggle-content { margin-top: 10px; font-size: 14px; color: #444; text-align: left; background-color: #f9f9f9; padding: 10px; border-radius: 8px; display: none; }\n"
        )
