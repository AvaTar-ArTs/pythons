...
def compare_analysis_coverage(original_data, enhanced_data):
    ...
    original_analyzed = sum(
        1 for row in original_data if row.get("has_analysis") == "True"
    )
    enhanced_analyzed = sum(
        1 for row in enhanced_data if row.get("has_analysis") == "True"
    )
    total_files = len(enhanced_data)

    print(f"Total Files: {total_files}")
    print(
        f"Original Analysis Coverage: {original_analyzed} files ({original_analyzed / total_files * 100:.1f}%)"
    )
    print(
        f"Enhanced Analysis Coverage: {enhanced_analyzed} files ({enhanced_analyzed / total_files * 100:.1f}%)"
    )
...
def compare_content_classification(original_data, enhanced_data):
    ...
    original_types = Counter(
        row.get("content_type", "unknown") for row in original_data
    )
    ...
    enhanced_types = Counter(
        row.get("content_type", "unknown") for row in enhanced_data
    )
...
def analyze_enhanced_features(enhanced_data):
    ...
    sentiment_dist = Counter(
        row.get("primary_sentiment", "unknown") for row in enhanced_data
    )
    ...
    visual_style_dist = Counter(
        row.get("primary_visual_style", "unknown") for row in enhanced_data
    )
    ...
    engagement_scores = [
        float(row.get("engagement_potential", 0))
        for row in enhanced_data
        if row.get("engagement_potential")
    ]
...
def analyze_resolution_distribution(enhanced_data):
    ...
    resolution_dist = Counter(
        row.get("resolution_category", "unknown") for row in enhanced_data
    )
...
def generate_top_content_recommendations(enhanced_data, n=10):
    ...
    analyzed_data.sort(
        key=lambda x: float(x.get("engagement_potential", 0)), reverse=True
    )
...
def main():
    ...
    enhanced_csv = (
        movies_dir / "enhanced_analysis" / "enhanced_content_analysis_improved.csv"
    )
    ...
    print(
        "✅ Demo completed! Check the enhanced_analysis/ directory for detailed results."
    )
    print("\n📁 Generated Files:")
    print(
        "  - enhanced_content_analysis_improved.csv (Enhanced CSV with all improvements)"
    )
