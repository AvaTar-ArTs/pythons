#!/usr/bin/env python3
"""
Script to parse the SUNO CSV file and create proper UUID to title mappings.
"""
...
def parse_suno_csv_for_mappings(csv_file_path):
    ...
    for line in lines:
        ...
        # Look for patterns where UUIDs appear with titles
        # Pattern: UUID followed by commas and then a title
        # Example: 01bbae67-5f8f-4285-8926-24545afe184e,,,,stitch by stitch4,,,,,,,,

        # More specific pattern based on the actual data format seen
        # Looking for: UUID,,,,title,
        ...
        for match in matches:
            uuid = match[0].strip()
            title_candidate = match[1].strip()

            # Clean up the title candidate
            title_candidate = re.sub(r'^"|"$', "", title_candidate)  # Remove leading/trailing quotes
            title_candidate = re.sub(r"^\'|\'$", "", title_candidate)  # Remove leading/trailing single quotes

            # Only add if it looks like a real title (not just a URL or ID)
            if (
                title_candidate
    ...
    # Additional pattern to catch titles in different positions
    # Looking for patterns like: UUID,title,description or UUID,,,,title,description
    for line in lines:
        if not line.strip():
            continue

        # Pattern to match UUID followed by title in various positions
        # This looks for UUID followed by up to 4 commas and then a title
        multi_comma_pattern = r'([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}),*,*,*,*,([^,\n"]+)'
        matches = re.findall(multi_comma_pattern, line)
