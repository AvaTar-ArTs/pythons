...
def analyze_directory(directory):
    ...
    print(f"   Duplicate function hashes: {len([h for h, funcs in hash_to_functions.items() if len(funcs) > 1])}")
...
def generate_report(analysis, output_dir):
    ...
    duplicate_hashes = {h: funcs for h, funcs in hash_to_functions.items() if len(funcs) > 1}

    # 2. Functions with same name/signature (potential duplicates)
    duplicate_names = {name: funcs for name, funcs in name_to_functions.items() if len(funcs) > 1}
    ...
    with open(txt_path, 'w') as f:
        ...
        f.write(f"Unique function signatures: {report['summary']['unique_signatures']}\n")
        f.write(f"Exact duplicate functions (by hash): {report['summary']['duplicate_hashes']}\n")
        f.write(f"Functions with same name (potential duplicates): {report['summary']['duplicate_names']}\n\n")
        ...
        if duplicate_hashes:
            for i, (hash_val, funcs) in enumerate(list(duplicate_hashes.items())[:30], 1):
        ...
        if duplicate_names:
            for i, (name, funcs) in enumerate(list(duplicate_names.items())[:50], 1):
                ...
                for func in funcs:
                    ...
                    f.write(f"  Hash: {func.get('hash', 'N/A')[:16] if func.get('hash') else 'N/A'}...\n")
                    f.write("\n")
        else:
