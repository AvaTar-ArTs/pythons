...
class ContentAnalyzer:
    ...
    def analyze(self):
        ...
        for file_path in self.root_path.rglob('*'):
            if file_path.is_file():
                try:
                    ...
                    elif ext in ['.txt', '.md', '.csv', '.json', '.html', '.yaml', '.yml', '.sh', '.js']:
        ...
        for file_info in all_files:
            if file_info['size'] > 0 and file_info['size'] < 100 * 1024 * 1024:  # < 100MB
                try:
                    file_hash = hashlib.md5()
                    with open(file_info['path'], 'rb') as f:
                        while chunk := f.read(8192):
                            file_hash.update(chunk)
                    hash_key = file_hash.hexdigest()
        ...
        print(f"   Found {len(self.stats['exact_duplicates'])} groups of exact duplicates")
        ...
        for file_info in python_files:
            ...
            if analysis:
                ...
                func_key = tuple(sorted(analysis['functions']))
                if func_key:
                    self.stats['python_functions'][func_key].append(rel_path)
                
                # Group by class names
                class_key = tuple(sorted(analysis['classes']))
                if class_key:
        ...
        print(f"   Comparing Python files with similar functions...")
        compared = set()
        ...
        for path, info in python_list:
            if info['functions']:
                func_key = tuple(sorted(info['functions']))
        ...
        for i, (path1, info1) in enumerate(python_list[:100]):  # Limit comparisons
            ...
            for path2, info2 in python_list[i+1:min(i+50, len(python_list))]:  # Limit to 50 comparisons per file
                ...
                if funcs1_set & funcs2_set:  # Has common functions
                    comparison = self.compare_python_files(
                        self.root_path / path1,
                        self.root_path / path2
                    )
                    
                    if comparison and comparison['similarity'] >= 0.5:
                        func_key = f"{path1} <-> {path2}"
        ...
        print(f"   Comparing text files for similar content...")
        compared = set()
    ...
    def print_report(self):
        ...
        exact_count = sum(len(files) for files in self.stats['exact_duplicates'].values())
        ...
        if self.stats['exact_duplicates']:
            ...
            for hash_key, file_list in list(self.stats['exact_duplicates'].items())[:10]:
        ...
        print(f"   Python files with similar functions: {len(self.stats['functional_duplicates'])}")
        ...
        if self.stats['functional_duplicates']:
            ...
            for key, info in list(self.stats['functional_duplicates'].items())[:15]:
                ...
                if info['details']['functions1']:
                    print(f"      Functions in file 1: {', '.join(info['details']['functions1'][:5])}")
                    if len(info['details']['functions1']) > 5:
                        print(f"        ... and {len(info['details']['functions1']) - 5} more")
                print()
        
        # Python files with same functions
        print(f"\n📦 PYTHON FILES WITH IDENTICAL FUNCTIONS")
        func_dupes = {k: v for k, v in self.stats['python_functions'].items() if len(v) > 1}
        if func_dupes:
        ...
        print(f"\n📄 SIMILAR CONTENT (Text Files)")
        print(f"   Text files with similar content: {len(self.stats['similar_content'])}")
        ...
        if self.stats['similar_content']:
            ...
            for key, info in list(self.stats['similar_content'].items())[:15]:
                ...
                if info['details']['is_subset']:
                    print(f"      ⚠️  One file appears to be a subset of the other")
                print()
