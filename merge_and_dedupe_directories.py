...
def merge_directories():
    ...
    duplicates_by_hash = {h: files for h, files in hash_to_files.items() if len(files) > 1}
    print(f"   Found {len(duplicates_by_hash)} duplicate groups (same content, different paths)")
    ...
    for source_dir in SOURCE_DIRS:
        source_files = {k: v for k, v in all_files.items() if v['source_dir'] == source_dir}
    ...
    # Step 6: Save report
    report_path = Path(TARGET_DIR) / f"merge_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    print(f"   Files already in target: {len(all_files) - len(files_to_copy) - len(conflicts)}")
    ...
    if conflicts:
        ...
        conflict_report_path = Path(TARGET_DIR) / f"conflicts_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
