...
class DeepZipAnalyzer:
    def __init__(self, zip_dir=None):
        self.zip_dir = Path(zip_dir or "/Volumes/2T-Xx/zip")
    ...
    def analyze_directory_structure(self):
        ...
        print("📁 Analyzing Directory Structure (Deep Scan)")
        print("=" * 60)
        ...
        print(f"  📏 Max depth: {max(s['depth'] for s in structure.values())}")
    ...
    def analyze_file_types(self):
            ...
            for file in files:
                ...
                try:
                    ...
                    if file_size > 100 * 1024 * 1024:
                        large_files.append(
                            {"path": file_path, "size": file_size, "extension": ext},
                        )

                    # Track ZIP files
                    if ext == ".zip":
                        zip_files.append(
                            {"path": file_path, "size": file_size, "name": file},
                        )
        ...
        print("📊 File Type Analysis Complete:")
    ...
    def find_duplicates(self):
        ...
        print(f"  📄 Duplicate files: {sum(d['count'] for d in duplicates)}")
        print(
            f"  💾 Space wasted: {self.format_size(sum(d['total_size'] - d['keep']['size'] for d in duplicates))}",
        )
    ...
    def categorize_content(self):
        ...
        print("📊 Content Categorization Complete:")
        for category, files in categories.items():
            total_size = sum(f["size"] for f in files)
            print(
                f"  {category.title()}: {len(files):,} files ({self.format_size(total_size)})",
            )
    ...
    def analyze_zip_files(self):
        ...
        for zip_info in self.results["zip_files"]:
            ...
            print(f"  🔍 Analyzing {zip_info['name']}...")
            ...
            try:
                with zipfile.ZipFile(zip_path, "r") as zip_file:
                    ...
                    compression_ratio = (
                        (1 - zip_info["size"] / total_size) * 100
                        if total_size > 0
                        else 0
                    )
                    ...
                    for file_name in file_list:
                        ...
                        if any(
                            kw in name_lower
                            for kw in ["design", "template", "mockup", "bundle"]
                        ):
                            content_keywords.append("design")
                        if any(
                            kw in name_lower for kw in ["svg", "png", "jpg", "jpeg"]
                        ):
                    ...
                    zip_analysis.append(
                        {
                            ...
                            "recommendation": self.get_zip_recommendation(
                                total_size, compression_ratio, file_types,
                            ),
                        },
                    )
        ...
        print(f"  📦 ZIPs analyzed: {len(zip_analysis)}")
        print(f"  ✅ Successful: {len([z for z in zip_analysis if 'error' not in z])}")
        print(f"  ❌ Errors: {len([z for z in zip_analysis if 'error' in z])}")

        return zip_analysis

    def get_zip_recommendation(self, uncompressed_size, compression_ratio, file_types):
        """Get recommendation for ZIP file handling"""
        if uncompressed_size > 500 * 1024 * 1024:  # 500MB
            return "extract_and_organize"
        if uncompressed_size > 100 * 1024 * 1024:  # 100MB
            if compression_ratio > 50:
                return "keep_zipped"
            return "extract"
        if uncompressed_size > 10 * 1024 * 1024:  # 10MB
    ...
    def identify_optimization_opportunities(self):
        ...
        duplicate_savings = sum(
            d["total_size"] - d["keep"]["size"] for d in self.results["duplicate_files"]
        )
            ...
            opportunities.append(
                {
                    ...
                    "files_affected": sum(
                        d["count"] for d in self.results["duplicate_files"]
                    ),
        ...
        large_files = [
            f for f in self.results["large_files"] if f["size"] > 500 * 1024 * 1024
        ]
        if large_files:
            opportunities.append(
                {
        ...
        zip_files = [
            z for z in self.results["zip_files"] if z["size"] > 100 * 1024 * 1024
        ]
        if zip_files:
            opportunities.append(
                {
        ...
        deep_dirs = [
            d for d in self.results["directory_structure"].items() if d[1]["depth"] > 5
        ]
        if deep_dirs:
            opportunities.append(
                {
        ...
        for opp in opportunities:
            ...
            if opp["space_savings"] > 0:
                print(f"    💾 Space savings: {self.format_size(opp['space_savings'])}")
    ...
    def generate_comprehensive_report(self):
        ...
        report = {
            ...
            "duplicate_analysis": {
                ...
                "space_wasted": sum(
                    d["total_size"] - d["keep"]["size"]
                    for d in self.results["duplicate_files"]
                ),
        ...
        report_file = (
            self.analysis_dir
            / f"deep_analysis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        print("📋 Reports saved:")
        print(f"  📄 Detailed JSON: {report_file}")
    ...
    def generate_markdown_summary(self, output_file, report):
        ...
        with open(output_file, "w") as f:
            ...
            f.write("|-----------|-------|\n")
            f.writelines(f"| {ext} | {count:,} |\n" for ext, count in sorted(
                report["file_analysis"]["file_types"].items(),
                key=lambda x: x[1],
                reverse=True,
            )[:20])
            f.write("\n")
            ...
            f.write("## 🔄 Duplicates\n\n")
            f.write(
                f"- **Duplicate Groups**: {report['duplicate_analysis']['duplicate_groups']}\n",
            )
            f.write(
                f"- **Duplicate Files**: {report['duplicate_analysis']['duplicate_files']}\n",
            )
            f.write(
                f"- **Space Wasted**: {self.format_size(report['duplicate_analysis']['space_wasted'])}\n\n",
            )
            ...
            for category, files in report["content_categories"].items():
                total_size = sum(f["size"] for f in files)
                f.write(
                    f"| {category.title()} | {len(files):,} | {self.format_size(total_size)} |\n",
                )
            f.write("\n")
            ...
            for opp in report["optimization_opportunities"]:
                ...
                if opp["space_savings"] > 0:
                    f.write(
                        f"- **Space Savings**: {self.format_size(opp['space_savings'])}\n",
                    )
                f.write("\n")

    def run_comprehensive_analysis(self):
        """Run complete comprehensive analysis"""
        print("🔍 Deep ZIP Directory Content Awareness Analysis")
        print("=" * 80)
        print(f"📁 Target Directory: {self.zip_dir}")
        print(
            f"💾 Directory Size: {self.format_size(sum(os.path.getsize(os.path.join(root, file)) for root, dirs, files in os.walk(self.zip_dir) for file in files if os.path.isfile(os.path.join(root, file))))}",
        )
        print("=" * 80)
        ...
        print(f"💾 Total size analyzed: {self.format_size(self.results['total_size'])}")
        print(f"🔄 Duplicates found: {len(self.results['duplicate_files'])} groups")
        print(
            f"🎯 Optimization opportunities: {len(self.results['optimization_opportunities'])}",
        )
