...
print(f"Loaded: {len(mp3s)} MP3s, {len(discography)} discography, {len(prompts)} prompts, {len(lyrics)} lyrics")
print()

# Group items by directory/project
related_groups = defaultdict(lambda: {
    'mp3s': [],
    'lyrics': [],
    'prompts': [],
    'discography': [],
...
with open(csv_file, 'w', newline='', encoding='utf-8') as f:
    ...
    sorted_groups = sorted(
        ...
        key=lambda x: len(x[1]['mp3s']) + len(x[1]['lyrics']) + len(x[1]['prompts']) + len(x[1]['discography']),
        reverse=True
    )
    ...
    for directory, group_data in sorted_groups:
        ...
        total_items = len(mp3s_list) + len(lyrics_list) + len(prompts_list) + len(discography_list)
...
for i, (directory, group_data) in enumerate(sorted_groups[:10], 1):
    total = len(group_data['mp3s']) + len(group_data['lyrics']) + len(group_data['prompts']) + len(group_data['discography'])
    print(f"{i:2d}. {total:3d} items | {len(group_data['mp3s'])} MP3s, {len(group_data['lyrics'])} lyrics, "
