...
def print_analysis(analysis):
    ...
    for depth in sorted(depth_dist.keys()):
        ...
        percentage = (count / analysis['total_dirs'] * 100) if analysis['total_dirs'] > 0 else 0
        bar = '█' * (count // 5)
    ...
    for subcat, stats in sorted_subs[:20]:
        print(f"   {subcat:50} {stats['dirs']:4,} dirs | {stats['files']:5,} files | {stats['size'] / (1024*1024):6.2f} MB")
...
def suggest_fixes(analysis):
    ...
    if deep_nesting:
        ...
        print(f"   • Flatten to depth 2 (merge intermediate levels)")
    ...
    if small_dirs:
        ...
        print(f"   • Merge into parent directories")
    ...
    subcat_stats = analysis['subcategory_stats']
    large_subs = [(cat, stats) for cat, stats in subcat_stats.items() if stats['dirs'] > 10]

    if large_subs:
        print(f"\n4️⃣  Organize Large Subcategories:")
        for cat, stats in sorted(large_subs, key=lambda x: -x[1]['dirs'])[:5]:
            print(f"   • {cat}: {stats['dirs']} directories → consider further organization")
