...
class EnhancedDuplicateFinder:
    ...
    def detect_duplicate_types(self, file1: Path, file2: Path) -> List[Tuple[str, float]]:
        ...
        copy_patterns = [
            r'copy\s*\d*$', r'_copy\d*$', r'_\d+$', r'\d+$',
            r'\(copy\)', r'\(copy \d+\)', r' - copy'
        ]
        for pattern in copy_patterns:
            if re.search(pattern, name1, re.IGNORECASE) or re.search(pattern, name2, re.IGNORECASE):
        ...
        version_patterns = [
            r'v\d+$', r'_v\d+$', r'version\s*\d+$', r'_version\d+$'
        ]
        ...
        for i, file1 in enumerate(python_files):
            ...
            if len(group) > 1:
                duplicate_groups.append({
                    'files': group,
                    'types': best_types,
                    'confidence': max([t[1] for t in best_types]) if best_types else 0.75
                })
                processed.add(file1)
    ...
    def save_results(self, output_file: str = "comprehensive_files_to_remove.csv"):
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=[
                'file_to_remove', 'keep_file', 'duplicate_type', 
                'confidence_score', 'confidence', 'reason', 'file_size'
            ])
            writer.writeheader()
