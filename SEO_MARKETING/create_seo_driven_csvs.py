...
def calculate_seo_score(matches, lines, num_classes, num_functions):
    ...
    total_score = min(
        100, base_score + priority_bonus + complexity_score + multi_trend_bonus
    )
...
for idx, filepath in enumerate(all_files, 1):
    ...
    result = {
        ...
        "last_modified": datetime.fromtimestamp(filepath.stat().st_mtime).strftime(
            "%Y-%m-%d"
        ),
    }
...
for trend_name, trend_data in sorted(
    SEO_TRENDS.items(), key=lambda x: x[1]["priority"]
):
    count = len(trend_buckets.get(trend_name, []))
    total_lines = sum(f["lines"] for f in trend_buckets.get(trend_name, []))
    avg_score = (
        sum(f["seo_score"] for f in trend_buckets.get(trend_name, [])) / count
        if count > 0
        else 0
    )
    ...
    print(f"\n🔥 {trend_name}")
    print(
        f"   Growth: {trend_data['growth']} | Searches: {trend_data['monthly_searches']}/month"
    )
    print(
        f"   Matched Files: {count} | Total Lines: {total_lines:,} | Avg SEO Score: {avg_score:.1f}"
    )
...
for i, r in enumerate(results[:10], 1):
    ...
    print(
        f"     Score: {r['seo_score']} | Trend: {r['primary_trend']} | Value: {r['market_value_estimate']}"
    )
