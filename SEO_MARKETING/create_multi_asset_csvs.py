...
for file in music_files:
    try:
        ...
        for i, part in enumerate(path_parts):
            if (
                "album" in part.lower()
                or "blues" in part.lower()
                or "symphony" in part.lower()
            ):
                album = part
            if i < len(path_parts) - 1 and (
                "music" in part.lower() or "audio" in part.lower()
            ):
...
if all_assets:
    ...
    print(
        f"✅ Complete Ecosystem: {len(all_assets)} total assets → {master_path.name}\n"
    )
...
print("  • Python Scripts: 234 (from SEO scan)")
