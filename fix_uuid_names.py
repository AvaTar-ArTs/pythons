    ...
    for item in Path(root_dir).iterdir():
        if item.is_dir():
            ...
            if re.match(uuid_pattern, dir_name.lower()):
                # Look up the proper title
                lookup_key = dir_name.lower().replace("-", "_")  # Normalize to underscore format
