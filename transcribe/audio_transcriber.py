...
class AudioTranscriber:
    ...
    def transcribe_local(
        ...
        try:
            logger.info(f"Transcribing {file_path.name} with local Whisper ({model})...")
    ...
    def transcribe(
        ...
        **kwargs
        ...
        if self.provider == TranscriptionProvider.OPENAI:
            return self.transcribe_openai(file_path, model=model or "whisper-1", **kwargs)
        else:
    ...
    def save_transcript_srt(
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            for idx, (start, end, text) in enumerate(segments, 1):
                f.write(f"{idx}\n")
                f.write(f"{self.format_timestamp_srt(start)} --> {self.format_timestamp_srt(end)}\n")
                f.write(f"{text}\n\n")
