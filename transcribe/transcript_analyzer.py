...
{transcript}""",
        ...
        def _call():
            return self.client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": prompt_config["user_template"].format(transcript=transcript)},
                ],
    ...
    def analyze(
        ...
        **kwargs
        ...
        if self.provider == AnalysisProvider.OPENAI:
            return self.analyze_openai(transcript, model=model or "gpt-3.5-turbo", **kwargs)
        else:
            return self.analyze_ollama(transcript, model=model or "llama3.1:8b-instruct", **kwargs)
    ...
    def batch_analyze(
        ...
        results = {}
        transcript_files = list(input_dir.rglob("*.txt")) + list(input_dir.rglob("*.transcript"))
...
def main():
    ...
    if input_path.is_file() and not args.batch:
        output_path = Path(args.output) if args.output else input_path.parent / f"{input_path.stem}_analysis.txt"
        ...
        results = analyzer.batch_analyze(input_path, output_dir, force=args.force, model=args.model)
        logger.info(f"Analyzed {len([r for r in results.values() if r])} files. Results saved to {output_dir}")
