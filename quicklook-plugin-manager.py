...
class QuickLookPluginManager:
        ...
        if scope in ("user", "all"):
            if self.USER_PLUGIN_DIR.exists():
                for item in self.USER_PLUGIN_DIR.iterdir():
                    if item.suffix == ".qlgenerator" or (item.is_dir() and item.suffix == ""):
                        plugins.append({
                            "name": item.name,
                            "path": str(item),
                            "scope": "user",
    ...
    def install_plugin(self, plugin_path, scope="user", backup=True):
        ...
        try:
            ...
            print(f"❌ Permission denied. For system-wide installation, run with sudo")
            return False
    ...
    def uninstall_plugin(self, plugin_name, scope="user"):
        ...
        try:
            ...
            print(f"❌ Permission denied. For system plugins, run with sudo")
            return False
        except Exception as e:
    ...
    def generate_plugin_list(self, output_file=None):
        ...
        if output_file:
            ...
            output_path = self.BACKUP_DIR / f"plugin_list_{datetime.now().strftime('%Y%m%d')}.txt"
        ...
        with open(output_path, "w") as f:
            ...
            if user_plugins:
                ...
                for plugin in user_plugins:
                    f.write(f"  • {plugin['name']} ({self.format_size(plugin['size'])})\n")
                f.write("\n")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  python quicklook-plugin-manager.py install <plugin_path> [user|system]")
        print("  python quicklook-plugin-manager.py uninstall <plugin_name> [user|system]")
        ...
        print("  python quicklook-plugin-manager.py install ~/Downloads/QLColorCode.qlgenerator user")
    ...
    if command == "list":
        ...
        if user_plugins:
            ...
            print("-" * 60)
            for plugin in user_plugins:
                print(f"  • {plugin['name']:40} {manager.format_size(plugin['size']):>10}")
