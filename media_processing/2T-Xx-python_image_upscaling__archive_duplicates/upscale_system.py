#!/usr/bin/env python3
"""
System-based Image Upscaler
Uses macOS built-in tools (sips) to process images with aspect ratios
Based on the proven 9mbs.py approach
"""
...
def resize_to_aspect_ratio(input_path, output_path, target_width, target_height, method='crop'):
...
def process_image(input_path, output_path, aspect_ratio, max_size_mb=9):
    ...
    try:
        ...
        success, message = resize_to_aspect_ratio(input_path, output_path, target_width, target_height, 'crop')
        if not success:
...
def process_images_with_aspect_ratios(directory, max_size_mb=9):
    ...
    print("🖼️  SYSTEM-BASED IMAGE UPSCALER")
    print("=" * 50)
...
def main():
    ...
    print("Maintains file size under 9MB using macOS sips")
    print("=" * 50)
