...
def get_latest_analysis() -> Path:
    ...
    if not analysis_dir.exists():
        raise FileNotFoundError("No functional_analysis directory found. Run functional_category_analyzer.py first.")
...
def create_category_structure(base_dir: Path, categories: List[str]):
    ...
    for category in categories:
        ...
        if not readme.exists():
            readme.write_text(f"# {category.replace('-', ' ').title()}\n\n"
...
def organize_files(analysis_data: Dict, dry_run: bool = True):
    ...
    if dry_run:
        ...
        print(f"📊 Would organize {len(analysis_data['file_categories'])} files into {len(category_counts)} categories")
        print("\n💡 To actually move files, run: python3 organize_by_category.py --execute")
    else:
        print(f"✅ Organized {moved_count} files into {len(category_counts)} categories")
...
Examples:
  ...
  python3 organize_by_category.py --execute --analysis my_analysis.json
    ...
    parser.add_argument(
        '--analysis',
        type=str,
        help='Path to analysis JSON file (default: uses latest)'
    )
