...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('canva_processing.log'),
...
class CanvaArchiveProcessor:
    ...
    def analyze_archives(self):
        ...
        logger.info(f"✅ Analysis complete: {self.stats['total_files']} files processed")
    ...
    def find_duplicates(self):
        ...
        logger.info(f"💾 Potential space savings: {self.stats['space_saved'] / (1024*1024):.1f} MB")
    ...
    def create_deduplicated_archives(self):
        ...
        for file_hash, dup_info in self.duplicates.items():
            ...
            for dup_file in dup_info['duplicates']:
                file_keep_map[dup_file['archive']] = file_keep_map.get(dup_file['archive'], [])
            ...
            try:
                with zipfile.ZipFile(original_archive, 'r') as source_zip:
                    with zipfile.ZipFile(dedup_archive, 'w', zipfile.ZIP_DEFLATED, compresslevel=9) as dest_zip:
                        ...
                        logger.info(f"✅ Deduplicated {archive_name}: {len(files_to_skip)} files removed")
    ...
    def compress_archives(self):
        ...
        for i, archive_path in enumerate(dedup_archives, 1):
            ...
            try:
                ...
                compressed_archive = self.temp_dir / f"compressed_{archive_path.name.replace('.zip', '.7z')}"
                cmd = [
    ...
    def generate_reports(self):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"- **Compression Ratio:** {self.stats['compression_ratio']:.1f}%\n\n")
        ...
        with open(csv_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Archive', 'File_Path', 'Size_Bytes', 'Hash', 'Is_Duplicate'])
            
            for archive_name, archive_info in self.archive_contents.items():
                for file_info in archive_info['files']:
                    is_duplicate = file_info['file_path'] in [
                        dup['file_path'] for dup in self.duplicates.get(file_info['hash'], {}).get('duplicates', [])
                    ]
                    writer.writerow([
                        archive_name,
                        file_info['file_path'],
    ...
    def process_all(self):
        ...
        try:
            ...
            logger.info(f"💾 Space saved: {self.stats['space_saved'] / (1024*1024):.1f} MB")
...
def main():
    parser = argparse.ArgumentParser(description='Canva Archive Deduplication, Compression & Organization')
    ...
    parser.add_argument('--temp', default='/tmp/canva_processing',
    ...
    if args.dry_run:
        ...
        logger.info(f"Would save {processor.stats['space_saved'] / (1024*1024):.1f} MB")
        return
