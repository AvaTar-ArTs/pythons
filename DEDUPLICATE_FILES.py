...
def choose_keep_file(file_group):
    ...
    file_group = sorted(file_group, key=lambda p: (
...
def analyze_duplicates(duplicates):
    ...
    total_duplicate_copies = total_duplicate_files - total_duplicate_groups  # Files to remove
    ...
    for file_hash, files in duplicates.items():
        try:
            ...
            group_size = file_size * (len(files) - 1)  # Size of duplicates (excluding keep)
    ...
    for group in size_by_group[:20]:
        ...
        if len(example) > 58:
            example = example[:55] + "..."
        print(f"{group['count']:<8} {group['size']/(1024*1024):<12.2f} MB "
...
def remove_duplicates(duplicates, dry_run=True):
        ...
        for remove_file in remove_files:
            try:
                ...
                with open(backup_log, 'a') as log:
                    log.write(f"{remove_file.relative_to(documents_dir)},"
    ...
    print(f"{'Would remove' if dry_run else 'Removed'}: {removed_count:,} duplicate files")
    print(f"Space {'to recover' if dry_run else 'recovered'}: {removed_size / (1024*1024):.2f} MB")
    if errors > 0:
