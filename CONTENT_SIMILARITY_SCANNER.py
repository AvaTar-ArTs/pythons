...
class ContentSimilarityScanner:
    ...
    def save_results(self, identical, similar_structure, similar_imports):
        ...
        with open(csv_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Group ID', 'File Count', 'File Paths', 'Function Count', 'Size (KB)'])

            for i, group in enumerate(identical, 1):
                paths = ' | '.join(str(f['path'].relative_to(self.pythons_dir)) for f in group)
        ...
        with open(csv_file2, 'w', newline='') as f:
            ...
            for group in similar_structure[:500]:
                paths = ' | '.join(str(f['path'].relative_to(self.pythons_dir)) for f in group['files'])
        ...
        with open(csv_file3, 'w', newline='') as f:
            ...
            for group in similar_imports[:200]:
                ...
                paths = ' | '.join(str(f['path'].relative_to(self.pythons_dir)) for f in group['files'][:10])
                writer.writerow([len(group['imports']), group['count'], imports_str, paths])
