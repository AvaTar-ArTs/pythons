...
def resize_image_to_max_size(:
    image_path, output_path, max_size_mb=9, aspect_ratio=None, method="crop"
):
    ...
    if is_large_image(image_path):
        print(
            f"Skipping {os.path.basename(image_path)} - Image size exceeds the limit."
        )
        return False
    ...
    with Image.open(image_path) as img:
        ...
        if aspect_ratio:
            ...
            print(
                f"Resized {os.path.basename(image_path)} to {aspect_ratio[0]}:{aspect_ratio[1]} aspect ratio ({img.size[0]}x{img.size[1]})"
            )
        ...
        if current_size <= max_size_bytes and not aspect_ratio:
            ...
            img.save(
                output_path, format="JPEG", dpi=target_dpi, quality=95, optimize=True
            )
            return True
        ...
        print(
            f"Processed {os.path.basename(image_path)}: {original_size} → {img.size} @ {quality}% quality, {final_size / (1024 * 1024):.2f} MB"
        )
        return True
