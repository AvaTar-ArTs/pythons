...
def get_resize_method():
    ...
    while True:
        ...
        methods = {"1": "crop", "2": "pad", "3": "stretch"}
...
def upscale_single_image(:
    input_path, output_path, width_ratio, height_ratio, method="crop"
):
...
def main():
    ...
    # Get file pattern
    pattern = input(
        "\n📄 File pattern (e.g., *.jpg, *.png, or specific filename): "
    ).strip()
    if not pattern:
        pattern = "*.jpg"
    ...
    for i, file_path in enumerate(files, 1):
        ...
        result = upscale_single_image(
            file_path, output_path, width_ratio, height_ratio, method
        )

        if result["success"]:
            successful += 1
            print(
                f"  ✅ {result['new_size']} @ {result['quality']}% quality, {result['file_size_mb']:.2f}MB"
            )
        else:
            failed += 1
