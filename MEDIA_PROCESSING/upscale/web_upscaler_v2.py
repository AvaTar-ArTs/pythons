...
def convert_and_upscale_webp(:
    ...
    if not PIL_AVAILABLE:
        return {"success": False, "error": "PIL/Pillow is required for this operation"}
    ...
    try:
        with Image.open(input_path) as img:
            ...
            opt_success, opt_message = optimize_file_size(
                output_path, max_size_mb, quality_range=(90, 20), quality_step=10
            )
...
def main():
    ...
    # Get source format
    source_format = input("Source format (webp/tiff) [webp]: ").strip().lower()
    if not source_format:
        source_format = "webp"

    # Get target format
    target_format = input("Target format (JPEG/PNG) [JPEG]: ").strip().upper()
    if not target_format:
        target_format = "JPEG"

    # Get remove original option
    remove_original = input("Remove original files? (y/N): ").strip().lower() == "y"
