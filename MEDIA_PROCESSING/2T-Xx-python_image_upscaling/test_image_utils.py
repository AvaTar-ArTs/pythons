...
class TestCalculateTargetDimensions:
    ...
    def test_landscape_16x9(self):
        ...
        width, height = calculate_target_dimensions(16, 9, base_size=2000)
        assert width == 3200
        assert height == 1800

    def test_portrait_9x16(self):
        """Test 9:16 portrait calculation."""
        width, height = calculate_target_dimensions(9, 16, base_size=2000)
        assert width == 1125
        assert height == 2000

    def test_square_1x1(self):
        """Test 1:1 square calculation."""
        width, height = calculate_target_dimensions(1, 1, base_size=2000)
        assert width == 2000
        assert height == 2000

    def test_max_dimension_limit(self):
        """Test max dimension constraint."""
        width, height = calculate_target_dimensions(
    ...
    def test_custom_base_size(self):
        ...
        width, height = calculate_target_dimensions(4, 3, base_size=1000)
        assert width == 1333
        assert height == 1000
