...
try:
    ...
    logging.error(f"OpenAI initialization failed: {e!s}")
    openai = None
...
def safe_file_access(filepath: Path):
    ...
    try:
        ...
        logging.error(f"Failed to access {filepath}: {e!s}")
        yield None
...
class FileProcessor:
    ...
    def get_creation_date(filepath: Path) -> str:
        try:
            ...
            logging.error(f"Creation date error for {filepath}: {e!s}")
            return "Unknown"
    ...
    def format_duration(seconds: Optional[float]) -> str:
        ...
        try:
            ...
            logging.error(f"Duration formatting error: {e!s}")
            return "Unknown"
...
class MediaProcessor(FileProcessor):
    ...
    def process_audio(cls, filepath: Path) -> Optional[Dict]:
        try:
            ...
            logging.error(f"Audio processing failed for {filepath}: {e!s}")
            return None
    ...
    def process_image(cls, filepath: Path) -> Optional[Dict]:
        try:
            ...
            logging.error(f"Image processing failed for {filepath}: {e!s}")
            return None
    ...
    def process_video(cls, filepath: Path) -> Optional[Dict]:
        try:
            ...
            logging.error(f"Video processing failed for {filepath}: {e!s}")
            return None
...
class ScriptAnalyzer:
    ...
    def analyze_scripts_batch(self, scripts: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            logging.error(f"AI analysis failed: {e!s}")
...
def write_results(results: List[Dict], output_path: Path):
    ...
    try:
        ...
        logging.error(f"Failed to write CSV: {e!s}")
...
if __name__ == "__main__":
    try:
        ...
        logging.error(f"Critical failure: {e!s}", exc_info=True)
