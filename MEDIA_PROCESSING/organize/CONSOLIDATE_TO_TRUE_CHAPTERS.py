...
chapters = {
    ...
    "other": {"expected_duration": (0, 999999), "files": []},
}
...
for chapter, data in chapters.items():
    ...
    print(
        f"  Expected: {expected[0] // 60}:{expected[0] % 60:02d} - {expected[1] // 60}:{expected[1] % 60:02d}"
    )
    ...
    if files:
        ...
        for f, dur in files:
            if expected[0] <= dur <= expected[1]:
                if best is None or abs(dur - (expected[0] + expected[1]) // 2) < abs(
                    best[1] - (expected[0] + expected[1]) // 2
                ):
                    best = (f, dur)
        ...
        if best:
            ...
            print("  ??  No version in expected range!")
...
print(
    f"Thought & Character versions: {len(chapters['thought_and_character']['files'])}"
)
