...
if not OPENAI_API_KEY:
    logger.info(
        colored(
            "❌ OPENAI_API_KEY missing. Ensure ~/.env or environment has it.", "red"
        )
    )
    sys.exit(1)
...
def slugify(name: str) -> str:
    ...
    name = re.sub(
        r"[^\w\-_\. ]+", "", name
    )  # allow word chars, dash, underscore, dot, space
    ...
    for line in transcript_text.splitlines():
        if "--" in line:
            try:
                ...
                segments.append(
                    {"timestamp": timestamp_part.strip(), "text": text.strip()}
                )
            except ValueError:
                continue
    return segments
...
def process_audio_file(:
    ...
    if transcript_path.exists() and analysis_path.exists() and not force:
        logger.info(
            colored(
                f"✔ Skipping {audio_file.name} (already done). Use --force to reprocess.",
                "green",
            )
        )
        return
    ...
    if not transcript:
        logger.info(
            colored(
                f"⚠️ Skipping {audio_file.name} due to transcription failure.", "yellow"
            )
        )
        return
...
def check_conda_env(expected="ai-media"):
    ...
    if expected and expected not in current:
        logger.info(
            colored(
                f"⚠️ You are in conda env '{current or 'none'}'; expected '{expected}'. Activate with: conda activate {expected}",
                "yellow",
            )
        )
...
def process_audio_directory(audio_dir: Path, max_workers: int, force: bool):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        list(
            tqdm(
                executor.map(worker, audio_files), total=len(audio_files), desc="Files"
            )
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of concurrent transcription workers.",
    )
    parser.add_argument(
        "-f",
        "--force",
        action="store_true",
        help="Reprocess even if transcript+analysis exist.",
    )
