...
class AnalyzeAndTranscribe:
    ...
    def check_folder_content(self, folder_path: Path):
        ...
        {self.normalize_filename(f.name): f for f in text_files}
        ...
        for mp3 in mp3_files:
            ...
            has_transcript = any(
                "transcript" in f.name.lower()
                and normalized in self.normalize_filename(f.name)
                for f in text_files
            )

            # Check for analysis
            has_analysis = any(
                "analysis" in f.name.lower()
                and normalized in self.normalize_filename(f.name)
                for f in text_files
            )
            ...
            if has_transcript:
                ...
                self.missing_transcripts.append(
                    {"folder": folder_path.name, "file": mp3.name, "path": str(mp3)}
                )
        ...
        if folder_missing_transcript:
            print(
                f"  {Colors.YELLOW}Missing transcripts: {len(folder_missing_transcript)}{Colors.END}"
            )
        ...
        if folder_missing_analysis:
            print(
                f"  {Colors.YELLOW}Missing analysis: {len(folder_missing_analysis)}{Colors.END}"
            )
    ...
    def save_reports(self):
        ...
        print(
            f"{Colors.GREEN}✅ Missing transcripts: {transcript_csv.name}{Colors.END}"
        )
        ...
        with open(summary_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"  Coverage: {self.stats['has_transcript'] / self.stats['total_mp3s'] * 100:.1f}%\n\n"
            )
            ...
            f.write(
                f"  Coverage: {self.stats['has_analysis'] / self.stats['total_mp3s'] * 100:.1f}%\n\n"
            )
    ...
    def run(self, folder_paths):
        ...
        print(
            f"{Colors.BOLD}Folders scanned:{Colors.END} {self.stats['total_folders']}"
        )
        print(f"{Colors.BOLD}Total MP3s:{Colors.END} {self.stats['total_mp3s']}\n")

        print(f"{Colors.BOLD}TRANSCRIPTS:{Colors.END}")
        print(
            f"  ✅ Has transcript: {Colors.GREEN}{self.stats['has_transcript']}{Colors.END}"
        )
        print(
            f"  ❌ Missing: {Colors.RED}{self.stats['missing_transcript']}{Colors.END}"
        )
        ...
        print(
            f"  ✅ Has analysis: {Colors.GREEN}{self.stats['has_analysis']}{Colors.END}"
        )
        ...
        if self.stats["missing_transcript"] > 0:
            print(
                f"{Colors.YELLOW}⚠️  {self.stats['missing_transcript']} MP3s need transcripts{Colors.END}"
            )
            print(
                f"{Colors.CYAN}💡 Review MISSING_TRANSCRIPTS.csv for details{Colors.END}\n"
            )
        else:
