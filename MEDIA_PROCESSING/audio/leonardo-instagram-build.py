...
CATEGORIES = [
    (
        "Audio & Transcription",
        ["transcribe", "lyrics", "audio", "spotifymp3", "savify"],
    ),
    ("YouTube/Twitch Automation", ["youtube", "yt-", "twitch", "shorts", "clip"]),
    (
        "Social Media Bots & Scrapers",
        ["reddit", "instagram", "tiktok", "fb-", "twitter", "scraper"],
    ),
    (
        "AI Art & POD Pipelines",
        [
            "redbubble",
            "pod",
            "photoshop",
...
def main(:
    ...
    out_al_md="content/alchemy_index.md",
):
    root = Path(repo_root)
    ...
    with open(out_csv, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["name", "category", "seo_title", "summary", "suggested_tags"]
        )
        writer.writeheader()
...
""".format(
    ...
    Path(out_py_md).write_text(
        "# AvatarArts · Python Portfolio\n\n"
        + Path("\n").join(md_section(r) for r in rows_sorted),
        encoding="utf-8",
    )

    al_lines = [
        "- **{name}** — *{category}*: {summary} · *Tags:* {tags}".format(
            name=r["name"],
            category=r["category"],
            summary=r["summary"],
            tags=r["suggested_tags"],
    ...
    Path(out_al_md).write_text(
        "# Alchemy · Creative Automation Index\n\n> Tools and rituals behind the art.\n\n"
        + Path("\n").join(al_lines),
        encoding="utf-8",
    )
