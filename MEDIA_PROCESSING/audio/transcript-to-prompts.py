...
STYLE_HINTS = {
    ...
    "typography": [
        ...
        "glowing glyphs",
        "kerning tight",
        "letterforms textured",
    ],
    "surreal": [
        "surreal symbolism",
        "double exposure",
        "scale-shifted architecture",
        "dreamlike haze",
    ],
...
def pick_tags(txt):
    ...
    styles = ["cinematic"]
    if any(
        w in txt_low for w in ("dream", "veil", "vision", "siren", "phantom", "myth")
    ):
    ...
    if any(
        w in txt_low
        for w in ("america", "nation", "civic", "republic", "flag", "liberty", "peril")
    ):
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate detailed image prompts from a transcript with timestamps."
    )
    ap.add_argument(
        "transcripts",
        nargs="+",
        help="Path(s) to transcript .txt files with [HH:MM:SS] lines",
    )
    args = ap.parse_args()
