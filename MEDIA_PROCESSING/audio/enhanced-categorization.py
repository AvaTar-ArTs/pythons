...
"""
    ...
    def generate_category_csv(self):
        ...
        with open(csv_path, "w", newline="") as f:
            writer = csv.DictWriter(
                f, fieldnames=["category", "filename", "path", "size_kb"]
            )
            writer.writeheader()
    ...
    def print_summary(self, uncategorized):
        ...
        for category, info in sorted(
            self.categories.items(), key=lambda x: -len(x[1]["scripts"])
        ):
            if info["scripts"]:
...
def main():
    ...
    # Generate docs
    categorizer.generate_enhanced_docs()

    # Generate CSV
    categorizer.generate_category_csv()
