    ...
    def detect_functionality(self, filepath, imports):
        ...
        try:
            ...
            for service, keywords in services.items():
                if any(kw in content for kw in keywords) or any(
                    kw in imports for kw in keywords
                ):
...
"""
        ...
        for i, (hash_val, files) in enumerate(
            sorted(dupes.items(), key=lambda x: -len(x[1]))[:20], 1
        ):
