...
class ComprehensiveTranscriptSearch:
    ...
    def __init__(self):
        ...
        self.output_dir = (
            self.home_dir / "Documents" / f"TRANSCRIPT_SEARCH_{self.timestamp}"
        )
    ...
    def search_by_filename(self, search_path: str):
        ...
        patterns = ["*transcript*", "*lyrics*", "*lyric*", "*transcription*"]
    ...
    def search_volumes(self):
        ...
        volumes = [
            v for v in volumes_path.iterdir() if v.is_dir() and v.name != "Macintosh HD"
        ]
    ...
    def analyze_nocturne_transcripts(self):
        ...
        sorted_folders = sorted(
            by_folder.items(), key=lambda x: len(x[1]), reverse=True
        )
    ...
    def save_comprehensive_report(self):
        ...
        with open(csv_file, "w", newline="", encoding="utf-8") as f:
            ...
            for item in sorted(
                self.found_files, key=lambda x: x["size_kb"], reverse=True
            ):
                writer.writerow(
                    [
                        item["path"],
        ...
        print(
            f"{Colors.GREEN}✅ Total unique files: {len(self.found_files)}{Colors.END}\n"
        )
    ...
    def run(self):
        ...
        print(
            f"  nocTurneMeLoDieS transcripts: {Colors.CYAN}{nocturne_count}{Colors.END}"
        )
        print(
            f"  Total unique files: {Colors.CYAN}{len(self.found_files)}{Colors.END}\n"
        )
