...
class BatchProcessor:
    ...
    def process_directory(:
        ...
        if self.transcribe:
            ...
            logger.info("=" * 60)
            self.transcriber.batch_transcribe(input_dir, transcripts_dir, force=force)
            logger.info(
                f"✓ Transcription complete. Transcripts saved to {transcripts_dir}"
            )
        ...
        if self.analyze:
            ...
            transcript_source = (
                transcripts_dir
                if self.transcribe and transcripts_dir.exists()
                else input_dir
            )
...
def main():
    ...
    parser.add_argument(
        "input", help="Input directory containing audio files or transcripts"
    )
