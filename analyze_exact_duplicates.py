...
def analyze_exact_duplicates(root_path):
    ...
    for file_hash, file_list in duplicates.items():
        ...
        by_category[primary_category]['size'] += size * (len(file_list) - 1)  # Wasted space
        
        by_file_type[ext]['groups'] += 1
        by_file_type[ext]['files'] += len(file_list)
    
    print(f"\n📁 BY LOCATION CATEGORY:")
    for category, data in sorted(by_category.items(), key=lambda x: x[1]['files'], reverse=True):
        wasted_gb = data['size'] / 1024 / 1024 / 1024
        print(f"   {category:30s} {data['groups']:4d} groups, {data['files']:5d} files, {wasted_gb:6.2f} GB wasted")
    
    print(f"\n📄 BY FILE TYPE:")
    for ext, data in sorted(by_file_type.items(), key=lambda x: x[1]['files'], reverse=True)[:15]:
    ...
    for file_hash, file_list in duplicates.items():
        ...
        categories = [f['category'] for f in file_list]
        rel_paths = [f['rel_path'] for f in file_list]
    ...
    if root_vs_organized:
        print(f"   (Files in root that also exist in organized subdirectories)")
    ...
    if archive_duplicates:
        print(f"   (Files duplicated in archive folders)")
    ...
    if project_duplicates:
        print(f"   (Files duplicated across project folders)")
    ...
    total_wasted = sum(size * (len(files) - 1) for hash_key, files in duplicates.items() 
                      for size in [files[0]['size']] if files)
    ...
    root_removable = sum(f['size'] for group in root_vs_organized 
                        for f in group['files'] if f['category'] == 'Root Directory')
    archive_removable = sum(f['size'] for group in archive_duplicates 
                           for f in group['files'][1:] if 'Archive' in f['category'])
    
    print(f"\n📋 Recommended cleanup:")
    print(f"   1. Remove root duplicates (keep organized versions): {root_removable/1024/1024/1024:.2f} GB")
    print(f"   2. Remove archive duplicates (keep originals): {archive_removable/1024/1024/1024:.2f} GB")
    print(f"   3. System files (.DS_Store, .pyc): Can be regenerated/removed")
    
    # Show top duplicate groups by size
    print(f"\n📦 TOP DUPLICATE GROUPS BY SIZE:")
    size_sorted = sorted(duplicates.items(), 
    ...
    for hash_key, file_list in size_sorted[:15]:
        if file_list:
            ...
            if wasted > 1024 * 1024:  # > 1MB wasted
                ...
                print(f"     Size: {size/1024/1024:.2f} MB, Copies: {len(file_list)}, Wasted: {wasted/1024/1024:.2f} MB")
