...
def create_advanced_summary_views(conn):
    ...
    cursor.execute('''
        ...
        SELECT 
            ...
            fc.color_code,
            COUNT(f.id) as file_count,
            SUM(CASE WHEN f.size_bytes IS NOT NULL THEN f.size_bytes ELSE 0 END) as total_size,
            AVG(CASE WHEN f.size_bytes IS NOT NULL THEN f.size_bytes ELSE 0 END) as avg_size
...
def main():
    ...
    print(f"\nProject Summary:")
    cursor.execute("""
        SELECT project_name, file_count, COALESCE(total_size, 0), revenue_potential 
