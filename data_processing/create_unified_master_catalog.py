...
def main():
    ...
    for filepath in sorted(all_paths):
        ...
        entry = {
            ...
            "file_size_mb": existing.get("file_size_mb")
            or new.get("Size_MB")
            or deep.get("Size_MB")
            or "",
            "bitrate_kbps": existing.get("bitrate") or new.get("Bitrate_kbps") or "",
            "file_hash": existing.get("file_hash")
            or new.get("File_Hash")
            or deep.get("Hash")
            or "",
            # Classification
            "content_type": existing.get("content_type")
            or new.get("Content_Type")
            or "",
            "is_your_music": new.get("Is_Your_Music")
            or ("YES" if "Steven Chaplinski" in existing.get("artist", "") else "NO"),
            # Organization
            ...
            "has_metadata_tags": existing.get("has_metadata")
            or new.get("Has_Metadata_Tags")
            or "NO",
            # Transcription (from existing catalog)
            "should_transcribe": existing.get("should_transcribe")
            or ("YES" if existing else "REVIEW"),
    ...
    in_all_three = sum(
        1
        for e in unified
        if e["in_existing_catalog"] == "YES"
        and e["in_new_analysis"] == "YES"
        and e["in_deep_scan"] == "YES"
    )
