...
class RobustFileProcessor:
    ...
    def load_and_analyze_files(self):
        ...
        logger.info(
            f"⏭️  Skipped {skipped_count} files (too large, wrong extension, or errors)"
        )
    ...
    def analyze_relationships(self):
        ...
        sorted_files = sorted(
            self.file_data.items(), key=lambda x: x[1]["priority_score"], reverse=True
        )
                ...
                similar_groups.append(
                    {
                        ...
                        "priority_score": sum(
                            top_files[f]["priority_score"] for f in similar_files
                        ),
                    }
                )
    ...
    def calculate_content_similarity(self, file1_info, file2_info):
        ...
        keyword_similarity = len(keywords1.intersection(keywords2)) / len(
        ...
        type_similarity = (
            1.0 if file1_info["content_type"] == file2_info["content_type"] else 0.0
        )
                ...
                chains.append(
                    {
                        ...
                        "priority_score": sum(
                            file_data[f]["priority_score"] for f in chain
                        ),
                    }
                )
                    ...
                    merge_opportunities.append(
                        {
                            ...
                            "type": "content_merge",
                            "priority_score": sum(
                                file_data[f]["priority_score"] for f in files
                            ),
                        }
                    )
    ...
    def create_optimization_plan(self):
        ...
        for merge in sorted(
            self.relationships["merge_opportunities"],
        ...
        for file_path, file_info in sorted_files[
            :CONSTANT_100
        ]:  # Top CONSTANT_100 files
            if not file_info.get("exists", True) or file_info.get(
                "is_truncated", False
            ):
                continue
        ...
        for chain in sorted(
            self.relationships["dependency_chains"],
            key=lambda x: x["priority_score"],
            reverse=True,
        )[:10]:
            if chain["count"] > 2:
    ...
    def execute_optimization_plan(self, dry_run=True):
        ...
        total_actions = sum(
            len(phase["actions"]) for phase in self.optimization_plan["phases"]
        )
    ...
    def generate_comprehensive_report(self):
        ...
        json_file = (
            self.analysis_dir
            / f"robust_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        report_data = {
        ...
        md_file = (
            self.analysis_dir
            / f"robust_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
    ...
    def generate_markdown_report(self, output_file):
        ...
        with open(output_file, "w") as f:
            ...
            f.write(
                f"- **Content Types**: {len(self.relationships.get('type_groups', {}))}\n"
            )
            f.write(
                f"- **Similar Groups**: {len(self.relationships.get('similar_groups', []))}\n"
            )
            f.write(
                f"- **Merge Opportunities**: {len(self.relationships.get('merge_opportunities', []))}\n"
            )
            f.write(
                f"- **Dependency Chains**: {len(self.relationships.get('dependency_chains', []))}\n"
            )
            f.write(
                f"- **Optimization Phases**: {len(self.optimization_plan.get('phases', []))}\n\n"
            )

            # Content types
            f.write("## 📄 Content Types\n\n")
            for content_type, files in self.relationships.get(
                "type_groups", {}
            ).items():
            ...
            for group in self.relationships.get("similar_groups", [])[:20]:
                f.write(
                    f"- **{group['count']} similar files** ({group['type']}) - Priority: {group['priority_score']}\n"
                )
            f.write(Path("\n"))
            ...
            for merge in self.relationships.get("merge_opportunities", [])[:20]:
                f.write(
                    f"- **{merge['count']} files** ({merge['content_type']}) - Priority: {merge['priority_score']}\n"
                )
            ...
            for phase_num, phase in enumerate(
                self.optimization_plan.get("phases", []), 1
            ):
