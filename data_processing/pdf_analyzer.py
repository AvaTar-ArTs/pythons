...
def get_file_stats(file_path):
    ...
    try:
        ...
        return {"size": 0, "modified": "N/A", "exists": False}
    ...
    for path in set(paths):
        ...
        if stats["exists"]:
            analysis["existing_files"].append(
                {"path": path, "size": stats["size"], "modified": stats["modified"]}
            )
...
def generate_report(analysis):
    ...
    top_dirs = sorted(analysis["path_stats"].items(), key=lambda x: x[1], reverse=True)[
        :10
    ]
    ...
    ext_counts = sorted(
        analysis["extension_analysis"].items(), key=lambda x: x[1], reverse=True
    )
