...
def analyze_csv_files(csv_files):
    ...
    print(
        f"🕐 Date range: {csv_files[-1].stem.split('-')[-1]} → {csv_files[0].stem.split('-')[-1]}"
    )
    ...
    try:
        with open(latest, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if row.get("error"):
                    errors.append(
                        {"title": row.get("title", "Unknown"), "error": row["error"]}
                    )
        ...
        for field in critical_fields:
            if field in field_stats:
                ...
                print(
                    f"   {color}✓{Colors.ENDC} {field:15} {present:4}/{total} ({pct:5.1f}%)"
                )
        ...
        for field in nice_to_have:
            if field in field_stats:
                ...
                icon = "✓" if pct > 10 else "✗"
                print(
                    f"   {color}{icon}{Colors.ENDC} {field:15} {present:4}/{total} ({pct:5.1f}%)"
                )
        ...
        lyrics_pct = (
            field_stats["lyrics"]["present"] / total * 100
            if "lyrics" in field_stats
            else 0
        )
        metadata_pct = (
            sum(field_stats[f]["present"] for f in nice_to_have if f in field_stats)
            / (len(nice_to_have) * total)
            * 100
            if total > 0
            else 0
        )
        ...
        print(
            f"   Lyrics coverage: {lyrics_pct:.1f}% ({'✅ Good' if lyrics_pct > 80 else '⚠️  Poor'})"
        )
        print(
            f"   Metadata richness: {metadata_pct:.1f}% ({'✅ Good' if metadata_pct > 50 else '⚠️  Poor'})"
        )
        ...
        if lyrics_pct < 50:
            print(
                f"   {Colors.WARNING}⚠️{Colors.ENDC}  Low lyrics coverage - try v3.0 extractor for better results"
            )

        if metadata_pct < 30:
            print(
                f"   {Colors.WARNING}⚠️{Colors.ENDC}  Missing metadata - v3.0 extractor captures more fields"
            )

        if total - unique > 10:
            print(
                f"   {Colors.WARNING}⚠️{Colors.ENDC}  Many duplicates - use deduplication tool"
            )

        if len(errors) > total * 0.1:
            print(
                f"   {Colors.WARNING}⚠️{Colors.ENDC}  High error rate ({len(errors) / total * 100:.1f}%) - consider re-extraction"
            )

        if lyrics_pct > 80 and metadata_pct > 50:
            print(
                f"   {Colors.OKGREEN}✅{Colors.ENDC} Data quality is good! Ready for processing."
            )
...
def recommend_best_file(csv_files, json_files):
    ...
    print(
        f"   🕐 Modified: {datetime.fromtimestamp(best['mtime']).strftime('%Y-%m-%d %H:%M:%S')}"
    )
