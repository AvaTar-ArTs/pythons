...
class DedupeMergeDiffTool:
    ...
    def find_duplicates(self, max_size_mb=50, algorithm="md5"):
        ...
        print(
            f"   Potential savings: {self.stats['potential_savings'] / (1024**2):.2f} MB"
        )
    ...
    def diff_files(self, file1, file2, output_format="unified"):
        ...
        if not path1.exists() or not path2.exists():
            print("⚠️  One or both files don't exist")
            return None
    ...
    def diff_directories(self, dir1, dir2, output_file=None):
        ...
        if not path1.exists() or not path2.exists():
            print("⚠️  One or both directories don't exist")
            return None
    ...
    def du(self, path=None, max_depth=3, sort_by="size"):
        ...
        for item in usage[:50]:  # Top 50
            size_str = (
                ...
                if item["size_mb"] < 1024
                else f"{item['size_gb']:.2f} GB"
            )
            path_str = (
                item["path"][:48] + ".." if len(item["path"]) > 50 else item["path"]
            )
    ...
    def generate_report(self, output_file=None):
        ...
        if output_file is None:
            output_file = (
                self.root
                / f"DEDUPE_MERGE_DIFF_REPORT_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
            )
...
def main():
    ...
    dedupe_parser.add_argument(
        "--strategy",
        choices=["newest", "oldest", "largest", "smallest", "shortest_path"],
        default="newest",
        help="Strategy for choosing which file to keep",
    )
    dedupe_parser.add_argument(
        "--max-size", type=int, default=50, help="Max file size in MB to check"
    )
    dedupe_parser.add_argument(
        "--keep-paths", nargs="+", help="Path patterns to prefer keeping"
    )
    dedupe_parser.add_argument(
        "--execute",
        action="store_true",
    ...
    merge_parser.add_argument(
        "--strategy",
        choices=["concatenate", "unique_lines", "overwrite", "rename"],
        default="concatenate",
        help="Merge strategy",
    )
    ...
    diff_parser.add_argument(
        "--format", choices=["unified", "context", "html"], default="unified"
    )
    ...
    find_parser.add_argument(
        "--max-size", type=int, default=50, help="Max file size in MB"
    )
    ...
    if args.command == "dedupe":
        ...
        tool.dedupe(
            strategy=args.strategy, keep_paths=getattr(args, "keep_paths", None)
        )
