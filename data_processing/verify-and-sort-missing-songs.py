...
class MissingSongVerifier:
    ...
    def scan_collection(self):
        ...
        for audio_file in self.music_dir.rglob("*"):
            if audio_file.suffix.lower() in audio_extensions:
                ...
                if any(
                    x in str(audio_file)
                    for x in ["ARCHIVE_", "BACKUP_", "CONSOLIDATION_", "SCAN_"]
                ):
                    continue
    ...
    def load_missing_songs(self):
        ...
        print(
            f"{Colors.CYAN}Loaded {len(self.missing_songs)} claimed missing songs{Colors.END}\n"
        )
    ...
    def verify_missing_songs(self):
        ...
        print(
            f"Comparing {len(self.missing_songs)} songs against {len(self.collection_files)} collection files...\n"
        )
        ...
        for missing in self.missing_songs:
            ...
            for collection_file in self.collection_files:
                ...
                if title_score > 0.7 or (title_score > 0.5 and duration_diff <= 3):
                    if title_score > best_score or (
                        title_score == best_score and duration_diff < best_duration_diff
                    ):
        ...
        print(
            f"{Colors.GREEN}✅ Found with different names: {len(self.found_with_diff_name)}{Colors.END}"
        )
        print(
            f"{Colors.YELLOW}⚠️  Truly missing: {len(self.truly_missing)}{Colors.END}\n"
        )
    ...
    def suggest_organization(self):
        ...
        for song in self.truly_missing:
            ...
            if (
                "moonly" in title_lower
                or "moonlit" in title_lower
                or "moonlight" in title_lower
            ):
                by_series["Moonlight_Serenade_Vol_1"].append(song)
            elif "alley" in title_lower and (
                "blues" in title_lower or "blues" in genre
            ):
                by_series["Blues_in_the_Alley_Vol_3"].append(song)
            elif (
                "junkyard" in title_lower
                or "trash" in title_lower
                or "rubbish" in title_lower
            ):
                by_series["Junkyard_Symphony_Vol_4"].append(song)
            elif (
                "king" in title_lower
                or "queen" in title_lower
                or "royalty" in title_lower
            ):
                by_series["Junkyard_Symphony_Vol_4"].append(song)
    ...
    def save_reports(self, by_series: Dict):
        ...
        with open(summary_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"| Found (Different Name) | {self.stats['found_different_name']} |\n\n"
            )

            if self.found_with_diff_name:
                f.write("## ✅ Found in Collection (Different Names)\n\n")
                f.write(
                    "These songs ARE in your collection with different filenames:\n\n"
                )
                for match in self.found_with_diff_name:
                    ...
                    f.write(
                        f"- **Duration match:** {match['missing_duration']}s vs {match['found_duration']}s (diff: {match['duration_diff']}s)\n\n"
                    )
            ...
            if self.truly_missing:
                ...
                for series, songs in by_series.items():
                    ...
                    for song in songs:
                        f.write(
                            f"- **{song['title']}** ({song['duration_str']}) - {song['genre']}\n"
                        )
    ...
    def run(self):
        ...
        print(
            f"  Claimed missing: {Colors.CYAN}{self.stats['missing_claimed']}{Colors.END}"
        )
        print(
            f"  Found (different names): {Colors.GREEN}{self.stats['found_different_name']}{Colors.END}"
        )
        print(
            f"  Truly missing: {Colors.YELLOW}{self.stats['truly_missing']}{Colors.END}\n"
        )

        if self.found_with_diff_name:
            print(
                f"{Colors.GREEN}✅ Good news! {len(self.found_with_diff_name)} songs are already in your collection!{Colors.END}"
            )
            print(f"{Colors.GREEN}They just have different filenames.{Colors.END}\n")

        if self.truly_missing:
            print(
                f"{Colors.YELLOW}📥 {len(self.truly_missing)} songs need to be downloaded from Suno{Colors.END}"
            )
            print(
                f"{Colors.CYAN}See TRULY_MISSING_WITH_ORGANIZATION.csv for download links and locations{Colors.END}\n"
            )
