    ...
    def identify_smart_actions(self):
        ...
        for entry in self.files_catalog:
            ...
            # Needs transcription: UUID or very bad name
                ...
                re.match(r"^[a-f0-9]{8,}", filename)
                or re.match(r"^\d+\.mp3$", filename)
            ) and entry.get("is_your_music") == "YES":
                actions["transcription_needed"].append(
                    {"file": filepath, "reason": "Cannot determine title from filename"}
                )

            # Needs metadata: has file but no tags
            elif (
                entry.get("has_metadata_tags") in ["NO", "False", "FALSE"]
                and entry.get("is_your_music") == "YES"
            ):
            ...
            # Needs bundle: your music without bundle
                ...
                entry.get("has_bundle") in ["NO", "False", "FALSE"]
                and entry.get("is_your_music") == "YES"
                and entry.get("content_type") == "SONG"
            ):
...
def main():
    ...
    print(
        f"  ?? Transcriptions: {len(actions['transcription_needed'])} (uses Whisper/AssemblyAI)"
    )
    print(
        f"  ???  Add metadata: {len(actions['metadata_addition'])} (uses catalog data)"
    )
    ...
    print("Next steps:")
    print(
        "  1. Quick cleanup: python3 ~/advanced_toolkit/fix_downloaded_filenames.py ? DONE"
    )
