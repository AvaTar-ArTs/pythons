...
def organize_from_csv(csv_path: Path, dry_run: bool = True):
    ...
    for artist in sorted(
        by_artist.keys(), key=lambda x: len(by_artist[x]), reverse=True
    )[:20]:
    ...
    avatararts_count = sum(
        len(files) for artist, files in by_artist.items() if "avatar" in artist.lower()
    )
...
def main():
    ...
    parser.add_argument(
        ...
        default=str(
            Path.home()
            / "Downloads/YOUR_SUNO_SONGS_COMPLETE - YOUR_SUNO_SONGS_COMPLETE.csv"
        ),
    )
