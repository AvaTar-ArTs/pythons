...
def main():
    ...
    wasted_space = sum(
        (len(files) - 1) * files[0]["size"] for files in duplicates.values()
    )
    ...
    report_file = (
        home
        / "AVATARARTS"
        / f"DUPLICATE_FILES_REPORT_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    )
    ...
    csv_file = (
        home
        / "AVATARARTS"
        / f"DUPLICATE_FILES_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
