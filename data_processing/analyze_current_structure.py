...
def main():
    ...
    # Top-level folders
    top_level = [
        f for f in base_dir.iterdir() if f.is_dir() and not f.name.startswith(".")
    ]
    ...
    print("CONTAINER FOLDERS (have subfolders):")
    for container in sorted(containers, key=lambda x: -x["mp3s_total"])[:15]:
        print(
            f"  ?? {container['name']:<40} {container['mp3s_total']:>4} MP3s  ({container['subfolders']} subfolders)"
        )
    ...
    with open(output, "w", newline="", encoding="utf-8") as f:
        ...
        for album in albums:
            recommendation = (
                "Keep as-is"
                if album["mp3s_total"] > 3
                else "Consider merging with related album"
            )
            writer.writerow(
                [
                    album["name"],
        ...
        for container in containers:
            recommendation = (
                "Review subfolders"
                if container["subfolders"] > 10
                else "Good organization"
            )
            writer.writerow(
                [
                    container["name"],
