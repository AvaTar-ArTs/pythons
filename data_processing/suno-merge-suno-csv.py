...
def main():
    ...
    parser.add_argument(
        "--output",
        "-o",
        default="suno_master_collection.csv",
    ...
    for i, (source, count) in enumerate(
        sorted(source_counts.items(), key=lambda x: x[1], reverse=True)[:10], 1
    ):
    ...
    for i, song in enumerate(unique_list[:5], 1):
        ...
        if song["style"]:
            style_preview = (
                song["style"][:50] + "..." if len(song["style"]) > 50 else song["style"]
            )
    ...
    if args.report:
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write("\n\nDUPLICATE ANALYSIS:\n")
            f.write("-" * 70 + "\n")
            for song_id, sources in sorted(
                multi_source.items(), key=lambda x: len(x[1]), reverse=True
            )[:20]:
