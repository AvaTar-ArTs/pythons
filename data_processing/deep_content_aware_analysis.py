...
def extract_song_info(metadata_entry: dict) -> dict:
    ...
    return {
        ...
        "created": (
            data.get("created_at") or data.get("date") or data.get("timestamp") or ""
        ),
        "duration": (
            data.get("duration")
            or data.get("length")
            or data.get("metadata", {}).get("duration")
            or 0
        ),
        "audio_url": (
            data.get("audio_url")
            or data.get("url")
            or data.get("song_url")
        ...
        "model": (
            data.get("model_name") or data.get("model") or data.get("version") or ""
        ),
...
def match_audio_with_metadata(:
    audio_files: list, json_metadata: list, master_csv: dict
) -> list:
...
def main():
    ...
    print("  COMPLETE ? Has audio + JSON + master")
    print("  HAS_JSON ? Has JSON metadata, add to master")
    print("  HAS_MASTER ? Has master entry")
    print("  MINIMAL ? Review metadata")
    print(f"\nOpen: open '{output}'")
