...
def compare_files(files_info):
    ...
    return {
        ...
        "similar_files": sorted(
            similar_files, key=lambda x: x["similarity"], reverse=True
        ),
    }
...
def calculate_similarity(file1, file2):
    ...
    if file1["size"] > 0 and file2["size"] > 0:
        size_diff = abs(file1["size"] - file2["size"]) / max(
...
def generate_report(files_info, comparisons, output_file=None):
    ...
    for info in files_info:
        ...
        if "csv_rows" in info:
            report.append(
                f"   CSV: {info['csv_rows']} rows, {info['csv_columns']} columns"
            )
        ...
        if "text_lines" in info:
            report.append(
                f"   Text: {info['text_lines']} lines, {info['text_words']} words"
            )
            if "has_script" in info:
                report.append(
                    f"   HTML features: script={info['has_script']}, style={info['has_style']}, links={info['has_links']}"
                )

        if "svgxml_lines" in info:
            report.append(f"   SVGXML: {info['svgxml_lines']} lines")
            report.append(
                f"   Contains: SVG={info.get('has_svg', False)}, XML={info.get('has_xml', False)}"
            )
    ...
    if comparisons["duplicates"]:
        for hash_val, files in comparisons["duplicates"].items():
            ...
            for f in files:
                report.append(f"   - {f['name']} ({f['size_kb']} KB)")
            report.append(
                f"   → Recommendation: Keep 1 file, remove {len(files) - 1} duplicate(s)"
            )
    else:
    ...
    total_duplicates = sum(
        len(files) - 1 for files in comparisons["duplicates"].values()
    )
    ...
    for file_type, files in by_type.items():
        if len(files) > 1:
            report.append(
                f"   - {len(files)} {file_type} files could be grouped together"
            )
