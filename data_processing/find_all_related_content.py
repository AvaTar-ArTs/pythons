...
def normalize_for_matching(text: str) -> str:
    ...
    text = re.sub(
        r"(\.mp3|\.wav|\.txt|\.jpg|\.png|\.mp4|\.json|\.md|\.html)$",
        "",
        text,
        flags=re.IGNORECASE,
    )
                ...
                if item.is_file():
                    ...
                    for pattern in patterns:
                        if (
                            ...
                            or item.suffix.lower() in pattern
                        ):
                            files.append(item)
                            break
                elif item.is_dir():
...
def main():
    ...
    for search_dir in [home / "Music", home / "Documents", home / "Downloads"]:
        if search_dir.exists():
            image_files.extend(
                find_content_files(
                    search_dir, [".jpg", ".jpeg", ".png", ".gif", ".webp"], max_depth=3
                )
            )
    ...
    for search_dir in [home / "Music", home / "Movies", home / "Downloads"]:
        if search_dir.exists():
            video_files.extend(
                find_content_files(
                    search_dir, [".mp4", ".mov", ".avi", ".mkv"], max_depth=3
                )
            )
    ...
    for search_dir in [home / "Documents", home / "Music"]:
        if search_dir.exists():
            doc_files.extend(
                find_content_files(
                    search_dir, [".md", ".html", ".pdf", ".doc", ".docx"], max_depth=3
                )
            )
    ...
    for i, song in enumerate(songs):
        ...
        for doc in doc_files:
            ...
            if score > 0.6:
                result["doc_files"].append({"path": str(doc), "score": f"{score:.2f}"})
    ...
    with open(output, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow(
            [
                ...
                "Local_Audio_File",
                "Has_Lyrics_In_CSV",
                "Related_Lyrics_Files",
                "Lyrics_Count",
                "Related_Prompt_Files",
                "Prompt_Count",
                "Related_Image_Files",
                "Image_Count",
                "Related_Video_Files",
        ...
        for result in matched_results:
            ...
            lyrics_str = (
                " | ".join(
                    [f"{m['path']} ({m['score']})" for m in result["lyrics_files"]]
                )
                if result["lyrics_files"]
                else ""
            )
            prompt_str = (
                " | ".join(
                    [f"{m['path']} ({m['score']})" for m in result["prompt_files"]]
                )
            ...
            image_str = (
                " | ".join(
                    [f"{m['path']} ({m['score']})" for m in result["image_files"]]
                )
            ...
            video_str = (
                " | ".join(
                    [f"{m['path']} ({m['score']})" for m in result["video_files"]]
                )
                if result["video_files"]
                else ""
            )
            doc_str = (
                " | ".join([f"{m['path']} ({m['score']})" for m in result["doc_files"]])
                if result["doc_files"]
                else ""
            )
    ...
    print(
        f"  ?? TOTAL: {total_lyrics + total_prompts + total_images + total_videos + total_docs}"
    )
    print()
    ...
    print("Filter by content type to find songs with prompts, images, etc.")
    print(f"\nOpen: open '{output}'")
