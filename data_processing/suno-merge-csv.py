...
class UltimateFolderCleanup:
    ...
    def get_folder_stats(self, folder: Path) -> dict:
        """Get folder info"""
        audio_count = len(list(folder.glob("files/*.mp3"))) + len(
            list(folder.glob("files/*.m4a"))
        )
    ...
    def find_and_merge_all_variants(self):
        ...
        folders = [
            f
            for f in self.nocturne_dir.iterdir()
            if f.is_dir() and not self.should_skip(f.name)
        ]
        ...
        for norm_name, variant_folders in sorted(merge_groups):
            ...
            primary, primary_stats = sorted(
                variants_with_stats, key=lambda x: (-x[1]["audio"], -x[1]["files"])
            )[0]

            print(f"{Colors.BOLD}{norm_name}{Colors.END}")
            print(
                f"  {Colors.GREEN}PRIMARY:{Colors.END} {primary.name} ({primary_stats['audio']} audio, {primary_stats['files']} files)"
            )
            ...
            for folder, stats in variants_with_stats:
                ...
                print(
                    f"  {Colors.YELLOW}MERGE:{Colors.END} {folder.name} ({stats['audio']} audio, {stats['files']} files)"
                )
    ...
    def remove_empty_folders(self):
        ...
        for folder in folders:
            ...
            if stats["files"] == 0 or (stats["audio"] == 0 and stats["files"] < 5):
                print(
                    f"{Colors.YELLOW}{folder.name}{Colors.END} (0 audio, {stats['files']} files)"
                )
    ...
    def run(self):
        ...
        print(
            f"  Empty folders removed: {Colors.CYAN}{self.stats['removed']}{Colors.END}\n"
        )
