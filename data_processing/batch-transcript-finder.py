...
class BatchTranscriptFinder:
    ...
    def __init__(self):
        ...
        self.output_file = (
            Path.home() / "Documents" / f"TRANSCRIPT_INVENTORY_{self.timestamp}.csv"
        )
    ...
    def save_csv(self):
        ...
        with open(self.output_file, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["Path", "Filename", "Parent_Folder", "Size_KB", "Location"]
            )
        ...
        print(
            f"{Colors.GREEN}✅ Total size: {self.stats['total_size_mb']:.2f} MB{Colors.END}\n"
        )
