...
class EtsyOrganizer:
    ...
    def analyze_filename(self, file_path):
        ...
        # T-shirt designs
        if any(
            keyword in name for keyword in ["tshirt", "t-shirt", "shirt", "apparel"]
        ):
        ...
        if any(
            keyword in name
            for keyword in ["halloween", "spooky", "skeleton", "ghost", "witch"]
        ):
        ...
        if any(
            keyword in name
            for keyword in ["funny", "quote", "sarcastic", "pun", "joke"]
        ):
        ...
        if any(
            keyword in name for keyword in ["mockup", "template", "psd", "ai", "svg"]
        ):
        ...
        if any(
            pattern in name
            for pattern in ["_1", "_2", "_3", "copy", "duplicate", "backup"]
        ):
    ...
    def analyze_zip_content(self, zip_path):
        ...
        try:
            with zipfile.ZipFile(zip_path, "r") as zip_file:
                ...
                len([f for f in file_list if f.lower().endswith(".psd")])
            ...
            for duplicate_file, original_file in duplicates:
                try:
                    ...
                    dest_file = (
                        self.etsy_dir
                        / "11_duplicates"
                        / f"duplicate_{duplicate_file.name}"
                    )
...
"""
    ...
    def generate_organization_report(self, stats):
        ...
        report_file = (
            self.etsy_dir
            / "00_archives"
            / f"etsy_organization_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def run_organization(self):
        ...
        print(
            f"🗑️  Empty directories removed: {report.get('empty_directories_removed', 0)}"
        )
