...
class RenameWithDocUpdate:
    ...
    def update_markdown_file(self, md_file, old_name, new_name):
        ...
        try:
            ...
            patterns = [
                ...
                (
                    f"from {old_name.replace('.py', '')} import",
                    f"from {new_name.replace('.py', '')} import",
                ),
    ...
    def update_all_documentation(self):
        ...
        for rename in self.renames:
            ...
            for md_file in self.md_files:
                was_updated, update_count = self.update_markdown_file(
                    md_file, old_name, new_name
                )
    ...
    def generate_update_report(self):
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                f"- **Documentation Files Updated:** {len(set(u['doc_file'] for u in self.updates_made))}\n"
            )
            f.write(
            ...
            for rename in self.renames:
                f.write(
                    f"### {rename['current_name']} → {rename['suggested_name']}\n\n"
                )
                ...
                docs_updated = [
                    u
                    for u in self.updates_made
                    if u["old_name"] == rename["current_name"]
                ]
                if docs_updated:
                    f.write("**Documentation Updated:**\n\n")
                    for doc in docs_updated:
                        f.write(
                            f"- `{doc['doc_file']}` ({doc['replacements']} replacements)\n"
                        )
                    f.write("\n")
    ...
    def execute_full_rename(self):
        ...
        print(
            f"   📝 Updated: {len(set(u['doc_file'] for u in self.updates_made))} markdown files"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Rename files and update documentation"
    )
    parser.add_argument(
        "--csv",
        default="FINAL_RENAME_PLAN_20251106_134344.csv",
