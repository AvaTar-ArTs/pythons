...
class ComprehensivePreservationSystem:
    ...
    def __init__(self):
        ...
        self.preservation_root = (
            Path.home() / "Documents" / f"MUSIC_ARCHIVE_PRESERVATION_{self.timestamp}"
        )
    ...
    def classify_file(self, filepath: Path) -> str:
        ...
        if (
            "lyric" in name_lower
            or "[verse" in content_preview
            or "[chorus" in content_preview
        ):
            return "lyrics"
        ...
        if (
            ...
            or "speaker" in content_preview
            or "timestamp" in content_preview
        ):
            return "transcripts"
    ...
    def preserve_orphaned_content(self):
        ...
        if not scan_dirs:
            print(
                f"{Colors.YELLOW}⚠️  No comprehensive scan found. Skipping orphaned content.{Colors.END}"
            )
            return
        ...
        for file_info in orphaned_files:
            ...
            try:
                ...
                print(
                    f"{Colors.RED}❌ Error preserving {source_path.name}: {e}{Colors.END}"
                )
                ...
                for filename in files:
                    ...
                    if ext in preserve_types:
                        try:
                            ...
                            safe_name = (
                                str(rel_path).replace("/", "_").replace(" ", "_")
                            )
                            ...
                            if preserved_count % 20 == 0:
                                print(
                                    f"  Preserved {preserved_count} files from {archive_dir.name}...",
                                    end="\r",
                                )
            ...
            for analysis_dir in music_dir.glob(pattern):
                if analysis_dir.is_dir():
                    ...
                    try:
                        ...
                        # Count files
                        file_count = sum(
                            1 for _ in target_dir.rglob("*") if _.is_file()
                        )
                        preserved_count += file_count

                        print(
                            f"{Colors.GREEN}✅ Preserved: {analysis_dir.name} ({file_count} files){Colors.END}"
                        )
    ...
    def create_master_inventory(self):
        ...
        with open(inventory_file, "w", newline="", encoding="utf-8") as f:
            ...
            for item in sorted(
                self.inventory, key=lambda x: (x["type"], x["filename"])
            ):
                writer.writerow(
                    [
                        item["filename"],
        ...
        with open(readme_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"This archive contains **{self.stats['files_preserved']:,} files** "
            )
            f.write(
                f"({self.stats['total_size'] / (1024**2):.2f} MB) preserved before cleanup.\n\n"
            )
            ...
            f.write(
                "1. **Browse by Type:** Each subfolder contains specific content type\n"
            )
            f.write(
                "2. **Check Inventory:** `MASTER_INVENTORY.csv` lists all files with original paths\n"
            )
            f.write(
                "3. **Search Content:** Use search tools to find specific songs/stories\n"
            )
            f.write(
                "4. **Restore if Needed:** Original paths are documented for restoration\n\n"
            )
    ...
    def run(self):
        ...
        print(
            "║        Comprehensive Preservation → Nothing Lost → Safe Cleanup              ║"
        )
        print(
            "║                                                                               ║"
        )
        ...
        print(
            f"  Total files preserved: {Colors.CYAN}{self.stats['files_preserved']:,}{Colors.END}"
        )
        print(
            f"  Total size: {Colors.CYAN}{self.stats['total_size'] / (1024**2):.2f} MB{Colors.END}\n"
        )
        ...
        print(
            f"{Colors.GREEN}✅ Safe to proceed with cleanup/merge operations.{Colors.END}\n"
        )

        print(
            f"{Colors.CYAN}Open archive: open '{self.preservation_root}'{Colors.END}\n"
        )
...
def main():
    ...
    print(
        f"{Colors.YELLOW}Files will be copied (not moved) to a timestamped archive.{Colors.END}\n"
    )

    response = (
        input(f"{Colors.BOLD}Proceed with preservation? [Y/n]: {Colors.END}")
        .strip()
        .lower()
    )
