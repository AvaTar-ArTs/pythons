...
class StructuralCleanup:
    def __init__(self, pythons_dir="/Users/steven/pythons"):
        ...
        # Find latest scan
        scans = sorted(
            self.pythons_dir.glob("RECURSIVE_FOLDER_SCAN_*.csv"), reverse=True
        )
        if scans:
    ...
    def load_issues(self):
        ...
        for folder in folders:
            ...
            # 3. Legacy folders
                ...
                kw in path.lower() for kw in ["legacy", "archived", "backup", "old"]
            ):
    ...
    def print_action_plan(self):
        ...
        if self.actions["delete_empty"]:
            print(
                f"🗑️  DELETE EMPTY/NEARLY EMPTY: {len(self.actions['delete_empty'])} folders"
            )
            for item in self.actions["delete_empty"][:10]:
                print(
                    f"   • {item['path']} (depth {item['depth']}, {item['files']} files)"
                )
        ...
        if self.actions["consolidate_legacy"]:
            print(
                f"📦 CONSOLIDATE LEGACY: {len(self.actions['consolidate_legacy'])} folders"
            )
    ...
    def execute_cleanup(self):
        ...
        empty_sorted = sorted(
            self.actions["delete_empty"], key=lambda x: x["depth"], reverse=True
        )
