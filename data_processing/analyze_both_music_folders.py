    ...
    for item in base_path.iterdir():
        ...
        if item.is_dir():
            ...
            result["top_level_folders"].append(
                {"name": item.name, "mp3_count": mp3_count}
            )
...
def main():
    ...
    if music_nocturne["exists"]:
        ...
        sorted_albums = sorted(
            music_nocturne["by_album"].items(), key=lambda x: x[1], reverse=True
        )[:20]
    ...
    if music_lower["exists"] and music_lower["total_folders"] > 0:
        ...
        sorted_albums = sorted(
            music_lower["by_album"].items(), key=lambda x: x[1], reverse=True
        )[:15]
    ...
    print(
        "  ??? avatararts/                                ? Container for subprojects"
    )
    print("  ?   ??? files/")
    print("  ?   ??? images/")
    print("  ?   ??? metadata/")
    ...
    with open(output, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["Location", "Folder_Name", "MP3_Count", "Type", "Recommendation"]
        )

        # ~/Music/nocTurneMeLoDieS folders
        if music_nocturne["exists"]:
            for name, count in sorted(
                music_nocturne["by_album"].items(), key=lambda x: -x[1]
            ):
                ...
                writer.writerow(["nocTurneMeLoDieS", name, count, folder_type, rec])

        # ~/music folders
        if music_lower["exists"]:
            for name, count in sorted(
                music_lower["by_album"].items(), key=lambda x: -x[1]
            ):
                writer.writerow(
                    [
                        "music (lowercase)",
