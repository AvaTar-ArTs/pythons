...
def main():
    ...
    with open(user_updates, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            filepath = normalize_path(
                row.get("filepath") or row.get("current_path") or ""
            )
            if filepath:
            ...
            for field in important_fields:
                if field in user_row and user_row[field]:
                    ...
                    if field == "is_music":
                        ...
                        entry["is_your_music"] = (
                            "YES" if user_row[field] == "True" else "NO"
                        )
                    else:
    ...
    for filepath, user_row in user_data.items():
        ...
        if not any(
            normalize_path(e.get("filepath", "")) == filepath for e in unified_data
        ):
            # New entry - add it
            ...
            # Fill from user data
            new_entry["filepath"] = (
                user_row.get("filepath") or user_row.get("current_path") or ""
            )
            new_entry["filename"] = (
                user_row.get("filename") or user_row.get("current_filename") or ""
            )
            ...
            new_entry["duration_seconds"] = (
                user_row.get("duration_seconds") or user_row.get("duration") or ""
            )
            new_entry["file_size_mb"] = user_row.get("file_size_mb") or ""
            new_entry["content_type"] = user_row.get("content_type") or ""
            new_entry["is_your_music"] = (
                "YES" if user_row.get("is_music") == "True" else "NO"
            )
            new_entry["should_transcribe"] = (
                user_row.get("should_transcribe") or "REVIEW"
            )
