    ...
    for search_dir in search_dirs:
        ...
        for img_ext in [".png", ".jpg", ".jpeg", ".webp"]:
            for img_file in search_dir.rglob(f"*{img_ext}"):
                if any(
                    keyword in img_file.name.lower()
                    for keyword in ["cover", "album", "art", "artwork"]
                ):
...
def save_comprehensive_reference(matches: list):
        ...
        for match in matches:
            writer.writerow(
                [
                    ...
                    "; ".join(match["dalle_prompts"][:3])
                    if match["dalle_prompts"]
                    else "",
                    "; ".join(match["sora_prompts"][:3])
                    if match["sora_prompts"]
                    else "",
                    "; ".join(match["other_prompts"][:3])
                    if match["other_prompts"]
                    else "",
    ...
    complete_matches = [
        m
        for m in matches
        if (m.get("transcripts") or m.get("lyrics"))
        and (m.get("dalle_prompts") or m.get("sora_prompts") or m.get("images"))
    ]
