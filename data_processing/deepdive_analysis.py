...
class DeepDiveAnalyzer:
    ...
    def analyze(self):
        ...
        for name, files in by_name.items():
            if len(files) > 1:
                ...
                if len(set(sizes)) == 1:
                    ...
                    self.stats["duplicates_by_name"][name] = [
        ...
        print("   Calculating file hashes (sampling large groups)...")
        ...
        for size, file_list in self.stats["duplicates_by_size"].items():
            if (
                len(file_list) > 1 and size < 100 * 1024 * 1024
        ...
        self.stats["largest_files"] = [
            {"path": f["rel_path"], "size": f["size"]} for f in sorted_by_size[:20]
        ]
        ...
        self.stats["oldest_files"] = [
            {
                "path": f["rel_path"],
                "date": datetime.fromtimestamp(f["mtime"]).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            }
            ...
            if dir_path.is_dir():
                try:
                    if not any(dir_path.iterdir()):
                        self.stats["empty_directories"].append(
                            str(dir_path.relative_to(self.root_path))
                        )
                except:
                    pass
    ...
    def print_report(self):
        ...
        print(
            f"   Average file size: {self.stats['total_size'] / self.stats['total_files'] / 1024 / 1024:.2f} MB"
        ...
        if hash_dupes > 0:
            total_dupe_files = sum(
                len(files) for files in self.stats["duplicates_by_hash"].values()
            )
            wasted_space = 0
            for file_hash, file_list in list(self.stats["duplicates_by_hash"].items())[
                :10
            ]:
                if file_list:
                    if file_path.exists():
        ...
        size_dupes = {
            k: v for k, v in self.stats["duplicates_by_size"].items() if len(v) > 1
        }
        if size_dupes:
            large_groups = sorted(
                size_dupes.items(), key=lambda x: (len(x[1]), x[0]), reverse=True
            )[:10]
            for size, file_list in large_groups:
                if len(file_list) > 1:
                    print(
                        f"   Size: {size / 1024 / 1024:.2f} MB ({len(file_list)} files)"
                    )
                    for rel_path in file_list[:3]:
                        print(f"     - {rel_path}")
                    if len(file_list) > 3:
        ...
        if self.stats["directory_depth"]:
            ...
            for depth in sorted(self.stats["directory_depth"].keys())[:10]:
                print(
                    f"   Depth {depth}: {self.stats['directory_depth'][depth]:,} files"
                )
    ...
    def export_json(self, output_file):
        ...
        with open(output_path, "w") as f:
            ...
            json_stats = {
                ...
                "duplicates_by_hash": {
                    k: v
                    for k, v in list(self.stats["duplicates_by_hash"].items())[:100]
                },
                "duplicates_by_name": dict(
                    list(self.stats["duplicates_by_name"].items())[:100]
                ),
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: python deepdive_analysis.py <directory> [--batch-size N] [--export json_file]"
        )
        print("\nExample:")
