...
for row in rows:
    ...
    # Count types
    type_count = sum(
        [mp3_count > 0, lyric_count > 0, prompt_count > 0, discography_count > 0]
    )
...
print(
    f"3. Large orphaned lyric collections: {len(analysis['orphaned_lyrics']):,} groups"
)
print("   These have 5+ lyrics but no MP3s")
print()

print(
    f"4. Large orphaned prompt collections: {len(analysis['orphaned_prompts']):,} groups"
)
...
if analysis["large_mp3_collections"]:
    ...
    for i, row in enumerate(
        sorted(
            analysis["large_mp3_collections"],
            key=lambda x: int(x["mp3_count"]),
            reverse=True,
        )[:10],
        1,
    ):
...
if analysis["orphaned_lyrics"]:
    ...
    for i, row in enumerate(
        sorted(
            analysis["orphaned_lyrics"],
            key=lambda x: int(x["lyric_count"]),
            reverse=True,
        )[:10],
        1,
    ):
...
if analysis["mp3s_no_lyrics"]:
    ...
    print("-" * 100)
    top_mp3s_no_lyrics = sorted(
        analysis["mp3s_no_lyrics"], key=lambda x: int(x["mp3_count"]), reverse=True
    )[:10]
...
if analysis["well_organized"]:
    ...
    for i, row in enumerate(
        sorted(
            analysis["well_organized"],
            key=lambda x: int(x["total_items"]),
            reverse=True,
        )[:10],
        1,
    ):
        print(f"   {i}. {row['base_directory']}")
        print(
            f"      {row['mp3_count']} MP3s, {row['lyric_count']} lyrics, "
...
with open(suggestions_file, "w", newline="", encoding="utf-8") as f:
    ...
    # Orphaned lyrics
    for row in sorted(
        analysis["orphaned_lyrics"], key=lambda x: int(x["lyric_count"]), reverse=True
    ):
        writer.writerow(
            [
                "Orphaned Lyrics",
...
if len(analysis["large_mp3_collections"]) > 0:
    recommendations.append(
        f"• Organize {len(analysis['large_mp3_collections'])} large MP3 collections into subfolders"
    )

if len(analysis["orphaned_lyrics"]) > 0:
    recommendations.append(
        f"• Investigate {len(analysis['orphaned_lyrics'])} orphaned lyric collections - find matching MP3s"
    )

if len(analysis["mp3s_no_lyrics"]) > 10:
    recommendations.append(
        f"• Consider adding lyrics to {len(analysis['mp3s_no_lyrics'])} MP3 collections without lyrics"
    )

if len(analysis["well_organized"]) > 0:
    recommendations.append(
        f"• {len(analysis['well_organized'])} groups are well-organized - use as examples"
    )
