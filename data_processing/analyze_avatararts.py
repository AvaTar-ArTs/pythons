...
class AvatarArtsAnalyzer:
    ...
    def generate_report(self):
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                f"- **Duplicate File Groups:** {len(self.stats['duplicates']):,}\n\n"
            )
            ...
            if self.stats["duplicates"]:
                ...
                f.write(
                    f"Found **{len(self.stats['duplicates'])}** groups of duplicate files.\n\n"
                )
                ...
                f.write(
                    f"**Potential space savings:** {total_duplicate_size / (1024**2):.1f} MB\n\n"
                )
            ...
            if self.stats["large_files"]:
                ...
                sorted_large = sorted(
                    self.stats["large_files"], key=lambda x: x["size"], reverse=True
                )[:50]
            ...
            if duplicate_scripts:
                ...
                for name, paths in sorted(
                    duplicate_scripts.items(), key=lambda x: len(x[1]), reverse=True
                )[:30]:
            ...
            sorted_csv_dirs = sorted(
                csv_by_dir.items(), key=lambda x: len(x[1]), reverse=True
            )[:20]
            ...
            readme_files = [
                doc
                for doc in self.stats["markdown_docs"]
    ...
    def generate_action_plan(self):
        ...
        with open(plan_path, "w") as f:
            ...
            if self.stats["duplicates"]:
                ...
                f.write("### Priority 1: Remove Duplicate Files\n")
                f.write(f"- **Impact:** Free up {duplicate_size / (1024**2):.1f} MB\n")
                f.write("- **Effort:** Low (automated)\n")
            ...
            if duplicate_scripts:
                f.write("### Priority 2: Consolidate Duplicate Python Scripts\n")
                f.write("- **Impact:** Reduce confusion, improve maintainability\n")
                f.write("- **Effort:** Medium (requires review)\n")
                f.write(f"- **Duplicate names:** {len(duplicate_scripts)}\n\n")

            # Priority 3: Organize by project
            f.write("### Priority 3: Project Structure Organization\n")
            f.write("- **Impact:** Improved navigation and clarity\n")
            f.write("- **Effort:** High (manual organization)\n")
            f.write(
    ...
    def export_inventory_csv(self):
        ...
        with open(csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["Type", "Path", "Name", "Size (bytes)", "Size (MB)", "Extension"]
            )
