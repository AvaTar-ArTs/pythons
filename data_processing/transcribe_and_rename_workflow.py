...
def extract_title_from_transcription(text: str, lyrics_patterns: list = None) -> str:
    ...
    # Clean up first line
    first_line = re.sub(
        r"^(music|song|audio|recording)\s*", "", first_line, flags=re.IGNORECASE
    )
...
def main():
    ...
    with open(needs_work_csv, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if (
                ...
                and row.get("Is_Your_Music") in ["YES", "True", "TRUE"]
                and row.get("Content_Type") == "SONG"
            ):
    ...
    for i, file_info in enumerate(files_to_process, 1):
        ...
        if not filepath or not Path(filepath).exists():
            print(
                f"[{i}/{len(files_to_process)}] ??  File not found: {file_info.get('Filename')}\n"
            )
            continue
        ...
        if transcription["success"]:
            ...
            new_filename = generate_proper_filename(
                extracted_title, file_info.get("Artist", ""), audio_path.name
            )
            ...
            results.append(
                {
                    ...
                    "suggested_new_path": str(
                        audio_path.parent / f"{new_filename}.mp3"
                    ),
    ...
    if success > 0:
        ...
        for i, result in enumerate(
            [r for r in results if r["status"] == "READY_TO_RENAME"], 1
        ):
