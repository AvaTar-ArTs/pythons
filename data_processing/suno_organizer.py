...
class SunoOrganizer:
    ...
    def _detect_series(self, fp: FileFingerprint) -> Optional[Dict]:
        ...
        for pattern in self.series_patterns:
            ...
            if match:
                ...
                return {"name": base_name, "version": version, "pattern": pattern}
    ...
    def organize_by_genre_and_mood(self, fingerprints: List[FileFingerprint]) -> Dict:
        ...
        for fp in fingerprints:
            ...
            if classification["series"]:
                organized["by_series"][classification["series"]].append(
                    (fp, classification)
                )
    ...
    def create_organization_structure(self, organized: Dict) -> Dict:
        ...
        plan = {
            ...
            "stats": {"total_moves": 0, "by_category": defaultdict(int)},
        }
    ...
    def generate_catalog(self, output_path: Optional[Path] = None) -> Dict:
        ...
        for series, tracks in organized["by_series"].items():
            catalog["by_series"][series] = [fp.path.name for fp, _ in tracks]
...
def main():
    ...
    parser.add_argument(
        "--execute", action="store_true", help="Execute organization (not dry run)"
    )
    parser.add_argument("--output", "-o", help="Output path for catalog")
