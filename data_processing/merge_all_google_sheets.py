...
def load_all_data_sources():
    ...
    # Local files
    local_csv = (
        Path.home() / "Downloads/YOUR_SUNO_SONGS_COMPLETE - MASTER_MERGED_DATA (1).csv"
    )
    if local_csv.exists():
...
def build_unified_database(sources: dict):
    ...
    if "google_sheet_2" in sources:
        for row in sources["google_sheet_2"]:
            title = (
                row.get("SongTitle", "") or row.get("Title", "") or row.get("title", "")
            )
            if title:
                title_norm = normalize_title(title)
                if title_norm not in unified:
...
def analyze_unified(unified: dict):
    ...
    for entry in unified.values():
        ...
        if entry["local_files"] and (
            entry["in_google_sheet_1"] or entry["in_google_sheet_2"]
        ):
...
def main():
    ...
    print("\nThis merges:")
    print("  ? Google Sheet 1 (466 rows)")
    print("  ? Google Sheet 2 (if different)")
    print("  ? Local files (341 files)")
    print("  ? Suno catalog")
    print("\nFilter by 'Status' column:")
    print("  COMPLETE ? Have file + metadata")
    print("  NEED_DOWNLOAD ? In sheets, need to download")
    print("  LOCAL_ONLY ? Have file, review if music or other")
    print(f"\nOpen: open {output}")
