...
with open(merge_file, "w", newline="", encoding="utf-8") as f:
    ...
    for name, files in sorted(
        merge_candidates.items(), key=lambda x: len(x[1]), reverse=True
    ):
        size_groups = defaultdict(list)
        for rel_path, size in files:
            size_groups[size].append(rel_path)
        ...
        for size, paths in size_groups.items():
            for rel_path in paths:
                writer.writerow(
                    [name, rel_path, size, format_size(size), len(files), len(paths)]
                )
...
with open(diff_file, "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)
    writer.writerow(
        ["File_Name", "File_Path", "Size", "Size_Formatted", "Hash", "Variants"]
    )
...
with open(du_file, "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)
    writer.writerow(
        ["Folder", "Size", "Size_Formatted", "File_Count", "Percent_Of_Total"]
    )
...
with open(summary_file, "w", encoding="utf-8") as f:
    ...
    f.write("|------|-------|------------|--------------|\n")
    sorted_dups = sorted(
        duplicates.items(), key=lambda x: len(x[1]) * x[1][0][2], reverse=True
    )
    for file_hash, files in sorted_dups[:20]:
        ...
        f.write(
            f"| `{file_hash[:16]}...` | {len(files)} | {format_size(total_sz)} | {format_size(wasted)} |\n"
        )
    f.write("\n")
    ...
    f.write(
        f"- **Files with Same Name, Different Content:** {len(diff_candidates):,}\n"
    )
    f.write(
        "These files have the same name but different content (different hashes).\n\n"
    )
    ...
    for folder, size in sorted_folders[:20]:
        ...
        folder_display = folder if folder != "." else "(root)"
        f.write(
            f"| `{folder_display}` | {format_size(size)} | {folder_counts[folder]:,} | {pct:.2f}% |\n"
        )
    f.write("\n")

    f.write("## Recommendations\n\n")
    f.write(
        "1. **Remove Duplicates:** Free up {format_size(duplicate_size)} by removing duplicate files\n"
    )
    f.write(
        "2. **Consolidate Files:** Review merge candidates to consolidate similar files\n"
    )
    ...
    f.write("- `deduplicates.csv` - Complete duplicate file analysis\n")
    f.write("- `merge_analysis.csv` - Files that could be merged/consolidated\n")
    f.write("- `diff_analysis.csv` - Files with same name but different content\n")
    f.write("- `disk_usage.csv` - Folder-by-folder disk usage\n")
    f.write("- `comprehensive_summary.md` - This summary report\n\n")
...
print("  - comprehensive_summary.md")
print("\n" + "=" * 70)
