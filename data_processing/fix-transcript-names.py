...
class TranscriptNameFixer:
    ...
    def scan_audio_files(self):
        ...
        print(
            f"{Colors.GREEN}✅ Found {self.stats['audio_files']} audio files{Colors.END}\n"
        )
    ...
    def scan_transcript_files(self):
        ...
        for root, dirs, files in os.walk(self.music_dir):
            ...
            for filename in files:
                if Path(filename).suffix.lower() in text_extensions:
                    ...
                    if any(
                        word in name_lower
                        for word in ["transcript", "analysis", "lyric"]
                    ):
        ...
        print(
            f"{Colors.GREEN}✅ Found {self.stats['transcript_files']} transcript files{Colors.END}\n"
        )
    ...
    def find_mismatches(self):
        ...
        print(
            f"Comparing {self.stats['transcript_files']} transcripts with {self.stats['audio_files']} audio files...\n"
        )
        ...
        for transcript in self.transcript_files:
            ...
            same_dir_audio = [
                a for a in self.audio_files if a["directory"] == trans_dir
            ]
            ...
            if not same_dir_audio:
                ...
                same_dir_audio = [
                    a for a in self.audio_files if a["directory"] == trans_dir.parent
                ]
            ...
            if best_match and best_score > 0.7:
                ...
                if transcript["normalized"] != best_match["normalized"]:
                    ...
                    suffix_match = re.search(
                        r"(_transcript|_analysis|_lyric.*?)(\.|$)",
                        transcript["filename"],
                        re.IGNORECASE,
                    )
                    ...
                    new_name = (
                        f"{best_match['stem']}{suffix}{transcript['path'].suffix}"
                    )
                    ...
                    self.rename_suggestions.append(
                        {
                            ...
                            "directory": str(
                                transcript["directory"].relative_to(self.music_dir)
                            ),
                        }
                    )
        ...
        print(
            f"{Colors.YELLOW}Found {self.stats['mismatches_found']} mismatched transcript names{Colors.END}\n"
        )
    ...
    def execute_renames(self):
        ...
        if not self.rename_suggestions:
            print(
                f"{Colors.GREEN}✅ No renames needed - all transcripts properly named!{Colors.END}"
            )
            return

        print(f"Processing {len(self.rename_suggestions)} rename(s)")
        print(
            f"Mode: {Colors.YELLOW}{'DRY RUN' if self.dry_run else 'LIVE'}{Colors.END}\n"
        )

        for i, rename in enumerate(self.rename_suggestions[:50], 1):  # Show first 50
            print(
                f"{Colors.BOLD}[{i}/{min(len(self.rename_suggestions), 50)}]{Colors.END}"
            )
            ...
            print(
                f"  Matches audio: {rename['audio_match']} ({rename['similarity']:.1%})"
            )
            ...
            if rename["name_conflict"]:
                ...
                while new_path.exists():
                    new_path = (
                        rename["new_path"].parent
                        / f"{rename['new_path'].stem}_{counter}{rename['new_path'].suffix}"
                    )
                    counter += 1
    ...
    def save_report(self):
        ...
        with open(report_file, "w", newline="", encoding="utf-8") as f:
            ...
            for rename in self.rename_suggestions:
                status = (
                    "RENAMED"
                    if not self.dry_run and self.stats["renames_executed"] > 0
                    else "SUGGESTED"
                )
        ...
        print(
            f"{Colors.CYAN}💡 CSV contains old_path & new_path - all you need to revert!{Colors.END}"
        )
    ...
    def run(self):
        ...
        print(
            "║     Find Mismatches → Compare Similarity → Rename                            ║"
        )
        print(
            "║                                                                               ║"
        )
        ...
        print(
            f"  Transcript files: {Colors.CYAN}{self.stats['transcript_files']:,}{Colors.END}"
        )
        print(
            f"  Mismatches found: {Colors.YELLOW}{self.stats['mismatches_found']:,}{Colors.END}"
        )

        if self.dry_run:
            print(
                f"  Renames suggested: {Colors.YELLOW}{self.stats['renames_suggested']:,}{Colors.END}\n"
            )
        else:
            print(
                f"  Renames executed: {Colors.GREEN}{self.stats['renames_executed']:,}{Colors.END}\n"
            )
        ...
        if self.stats["mismatches_found"] > 0:
            print(
                f"{Colors.CYAN}Review TRANSCRIPT_RENAMES.csv for all suggested changes{Colors.END}\n"
            )

        if self.dry_run:
            print(
                f"{Colors.YELLOW}⚠️  This was a DRY RUN. Run with --live to execute renames.{Colors.END}\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--live", action="store_true", help="Live mode (execute renames)"
    )
