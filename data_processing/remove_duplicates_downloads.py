#!/usr/bin/env python3
"""
Safely remove duplicate files from Downloads directory.
"""
...
log_file = Path("/Users/steven/downloads_duplicate_removal_log.txt")
...
def remove_duplicates(dry_run=True, backup=True, confirm_each=False):
    ...
    if not duplicates_file.exists():
        ...
        print("Please run find_duplicates_downloads.py first.")
        return
    ...
    with open(duplicates_file, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            file_path = downloads_path / row["File_Path"]
            if file_path.exists():
    ...
    print(f"\n{'=' * 60}")
    print("DUPLICATE FILE REMOVAL - DOWNLOADS")
    print(f"{'=' * 60}")
    print(
    ...
    if backup and not dry_run:
        backup_dir = Path(
            f"/Users/steven/Downloads_Duplicates_Backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    log_entries = []
    log_entries.append(f"Downloads Duplicate Removal Log - {datetime.now()}")
    ...
    for i, file_info in enumerate(files_to_remove, 1):
        ...
        if not file_path.exists():
            log_entries.append(
                f"[{i}/{len(files_to_remove)}] SKIP: {rel_path} (file not found)"
            )
            continue
        ...
        if confirm_each and not dry_run:
            ...
            if response != "y":
                log_entries.append(
                    f"[{i}/{len(files_to_remove)}] SKIP: {rel_path} (user declined)"
                )
                continue
        ...
        try:
            if dry_run:
                if i <= 20 or i % 100 == 0:  # Show first 20 and every 100th
                    print(
                        f"[{i}/{len(files_to_remove)}] WOULD REMOVE: {rel_path} ({format_size(file_info['size'])})"
                    )
                log_entries.append(
                    f"[{i}/{len(files_to_remove)}] DRY RUN: {rel_path} ({format_size(file_info['size'])})"
                )
                removed_count += 1
                ...
                if backup and backup_dir:
                    ...
                    if i <= 20 or i % 100 == 0:
                        log_entries.append(
                            f"[{i}/{len(files_to_remove)}] BACKED UP: {backup_path}"
                        )
                ...
                if i <= 20 or i % 100 == 0:
                    print(
                        f"[{i}/{len(files_to_remove)}] REMOVED: {rel_path} ({format_size(file_info['size'])})"
                    )
                log_entries.append(
                    f"[{i}/{len(files_to_remove)}] REMOVED: {rel_path} ({format_size(file_info['size'])})"
                )
    ...
    if dry_run:
        ...
        print(
            "To actually remove files, run with: python3 remove_duplicates_downloads.py --live"
        )
    ...
    if len(sys.argv) > 1:
        if "--live" in sys.argv or "--remove" in sys.argv:
            ...
            if "--force" not in sys.argv:
                print("WARNING: This will DELETE files from Downloads!")
                try:
                    ...
                    print("ERROR: Interactive confirmation required.")
                    print(
                        "Use --force flag to skip confirmation in non-interactive mode."
                    )
                    sys.exit(1)
        ...
        if "--confirm" in sys.argv:
            confirm_each = True
        if "--force" in sys.argv and ("--live" in sys.argv or "--remove" in sys.argv):
            print(
                "WARNING: Running in FORCE mode - files will be deleted without confirmation!"
            )
