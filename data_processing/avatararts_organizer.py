...
class AvatarArtsOrganizer:
    ...
    def __init__(self, base_music_dir: Optional[Path] = None):
        ...
        self.base_music_dir = (
            base_music_dir or Path.home() / "Music/nocTurneMeLoDieS/FINAL_ORGANIZED"
        )
    ...
    def categorize_by_metadata(:
        self, fingerprints: List[FileFingerprint]
        ...
        for fp in fingerprints:
            ...
            if not artist:
                ...
                if (
                    "avatararts" in fp.path.name.lower()
                    or "avatar" in fp.path.name.lower()
                ):
                    artist = "avatararts"
    ...
    def _create_organization_plan(self, categorized: Dict) -> Dict:
        ...
        plan = {"moves": [], "stats": {"total": 0, "by_artist": {}, "by_category": {}}}
        ...
        for artist, files in categorized["by_artist"].items():
            ...
            for fp in files:
                ...
                plan["stats"]["total"] += 1
                plan["stats"]["by_artist"][artist] = (
                    plan["stats"]["by_artist"].get(artist, 0) + 1
                )
                plan["stats"]["by_category"][category] = (
                    plan["stats"]["by_category"].get(category, 0) + 1
                )
    ...
    def _show_plan_summary(self, plan: Dict):
        ...
        for artist in sorted(plan["stats"]["by_artist"].keys()):
            ...
            artist_categories = set(
                move["category"] for move in plan["moves"] if move["artist"] == artist
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--execute", action="store_true", help="Actually move files (not dry run)"
    )
