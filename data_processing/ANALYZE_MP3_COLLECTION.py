...
results = {"mp3": [], "discography": [], "prompts": [], "lyrics": []}
...
for depth in sorted(depth_stats.keys()):
    ...
    avg_mb = (
        (stats["size"] / stats["count"]) / (1024 * 1024) if stats["count"] > 0 else 0
    )
...
sorted_prompt_dirs = sorted(
    prompt_by_dir.items(), key=lambda x: x[1]["count"], reverse=True
)
...
sorted_lyric_dirs = sorted(
    lyric_by_dir.items(), key=lambda x: x[1]["count"], reverse=True
)
print(f"{'Count':<8} {'Size (MB)':<12} {'Directory':<70}")
print("-" * 100)
...
with open(csv_disc, "w", newline="", encoding="utf-8") as f:
    ...
    for item in results["discography"]:
        writer.writerow([item["name"], item["path"], item["type"], item["depth"]])
...
with open(csv_summary, "w", newline="", encoding="utf-8") as f:
    ...
    writer.writerow(
        ["Duplicate instances", sum(len(v) for v in duplicates.values()), 0, 0]
    )
