...
def resize_image(im, output_path):
    ...
    while quality >= 20:
        ...
        quality -= 5
    logging.warning(
        f"Could not shrink {os.path.basename(output_path)} below {MAX_IMAGE_SIZE_MB}MB"
    )
    return False
...
def upscale_image(im, output_path):
    ...
    logging.info(
        f"Upscaled {os.path.basename(output_path)} -> {new_size[0]}x{new_size[1]}"
    )
...
# 📦 Main processing
def process_images(:
    source, mode, from_csv=False, column="path", log_file=None, verbose=False
):
