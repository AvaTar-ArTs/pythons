...
def analyze_scattered_files(inventory_file):
    ...
    scattered = {name: files for name, files in files_by_name.items() if len(files) > 1}
...
def generate_report(categories, scattered):
    ...
    with open(csv_file, "w", newline="", encoding="utf-8") as f:
        ...
        for category_name, items in categories.items():
            ...
            for filename, locations in sorted(
    ...
    with open(md_file, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"- **Exact Duplicates:** {total_exact_dupes} (same content, can be consolidated)\n"
        )
        f.write(
            f"- **Different Content:** {total_different} (same name, different implementations)\n"
        )
        f.write(
            f"- **Package Files:** {len(categories['package_files'])} (expected duplicates)\n\n"
        )

        # Exact duplicates section
        f.write("## Exact Duplicates (High Priority)\n\n")
        f.write(
            "These files have identical content across multiple locations. **Action:** Keep one canonical copy, delete others.\n\n"
        )

        sorted_dupes = sorted(
            categories["exact_duplicates"], key=lambda x: len(x[1]), reverse=True
        )

        for i, (filename, locations) in enumerate(sorted_dupes[:30], 1):
            location_counts = {}
            for loc in locations:
                location_counts[loc["location"]] = (
                    location_counts.get(loc["location"], 0) + 1
                )
            ...
            f.write(
                f"**Total waste:** {sum(loc['size_kb'] for loc in locations[1:]):.2f} KB\n\n"
            )
        ...
        f.write(
            "These files share the same name but have different implementations. **Action:** Rename or merge intelligently.\n\n"
        )

        sorted_diff = sorted(
            categories["different_content"], key=lambda x: len(x[1]), reverse=True
        )
        ...
        for i, (filename, locations) in enumerate(sorted_diff[:20], 1):
            ...
            f.write(
                f"### {i}. {filename} ({len(locations)} copies, {len(hashes)} unique versions)\n\n"
            )
            f.write("**Locations:**\n")
            for loc_name, count in sorted(
                location_counts.items(), key=lambda x: x[1], reverse=True
            ...
            for j, (hash_val, locs) in enumerate(by_hash.items(), 1):
                f.write(
                    f"- Version {j}: {len(locs)} file(s), {locs[0]['code_lines']} lines\n"
                )
