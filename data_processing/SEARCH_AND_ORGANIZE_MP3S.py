    ...
    json.dump(
        {
            ...
            "summary": {
                ...
                "discography": {"count": disc_count},
                "prompts": {"count": prompt_count, "size_mb": round(prompt_size, 2)},
                "lyrics": {"count": lyric_count, "size_mb": round(lyric_size, 2)},
                "duplicates": {
...
with open(csv_file_depth, "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)
    writer.writerow(["depth", "file_type", "count", "total_size_mb", "total_size_gb"])
...
sorted_locations = sorted(
    mp3_locations.items(), key=lambda x: x[1]["count"], reverse=True
)
...
for loc, data in sorted_locations[:20]:
    ...
    avg_depth = (
        sum(data["avg_depth"]) / len(data["avg_depth"]) if data["avg_depth"] else 0
    )
