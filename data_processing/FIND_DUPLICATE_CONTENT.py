    ...
    def find_name_pattern_dupes(self):
        ...
        for file_info in self.files:
            ...
            base_patterns = [
                (
                    r"_\d+\.py$",
                    name.replace("_1.py", "").replace("_2.py", "").replace("_3.py", ""),
                ),
                (
                    r" \(\d+\)\.py$",
                    name.replace(" (1).py", "")
                ...
                (
                    r"-\d+\.py$",
                    name.replace("-1.py", "").replace("-2.py", "").replace("-3.py", ""),
                ),
                (
                    r"(_copy|_Copy| copy| Copy)\.py$",
                    name.replace("_copy.py", ".py").replace(" copy.py", ".py"),
                ),
                (
                    r"(-v\d+|_v\d+)\.py$",
                    name.replace("-v1.py", ".py").replace("-v2.py", ".py"),
                ),
            ]
        ...
        for base_name, paths in patterns.items():
            if len(paths) > 1:
                pattern_dupes.append(
                    {"base": base_name, "count": len(paths), "files": paths}
                )
    ...
    def save_report(self, exact_dupes, similar_names, pattern_dupes):
        ...
        with open(csv_file, "w", newline="") as f:
            ...
            for dup in exact_dupes:
                paths_str = " | ".join(
                    str(p.relative_to(self.pythons_dir)) for p in dup["files"]
                )
                writer.writerow(
                    [dup["hash"], dup["count"], f"{dup['size'] / 1024:.2f}", paths_str]
                )
        ...
        with open(csv_file2, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["Normalized Name", "Variant Count", "Actual Names", "File Paths"]
            )

            for sim in similar_names[:500]:  # Top 500
                names_str = " | ".join(sim["names"])
                paths_str = " | ".join(
                    str(p.relative_to(self.pythons_dir)) for p in sim["files"]
                )
                writer.writerow([sim["normalized"], sim["count"], names_str, paths_str])
        ...
        with open(csv_file3, "w", newline="") as f:
            ...
            for pat in pattern_dupes[:500]:
                paths_str = " | ".join(
                    str(p.relative_to(self.pythons_dir)) for p in pat["files"]
                )
                writer.writerow([pat["base"], pat["count"], paths_str])
