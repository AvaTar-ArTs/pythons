...
class WorkflowEngine:
        ...
        for i, step in enumerate(workflow_config.get("steps", []), 1):
            ...
            if "if" in step:
                if not self._evaluate_condition(step["if"], context):
                    logger.info(
                        f"  [{i}] ⏭️  Skipping: {step_name} (condition: {step['if']})"
                    )
                    continue
    ...
    def _evaluate_condition(self, condition: str, context: Dict) -> bool:
        """Evaluate conditional logic safely.

        SECURITY FIX: Replaced eval(condition, {"__builtins__": {}}) with a
        safe parser. The old approach was vulnerable to attribute-access attacks
        (e.g. ().__class__.__bases__[0].__subclasses__()).

        Supported conditions: simple comparisons like:
            media_type == 'audio'
            audio_detected == true
            count > 5
        """
        try:
            ...
            # SECURITY: Use ast.literal_eval instead of eval for safe parsing.
            # For boolean/string/numeric comparisons, we parse and evaluate
            # using a restricted set of operators rather than raw eval.
            return self._safe_bool_eval(condition)
        except Exception as e:
            logger.warning(f"Condition evaluation error: {e}")
            return True

    def _safe_bool_eval(self, expr: str) -> bool:
        """Safely evaluate a simple boolean expression without eval().

        Supports: ==, !=, >, <, >=, <= with string and numeric operands.
        """
        import ast
        ...
        def _eval_node(node):
            if isinstance(node, ast.Expression):
                ...
                # Bare variable name — treat as truthy if it exists in context
                return True
        ...
        "steps": [
            {
                ...
                "params": {"source_dir": "${source_dir}", "method": "hash"},
            },
            {
                "name": "rename_files",
                "component": "file_renamer",
                "params": {"source_dir": "${source_dir}", "strategy": "${strategy}"},
            },
        ],
    },
    "data_processor": {
        ...
        "steps": [
            {
                "name": "load_data",
                "component": "data_validator",
                "params": {"input_file": "${input_file}", "format_type": "auto"},
            },
            {
                "name": "enrich_data",
                "component": "data_enricher",
                "if": "operation == 'enrich'",
                "params": {"input_data": "${input_file}", "model": "gpt-4-vision"},
            },
        ],
    },
    "gallery_builder": {
        ...
        "steps": [
            ...
            },
            {
                ...
                "params": {"source_dir": "${source_dir}", "template": "${template}"},
            },
        ],
    },
}
...
def main():
    ...
    run_parser.add_argument(
        "--strategy", type=str, default="intelligent", help="Organization strategy"
    )
    run_parser.add_argument(
        "--template", type=str, default="grid", help="Gallery template"
    )
    ...
    engine.register_component(
        "whisper_transcriber", WhisperTranscriber("whisper_transcriber")
    )
    engine.register_component("image_upscaler", ImageUpscaler("image_upscaler"))
    engine.register_component(
        "file_duplicate_detector", FileDuplicateDetector("file_duplicate_detector")
    )
    ...
    engine.register_component(
        "html_gallery_generator", HtmlGalleryGenerator("html_gallery_generator")
    )
