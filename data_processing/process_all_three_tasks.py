...
def task2_extract_metadata(ultimate_data: list, output_dir: Path):
    ...
    has_file = [
        item for item in ultimate_data if item.get("Overall_Status") == "HAS_FILE"
    ]
...
def task3_review_complete(ultimate_data: list, output_dir: Path):
    ...
    complete = [
        item for item in ultimate_data if item.get("Overall_Status") == "COMPLETE"
    ]
...
def main():
    ...
    print(
        f"  ?? Task 2: Extracted metadata from {len(extracted_metadata)} files (sample)"
    )
