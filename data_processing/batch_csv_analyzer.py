...
def classify_content_type(content: str, role: str) -> str:
    ...
    if any(
        ...
        for word in ["analysis", "analyze", "analyzing", "examined"]
    ):
        return "Analysis"
    elif any(
        word in content_lower for word in ["created", "generated", "built", "made"]
    ):
        return "Creation"
    elif any(
        word in content_lower for word in ["explain", "describe", "summary", "overview"]
    ):
        return "Explanation"
    elif any(word in content_lower for word in ["fix", "error", "issue", "problem"]):
        return "Troubleshooting"
    elif any(
        word in content_lower for word in ["improve", "enhance", "optimize", "better"]
    ):
        return "Improvement"
    else:
        return "Response"
    ...
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                stats["max_content_length"] = max(
                    stats["max_content_length"], content_length
                )
                if content_length > 0:
                    stats["min_content_length"] = min(
                        stats["min_content_length"], content_length
                    )
        ...
        if stats["total_rows"] > 0:
            stats["avg_content_length"] = (
                stats["total_content_length"] // stats["total_rows"]
            )
        else:
...
def analyze_directory(directory_path: str) -> Dict:
    ...
    for i, csv_file in enumerate(csv_files, 1):
        ...
        if stats.get("first_date"):
            if (
                not overall_stats["date_range"]["earliest"]
                or stats["first_date"] < overall_stats["date_range"]["earliest"]
            ):
                overall_stats["date_range"]["earliest"] = stats["first_date"]

        if stats.get("last_date"):
            if (
                not overall_stats["date_range"]["latest"]
                or stats["last_date"] > overall_stats["date_range"]["latest"]
            ):
                overall_stats["date_range"]["latest"] = stats["last_date"]

        overall_stats["conversation_lengths"].append(stats["conversation_turns"])

    overall_stats["avg_rows_per_file"] = (
        overall_stats["total_rows"] / len(csv_files) if csv_files else 0
    )
    overall_stats["avg_content_length"] = (
        overall_stats["total_content_length"] / overall_stats["total_rows"]
        if overall_stats["total_rows"] > 0
        else 0
    )
    overall_stats["avg_conversation_length"] = (
        statistics.mean(overall_stats["conversation_lengths"])
        if overall_stats["conversation_lengths"]
        else 0
    )
...
"""
    ...
    for content_type, count in overall["content_types"].most_common(10):
        percentage = (
            (count / overall["total_assistant_messages"] * 100)
            if overall["total_assistant_messages"] > 0
            else 0
        )
...
"""
    ...
    long_content_files = [
        s
        for s in analysis_results["individual_stats"]
        if s.get("has_long_content", False)
    ]

    report += f"- **Files with Long Content:** {len(long_content_files)}\n"
    for stats in sorted(
        long_content_files, key=lambda x: x.get("max_content_length", 0), reverse=True
    )[:10]:
...
"""
    ...
    details += (
        "| Filename | Rows | Human | Assistant | Avg Length | Topics | Actions |\n"
    )
    details += (
        "|----------|------|-------|-----------|------------|--------|---------|\n"
    )
    ...
    for stats in sorted(
        analysis_results["individual_stats"], key=lambda x: x.get("filename", "")
    ):
...
def main():
    ...
    directory = (
        sys.argv[1] if len(sys.argv) > 1 else "/Users/steven/claude/conversations"
    )
    ...
    print(
        f"   - Total messages: {results['overall_stats']['total_human_messages'] + results['overall_stats']['total_assistant_messages']:,}"
    )
