...
class BatchCleanupAnalyzer:
    ...
    def generate_cleanup_csv(self, volume_name, duplicates):
        ...
        with open(csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["External_Path", "Master_Path", "Size_KB", "Filename", "Action"]
            )
        ...
        for i, row in enumerate(rows, 1):
            ...
            if dry_run:
                ...
                try:
                    ...
                    if i % 100 == 0:
                        print(
                            f"  [{i}/{len(rows)}] Removed {removed_count} files ({removed_size / (1024**2):.1f} MB)"
                        )
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print(
            "  python batch-cleanup-analyzer.py index                    # Build master index"
        )
        print(
            "  python batch-cleanup-analyzer.py scan <volume>            # Scan volume (2T-Xx or DeVonDaTa)"
        )
        print(
            "  python batch-cleanup-analyzer.py find <volume>            # Find duplicates"
        )
        print(
            "  python batch-cleanup-analyzer.py execute <csv> [--live]   # Execute cleanup"
        )
        print()
        ...
        print(
            "  5. python batch-cleanup-analyzer.py execute 2T-Xx_duplicates_to_remove.csv --live"
        )
        return
    ...
    if command == "index":
        ...
        # Find duplicates
        duplicates, unique = analyzer.find_duplicates_in_batch(
            volume_name, master_index
        )
