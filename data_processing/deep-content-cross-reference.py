...
class DeepContentCrossReference:
    ...
    def __init__(self):
        ...
        self.output_dir = (
            Path.home() / "Music" / f"CONTENT_CROSS_REFERENCE_{self.timestamp}"
        )
    ...
    def scan_documents_for_content(self):
        ...
        # Summary
        print(
            f"{Colors.GREEN}✅ Scanned {self.stats['documents_scanned']} text files{Colors.END}\n"
        )
    ...
    def load_audio_files_from_analysis(self):
        ...
        print(
            f"{Colors.GREEN}✅ Loaded {len(self.audio_files)} audio files{Colors.END}\n"
        )
    ...
    def cross_reference_content(self):
        ...
        print(
            f"Matching {self.stats['documents_scanned']} content files with {len(self.audio_files)} audio files...\n"
        )
                ...
                for content_type, files in self.content_files.items():
                    for content_file in files:
                        score = self.similarity_score(
                            audio_file["name"], content_file["name"]
                        )
        ...
        print(
            f"{Colors.GREEN}✅ Matched {len(self.matches)} audio-content pairs{Colors.END}"
        )
        print(
            f"{Colors.YELLOW}⚠️  Orphaned audio (no content): {len(orphaned_audio)}{Colors.END}"
        )
        print(
            f"{Colors.YELLOW}⚠️  Orphaned content (no audio): {len(orphaned_content)}{Colors.END}\n"
        )
    ...
    def save_reports(self, orphaned_audio: List, orphaned_content: List):
        ...
        print(
            f"{Colors.GREEN}✅ Orphaned audio: {orphaned_audio_file.name}{Colors.END}"
        )
        ...
        with open(orphaned_content_file, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["Content_File", "Path", "Type", "Size_KB", "Depth", "Recommendation"]
            )
        ...
        print(
            f"{Colors.GREEN}✅ Orphaned content: {orphaned_content_file.name}{Colors.END}"
        )
        ...
        with open(by_type_file, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["Content_Type", "Total_Files", "Matched_to_Audio", "Orphaned"]
            )
            ...
            for content_type, files in self.content_files.items():
                ...
                matched = sum(
                    1 for m in self.matches if m["content_type"] == content_type
                )
                orphaned = len(files) - matched

                writer.writerow([content_type, len(files), matched, orphaned])
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"Found **{self.stats['orphaned_audio']}** audio files with no associated content.\n"
            )
            f.write("These may be safe to archive/remove.\n\n")

            f.write("### Content Without Audio\n\n")
            f.write(
                f"Found **{self.stats['orphaned_content']}** content files with no associated audio.\n"
            )
            ...
            f.write(
                "1. **Review Orphaned Content** - Check `ORPHANED_CONTENT_NO_AUDIO.csv` before removing anything\n"
            )
            f.write(
                "2. **Preserve Matched Pairs** - Ensure audio-content pairs stay together during consolidation\n"
            )
            f.write(
                "3. **Archive Safely** - When archiving duplicates, verify content files are preserved\n\n"
            )
    ...
    def run(self):
        ...
        print(
            "║        Stories · Lyrics · Transcripts · Prompts → Audio Matching             ║"
        )
        print(
            "║                                                                               ║"
        )
        ...
        # 4. Save reports
        self.save_reports(orphaned_audio, orphaned_content)
        ...
        print(
            f"  Content files scanned: {Colors.CYAN}{self.stats['documents_scanned']:,}{Colors.END}"
        )
        print(
            f"  Audio files analyzed: {Colors.CYAN}{self.stats['audio_files']:,}{Colors.END}"
        )
        print(
            f"  Matched pairs: {Colors.GREEN}{self.stats['matched_pairs']:,}{Colors.END}"
        )
        print(
            f"  Orphaned audio: {Colors.YELLOW}{self.stats['orphaned_audio']:,}{Colors.END}"
        )
        print(
            f"  Orphaned content: {Colors.YELLOW}{self.stats['orphaned_content']:,}{Colors.END}\n"
        )
        ...
        print(
            "  2. ORPHANED_AUDIO_NO_CONTENT.csv - Audio without content (may be safe to archive)"
        )
        print(
            "  3. ORPHANED_CONTENT_NO_AUDIO.csv - Content without audio (REVIEW before removing!)"
        )
        print("  4. CROSS_REFERENCE_REPORT.md - Full analysis\n")

        print(f"{Colors.YELLOW}⚠️  IMPORTANT:{Colors.END}")
        print(
            f"{Colors.YELLOW}Review ORPHANED_CONTENT_NO_AUDIO.csv before removing any duplicates!{Colors.END}"
        )
        print(
            f"{Colors.YELLOW}This ensures stories/lyrics aren't lost during consolidation.{Colors.END}\n"
        )
