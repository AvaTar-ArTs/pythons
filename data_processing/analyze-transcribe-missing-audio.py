...
class AnalyzeAndTranscribe:
    ...
    def scan_all_folders(self):
        ...
        for root, dirs, files in os.walk(self.root_dir):
            ...
            dirs[:] = [
                d for d in dirs if not d.startswith(".") and d != self.output_dir.name
            ]
    ...
    def save_reports(self):
        ...
        if self.missing_transcripts:
            ...
            print(
                f"{Colors.GREEN}✅ Missing transcripts saved: {transcript_csv.name} ({len(self.missing_transcripts)} files){Colors.END}"
            )
        ...
        if self.missing_analysis:
            ...
            print(
                f"{Colors.GREEN}✅ Missing analysis saved: {analysis_csv.name} ({len(self.missing_analysis)} files){Colors.END}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze and transcribe missing content in audio directories."
    )
    parser.add_argument(
        "-d",
        "--directory",
