...
def main():
    ...
    csv_file = (
        output_dir
        / f"DATABASE_INVENTORY_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
    ...
    report_file = (
        output_dir / f"DATABASE_REPORT_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    )
    ...
    with open(report_file, "w") as f:
        ...
        for purpose, dbs in sorted(
            by_purpose.items(), key=lambda x: len(x[1]), reverse=True
        ):
            total_size_cat = sum(db["size_mb"] for db in dbs)
            f.write(
                f"{purpose.upper().replace('_', ' ')} ({len(dbs)} databases, {total_size_cat:.2f} MB)\n"
            )
            f.write("-" * 80 + "\n")

            for db in sorted(dbs, key=lambda x: x["size_mb"], reverse=True)[:10]:
                f.write(
                    f"  {Path(db['path']).name:<50} {db['size_mb']:>8.2f} MB, {db['tables']:>3} tables\n"
                )
        ...
        for r in sorted(results, key=lambda x: x["size_mb"], reverse=True)[:20]:
            f.write(
                f"{Path(r['path']).name:<50} {r['size_mb']:>12.2f} {r['tables']:>8} {r['purpose']:>20}\n"
            )
