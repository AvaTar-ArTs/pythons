...
def compare_and_merge(google_data: list, local_data: list):
    ...
    print("   (In both Google Sheet and local)")
    print()

    print(f"?? IN GOOGLE, NOT LOCAL: {len(in_google_not_local)} songs")
    print("   (Songs in your Google Sheet you haven't downloaded)")
    print()

    print(f"?? IN LOCAL, NOT GOOGLE: {len(in_local_not_google)} files")
    print("   (Files you have but not in Google Sheet)")
    print()
...
def save_master_comparison(matched, in_google_not_local, in_local_not_google):
    ...
    with open(output, "w", newline="", encoding="utf-8") as f:
        ...
        for google_row in in_google_not_local[:100]:  # Limit to 100
            title = (
                google_row.get("SongTitle")
                or google_row.get("Title")
                or google_row.get("title")
                or ""
            )
            writer.writerow(
...
def main():
    ...
    print("\nNow you can see:")
    print("  ? Which files match your Google Sheet")
    print("  ? Which songs you need to download")
    print("  ? Which local files aren't in the sheet")
    print(f"\nOpen it: open {output}")
