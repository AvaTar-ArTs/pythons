...
def main():
    ...
    csv_backup_file = (
        fstr(Path.home()) + "/python_aggressive_renaming_backup_{timestamp}.csv"
    )
    renamer.create_csv_backup(csv_backup_file)

    # Generate execution script
    execution_script = (
        fstr(Path.home()) + "/python_aggressive_rename_execute_{timestamp}.py"
    )
    ...
    logger.info("   This will actually suggest changes for your files!")
