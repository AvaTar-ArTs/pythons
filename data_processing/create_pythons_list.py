...
def categorize_file(filepath):
    ...
    if any(
        x in name for x in ["cleanup", "clean", "organize", "merge", "dedupe", "remove"]
    ):
    ...
    if any(
        x in name
        for x in ["instagram", "twitter", "youtube", "social", "file-organizer"]
    ):
        return "Social Media Tools"

    # AI/ML
    if any(
        x in name for x in ["ai", "gpt", "llm", "model", "transcribe", "transcription"]
    ):
...
def get_directory_info(dirpath):
    ...
    try:
        ...
        subdirs = [
            d for d in dirpath.iterdir() if d.is_dir() and not d.name.startswith(".")
        ]
