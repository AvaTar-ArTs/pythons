...
class DeepMusicDirectoryAnalyzer:
    ...
    def classify_content(self, filepath: Path) -> str:
        ...
        # Audiobooks
        if (
            "audiobook" in path_lower
            or "elion" in path_lower
            or "thinketh" in path_lower
        ):
            return "AUDIOBOOK"
        ...
        if filepath.suffix.lower() in [".json", ".csv", ".txt", ".md"]:
            if (
                "metadata" in path_lower
                or "prompt" in path_lower
                or "lyric" in path_lower
            ):
                return "METADATA"
            return "TEXT"
    ...
    def scan_directory(self, directory: str) -> Dict:
            ...
            for item in dir_path.rglob("*"):
                if item.is_dir() and not item.name.startswith("."):
                    ...
                    result["subdirectories"].append(
                        {"path": str(rel_path), "depth": depth, "full_path": str(item)}
                    )
        ...
        print(
            f"  Found {result['total_files']} files ({result['total_size'] / (1024**3):.2f} GB)"
        )
    ...
    def find_duplicates(self) -> Dict:
        ...
        print(
            f"{Colors.GREEN}Found {len(duplicates)} files duplicated across directories{Colors.END}"
        )

        # Calculate space wasted
        space_wasted = sum(
            dup["size"] * (dup["count"] - 1) for dup in duplicates.values()
        )
        print(
            f"{Colors.YELLOW}Space wasted on duplicates: {space_wasted / (1024**2):.2f} MB{Colors.END}"
        )
    ...
    def generate_consolidation_plan(self, scan_results: List[Dict], duplicates: Dict):
        ...
        for title, files in audiobook_by_title.items():
            ...
            if len(sources) > 1:
                ...
                print(
                    f"    {Colors.GREEN}→ Consolidate to: ~/Music/Audiobooks/{title}/{Colors.END}"
                )
        ...
        for hash_val, dup_info in list(duplicates.items())[:10]:
            print(f"\n  {dup_info['name']}:")
            print(
                f"    Duplicated {dup_info['count']} times across {len(dup_info['sources'])} directories"
            )
            ...
            priority_order = [
                "Audiobooks",
                "Other_Content",
                "_OLD_DIRECTORIES_ARCHIVED",
                "_ARCHIVED_DIRECTORIES",
            ]
            keep_instance = None
            ...
            print(
                f"    {Colors.GREEN}Keep: {keep_instance['relative_path']}{Colors.END}"
            )
        ...
        print(
            "  • Move useful content from Other_Content/Albums/AUDIOBOOKS to ~/Music/Audiobooks"
        )
    ...
    def save_reports(:
        self, scan_results: List[Dict], duplicates: Dict, recommendations: List[Dict]
    ):
        ...
        with open(inventory_file, "w", newline="", encoding="utf-8") as f:
            ...
            for file_info in sorted(
                all_files_list,
                key=lambda x: (x["source_directory"], x["relative_path"]),
            ):
                writer.writerow(
                    [
                        file_info["name"],
        ...
        with open(duplicates_file, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["Filename", "Count", "Directories", "Size_MB", "Hash", "Paths"]
            )

            for hash_val, dup_info in sorted(
                duplicates.items(), key=lambda x: x[1]["count"], reverse=True
            ):
                paths = " | ".join(
                    [inst["relative_path"] for inst in dup_info["instances"]]
                )
                writer.writerow(
                    [
                        dup_info["name"],
        ...
        with open(audiobook_file, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["Filename", "Source_Directory", "Audiobook_Title", "Size_MB", "Path"]
            )
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                "1. **Consolidate Audiobooks:** Move all audiobook files to `~/Music/Audiobooks/`\n"
            )
            f.write(
                "2. **Remove Duplicates:** Eliminate cross-directory duplicates to save space\n"
            )
            f.write(
                "3. **Archive System Files:** Clean up `_ARCHIVED_DIRECTORIES` (mostly system files)\n"
            )
            f.write(
                "4. **Organize Content:** Review `Other_Content` structure for optimization\n\n"
            )
    ...
    def run(self):
        ...
        print(
            "║                    Multi-Depth Archive Consolidation                          ║"
        )
        print(
            "║                                                                               ║"
        )
        ...
        print(
            f"  Total Size: {Colors.CYAN}{self.stats['total_size'] / (1024**3):.2f} GB{Colors.END}"
        )
        print(f"  Audio Files: {Colors.CYAN}{self.stats['total_audio']:,}{Colors.END}")
        print(
            f"  Audiobook Files: {Colors.CYAN}{self.stats['total_audiobooks']:,}{Colors.END}"
        )
        ...
        print(
            f"  2. Check {Colors.CYAN}CROSS_DIRECTORY_DUPLICATES.csv{Colors.END} for duplicates"
        )
        print(
            f"  3. Review {Colors.CYAN}AUDIOBOOK_CONSOLIDATION.csv{Colors.END} for audiobook merging"
        )
        print(
            f"  4. Use {Colors.CYAN}CONSOLIDATION_PLAN.json{Colors.END} for automated merging\n"
        )
...
def main():
    ...
    parser.add_argument(
        "--live", action="store_true", help="Live mode (execute changes)"
    )
