...
class BatchVolumeCleanup:
    ...
    def quick_hash(self, filepath):
        ...
        try:
            ...
            with open(filepath, "rb") as f:
                chunk = f.read(1024)
            return hashlib.md5(
                f"{filepath.name}{stat.st_size}{chunk}".encode()
            ).hexdigest()
        except:
            return None
    ...
    def step1_index_master(self):
        ...
        py_files = [
            f
            for f in self.master_dir.rglob("*.py")
    ...
    def step2_scan_volume_sample(self, volume_name, sample_size=5000):
        ...
        for fp in all_py_files:
            ...
            if count % 500 == 0:
                print(
                    f"    Processed: {count} ({len(dupes)} dupes, {len(unique)} unique)"
                )
        ...
        with open(sample_file, "w") as f:
            json.dump(
                {"sample": sample, "duplicates": dupes, "unique": unique}, f, indent=2
            )
    ...
    def step3_summarize_findings(self):
        ...
        for vol_name, stats in volumes_analyzed.items():
            ...
            report_lines.append(
                f"- **Space Reclaimable**: {stats['size_gb']:.2f} GB\n\n"
            )
        ...
        report_lines.append(
            "3. Run: `python batch-volume-cleanup.py execute <volume> --dry-run`\n"
        )
        report_lines.append(
            "4. If satisfied: `python batch-volume-cleanup.py execute <volume> --live`\n"
        )
