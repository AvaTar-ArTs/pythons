...
def main():
    ...
    csv_file = (
        output_dir
        / f"DOWNLOADS_PYTHON_INVENTORY_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
    ...
    with open(csv_file, "w", newline="") as f:
        ...
        for dir_name, files in sorted(
            projects.items(), key=lambda x: len(x[1]), reverse=True
        ):
    ...
    report_file = (
        output_dir
        / f"DOWNLOADS_CATEGORIZATION_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    )
    ...
    with open(report_file, "w") as f:
        ...
        for cat_name, dirs in sorted(
            categories.items(), key=lambda x: sum(d[1] for d in x[1]), reverse=True
        ):
            if not dirs:
                continue
            ...
            f.write(
                f"{cat_name.upper()} ({len(dirs)} directories, {total_files} files)\n"
            )
            f.write("-" * 80 + "\n")

            for dir_name, file_count in sorted(dirs, key=lambda x: x[1], reverse=True)[
                :20
            ]:
    ...
    with open(migration_script, "w") as f:
        ...
        for cat_name, dirs in categories.items():
            ...
            for dir_name, file_count in sorted(dirs, key=lambda x: x[1], reverse=True)[
                ...
                f.write(
                    f"# mv ~/Downloads/{dir_name} ~/pythons/recovered/{cat_name}/\n"
                )
            f.write("\n")
    ...
    for cat_name, dirs in sorted(
        ...
        if dirs:
            total_files = sum(d[1] for d in dirs)
            print(
                f"  {cat_name:<20} {len(dirs):>3} directories, {total_files:>6,} files"
            )
