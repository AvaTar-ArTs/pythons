...
class CleanupAnalyzer:
    ...
    def _find_cache_files(self, max_depth: int, skip_dirs: Set[str]):
        ...
        for root, dirs, files in os.walk(self.home_dir):
            ...
            if any(
                pattern.lower() in dir_name.lower()
                for pattern in self.safe_patterns["cache"]
            ):
                for file in files:
                    filepath = Path(root) / file
                    try:
    ...
    def _find_large_files(:
        self, max_depth: int, skip_dirs: Set[str], min_size_mb: int = 100
    ):
        """Find large files'\''
    ...
    def _find_backup_files(self, max_depth: int, skip_dirs: Set[str]):
        ...
        for root, dirs, files in os.walk(self.home_dir):
            ...
            for file in files:
                ...
                if any(
                    filepath.name.endswith(ext) or filepath.name.endswith("~")
...
def generate_report(findings: Dict, output_file: Path):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        f.write("## 📊 Summary\n\n")
        f.write(
            f"- **Total Removable Size:** {format_size(findings['total_size_removable'])}\n"
        )
        ...
        if findings["duplicates"]:
            ...
            for i, dup in enumerate(findings["duplicates"][:20], 1):  # Top 20
                ...
                f.write(
                    f"**Keep:** `{dup['keep']['path']}` ({format_size(dup['keep']['size'])})\n\n"
                )
                f.write("**Remove:**\n")
                for file in dup["remove"]:
                    f.write(
                        f"- `{file['path']}` ({format_size(file['size'])}, {file['modified'].strftime('%Y-%m-%d')})\n"
                    )
        ...
        if findings["temp_files"]:
            ...
            for file in findings["temp_files"][:50]:  # Top 50
                f.write(
                    f"- `{file['path']}` ({format_size(file['size'])}, {file['age_days']} days old)\n"
                )
            f.write("\n")
        ...
        if findings["large_files"]:
            ...
            for file in findings["large_files"][:20]:  # Top 20
                f.write(
                    f"- `{file['path']}` ({format_size(file['size'])}, modified {file['modified'].strftime('%Y-%m-%d')})\n"
                )
            f.write("\n")
