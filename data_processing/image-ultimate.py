...
class ImageProcessor:
    ...
    def needs_dpi_fix(self, image: Image.Image) -> bool:
        ...
        return (
            dpi_x < self.config.min_dpi_threshold
            or dpi_y < self.config.min_dpi_threshold
        )
    ...
    def calculate_new_dimensions(:
        self, width: int, height: int, target_size_mb: float
    ) -> Tuple[int, int]:
    ...
    def resize_image(:
        self, image: Image.Image, output_path: str, target_size_mb: float = None
    ) -> bool:
        ...
        original_size_mb = (
            os.path.getsize(output_path) / (CONSTANT_1024 * CONSTANT_1024)
        ...
        new_width, new_height = self.calculate_new_dimensions(
            width, height, target_size_mb
        )

        # Ensure we don't exceed maximum dimensions
        if new_width > self.config.max_width or new_height > self.config.max_height:
            scale_factor = min(
                self.config.max_width / new_width, self.config.max_height / new_height
            )
        ...
        for quality in range(
            ...
            current_size_mb = os.path.getsize(output_path) / (
                CONSTANT_1024 * CONSTANT_1024
            )
            if current_size_mb <= target_size_mb:
                self.logger.info(
                    f"Successfully resized to {current_size_mb:.2f}MB at quality {quality}"
                )
                return True
    ...
    def upscale_image(self, image: Image.Image, output_path: str) -> bool:
        ...
        if (
            output_path.lower().endswith((".jpg", ".jpeg"))
            and upscaled_image.mode != "RGB"
        ):
    ...
    def fix_dpi(self, image: Image.Image, output_path: str) -> bool:
        ...
        dpi_x, dpi_y = self.get_image_dpi(image)
        self.logger.info(
            f"Fixing DPI from ({dpi_x:.1f}, {dpi_y:.1f}) to ({self.config.target_dpi}, {self.config.target_dpi})"
        )
    ...
    def process_single_image(:
        self, input_path: str, output_path: str, mode: ProcessingMode
    ) -> Dict:
        ...
        try:
            # Get file size
            original_size_mb = os.path.getsize(input_path) / (
                CONSTANT_1024 * CONSTANT_1024
            )
            ...
            with Image.open(input_path) as image:
                ...
                if mode in [
                    ProcessingMode.UPSCALE_ONLY,
                    ProcessingMode.BOTH,
                    ProcessingMode.SMART,
                    ProcessingMode.COMPREHENSIVE,
                ]:
                    closest_ratio, (min_width, min_height) = (
                        self.get_closest_aspect_ratio(width, height)
                    )
                ...
                if os.path.exists(output_path):
                    final_size_mb = os.path.getsize(output_path) / (
                        CONSTANT_1024 * CONSTANT_1024
                    )
    ...
    def process_directory(:
        self, input_dir: str, output_dir: str, mode: ProcessingMode
    ) -> List[Dict]:
        ...
        for i in range(0, len(image_files), self.config.batch_size):
            ...
            for image_file in tqdm(
                batch, desc=f"Processing batch {i // self.config.batch_size + 1}"
            ):
                ...
                result = self.process_single_image(
                    str(image_file), str(output_file), mode
                )
...
Examples:
  ...
  python ultimate_image_processor_v2.py input_dir output_dir --mode smart
    ...
    logger.info(
        f"Mode: {args.mode}, Max size: {args.max_size}MB, Upscale: {args.upscale_factor}x, Target DPI: {args.dpi}"
    )
