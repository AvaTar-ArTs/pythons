...
class ConversationExporter:
    def __init__(self, transcript_path: str, output_dir: str = None):
        ...
        self.output_dir = (
            Path(output_dir) if output_dir else Path.home() / "claude_conversations"
        )
    ...
    def read_transcript(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Error: Transcript file not found at {self.transcript_path}",
                file=sys.stderr,
            )
            return []
...
<body>
    <div class="header">
        ...
        <div class="meta">
            ...
            + """</div>
            <div>Source: """
            + html.escape(str(self.transcript_path))
            + """</div>
        </div>
    </div>
...
def main():
    ...
    try:
        ...
        if not transcript_path:
            logger.info(
                "Error: No transcript_path provided in hook input", file=sys.stderr
            )
            sys.exit(1)
        ...
        output_dir = os.environ.get(
            "CLAUDE_CONVERSATION_DIR", str(Path.home() / "claude_conversations")
        )
        ...
        if len(sys.argv) > 1:
            ...
            logger.info(
                "Usage: conversation_exporter.py <transcript_path> [output_dir]"
            )
