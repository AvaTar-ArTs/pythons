"""
Summary of fix-bare-except-statements.py
...
class BareExceptFixer:
    ...
    def detect_context(self, code_before: str) -> str:
        ...
        if any(word in code_lower for word in ["open(", "read", "write", "file"]):
            return "file"
        if any(word in code_lower for word in ["request", "urllib", "http", "url"]):
            return "network"
        if "json" in code_lower:
            return "json"
        if "import" in code_lower:
            return "import"
        if "[" in code_before and "]" in code_before:
            return "index"
        if any(word in code_lower for word in ["int(", "float(", "str("]):
            return "value"
