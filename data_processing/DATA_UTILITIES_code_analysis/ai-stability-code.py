...
class ContextAnalyzer:
    ...
    LANGUAGE_SIGNATURES = {
        ...
        "json": {"extensions": [".json"], "patterns": [r"^\s*[\{\[]", r'"[\w-]+":\s*']},
        "markdown": {
    ...
    @staticmethod
    def _detect_language(filepath: Path, content: str) -> str | None:
    ...
    def _detect_purpose(filepath: Path, content: str) -> str:
        ...
        if "/lib/" in str(filepath) or "/package/" in str(filepath):
            return "library"
        if "main." in filepath.name or "index." in filepath.name:
            return "application"
        return "module"

    @staticmethod
    def _analyze_metrics(content: str, language: str | None) -> dict:
        """Analyze code metrics"""
        lines = content.split("\n")
        ...
        return {
            ...
            "imports": len(
                re.findall(r"^(import|from|require|use)\s+", content, re.MULTILINE),
            ),
        }
...
class InsightGenerator:
    ...
    def _security_insights(:
        ...
        if re.search(
            r'(password|passwd|pwd|secret|api_key)\s*=\s*["\'][^"\']{3,}[""\']',
            content,
            re.IGNORECASE,
        ):
            insights.append(
    ...
    def _quality_insights(:
        ...
        if context.language == "python":
            ...
            if try_blocks == 0 and (
                "open(" in content or "requests." in content or "urllib" in content
            ):
                insights.append(
                    Insight(
                        category="quality",
    ...
    def _performance_insights(:
        ...
        # Nested loops
        nested_for = len(
            re.findall(r"for\s+\w+\s+in[^:]+:\s*.*for\s+\w+\s+in", content, re.DOTALL),
        )
        if nested_for > 2:
            insights.append(
                Insight(
    ...
    def _style_insights(:
        ...
        if context.language in ["python", "javascript", "typescript"]:
            magic_numbers = re.findall(
                r"\b(CONSTANT_100|CONSTANT_1000|CONSTANT_256|CONSTANT_512|CONSTANT_1024|CONSTANT_86400|CONSTANT_3600)\b",
                content,
            )
    ...
    def _documentation_insights(:
        ...
        if "readme" in filepath.name.lower():
            ...
            has_install = bool(
                re.search(
                    r"##?\s+(install|installation|setup)",
                    content,
                    re.IGNORECASE,
                ),
            )
            ...
            has_examples = bool(
                re.search(r"##?\s+(example|demo)", content, re.IGNORECASE),
            )
...
class CodeAnalyzer:
    ...
    def _analyze_directory(self, dirpath: Path) -> AnalysisResult:
        ...
        logger.info(
            f"Found {len(files_to_analyze)} files to analyze (out of {len(all_files)} total)",
        )
        ...
        for i, filepath in enumerate(
            ...
            try:
                ...
                file_insights = self.insight_generator.generate(
                    filepath,
                    content,
                    context,
                )
...
class AnalysisReporter:
    ...
    # ANSI color codes
    COLORS = {
        "critical": r"\CONSTANT_33[91m",  # Red
        "high": r"\CONSTANT_33[93m",  # Yellow
        "medium": r"\CONSTANT_33[94m",  # Blue
        "low": r"\CONSTANT_33[92m",  # Green
        "info": r"\CONSTANT_33[96m",  # Cyan
        "reset": r"\CONSTANT_33[0m",
        "bold": r"\CONSTANT_33[1m",
    }
    ...
    def print_report(result: AnalysisResult, show_details: bool = True):
        ...
        logger.info(Path("\n") + "=" * 80)
        logger.info(
            f"{AnalysisReporter.COLORS['bold']}📊 INTELLIGENT CODE ANALYSIS REPORT{AnalysisReporter.COLORS['reset']}",
        )
        logger.info("=" * 80)
        ...
        logger.info(
            f"{AnalysisReporter.COLORS['bold']}⏱  Timestamp:{AnalysisReporter.COLORS['reset']} {result.timestamp}",
        )
        logger.info(
            f"{AnalysisReporter.COLORS['bold']}📄 Files Analyzed:{AnalysisReporter.COLORS['reset']} {result.files_analyzed}",
        )
        ...
        if summary.get("by_severity"):
            ...
            for severity in AnalysisReporter.SEVERITY_ORDER:
                ...
                if count > 0:
                    ...
                    logger.info(
                        f"  {color}{severity.upper()}: {count}{AnalysisReporter.COLORS['reset']}",
                    )
        ...
        logger.info(Path("\n") + "=" * 80)
        logger.info(
            f"{AnalysisReporter.COLORS['bold']}✅ Analysis Complete{AnalysisReporter.COLORS['reset']}",
        )
        logger.info("=" * 80 + Path("\n"))
...
Examples:
  ...
  python code_analyzer.py /path/to/project --summary-only
    ...
    parser.add_argument(
        ...
        "--summary-only",
        action="store_true",
        help="Show summary only (no detailed insights)",
    )
