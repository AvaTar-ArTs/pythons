                ...
                for row in rows:
                    ...
                    if title:
                        ...
                        if key not in all_songs:
                            all_songs[key] = {"title": title, "sources": [], "data": []}
...
def compare_datasets(suno_audio: list, suno_csvs: dict, master_csv: dict) -> dict:
    ...
    for title, files in audio_by_title.items():
        if len(files) > 1:
            comparison["duplicates"].append(
                {"title": title, "count": len(files), "files": files}
            )
    ...
    print(
        f"  ? In all three (audio + SUNO CSV + master): {len(comparison['in_all_three'])}"
    )
    print(
        f"  ?? In audio + SUNO CSV (not master): {len(comparison['in_audio_and_suno_csv'])}"
    )
    print(
        f"  ?? In audio + master (not SUNO CSV): {len(comparison['in_audio_and_master'])}"
    )
...
def main():
    ...
    print("  COMPLETE ? Verified everywhere")
    print("  NEED_MASTER_UPDATE ? In SUNO, add to master")
    print("  SUNO_VERIFIED ? Has master entry")
    print("  UNVERIFIED ? Review for duplicates/issues")
    print(f"\nOpen: open '{output}'")
