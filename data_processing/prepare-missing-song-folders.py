...
class MissingSongFolderPrep:
    ...
    def __init__(self, dry_run: bool = True):
        ...
        self.stats = {"folders_created": 0, "subfolders_created": 0}
    ...
    def create_series_folders(self):
        ...
        for series_name, info in self.series_folders.items():
            ...
            if location == self.nocturne_dir:
                ...
                if existing:
                    print(
                        f"  {Colors.GREEN}✓ Found existing: {existing.name}{Colors.END}"
                    )
                    print(
                        f"  {Colors.CYAN}→ Will merge into existing folder{Colors.END}"
                    )
                    ...
                    if not self.dry_run:
                        ...
                        print(
                            f"  {Colors.GREEN}✅ Created: {series_path.name}{Colors.END}"
                        )
                    else:
                        print(
                            f"  {Colors.YELLOW}[DRY RUN] Would create: {series_path.name}{Colors.END}"
                        )
                ...
                if not series_path.exists():
                    if not self.dry_run:
                        ...
                        print(
                            f"  {Colors.YELLOW}[DRY RUN] Would create: {series_path}{Colors.END}"
                        )
    ...
    def generate_download_checklist(self, echoes_series: str, series_mapping: dict):
        ...
        # Load truly missing
        verification_dirs = sorted(
            Path.home().glob("Music/MISSING_SONG_VERIFICATION_*")
        )
    ...
    def run(self):
        ...
        print(
            f"  Series folders created: {Colors.CYAN}{self.stats['folders_created']}{Colors.END}"
        )
        print(
            f"  Subfolders created: {Colors.CYAN}{self.stats['subfolders_created']}{Colors.END}\n"
        )
        ...
        if self.dry_run:
            print(
                f"{Colors.YELLOW}⚠️  This was a DRY RUN. Run with --live to create folders.{Colors.END}\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--live", action="store_true", help="Live mode (create folders)"
    )
