...
def is_your_music(artist: str, title: str, filename: str, filepath: str) -> bool:
    ...
    if artist:
        ...
        if any(
            term in artist_lower
            for term in ["avatar", "avatararts", "ava tar", "steven chaplinski"]
        ):
            return True
...
def load_all_knowledge_bases(home: Path) -> dict:
    ...
    if gs2.exists():
        ...
        print(
            f"? Google Sheet 2: {len(knowledge['google_sheet_2'])} songs (with lyrics)"
        )

    # SUNO master catalog
    suno_master = (
        home / "Music/nocTurneMeLoDieS/SUNO/suno-sept/songs_master_combined.csv"
    )
...
def comprehensive_analysis(mp3_file: Path, knowledge: dict) -> dict:
    ...
    for gs_title, gs_data in knowledge["google_sheet_1"].items():
        ...
        if score > 0.7:
            if (
                "google_sheet_1" not in best_scores
                or score > best_scores["google_sheet_1"]
            ):
    ...
    for gs_title, gs_data in knowledge["google_sheet_2"].items():
        ...
        if score > 0.7:
            if (
                "google_sheet_2" not in best_scores
                or score > best_scores["google_sheet_2"]
            ):
...
def main():
    ...
    # Find all MP3s
    search_paths = [home / "Music", home / "Downloads", home / "Documents"]
    ...
    with open(output, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow(
            [
                ...
                "In_Master_CSV",
                "Has_Bundle",
                "Has_Downloads_Content",
                "Has_Lyrics",
                "Has_Metadata_Tags",
                "Completeness_Score",
                "Status",
                "Recommendation",
                "Google_Sheet_1_Match_Score",
                "Google_Sheet_2_Match_Score",
                "Genre",
                "Album",
                "Comment",
            ]
