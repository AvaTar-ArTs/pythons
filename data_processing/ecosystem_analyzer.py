...
class EcosystemAnalyzer:
    ...
    def scan_repository(self, name: str, path: Path) -> Dict:
        ...
        print(
            f"  ✓ Found {stats['total_files']} files ({self._format_size(stats['total_size'])})"
        )
        return stats
    ...
    def scan_all_repositories(self) -> Dict:
        ...
        output_file = (
            self.analysis_dir
            / f"ecosystem_scan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def generate_summary_report(self) -> str:
        ...
        for repo_name, repo_data in sorted(self.results.items()):
            ...
            # Top 3 extensions
            top_exts = sorted(
                repo_data["by_extension"].items(), key=lambda x: x[1], reverse=True
            )[:3]
        ...
        for ext, count in sorted(all_exts.items(), key=lambda x: x[1], reverse=True)[
            :15
        ]:
        ...
        total_small = sum(
            r.get("by_size", {}).get("small", 0)
            for r in self.results.values()
            if "error" not in r
        )
        total_medium = sum(
            r.get("by_size", {}).get("medium", 0)
            for r in self.results.values()
            if "error" not in r
        )
        total_large = sum(
            r.get("by_size", {}).get("large", 0)
            for r in self.results.values()
            if "error" not in r
        )
        ...
        report_lines.append("|---------------|-------|------------|")
        report_lines.append(
            f"| Small (<10KB) | {total_small:,} | {total_small / total_files * 100:.1f}% |"
        )
        report_lines.append(
            f"| Medium (10KB-1MB) | {total_medium:,} | {total_medium / total_files * 100:.1f}% |"
        )
        report_lines.append(
            f"| Large (>1MB) | {total_large:,} | {total_large / total_files * 100:.1f}% |"
        )
        ...
        report_file = (
            self.analysis_dir
            / f"ECOSYSTEM_REPORT_{datetime.now().strftime('%Y%m%d')}.md"
        )
...
def main():
    ...
    parser.add_argument(
        "--generate-report", action="store_true", help="Generate summary report"
    )
    parser.add_argument(
        "--base-dir", type=Path, default=Path.home(), help="Base directory (default: ~)"
    )
    ...
    if not any(
        [args.scan_all, args.find_duplicates, args.compare_repos, args.generate_report]
    ):
