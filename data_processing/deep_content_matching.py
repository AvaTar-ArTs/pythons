...
def normalize_for_matching(text: str) -> str:
    ...
    text = re.sub(
        r"\.(mp3|wav|txt|jpg|png|json|md|html)$", "", text, flags=re.IGNORECASE
    )
    # Remove common words
    text = re.sub(
        r"\b(the|a|an|by|suno|avatararts?|avatar|arts?)\b",
        "",
        text,
        flags=re.IGNORECASE,
    )
...
def similarity_score(s1: str, s2: str) -> float:
    ...
    return SequenceMatcher(
        None, normalize_for_matching(s1), normalize_for_matching(s2)
    ).ratio()


def find_content_in_directory(:
    search_dir: Path, patterns: list, max_depth: int = 4
) -> list:
...
def main():
    ...
    content_files = {"lyrics": [], "prompts": [], "images": [], "docs": []}
    ...
    for dir_name, dir_path in search_dirs.items():
        ...
        lyrics = find_content_in_directory(
            dir_path, [".txt", ".lrc", ".srt", ".vtt", "lyric", "lyrics"], max_depth=3
        )
        ...
        images = find_content_in_directory(
            dir_path, [".jpg", ".jpeg", ".png", ".webp"], max_depth=3
        )
        content_files["images"].extend(images)

        # Docs
        docs = find_content_in_directory(
            dir_path, [".md", ".html", ".pdf"], max_depth=3
        )
        ...
        for img in content_files["images"]:
            ...
            if score > 0.6:
                matches["image_files"].append(
                    {"path": str(img), "name": img.name, "score": score}
                )
        ...
        for doc in content_files["docs"]:
            ...
            if score > 0.6:
                matches["doc_files"].append(
                    {"path": str(doc), "name": doc.name, "score": score}
                )
    ...
    with open(output, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow(
            [
                ...
                "Has_Lyrics_In_CSV",
                "Audio_Filepath",
                "Matched_Lyrics_Files",
                "Lyrics_Count",
                "Matched_Prompt_Files",
                "Prompt_Count",
                "Matched_Images",
                "Image_Count",
                "Matched_Docs",
        ...
        for match in matched_songs:
            ...
            # Format file lists
            lyrics_str = " | ".join(
                [f"{m['name']} ({m['score']:.2f})" for m in match["lyrics_files"]]
            )
            prompt_str = " | ".join(
                [f"{m['name']} ({m['score']:.2f})" for m in match["prompt_files"]]
            )
            image_str = " | ".join(
                [f"{m['name']} ({m['score']:.2f})" for m in match["image_files"]]
            )
            doc_str = " | ".join(
                [f"{m['name']} ({m['score']:.2f})" for m in match["doc_files"]]
            )
    ...
    songs_with_any = sum(
        1
        for m in matched_songs
        if any([m["lyrics_files"], m["prompt_files"], m["image_files"], m["doc_files"]])
    )
    ...
    best_bundles = sorted(
        matched_songs,
        key=lambda x: len(x["lyrics_files"])
        + len(x["prompt_files"])
        + len(x["image_files"])
        + len(x["doc_files"]),
        reverse=True,
    ...
    for i, match in enumerate(best_bundles[:10]):
        song = match["song"]
        total = (
            len(match["lyrics_files"])
            + len(match["prompt_files"])
            + len(match["image_files"])
            + len(match["doc_files"])
        )
        ...
        print(
            f"   {len(match['lyrics_files'])} lyrics, {len(match['prompt_files'])} prompts, "
