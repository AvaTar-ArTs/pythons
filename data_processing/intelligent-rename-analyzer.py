...
class IntelligentRenameAnalyzer:
    ...
    def suggest_for_all_caps(self, script):
        ...
        if base in all_caps_mapping:
            return all_caps_mapping[
                base
            ] + ".py", "Converted ALL_CAPS to meaningful lowercase"

        # Generic conversion: remove ULTIMATE, convert to lowercase with hyphens
        better_name = (
            base.replace("_ULTIMATE", "").replace("_FINAL", "").replace("_MASTER", "")
        )
        ...
        if "metadata" in current:
            if "image prompt" in desc or "transcript" in desc:
                return (
                    ...
                    "Generates image prompts from transcripts",
                )
            ...
            if code_info and "gpt" in str(code_info["imports"]).lower():
                if "csv" in desc or "enrich" in desc:
                    ...
                    return (
                        "gpt-vision-prompt-generator.py",
                        "Generates prompts with GPT Vision",
                    )
            ...
            if "json" in current and "image" in desc:
                return (
                    ...
                    "Helper utilities for image metadata",
                )
            else:
    ...
    def analyze_and_suggest(self):
        ...
        for i, script in enumerate(self.scripts, 1):
            ...
            if (
                ...
                or current.isupper()
                or "_" in current
                and current.replace("_", "").replace(".py", "").isupper()
            ):
        ...
        for category, scripts in sorted_cats[:8]:  # Top 8 categories
            ...
            if renames:
                ...
                for i, script in enumerate(
                    sorted(renames, key=lambda x: x["current_name"])[:limit_per_cat], 1
                ):
    ...
    def save_suggestions_csv(self):
        ...
        def sort_key(x):
            ...
            return (
                action_priority.get(x["action"], 4),
                x["category"],
                x["current_name"],
            )
    ...
    def generate_summary(self):
        ...
        for action in ["RENAME", "KEEP", "REVIEW", "DELETE"]:
            ...
            emoji = {"RENAME": "🏷️", "KEEP": "✅", "REVIEW": "📝", "DELETE": "🗑️"}.get(
                action, "📌"
            )
        ...
        actual_renames = [
            s
            for s in self.smart_suggestions
            if s["action"] == "RENAME" and s["current_name"] != s["suggested_name"]
        ]
