...
def parse_pythons_files_txt(file_path):
    ...
    for qp in quoted_paths:
        content_no_quotes = content_no_quotes.replace(f"'{qp}'", "").replace(
            f'"{qp}"', ""
        )
...
def main():
    ...
    print(
        f"   Total size needing work: {needs_work_size:.2f} KB ({needs_work_size / 1024:.2f} MB)"
    )
    ...
    for dir_path, count in top_dirs:
        rel_dir = (
            dir_path.replace(str(Path("/Users/steven/pythons")), "").lstrip("/")
            or "root"
        )
