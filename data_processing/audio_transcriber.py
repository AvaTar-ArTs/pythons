...
class AudioTranscriptionDB:
    ...
    def save_similarity(:
        self, file1: str, file2: str, score: float, match_type: str, notes: str = ""
    ):
...
class AudioTranscriber:
    ...
    def transcribe_audio(self, file_path: Path) -> Optional[Dict]:
        ...
        transcription = self._transcribe_with_service(
            file_path, self.available_services[0]
        )
    ...
    def find_similar_audio(:
        self, target_file: Path, all_files: List[Path], threshold: float = 0.8
        ...
        for other_file in all_files:
            ...
            similarity = self.compare_transcriptions(
                target_trans["text"], other_trans["text"]
            )

            if similarity >= threshold:
                similar.append((other_file, similarity))
                self.db.save_similarity(
                    str(target_file), str(other_file), similarity, "audio_duplicate"
                )

        return sorted(similar, key=lambda x: x[1], reverse=True)

    def find_related_text_files(:
        self, audio_file: Path, search_dirs: List[Path]
    ) -> List[Tuple[Path, float]]:
...
if __name__ == "__main__":
    ...
    # Example usage
    dirs = [Path.home() / "Music/nocTurneMeLoDieS/FINAL_ORGANIZED/YOUR_SUNO_SONGS"]
