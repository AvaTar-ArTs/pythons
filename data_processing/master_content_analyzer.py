...
class MasterContentAnalyzer:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        self.config_path = (
            config_path or self.intelligence_dir / "directories_config.json"
        )
    ...
    def save_processed_files(self):
        ...
        data = {
            "processed_files": {
                k: v.isoformat() for k, v in self.processed_files.items()
            },
            "directory_stats": {
                k: {
                    **v,
    ...
    def analyze_directory(:
        self, dir_config: Dict, limit: Optional[int] = None
    ) -> List[Dict]:
        ...
        results_file = (
            self.results_dir
            / f"{dir_name.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def process_all_directories(self, limit_per_dir: Optional[int] = None):
        ...
        master_results = (
            self.results_dir
            / f"master_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def process_directory(self, dir_name: str, limit: Optional[int] = None):
        ...
        dir_config = next(
            (d for d in self.config["directories"] if d["name"] == dir_name), None
        )
    ...
    def generate_master_report(self):
        ...
        for dir_name, count in sorted(
            stats["by_directory"].items(), key=lambda x: -x[1]
        ):
    ...
    def list_directories(self):
        ...
        for dir_config in self.config["directories"]:
            ...
            print(
                f"{dir_config['name']:<30} {path:<40} {dir_config.get('type', 'unknown'):<15} {dir_config.get('priority', 'low'):<10} {status}"
            )
