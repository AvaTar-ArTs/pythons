        ...
        try:
            if env_file.suffix == ".json":
                with open(env_file, "r") as f:
                    data = json.load(f)
                    config.update(data)
            elif env_file.suffix in [".env", ".txt", ""] or env_file.name.endswith(
                ".env"
            ):
                with open(env_file, "r") as f:
                    for line in f:
...
def load_all_tools():
    ...
    for tool in sorted(tools):
        ...
        if any(term in name.lower() for term in ["analyz", "scan", "reanalys", "deep"]):
            ...
            term in name.lower()
            for term in ["organiz", "consolidat", "bundle", "cleanup"]
        ):
            tool_categories["Organization"].append(name)
        elif any(
            term in name.lower() for term in ["match", "compare", "cross", "find"]
        ):
            ...
            term in name.lower() for term in ["integrat", "merge", "unif", "master"]
        ):
...
def create_master_workflow_plan():
    ...
    for entry in entries:
        ...
        if entry.get("should_transcribe") == "YES" and entry.get("transcribed") in [
            "NO",
            "",
        ]:
        ...
        if entry.get("has_bundle") in ["NO", "False", "FALSE"] and entry.get(
            "is_your_music"
        ) in ["YES", "True", "TRUE"]:
    ...
    with open(workflow_output, "w", encoding="utf-8") as f:
        ...
        if stats["need_transcription"] > 0:
            f.write(
                f"### 1. Transcribe Files ({stats['need_transcription']:,} files)\n\n"
            )
            f.write(
                "**Why:** Many files have bad names that can only be fixed after transcription\n\n"
            )
            ...
            f.write("```\n\n")
            f.write("**API Key needed:** WHISPER (local) or ASSEMBLYAI_API_KEY\n\n")

        # Priority 2: Remove duplicates
        if stats["duplicates_to_check"] > 0:
            f.write(
                f"### 2. Remove Duplicates ({stats['duplicates_to_check']:,} groups)\n\n"
            )
        ...
        if stats["need_bundles"] > 0:
            ...
            f.write(
                "python3 ~/advanced_toolkit/extract_relevant_content_to_bundles.py\n"
            )
            f.write("```\n\n")
        ...
        f.write(
            "- `transcribe_and_rename_workflow.py` - Transcribe + rename bad files\n"
        )
        ...
        f.write(
            "- `FILES_NEEDING_TRANSCRIPTION_TO_RENAME.csv` - Bad filenames (5,709 files)\n"
        )
        f.write(
            "- `DEEP_SCAN_*.csv` - Deep scan results (6,068 audio, 17,860 lyrics, etc.)\n"
        )
