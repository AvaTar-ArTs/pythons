...
class CleanFolderNames:
    ...
    def __init__(self, dry_run: bool = True):
        ...
        self.stats = {"folders_renamed": 0, "folders_merged": 0}
    ...
    def scan_and_rename_folders(self):
        ...
        all_dirs = [
            d
            for d in self.nocturne_dir.iterdir()
            ...
            if action == "MERGE":
                ...
                if not self.dry_run:
                    try:
                            ...
                            if target.exists():
                                ...
                                if item_in_old.is_dir() and target.is_dir():
                                    ...
                                    while target.exists():
                                        target = (
                                            new_path
                                            / f"{item_in_old.stem}_{counter}{item_in_old.suffix}"
                                        )
                                        counter += 1
                        ...
                        print(
                            f"  {Colors.GREEN}✅ Merged and removed old folder{Colors.END}"
                        )
                    ...
                    print(
                        f"  {Colors.YELLOW}[DRY RUN] Would merge contents{Colors.END}"
                    )
                    self.stats["folders_merged"] += 1
    ...
    def save_log(self):
        ...
        print(
            f"{Colors.CYAN}💡 Contains old_path & new_path - use to revert if needed{Colors.END}\n"
        )
    ...
    def run(self):
        ...
        if self.renames:
            ...
            print(
                f"  Folders renamed: {Colors.CYAN}{self.stats['folders_renamed']}{Colors.END}"
            )
            print(
                f"  Folders merged: {Colors.CYAN}{self.stats['folders_merged']}{Colors.END}\n"
            )
            ...
            if self.dry_run:
                print(
                    f"{Colors.YELLOW}⚠️  This was a DRY RUN. Run with --live to execute.{Colors.END}\n"
                )
            else:
                print(
                    f"{Colors.GREEN}✅ All folder names cleaned! No more Vol numbers.{Colors.END}\n"
                )
