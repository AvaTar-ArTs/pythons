...
def normalize_for_matching(text: str) -> str:
    ...
    if not text:
        return ""
    text = re.sub(
        r"\.(mp3|wav|txt|jpg|png|json|md|html|pdf)$", "", text, flags=re.IGNORECASE
    )
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                ...
                if (
                    similarity_score(song_title, str(key)) > 0.5
                    or similarity_score(song_title, str(value)) > 0.5
                ):
                    relevant[key] = value
...
def create_extracted_content_file(:
    bundle_path: Path,
    song_title: str,
    lyrics_files: list,
    prompt_files: list,
...
def main():
    ...
    results = {"processed": 0, "files_extracted": 0, "bundles_updated": 0}
    ...
    for i, song in enumerate(songs_data):
        ...
        if not bundle_path:
            print("  ??  Bundle not found, skipping\n")
            continue
