...
def temporal_analysis(:
    df: pd.DataFrame, date_column: str = "Created time"
) -> pd.DataFrame:
    """
    ...
    monthly_stats = (
        ...
        .rename(columns={df.columns[0]: "Total Songs", "Song URL": "Unique Releases"})
    )
...
def load_suno_csv(csv_path: Path) -> pd.DataFrame:
    ...
    df = pd.read_csv(
        ...
        if "Created time" in pd.read_csv(csv_path, nrows=1).columns
        else None,
    )
    ...
    if "Created time" in df.columns:
        print(
            f"?? Date range: {df['Created time'].min()} to {df['Created time'].max()}"
        )
...
def export_analysis_report(df: pd.DataFrame, output_dir: Path):
    ...
    if "Analysis" in df.columns:
        ...
        df[["Song Title", "Sentiment Score"]].to_csv(
            output_dir / "sentiment_analysis.csv", index=False
        )
