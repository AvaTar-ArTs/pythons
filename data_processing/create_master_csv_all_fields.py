...
def main():
    ...
    for key in sorted(all_keys):
        ...
        entry["Title"] = (
            safe_get(s1, "SongTitle", "Title")
            or safe_get(s2, "songName")
            or (local_files[0].get("title") if local_files else "")
            or key
        )
        entry["Artist"] = (
            safe_get(s1, "Artist")
            or safe_get(s2, "author")
            or (local_files[0].get("artist") if local_files else "")
        )
        ...
        if local_files:
            ...
            entry["Local_Filenames"] = " | ".join(
                [f.get("filename", "") for f in local_files]
            )
    ...
    print("  ? Google Sheet 1 (metadata, URLs, prompts)")
    print("  ? Google Sheet 2 (FULL LYRICS!)")
    print("  ? Local files (paths, sizes, durations)")
