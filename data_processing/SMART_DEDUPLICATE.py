...
class SmartDeduplicator:
    ...
    def load_duplicates(self):
        ...
        # Find latest reports
        exact_files = sorted(
            self.pythons_dir.glob("EXACT_DUPLICATES_*.csv"), reverse=True
        )
        similar_files = sorted(
            self.pythons_dir.glob("SIMILAR_NAMES_*.csv"), reverse=True
        )
    ...
    def analyze_similar_group(self, files):
        ...
        for f in files:
            ...
            if not any(
                x in name
                for x in ["_1", "_2", "_3", "(1)", "(2)", " copy", "-v1", "-v2"]
            ):
                score += 50
            ...
            path_str = str(f)
            if any(
                x in path_str for x in ["batch", "misc", "other", "general", "temp"]
            ):
                score -= 20
    ...
    def process_exact_duplicates(self):
        ...
        print("✅ Processed exact duplicates\n")
                ...
                if len(existing_files) >= 2:
                    ...
                    if keep and keep not in self.to_keep:
                        ...
                        self.to_delete.extend(
                            [d for d in delete if d not in self.to_delete]
                        )
                        processed += 1
    ...
    def execute_deduplication(self, dry_run=True):
        ...
        print(
            f"📊 Plan: Keep {len(self.to_keep)} files, delete {len(self.to_delete)} duplicates\n"
        )
