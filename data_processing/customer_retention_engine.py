...
class CustomerRetentionEngine:
    ...
    def calculate_churn_risk(self, customer_id: str) -> float:
        ...
        if purchase_count == 1 and days_since > 30:
            ...
            avg_days_between = (
                (last_interaction - first_purchase) / (purchase_count - 1) / 86400
            )
...
"""
    ...
    def execute_retention_action(self, action: RetentionAction) -> bool:
        ...
        content = self.generate_personalized_content(
            action.customer_id, action.action_type
        )
...
def main():
    ...
    for action in actions:
        print(
            f"  - {action.action_type} (Priority: {action.priority}, ROI: ${action.estimated_roi:.2f})"
        )
    print()
