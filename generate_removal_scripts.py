...
phase1 = [r for r in data if r['confidence'] == 'Very High' and r['agreement'] == 'Both methods']

# Phase 2: High confidence, both methods
phase2 = [r for r in data if r['confidence'] == 'High' and r['agreement'] == 'Both methods']
...
with open('remove_phase2_review.sh', 'w') as f:
    ...
    for r in phase2:
        ...
        f.write(f'  read -p "  Remove this file? (y/n): " answer\n')
        f.write(f'  if [ "$answer" = "y" ]; then\n')
        f.write(f'    cp "{filename}" "$BACKUP_DIR/"\n')
        f.write(f'    rm "{filename}"\n')
        f.write(f'    echo "  ✅ Removed: {filename}"\n')
        f.write(f'  else\n')
