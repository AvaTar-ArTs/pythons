...
class WorkspaceCleanup:
    ...
    def find_duplicates(self):
        ...
        for filepath in self.root.rglob('*'):
            ...
            if any(x in str(filepath) for x in ['.venv', '_build', '__pycache__', '.git']):
                continue
    ...
    def cleanup_duplicates(self, duplicates, dry_run=True, keep_first=True):
        ...
        for hash_val, files in sorted_dupes[:50]:  # Process top 50 groups
            ...
            print(f"     Size: {file_size / 1024:.1f} KB × {len(remove_files)} copies = {group_size / 1024:.1f} KB")
            ...
            for remove_file in remove_files:
                ...
                if dry_run:
                    ...
                    try:
                        remove_file.unlink()
                        self.removed_items.append(('duplicate', str(rel_remove), file_size))
    ...
    def generate_cleanup_log(self):
        ...
        log_path = self.root / "docs" / "reports" / f"CLEANUP_LOG_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
