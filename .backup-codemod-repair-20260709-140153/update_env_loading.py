...
def update_file(file_path: Path) -> Tuple[bool, str]:
    ...
    try:
        ...
        has_simple_pattern = bool(re.search(
            r'env_dir.*\.env\.d|PathLib\.home.*\.env\.d',
            content,
            re.IGNORECASE
        ))
        ...
        if has_simple_pattern:
            ...
            for i, line in enumerate(lines):
                ...
                if 'env_dir = PathLib.home() / ".env.d"' in line or \
                   'env_dir = Path.home() / ".env.d"' in line:
                    # Skip this block
                    j = i
                    while j < len(lines) and ('load_dotenv' in lines[j] or 
                                             'env_dir' in lines[j] or
                                             not lines[j].strip()):
                        j += 1
                    skip_next = (j > i)
                    continue
...
def main():
    ...
    exclude_paths = {'.git', 'axolotl-main', 'site', '.ruff_cache', '.claude', 
                     '.context7', '.aider.tags.cache.v4', '__pycache__', '.venv'}
