...
class UnifiedOrganizer:
    ...
    def organize_by_csv(self, csv_file, destination_base):
        ...
        with open(csv_file, "r") as csv_file:
            ...
            for row in reader:
                original_path = row.get("Original Path") or row.get("source") or row.get("filepath")
                if not original_path:
                    continue

                # Support multiple CSV formats
                destination_path = row.get("Destination Path") or row.get("dest") or row.get("target")
                if destination_path:
                    ...
                    dest_path = destination_base / Path(original_path).relative_to(Path(original_path).anchor)
    ...
    def organize_music_by_albums(self, music_dir, strategy="filename", metadata_csv=None):
        """
        ...
        for album, files in classifications.items():
            ...
            for audio_file in files:
                ...
                while dest_path in [m['dest'] for m in self.moves]:
                    counter += 1
                    dest_path = album_dir / f"{clean_name}_v{counter}{audio_file.suffix}"
    ...
    def _clean_music_filename(self, filename):
        ...
        name = re.sub(r'[_-]?(Remastered|Remix|Edit|Cover|Live|Extended?)', '', name, flags=re.IGNORECASE)
    ...
    def _execute_moves(self, operation_name):
        ...
        if self.dry_run:
            ...
            for move in self.moves:
                print(f"  ? {move['source'].name} -> {move['dest'].relative_to(move['dest'].parent.parent)}")
            return
...
def main():
    ...
    parser.add_argument("--mode", choices=["filetype", "csv", "music"],
                       default="filetype", help="Organization mode")
    parser.add_argument("--csv-file", help="CSV file for CSV mode")
    parser.add_argument("--destination", help="Destination for CSV mode")
    parser.add_argument("--music-strategy", choices=["filename", "metadata"],
                       default="filename", help="Music organization strategy")
