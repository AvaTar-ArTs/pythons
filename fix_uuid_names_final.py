...
def load_uuid_mapping(csv_file_path):
    """
    Load UUID to title mapping from SUNO CSV file.
    Creates mappings for both hyphen and underscore formats.
    """
    uuid_mapping = {}

    with open(csv_file_path, newline="", encoding="utf-8") as csvfile:
        # Read the entire file as text to handle irregular formatting
        ...
        for line in lines:
            # Look for UUIDs followed by titles in the line
            # Pattern: UUID followed by commas and then a title

            # More flexible pattern to catch various formats in the CSV
            matches = re.findall(
            ...
            for match in matches:
                ...
                # Clean up the title - remove extra quotes and other artifacts
                title = re.sub(r'^"|"$', "", title)  # Remove leading/trailing quotes
                title = re.sub(r"^\'|\'$", "", title)  # Remove leading/trailing single quotes

                # Skip if title is empty or just numbers
    ...
    # Also try a more direct approach by parsing the known patterns from the grep results
    with open(csv_file_path, newline="", encoding="utf-8") as csvfile:
        content = csvfile.read()

        # Find patterns like: UUID,,,,title,,,, or UUID,title,
        pattern1 = r"([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}),,*([^,\n]*?)[,]{4,}"
        matches1 = re.findall(pattern1, content)

        for uuid, title in matches1:
            # Clean up the title
        ...
        # Find patterns like: UUID,title, (when title is in the second column)
        pattern2 = r"([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}),([^,\n]*?),"
        matches2 = re.findall(pattern2, content)

        for uuid, title in matches2:
            # Clean up the title
            title = re.sub(r'^"|"$', "", title.strip())
            title = re.sub(r"^\'|\'$", "", title)
