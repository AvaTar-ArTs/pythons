...
def prompt_for_csv_file():
    ...
    while True:
        ...
        if os.path.isfile(csv_input):
            ...
            print(f"Invalid file path: {csv_input}. Please enter a valid CSV file path.")
...
def prompt_for_output_directory():
    ...
    while True:
        ...
        if os.path.isdir(dir_output):
            print(f"Output CSV will be saved in: {dir_output}")
            return dir_output
        else:
            print(f"Invalid directory path: {dir_output}. Please enter a valid directory path.")
...
def remove_duplicates(duplicate_report_path, output_csv_path, delete_files=False):
    ...
    count_column = next(
        (col for col in possible_count_columns if col in duplicate_report.columns), None
    )

    if count_column is None:
        print("No suitable column found for duplicate count.")
        return
