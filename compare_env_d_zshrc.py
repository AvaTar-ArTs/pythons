...
class EnvDZshrcComparator:
    ...
    def analyze_zshrc(self):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if any(pattern in line.lower() for pattern in ["load", "dotenv", ".env", "export"]):
            ...
            if line.strip().startswith("function ") or re.match(r'^\w+\(\)\s*\{', line.strip()):
                func_match = re.search(r'(function\s+)?(\w+)', line)
                if func_match:
                    analysis["functions"].append({
                        "line": i,
                        "name": func_match.group(2) if func_match.group(2) else func_match.group(1)
                    })
    ...
    def save_results(self):
        ...
        output_path = PathLib.home() / "pythons" / f"ENV_D_ZSHRC_COMPARISON_{self.timestamp}"
    ...
    def generate_report(self, output_path: Path):
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(f"- **.env.d References:** {len(zshrc.get('env_d_references', []))}\n")
            f.write(f"- **Source Statements:** {len(zshrc.get('source_statements', []))}\n")
            f.write(f"- **Export Statements:** {len(zshrc.get('export_statements', []))}\n")
            ...
            f.write("## 🔍 Comparison\n\n")
            f.write(f"- **Integration Status:** {comp.get('integration_status', 'unknown')}\n\n")
