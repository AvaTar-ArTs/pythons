...
def organize_zip_folder():
    ...
    for zip_file in zip_path.glob("*.zip"):
        if zip_file.is_file():
            ...
            elif any(keyword in name_lower for keyword in ["alley", "inthis", "hiide", "aLLey"]):
    ...
    for subdir in zip_path.iterdir():
        if subdir.is_dir() and subdir.name not in categories:
    ...
    for loose_file in zip_path.iterdir():
