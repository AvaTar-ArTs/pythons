...
sys.path.insert(
    ...
    str(
        Path.home()
        / ".local/share/mamba/envs/sales-empire/lib/python3.11/site-packages"
    ),
)
...
class IntelligentDocsBuilder:
    ...
    def __init__(self, base_dir: Path):
        ...
        self.results = {"analyzed": 0, "categorized": {}, "scripts": [], "insights": []}
...
"""
            ...
            response = self.openai.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a Python code analyst. Be concise and accurate.",
                    },
...
"""
        # Category summary
        for category, scripts in sorted(
            self.results["categorized"].items(), key=lambda x: -len(x[1])
        ):
    ...
    def print_summary(self):
        ...
        print(
            f"   AI-Enhanced: {sum(1 for s in self.results['scripts'] if s.get('ai_analyzed', False))}"
        )
