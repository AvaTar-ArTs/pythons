...
class APIKeyInventoryV2:
    ...
    def __init__(self):
        self.output_file = (
            self.env_dir / f"API_KEY_INVENTORY_COMPLETE_{self.timestamp}.csv"
        )
    ...
    def scan_all_env_files(self):
        ...
        print(
            f"{Colors.GREEN}✅ Total unique keys found: {len(self.all_keys)}{Colors.END}\n"
        )
    ...
    def display_by_category(self):
        ...
        for category in sorted(by_category.keys()):
            ...
            print(
                f"{Colors.BOLD}{category}{Colors.END} ({active_count}/{len(keys)} active)"
            )

            for key_name, info in sorted(keys):
                status_color = (
                    Colors.GREEN if info["status"] == "ACTIVE" else Colors.YELLOW
                )
    ...
    def run(self):
        ...
        print(
            "║              🔑 API KEY INVENTORY (SECURE - NO EXPOSURE) 🔑                   ║"
        )
        print(
            "║                                                                               ║"
        )
        ...
        print(
            f"  {Colors.GREEN}ACTIVE:{Colors.END}       {status_counts['ACTIVE']} (ready to use)"
        )
        print(
            f"  {Colors.CYAN}CONFIG:{Colors.END}       {status_counts['CONFIG']} (config values)"
        )
        print(
            f"  {Colors.YELLOW}PLACEHOLDER:{Colors.END}  {status_counts['PLACEHOLDER']} (need real keys)"
        )
        print(
            f"  {Colors.RED}NOT_SET:{Colors.END}      {status_counts['NOT_SET']} (empty)\n"
        )
