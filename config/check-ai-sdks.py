...
def main():
    ...
    if configured < len(env_vars):
        print(
            f"{YELLOW}⚠️{RESET}  Some API keys are not loaded. Run: {BLUE}source ~/.env.d/loader.sh llm-apis{RESET}"
        )
    ...
    if installed < len(sdks):
        ...
        if missing:
            ...
            print(
                f"   Install with: {BLUE}pip3 install openai anthropic groq google-generativeai cohere{RESET}"
            )
    ...
    if all_good:
        print(
            f"{GREEN}{BOLD}✅ All AI services are properly installed and configured!{RESET}\n"
        )
        return 0
    else:
        print(
            f"{YELLOW}{BOLD}⚠️  Some services need attention. See recommendations above.{RESET}\n"
        )
        return 1
