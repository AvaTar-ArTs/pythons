...
class IntelligentCodeOrchestrator:
    ...
    async def _analyze_single_file(:
        self, file_path: Path, focus_areas: List[str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for area in focus_areas:
                analyses[area] = await self._analyze_focus_area(
                    content, area, file_path.name
                )
    ...
    async def _analyze_focus_area(:
        self, content: str, focus_area: str, filename: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "issues_found": len(analysis_text.split("Issue:")) - 1
                if "Issue:" in analysis_text
                else 0,
                "recommendations": len(analysis_text.split("Recommendation:")) - 1
                if "Recommendation:" in analysis_text
                else 0,
            }

        except Exception as e:
            return {"model_used": model, "error": str(e), "analysis": "Analysis failed"}
    ...
    def _select_model_for_analysis(self, focus_area: str) -> str:
        ...
        return (
            "anthropic" if "anthropic" in self.clients else list(self.clients.keys())[0]
        )

    def _create_analysis_prompt(:
        self, focus_area: str, content: str, filename: str
    ) -> str:
    ...
    def _aggregate_analyses(self, file_analyses: List[Dict]) -> Dict[str, Any]:
        ...
        aggregated = {
            ...
            "total_lines": sum(
                f.get("metrics", {}).get("total_lines", 0) for f in file_analyses
            ),
            "total_functions": sum(
                f.get("metrics", {}).get("functions", 0) for f in file_analyses
            ),
            "total_classes": sum(
                f.get("metrics", {}).get("classes", 0) for f in file_analyses
            ),
            "focus_areas": {},
        }
    ...
    async def _generate_recommendations(:
        ...
        try:
            ...
            recommendations["strategic"] = (
                "Strategic analysis failed - check individual file analyses for insights"
            )
...
async def main():
    ...
    logger.info(
        f"Analyzed {analysis_result['files_analyzed']} files in {analysis_result['processing_time']:.2f} seconds"
    )
    ...
    final_result = {
        ...
        "summary": {
            ...
            "total_issues": sum(
                ...
                for data in analysis_result["aggregated_analysis"][
                    "focus_areas"
                ].values()
            ),
    ...
    logger.info(
        f"Summary: {final_result['summary']['total_issues']} issues found, {final_result['summary']['total_recommendations']} recommendations made"
    )
