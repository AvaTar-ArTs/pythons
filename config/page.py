...
def load_pages(input_dir: Path) -> List[Page]:
    ...
    for path in sorted(input_dir.rglob("*")):
        ...
        if ext in [".md", ".markdown"]:
            ...
            updated = meta.get("updated") or datetime.datetime.now().isoformat(
                timespec="seconds"
            )
            extras = {
                k: v
...
def build_env(templates_dir: Path):
    ...
    env.filters["datefmt"] = (
        lambda s, fmt="%b %d, %Y": datetime.datetime.fromisoformat(
            str(s).replace("Z", "").replace(" ", "T")
        ).strftime(fmt)
        if s
        else ""
    )
    env.filters["rfc822"] = (
        lambda s: datetime.datetime.fromisoformat(
            str(s).replace("Z", "").replace(" ", "T")
        ).strftime("%a, %d %b %Y %H:%M:%S %z")
        if s
        else ""
    )
    env.filters["slug"] = lambda t: re.sub(
        r"-+", "-", re.sub(r"[^a-z0-9-]", "-", (t or "").lower())
    ).strip("-")
    ...
    for idx, p in enumerate(pages_sorted):
        ...
        if out_format in ("both", "html"):
            html = env.get_template("base.html").render(
                ...
                page=p,
                body=body_html,
                toc_html=meta.get("toc_html", ""),
                prev_page=prevp,
                next_page=nextp,
                json_ld=article_jsonld(site, p),
            )
            write_text(output_dir / f"{p.slug}.html", html)
        if out_format in ("both", "md"):
            body = (
                p.body_md
                if re.search(r"^#\s+", p.body_md, flags=re.M)
                else f"# {p.title}\n\n{p.body_md}"
            )
    ...
    if out_format in ("both", "html"):
        ...
        for page_num, chunk in paginate(pages_sorted, per_page):
            html = env.get_template("index.html").render(
                site=site, pages=chunk, page_num=page_num, total=len(pages_sorted)
            )
        ...
        for tag, plist in sorted(tags.items()):
            html = tag_t.render(
                site=site,
                tag=tag,
                pages=sorted(plist, key=lambda x: (x.date, x.updated), reverse=True),
            )
        ...
        write_text(
            output_dir / "rss.xml",
            env.get_template("rss.xml").render(site=site, pages=pages_sorted),
        )
        write_text(
            output_dir / "sitemap.xml",
            env.get_template("sitemap.xml").render(site=site, pages=pages_sorted),
        )
        write_text(
            output_dir / "robots.txt", env.get_template("robots.txt").render(site=site)
        )
        write_text(
            output_dir / "CONSTANT_404.html",
            env.get_template("CONSTANT_404.html").render(site=site),
        )
...
def main():
    ...
    site = {
        ...
        or cfg.get("description", "Clean pages, zero fuss."),
