...
class AdvancedSystemsCatalog:
    ...
    def _identify_system(self, path: Path) -> AdvancedSystem:
        ...
        system = AdvancedSystem(
            name=path.name, path=str(path), system_type="unknown", technologies=[]
        )
    ...
    def _analyze_package_json(self, path: Path, system: AdvancedSystem):
        ...
        try:
            ...
            if "express" in system.dependencies:
                ...
                system.has_api = True
            if "tailwindcss" in system.dependencies or "tailwindcss" in pkg.get(
                "devDependencies", {}
            ):
    ...
    def _determine_system_type(self, path: Path, system: AdvancedSystem) -> str:
        ...
        # Web app
        if (
            "React" in system.technologies
            or "Next.js" in system.technologies
            or "Vue.js" in system.technologies
        ):
            return "web_app"
    ...
    def _analyze_systems(self):
        ...
        print("\nSystems by Type:")
        for sys_type, systems in sorted(
    ...
    def _generate_catalog_csv(self):
        ...
        with open(filename, "w", newline="", encoding="utf-8") as f:
            ...
            for system in sorted(
                self.systems, key=lambda s: s.value_score, reverse=True
            ):
                writer.writerow(
                    [
                        system.name,
    ...
    def _generate_index_md(self):
        ...
        for sys_type in sorted(by_type.keys()):
            systems = sorted(
                by_type[sys_type], key=lambda s: s.value_score, reverse=True
            )
        ...
        top_systems = sorted(self.systems, key=lambda s: s.value_score, reverse=True)[
            :10
        ]

        for i, system in enumerate(top_systems, 1):
            lines.append(
                f"{i}. **{system.name}** ({system.value_score}/10) - {system.system_type}"
            )
            lines.append(f"   - Path: `{system.path}`")
...
"""
    ...
    def _generate_deployment_plan(self):
        ...
        for i, system in enumerate(
            sorted(ready, key=lambda s: s.value_score, reverse=True), 1
        ):
