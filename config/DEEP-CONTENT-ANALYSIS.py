...
class DeepContentAnalyzer:
    ...
    def categorize_file(self, info, content):
        ...
        if "openai" in services or "anthropic" in services:
            ...
            if "transcribe" in ops:
                return "AI Transcription"
            return "AI Tool"

        # Media processing
        if "upscale" in ops or "resize" in ops:
        ...
        # File operations
        if "organize" in ops or "rename" in ops:
        ...
        # Suno
        if "suno" in services or "suno" in purpose:
            return "Suno Music"
    ...
    def analyze_all(self):
        ...
        files = [
            f
            for f in files
            if "_BROKEN_" not in f.name
            and not any(x in f.name for x in ["DEEP_CONTENT", "STANDARD_ENV"])
        ]
