...
class SmartRenameSuggester:
    ...
    def suggest_rename(self, script):
        ...
        if current in [
            ...
            "personal-ai-assistant.py",
            "seo-content-organizer.py",
            "file-diff-viewer.py",
        ]:
        ...
        if "instagram" in current:
            if "follow" in desc and "unfollow" in desc:
                return (
                    "instagram-auto-follow-unfollow.py",
                    "RENAME",
                    "Auto-follows and unfollows users",
                )
            elif "bot" in desc or "automat" in desc:
                if "like" in desc:
                    return (
                        "instagram-auto-liker.py",
                        "RENAME",
                        "Automatically likes posts",
                    )
                elif "comment" in desc:
                    return (
                        "instagram-auto-commenter.py",
                        "RENAME",
                        "Automatically comments",
                    )
                elif "upload" in desc or "post" in desc:
                    return (
                        "instagram-auto-poster.py",
                        "RENAME",
                        "Automatically posts content",
                    )
                else:
                    return "instagram-bot.py", "RENAME", "General Instagram bot"
            elif "scrap" in desc or "download" in desc:
                return (
                    "instagram-content-scraper.py",
                    "RENAME",
                    "Scrapes Instagram content",
                )
            elif "whisper" in current:
                return (
                    "instagram-whisper-captions.py",
                    "RENAME",
                    "Generates captions with Whisper",
                )
            ...
            if "gpt" in apis or "vision" in desc:
                if "csv" in desc or "enrich" in desc:
                    return (
                        ...
                        "RENAME",
                        "Enriches CSV with GPT Vision",
                    )
                elif "batch" in desc:
                    return (
                        "gpt-vision-batch-analyzer.py",
                        "RENAME",
                        "Batch analyzes images",
                    )
                else:
                    return (
        ...
        if category == "File Organization":
            if "duplicate" in desc:
                ...
                return (
                    "sort-files-intelligent.py",
                    "RENAME",
                    "Intelligently sorts files",
                )
    ...
    def print_suggestions_by_category(self, suggestions, limit_per_cat=20):
        ...
        for category, scripts in sorted_cats:
            ...
            for i, script in enumerate(
                sorted(scripts, key=lambda x: x["current_name"])[:limit_per_cat], 1
            ):
                current = script["current_name"]
    ...
    def save_suggestions_csv(self):
        ...
        sorted_scripts = sorted(
            self.scripts, key=lambda x: (x["category"], x["current_name"])
        )
