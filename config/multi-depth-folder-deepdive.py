...
class MultiDepthFolderDeepDive:
    ...
    def _initialize_ai_clients(self) -> Dict[str, any]:
        ...
        clients = {"openai": None, "anthropic": None, "gemini": None}
...
{{
    ...
    def _analyze_with_anthropic(:
        self, file_path: Path, code_sample: str
    ) -> Optional[Dict]:
...
{{
    ...
    def scan_directory_structure(self, max_depth: int = 10):
        ...
        structure["largest_dirs"] = sorted(dir_sizes, key=lambda x: x[1], reverse=True)[
            :20
        ]
        ...
        structure["deepest_paths"] = sorted(
            all_paths, key=lambda x: x[1], reverse=True
        )[:20]
    ...
    def analyze_python_files(self):
        ...
        files_to_analyze = (
            python_files[:100] if len(python_files) > 100 else python_files
        )
        if self.ai_enabled:
                ...
                for file_path in files_to_analyze:
                    try:
                        ...
                        future = executor.submit(
                            self.analyze_code_with_ai, file_path, content
                        )
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if result:
                            ...
                            if completed % 10 == 0:
                                print(
                                    f"      Analyzed {completed}/{len(files_to_analyze)} files..."
                                )
        ...
        for file_path in python_files:
            try:
                ...
                if str(file_path) in ai_results:
                    ...
                    analysis["ai_analysis"]["by_category"][
                        ai_data.get("category", "other")
                    ] += 1
                    ...
                    for suggestion in ai_data.get("suggestions", []):
                        analysis["ai_analysis"]["suggestions"][suggestion].append(
                            rel_path
                        )
        ...
        analysis["largest_files"] = sorted(
            analysis["largest_files"], key=lambda x: x[2], reverse=True
        )[:30]
        analysis["most_imports"] = analysis["imports"].most_common(30)

        # Convert defaultdict to dict
        analysis["by_directory"] = dict(
            sorted(
        ...
        if analysis["ai_analysis"]["analyzed_count"] > 0:
            analysis["ai_analysis"]["by_category"] = dict(
                analysis["ai_analysis"]["by_category"]
            )
            analysis["ai_analysis"]["avg_quality"] = (
                sum(analysis["ai_analysis"]["quality_scores"])
                / len(analysis["ai_analysis"]["quality_scores"])
                if analysis["ai_analysis"]["quality_scores"]
                else 0
            )
            analysis["ai_analysis"]["suggestions"] = dict(
                analysis["ai_analysis"]["suggestions"]
            )

        self.results["file_analysis"] = analysis
        ai_msg = (
            f" (🤖 AI analyzed {analysis['ai_analysis']['analyzed_count']})"
            if analysis["ai_analysis"]["analyzed_count"] > 0
            else ""
        )
        print(
            f"   ✓ Analyzed {analysis['file_count']} Python files ({analysis['total_lines']:,} lines){ai_msg}"
        )
    ...
    def find_duplicates(self):
        ...
        duplicates["exact_duplicates"] = sorted(
            duplicates["exact_duplicates"], key=lambda x: x["count"], reverse=True
        )[:20]
    ...
    def identify_organization_issues(self):
                ...
                for file in files:
                    if file.endswith(".py"):
                        issues["deeply_nested"].append(
                            {
                                "path": str(
                                    (root_path / file).relative_to(self.root_path)
                                ),
                                "depth": depth,
                            }
                        )

        issues["deeply_nested"] = sorted(
            issues["deeply_nested"], key=lambda x: x["depth"], reverse=True
        )[:20]
        ...
        for root, dirs, files in os.walk(self.root_path / "transcribe"):
            ...
            for file in files:
                if file.endswith(".py") and file not in [
                    ...
                    transcribe_files.append(
                        str((root_path / file).relative_to(self.root_path))
                    )
        ...
        print(
            f"   ✓ Identified {len(issues['root_level_clutter'])} organization issues"
        )
    ...
    def generate_statistics(self):
        ...
        stats = {
            ...
            "top_imports": [
                {"module": k, "count": v}
                for k, v in self.results["file_analysis"].get("most_imports", [])[:10]
            ],
        }
    ...
    def save_results(self):
        ...
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["Depth", "Path", "File Count", "Dir Count", "Size (bytes)"]
            )
        ...
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["Directory", "File Count", "Total Lines", "Total Size (bytes)"]
            )
            for dir_name, stats in list(
                self.results["file_analysis"]["by_directory"].items()
            )[:50]:
                writer.writerow(
                    [dir_name, stats["count"], stats["lines"], stats["size"]]
                )
        ...
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            ...
            for dup in self.results["duplicates"]["exact_duplicates"][:20]:
                writer.writerow(
                    ["exact", dup["hash"], "; ".join(dup["files"]), dup["count"]]
                )
    ...
    def generate_summary_report(self, output_dir: Path):
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- **Files with Environment Loading:** {file_analysis['patterns']['has_env_loading']}\n"
            )
            f.write(
                f"- **Files with CONSTANT_ placeholders:** {file_analysis['patterns']['has_constants']}\n"
            )
            f.write(
                f"- **Files with TODO/FIXME:** {file_analysis['patterns']['has_todos']}\n"
            )
            f.write(
                f"- **Files with Docstrings:** {file_analysis['patterns']['has_docstrings']}\n"
            )
            f.write(
                f"- **Files with Type Hints:** {file_analysis['patterns']['has_type_hints']}\n"
            )
            f.write(
                f"- **Files with Logging:** {file_analysis['patterns']['has_logging']}\n"
            )
            f.write(
                f"- **Files with __main__:** {file_analysis['patterns']['has_main']}\n\n"
            )

            f.write("## 📁 Top Directories by File Count\n\n")
            for dir_name, dir_stats in list(file_analysis["by_directory"].items())[:10]:
                f.write(
                    f"- **{dir_name}:** {dir_stats['count']} files, {dir_stats['lines']:,} lines\n"
                )
            ...
            f.write("\n## 🔄 Duplicates Found\n\n")
            f.write(
                f"- **Exact Duplicates:** {len(duplicates['exact_duplicates'])} groups\n"
            )
            ...
            if issues.get("root_level_clutter"):
                f.write(
                    f"- **Root Level Clutter:** {issues['root_level_clutter']['count']} Python files in root\n"
                )
            if issues.get("deeply_nested"):
                f.write(
                    f"- **Deeply Nested Files:** {len(issues['deeply_nested'])} files at depth > 5\n"
                )
            if issues.get("potential_consolidation"):
                for consol in issues["potential_consolidation"]:
                    f.write(
                        f"- **{consol['directory']}:** {len(consol['files'])} files could be archived\n"
                    )

            f.write("\n## 📈 Depth Distribution\n\n")
            for depth, count in sorted(
                dir_structure["statistics"]["depth_distribution"].items()
            ):
    ...
    def run_full_scan(self):
        ...
        if self.ai_enabled:
            ...
            print(
                f"   Available APIs: {', '.join([k for k, v in self.ai_clients.items() if v])}\n"
            )
        else:
