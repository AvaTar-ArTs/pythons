...
# SECURITY FIX: Replaced dangerous eval() with a safe type-conversion map.
# eval(checks["type"])(value) allowed arbitrary code execution if an attacker
# could control the "type" field in the checks dict.
_SAFE_TYPE_MAP = {
    "str": str,
    "int": int,
    "float": float,
...
def _safe_type_convert(type_name: str):
    """Return a callable for type conversion from a whitelist, or str as fallback."""
    return _SAFE_TYPE_MAP.get(type_name, str)
...
def check(value, checks, name):
    ...
    if not incorrect and "type" in checks:
        try:
            ...
            type_fn = _safe_type_convert(type_name)
            value = type_fn(value)
        except (IndexError, KeyError, ValueError, SyntaxError, TypeError):
            incorrect = True
    ...
    if incorrect:
        value = handle_input(
            ...
            # SECURITY FIX: Was eval(get_check_value("type", "False")) — replaced
            # with safe type map lookup to prevent arbitrary code execution.
            check_type=_safe_type_convert(get_check_value("type", "bool")),
...
Overwrite it?(y/n)"""
        ...
        if not input().startswith("y"):
            logger.info(
                "Unable to read config, and not allowed to overwrite it. Giving up."
            )
            return False
        else:
            try:
