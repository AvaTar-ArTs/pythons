...
class APIKeyInventory:
    ...
    def scan_all_env_files(self):
        ...
        print(
            f"{Colors.GREEN}✅ Found {len(self.found_keys)} unique API keys{Colors.END}\n"
        )
        ...
        for key_name, service_name in self.COMMON_APIS.items():
            if key_name not in self.found_keys:
                self.missing_keys.append(
                    {"key_name": key_name, "service": service_name, "status": "MISSING"}
                )

        print(
            f"{Colors.YELLOW}Found {len(self.missing_keys)} missing common API keys{Colors.END}\n"
        )
        ...
        with open(self.output_file, "w", newline="", encoding="utf-8") as f:
            ...
            for key_name, info in sorted(self.found_keys.items()):
                ...
                if any(x in service for x in ["AI", "GPT", "Claude", "Gemini", "Groq"]):
                    ...
                    x in service for x in ["Image", "Stability", "DALL-E", "Leonardo"]
                ):
                    category = "Image Generation"
                elif any(
                    x in service for x in ["Audio", "TTS", "Music", "Eleven", "Suno"]
                ):
                    category = "Audio/Music"
                elif any(
                    x in service
                    for x in ["Instagram", "YouTube", "Reddit", "Twitter", "TikTok"]
                ):
                    category = "Social Media"
                elif any(
                    x in service for x in ["AWS", "Google Cloud", "Azure", "Storage"]
                ):
                    category = "Cloud/Storage"
                else:
                    category = "Other"
    ...
    def display_summary(self):
        ...
        print(
            f"  {Colors.GREEN}CONFIGURED:{Colors.END}   {status_counts['CONFIGURED']} keys (ready to use)"
        )
        print(
            f"  {Colors.YELLOW}PLACEHOLDER:{Colors.END}  {status_counts['PLACEHOLDER']} keys (need real values)"
        )
        print(
            f"  {Colors.YELLOW}NOT_SET:{Colors.END}      {status_counts['NOT_SET']} keys (empty)"
        )
        print(
            f"  {Colors.YELLOW}INVALID:{Colors.END}      {status_counts['INVALID']} keys (check format)"
        )
        print(
            f"  {Colors.RED}MISSING:{Colors.END}      {status_counts['MISSING']} keys (not in any .env)"
        )
        print(
        ...
        important_missing = [
            m
            for m in self.missing_keys
            if any(
                x in m["service"] for x in ["Perplexity", "Claude", "Suno", "Leonardo"]
            )
        ]
    ...
    def run(self):
        ...
        print(
            "║                  Secure Key Status Check (No Exposure)                       ║"
        )
        print(
            "║                                                                               ║"
        )
