...
def to_iso(dt):
    ...
    if isinstance(dt, str):
        for fmt in (
            ...
            try:
                return datetime.datetime.strptime(dt, fmt).isoformat()
            except Exception:
                continue
        return dt
...
def render_user(user: dict) -> str:
    ...
    created = (
        user.get("created_at") or user.get("createdAt") or user.get("created") or ""
    )
    ...
    if extras:
        parts += [
            "",
            "<details><summary>Raw User JSON</summary>",
            "",
            "```json",
...
def render_conversation(conv: dict) -> str:
    ...
    title = (
        conv.get("title")
        or conv.get("name")
        or (cid and f"Conversation {cid}")
        or "Conversation"
    )
    created = (
        conv.get("created_at") or conv.get("createdAt") or conv.get("start_time") or ""
    )
    updated = (
        conv.get("updated_at")
        or conv.get("updatedAt")
        or conv.get("last_activity")
        or ""
    )
    ...
    if not msgs and isinstance(conv.get("data"), dict):
        msgs = conv["data"].get("messages", [])
...
def main():
    ...
    user_obj = (
        ...
        else (user_data if isinstance(user_data, dict) else {})
    )
    ...
    for conv in conversations:
        title = (
            conv.get("title") or conv.get("name") or conv.get("id") or "conversation"
        )
