...
class APIKeyManager:
                        ...
                        if "=" in line:
                            ...
                            if (
                                ...
                                results[category].append(
                                    (key, value, f"{env_file.name}:{line_num}")
                                )
    ...
    def display_summary(self):
        ...
        if self.invalid_keys:
            ...
            for key, value, category, location in self.invalid_keys:
                ...
                logger.info(f"  • {service_name} ({category}) - {location}")
                logger.info(
                    f"    Current value: {value[:20]}{'...' if len(value) > 20 else ''}"
                )
    ...
    def interactive_setup(self):
        ...
        logger.info(
            "For each key, I'll open the creation page and wait for you to fill it in.\n"
        )
        ...
        for i, (key, category, location) in enumerate(self.missing_keys, 1):
            ...
            logger.info(
                f"{Colors.CYAN}[{i}/{len(self.missing_keys)}] Setting up {service_name}{Colors.NC}"
            )
            ...
            if is_required:
                logger.info(
                    f"{Colors.YELLOW}This is a REQUIRED key for core functionality{Colors.NC}"
                )
            ...
            while True:
                ...
                if api_key.lower() == "quit":
                    ...
                    logger.info(
                        f"{Colors.RED}Please enter a valid API key or 'skip'{Colors.NC}"
                    )
                    continue
                ...
                if self.validate_api_key(key, api_key):
                    ...
                    if self.update_env_file(key, api_key, location):
                        self.print_success(
                            f"Successfully saved {service_name} API key!"
                        )
                        break
                    else:
                        logger.info(
                            f"{Colors.RED}Failed to save API key. Please try again.{Colors.NC}"
                        )
                else:
                    logger.info(
    ...
    def fix_invalid_keys(self):
        ...
        logger.info(
            "I found some API keys that appear to be invalid or placeholder values."
        )
        ...
        for i, (key, value, category, location) in enumerate(self.invalid_keys, 1):
            ...
            logger.info(
                f"{Colors.CYAN}[{i}/{len(self.invalid_keys)}] Fixing {service_name}{Colors.NC}"
            )
            logger.info(f"Description: {description}")
            logger.info(f"Category: {category}")
            logger.info(
                f"Current value: {value[:30]}{'...' if len(value) > 30 else ''}"
            )
            ...
            while True:
                ...
                if self.validate_api_key(key, new_key):
                    if self.update_env_file(key, new_key, location):
                        self.print_success(
                            f"Successfully updated {service_name} API key!"
                        )
                        break
                    else:
                        logger.info(
                            f"{Colors.RED}Failed to update API key. Please try again.{Colors.NC}"
                        )
                else:
                    logger.info(
                        f"{Colors.RED}Invalid API key format. Please check and try again.{Colors.NC}"
...
def main():
    ...
    parser.add_argument(
        "--no-interactive", action="store_true", help="Skip interactive setup"
    )
    parser.add_argument(
        "--no-fix", action="store_true", help="Skip fixing invalid keys"
    )
    parser.add_argument(
        "--no-report", action="store_true", help="Skip generating report"
    )
    parser.add_argument(
        "--env-dir", default="~/.env.d", help="Environment directory path"
    )
